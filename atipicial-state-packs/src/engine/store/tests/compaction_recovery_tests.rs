use crate::engine::platform_io::{read_exact_at, write_all_at};
...
fn flip_persisted_run_byte(path: &Path, offset: u64) {
    ...
    let mut byte = [0u8; 1];
    read_exact_at(&file, &mut byte, offset)
        .expect("read prepared compaction byte");
    write_all_at(&file, &[byte[0] ^ 0x40], offset)
...
fn rewrite_frame_with_noncanonical_metadata(path: &Path, receipt: PackFrameReceipt) {
    ...
    read_exact_at(&file, &mut header_bytes, receipt.frame_start)
    ...
    read_exact_at(&file, &mut metadata, metadata_start)
    ...
    write_all_at(&file, &header_bytes, receipt.frame_start)
        .expect("rewrite committed frame header");
    write_all_at(&file, &metadata, metadata_start)
        .expect("rewrite committed frame metadata");
    write_all_at(&file, &footer, footer_start)
...
fn rewrite_prepared_filter_with_valid_structure_checksum(
    ...
    read_exact_at(&file, &mut structure, INDEX_HEADER_LEN as u64)
    ...
    write_all_at(&file, &structure, INDEX_HEADER_LEN as u64)
...
fn recovery_rebuilds_middle_record_corruption_in_a_non_tail_v5_run() {
    ...
    let mut byte = [0u8; 1];
    read_exact_at(&file, &mut byte, corrupt_offset)
        .expect("read middle record byte");
    write_all_at(&file, &[byte[0] ^ 0x40], corrupt_offset)
...
fn noncanonical_committed_rows_fail_before_cleanup_or_truncation() {
    ...
    write_all_at(&pack, b"orphan suffix", receipt.frame_end)
