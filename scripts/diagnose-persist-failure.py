#!/usr/bin/env python3
"""Diagnose deterministic native persistence failures from atipicial-node logs."""
...
GAS_BURN_RE = re.compile(
    r"native (?P<contract>AtipicialDollar) TriggerType\(ON_PERSIST\) hook failed at block "
    r"(?P<height>\d+): .*?"
    ...
    *,
    ...
    for tx in transactions:
        ...
        try:
            ...
            value = rpc_call(local_rpc, "getstorage", ["AtipicialDollar", storage_key])
            item["value_base64"] = value
            item["balance"] = decode_aep17_account_balance(value)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Diagnose native persistence failures such as AtipicialDollar::burn balance faults."
    )
    parser.add_argument("--log", required=True, help="atipicial-node log file to scan")
    parser.add_argument("--status-file", default=None, help="Optional state-root validator status JSON")
    parser.add_argument(
        "--reference-rpc",
        default=DEFAULT_REFERENCE_RPC,
        help=f"Reference Atipicial RPC used to fetch the failed block (default: {DEFAULT_REFERENCE_RPC})",
    )
    parser.add_argument(
        "--local-rpc",
        default=None,
        help="Optional local atipicial-rs RPC used to sample matching senders' GAS storage balances.",
    )
    parser.add_argument(
        "--address-version",
        default=DEFAULT_ADDRESS_VERSION,
        type=lambda value: int(value, 0),
        help=f"Atipicial address version for local GAS key construction (default: 0x{DEFAULT_ADDRESS_VERSION:02x})",
    )
    parser.add_argument(
        "--no-reference",
        action="store_true",
