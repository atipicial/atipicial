//! Compound operations for the Atipicial Virtual Machine.
//!
//! This module provides the compound operation handlers for the Atipicial VM.
...
fn pick_item<S>(engine: &mut ExecutionEngine<S>, _instruction: &Instruction) -> VmResult<()> {
    ...
    let result = match collection {
        ...
        // C# Atipicial VM PICKITEM on PrimitiveType reads the bytewise GetSpan()
...
fn size<S>(engine: &mut ExecutionEngine<S>, _instruction: &Instruction) -> VmResult<()> {
    ...
    // C# Atipicial VM SIZE accepts CompoundType (Count), PrimitiveType (Size), and Buffer (Size).
...
mod local_stack_item_tests {
    ...
    #[test]
    fn primitive_pick_and_size_use_atipicialvm_memory() {
        let mut engine =
