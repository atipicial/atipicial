...
fn rpc_unclaimed_gas_roundtrip() {
    let gas = RpcUnclaimedAtipicialDollar {
        unclaimed: 1234,
        address: "NQ7cbaBqX1p5quJDQr6b1qnBZBHae3mJzA".to_string(),
    };
    let json = gas.to_json();
    let parsed = RpcUnclaimedAtipicialDollar::from_json(&json).expect("unclaimed");
...
fn rpc_unclaimed_gas_rejects_invalid_value() {
    ...
    assert!(RpcUnclaimedAtipicialDollar::from_json(&json).is_err());
}
...
fn unclaimed_gas_to_json_matches_rpc_test_case() {
    ...
    let parsed = RpcUnclaimedAtipicialDollar::from_json(&expected).expect("parse");
