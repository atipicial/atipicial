...
class DockerEntrypointTests(unittest.TestCase):
        ...
        stub.write_text(
            textwrap.dedent(
                ...
                with open({str(capture_path)!r}, "w", encoding="utf-8") as output:
                    json.dump({{
                        "args": sys.argv[1:],
                        "atipicial_rpc_port": os.environ.get("ATC_RPC_PORT"),
                    }}, output)
                """
            ),
            encoding="utf-8",
    ...
    def test_service_profile_selects_network_service_preset(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            etc_atipicial = root / "etc" / "atipicial"
            config_dir = etc_atipicial / "config"
            ...
            (etc_atipicial / "atipicial_testnet_node.toml").write_text(
                "[rpc]\nport = 29999\n",
                encoding="utf-8",
            )

            capture_path = root / "atipicial-node-args.json"
            bin_dir = root / "bin"
            bin_dir.mkdir()
            ...
            env.update(
                {
                    ...
                    "ATC_CONFIG_ROOT": str(etc_atipicial),
                    "ATC_NETWORK": "testnet",
                    "ATC_PROFILE": "service",
                    "ATC_STORAGE": str(root / "data" / "testnet"),
                    "ATC_PLUGINS_DIR": str(root / "plugins"),
                    "ATC_LOGS_DIR": str(root / "logs"),
                }
            )
            ...
            self.assertEqual(captured["atipicial_rpc_port"], "20332")
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            service_config.write_text(
                textwrap.dedent(
                    ...
                    [logging]
                    file_path = "./logs/atipicial-node-testnet-service.log"
                    """
                ),
                encoding="utf-8",
            )
            ...
            env.update(
                {
                    ...
                    "ATC_NETWORK": "testnet",
                    "ATC_PROFILE": "service",
                    "ATC_STORAGE": str(storage),
                    "ATC_PLUGINS_DIR": str(root / "plugins"),
                    "ATC_LOGS_DIR": str(logs),
                }
            )
            ...
            self.assertIn(f'file_path = "{logs}/atipicial-node-testnet-service.log"', text)
    ...
    def test_unknown_profile_is_rejected(self):
        with tempfile.TemporaryDirectory() as tmpdir:
            root = Path(tmpdir)
            etc_atipicial = root / "etc" / "atipicial"
            etc_atipicial.mkdir(parents=True)
            (etc_atipicial / "atipicial_testnet_node.toml").write_text(
                "[rpc]\nport = 20332\n",
                encoding="utf-8",
            )
            ...
            self.assertIn("unsupported ATC_PROFILE", result.stderr)
