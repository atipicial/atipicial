...
use atipicial_runtime::ServiceError;
use std::sync::Arc;
...
impl ChainSpecProvider for TestContext {
    ...
    fn chain_spec(&self) -> Arc<Self::ChainSpec> {
        atipicial_test_fixtures::test_chain_spec(atipicial_config::ProtocolSettings::default())
    }
}
...
async fn run_loop_processes_simple_command() {
    ...
    let hash = atipicial_primitives::UInt256::zero();
...
async fn handle_block_import_check_rejects_bad_empty_block_merkle_root() {
    ...
    header.set_index(1);
    header.set_merkle_root(atipicial_primitives::UInt256::from([0x42; 32]));
    let block = atipicial_payloads::Block::from_parts(header, Vec::new());

    let err = atipicial_runtime::BlockImport::check(&handle, &block)
        .await
...
async fn handle_submits_checker_typed_inventory_batches_without_exposing_command_enum() {
    ...
    let queue = atipicial_runtime::BlockImportQueue::new(Arc::new(handle.clone()), 1);
    let checked = queue
...
async fn handle_submits_extensible_inventory_without_exposing_command_enum() {
    let (handle, mut cmd_rx, _event_tx) = BlockchainHandle::channel(4, 4);
    let payload = atipicial_payloads::ExtensiblePayload::default();
...
async fn handle_add_transaction_round_trips_service_reply() {
    let (handle, mut cmd_rx, _event_tx) = BlockchainHandle::channel(4, 4);
    let transaction = atipicial_payloads::Transaction::new();
