...
//! run when `ATC_TRACE_TX` matches the current transaction,
//! `ATC_TRACE_BLOCK` contains the current block, or the opt-in slow-transaction
//! profiler is enabled. Targeted opcode/stack profiling is selected with
//! `ATC_PROFILE_VM_TX` or `ATC_PROFILE_VM_BLOCK`.
...
use atipicial_vm::{OpcodeClass, StackItem, StackItemRpcJson, VmError, VmExecutionProfile};
...
impl TraceTxFilter {
    pub(crate) fn from_env() -> Self {
        ...
        FILTER
            .get_or_init(|| {
                Self::from_raw_parts(
                    std::env::var("ATC_TRACE_TX").ok().as_deref(),
                    std::env::var("ATC_TRACE_BLOCK").ok().as_deref(),
                )
            })
            .clone()
    }
...
impl SlowTxFilter {
    pub(crate) fn from_env() -> Self {
        ...
        *FILTER.get_or_init(|| {
            Self::from_raw_parts(
                std::env::var("ATC_PROFILE_SLOW_TX_US").ok().as_deref(),
                std::env::var("ATC_PROFILE_SLOW_TX_BLOCK").ok().as_deref(),
            )
        })
    }
...
impl VmProfileFilter {
    pub(crate) fn from_env() -> &'static Self {
        ...
        FILTER.get_or_init(|| {
            Self::from_raw_parts(
                std::env::var("ATC_PROFILE_VM_TX").ok().as_deref(),
                std::env::var("ATC_PROFILE_VM_BLOCK").ok().as_deref(),
            )
        })
    }
...
pub(crate) fn trace_tx_frames<P, B>(
    engine: &ApplicationEngine<P, atipicial_execution::NoDiagnostic, B>,
) -> String
where
