...
class RunStateRootMilestonesTests(unittest.TestCase):
    ...
    def fake_probe_result(self, command: list[str]) -> SimpleNamespace | None:
        if "atipicial-db-probe" not in str(command[0]):
            return None
            ...
            "height_samples": [
                {
                    "elapsed_seconds": 0.0,
                    "height": 0,
                    "metrics": {"atipicial_sync_native_persist_avg_tx_count": 0.0},
                },
                {
                    "elapsed_seconds": 10000.0,
                    "height": 100000,
                    "metrics": {"atipicial_sync_native_persist_avg_tx_count": 1.0},
                },
            ],
            "sync_proof": {
                "sync_source": "fast-sync",
    ...
    def test_build_plan_includes_reference_checked_bounded_and_checkpoint_commands(self):
        ...
        plan = module.build_plan(
            config=Path("clean/atipicial_mainnet_validate.toml"),
            node_bin=Path("target/debug/atipicial-node"),
            rpc_url="http://127.0.0.1:21332",
            milestones=[10, 20],
            poll_interval=5.0,
            max_seconds=120.0,
            chain_db=Path("clean/chain"),
            probe_bin=Path("target/debug/atipicial-db-probe"),
            references=["http://seed1.atipicial.com:10332"],
            data_dir=Path("clean"),
        ...
        self.assertEqual(plan["node_bin"], "target/debug/atipicial-node")
        self.assertEqual(plan["probe_bin"], "target/debug/atipicial-db-probe")
        ...
        self.assertIn("clean/logs/atipicial-node-milestone-h10.log", bounded)
    ...
    def test_main_rejects_fewer_milestones_than_minimum_checkpoint_count(self):
        ...
        try:
            module.sys.argv = [
                ...
                "--config",
                "clean/atipicial_mainnet_validate.toml",
                "--milestone",
                "10",
                "--milestone",
                "20",
    ...
    def test_parse_args_keeps_user_reference_without_default_duplicates(self):
        ...
        self.assertEqual(args.reference, ["http://seed1.atipicial.com:10332"])
    ...
    def test_run_milestones_checkpoints_each_successful_height(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
                                ...
                                "height_samples": [
                                    {
                                        ...
                                        "height": 0,
                                        "metrics": {
                                            "atipicial_sync_avg_persist_us": 100.0,
                                            "atipicial_state_service_mpt_apply_avg_total_us": 10.0,
                                            "atipicial_sync_native_persist_avg_tx_count": 0.0,
                                        },
                                    },
                                    {
                                        "elapsed_seconds": 2.0,
                                        "height": 4,
                                        "metrics": {
                                            "atipicial_sync_avg_persist_us": 200.0,
                                            "atipicial_state_service_mpt_apply_avg_total_us": 20.0,
                                            "atipicial_sync_native_persist_avg_tx_count": 1.0,
                                        },
                                    },
                                    {
                                        "elapsed_seconds": 5.0,
            ...
            self.assertEqual(
                metrics_summary["metrics"]["atipicial_sync_avg_persist_us"]["last"],
                200.0,
            )
            self.assertEqual(
                metrics_summary["metrics"]["atipicial_state_service_mpt_apply_avg_total_us"]["average"],
                15.0,
            )
            ...
            probe_calls = [command for command in calls if "atipicial-db-probe" in str(command[0])]
    ...
    def test_run_milestones_restore_probes_checkpoint_before_counting_completed(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def fake_run(command, **kwargs):
                if "atipicial-db-probe" in str(command[0]):
    ...
    def test_metrics_sample_summary_ranks_hot_latency_metrics(self):
        ...
        summary = module.metrics_sample_summary(
            {
                "height_samples": [
                    {
                        "metrics": {
                            "atipicial_sync_avg_total_us": 4000,
                            ...
                            "atipicial_state_service_mpt_apply_avg_items": 700,
                        }
                    },
                    {
                        "metrics": {
                            "atipicial_sync_avg_total_us": 6000,
                            'atipicial_sync_atipicialtoken_committee_compute_stage_avg_us{stage="candidate_state_decode"}': 3100,
                            "atipicial_state_service_mpt_apply_avg_changes": 19,
                            "atipicial_state_service_mpt_apply_avg_items": 900,
                        }
                    },
                ]
            }
        ...
        self.assertEqual(hot[0]["name"], "atipicial_sync_avg_total_us")
        ...
        self.assertEqual(hot_counts[0]["name"], "atipicial_state_service_mpt_apply_avg_items")
        self.assertEqual(hot_counts[0]["average"], 800.0)
        self.assertEqual(hot_counts[1]["name"], "atipicial_state_service_mpt_apply_avg_changes")
    ...
    def test_append_summary_jsonl_writes_one_compact_history_record(self):
        module = load_module()
        plan = {
            "config": "clean/atipicial_mainnet_validate.toml",
            "node_bin": "target/release/atipicial-node",
            "probe_bin": "target/release/atipicial-db-probe",
        ...
        self.assertEqual(record["config"], "clean/atipicial_mainnet_validate.toml")
        self.assertEqual(record["node_bin"], "target/release/atipicial-node")
        self.assertEqual(record["probe_bin"], "target/release/atipicial-db-probe")
