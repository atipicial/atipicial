//! # atipicial-test-fixtures
//!
//! Shared test fixtures for Atipicial workspace integration tests.
//!
...
//! copy-pasted fixture code that was duplicated between `atipicial-rpc`'s
...
//! ## Contents
//!
//! - [`TestTransactionBuilder`]: fluent builder for Atipicial transactions with
...
use atipicial_payloads::header::Header as BlockHeader;
...
use atipicial_storage::persistence::{Store, StoreCache};
...
/// runtime root.
#[must_use]
pub fn test_chain_spec(settings: atipicial_config::ProtocolSettings) -> Arc<atipicial_config::AtipicialChainSpec> {
    let committee: Vec<String> = settings
        .standby_committee
        .iter()
        .map(|key| atipicial_primitives::hex_util::encode_hex(key.as_bytes()))
        .collect();
    ...
    let validators = committee
        ...
        .cloned()
        .map(|public_key| atipicial_config::GenesisValidator {
            public_key,
            name: None,
        })
        .collect();
    let genesis = atipicial_config::GenesisConfig {
        timestamp: atipicial_config::GENESIS_TIMESTAMP_MS,
        nonce: atipicial_config::GENESIS_NONCE,
        validators,
        committee,
    ...
    Arc::new(
        atipicial_config::AtipicialChainSpec::private("workspace-test", settings, genesis, None)
    ...
    S: Store,
{
    ...
    let merkle_root = if transactions.is_empty() {
        ...
        atipicial_crypto::MerkleTree::compute_root(&hashes).unwrap_or_else(UInt256::zero)
    };
