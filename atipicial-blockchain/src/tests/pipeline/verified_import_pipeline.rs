...
use atipicial_payloads::{Block, Header, Transaction, Witness};
...
> {
    VerifiedImportPipeline::new(
        atipicial_test_fixtures::test_chain_spec(ProtocolSettings::default()),
...
fn verified_import_pipeline_requires_explicit_native_provider() {
    ...
    assert!(
        source.contains(
            "consensus_witness: AtipicialConsensusWitnessStage<SnapshotConsensusWitnessContext<P, B>>"
        ),
    ...
    assert!(context.contains("ChainSpecProvider<ChainSpec = AtipicialChainSpec>"));
    assert!(
...
fn cached_aborting_block(snapshot: &DataCache) -> (Block, Arc<PreverifiedSignatureCache>) {
    ...
    let sign_data =
        atipicial_payloads::get_sign_data_vec(&child_header, settings.network).expect("child sign data");
