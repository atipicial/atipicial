...
where
    S: atipicial_storage::persistence::store::Store + 'static,
{
...
pub(in crate::node) fn validate_state_service_storage(
    ...
    canonical_store: &Arc<atipicial_storage::persistence::providers::RuntimeStore>,
) -> anyhow::Result<()> {
    if !config.state_service.enabled {
        return Ok(());
    }
        ...
        && canonical_store.as_mdbx().is_some()
    {
        ...
        let label = format!(
            ...
            atipicial_state_service::MDBX_STATE_SERVICE_NAMESPACE
        );
...
{
    use atipicial_storage::persistence::RawReadOnlyStore;
...
fn read_state_service_mpt_height(
    ...
    use atipicial_storage::persistence::StoreFactory;
...
pub(in crate::node) fn open_store(
    config: &NodeConfig,
    storage_override: Option<&Path>,
) -> anyhow::Result<Arc<atipicial_storage::persistence::providers::RuntimeStore>> {
    use atipicial_storage::persistence::StoreFactory;
    ...
    let store = if let Some(provider) = provider {
        ...
        info!(target: "atipicial", backend = provider, path = %path.display(), "opening persistent store");
        ...
        info!(target: "atipicial", "using in-memory store (state is not persisted across restarts)");
        open_memory_store()?
    };
