...
use super::Notary;
use crate::{AtipicialDollar, LedgerContract, Role, RoleManagement};
use atipicial_error::{CoreError, CoreResult};
use atipicial_execution::ApplicationEngine;
use atipicial_payloads::get_sign_data;
use atipicial_primitives::{TransactionAttributeType, WitnessScope};
...
impl Notary {
    ...
    >(
        ...
        let from = Self::parse_account(args, crate::AEP17_PAYMENT_METHOD)?;
        let amount =
            crate::args::raw_required_integer_arg(args, 1, "Notary::onAEP17Payment", "an amount")?;
        let data = args.get(2).map(Vec::as_slice).unwrap_or(&[]);

        if engine.get_calling_script_hash() != Some(AtipicialDollar::script_hash()) {
            return Err(CoreError::invalid_operation(
                "Notary::onAEP17Payment: only ATD can be accepted for deposit",
            ));
        }
        let (to, till) = Self::parse_onnep17_data(&from, data)?;
        // C# `allowedChangeTill = tx.Sender == to`; the script container is
        // the persisting transaction (the ATD transfer that triggered this).
        let sender = engine
            .script_container()
    ...
    >(
        ...
        let ok = AtipicialDollar::transfer_core(
            engine,
            notary_hash,
            &notary_hash,
            &receive,
