...
use atipicial_payloads::{Signer, Transaction, TransactionAttribute, WitnessScope};

use atipicial_primitives::UInt160;
use atipicial_tests::mempool::{Mempool, MempoolConfig};
use atipicial_tests::state::{
    AccountState, MemoryWorldState, StateChanges, StorageItem, StorageKey, WorldState,
};
use atipicial_vm::OpCode;

// Creates a test account with ATC and ATD balances
fn create_test_account(atipicial_balance: u64, gas_balance: u64) -> (UInt160, AccountState) {
    let hash = UInt160::from([0x01u8; 20]);
    let account = AccountState::with_balances(hash, atipicial_balance, gas_balance);
    (hash, account)
}
...
async fn test_transaction_state_changes() {
    ...
    assert_eq!(sender_state.atipicial_balance(), 1000);
