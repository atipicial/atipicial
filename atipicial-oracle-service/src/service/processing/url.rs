    ...
    P: NativeContractProvider + OracleContractReadProvider,
{
    pub(in super::super) async fn process_url(
        ...
        // Handle AtipicialFs protocol
        if url.len() >= 6 && url[..6].eq_ignore_ascii_case("atipicialfs:") {
            #[cfg(feature = "oracle")]
            {
                return self.atipicialfs.process(&self.settings, url, _oracle_key).await;
            }
        ...
        if scheme.eq_ignore_ascii_case("atipicialfs") {
            #[cfg(feature = "oracle")]
            {
                return self.atipicialfs.process(&self.settings, url, _oracle_key).await;
            }
