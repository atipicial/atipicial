...
```bash
./target/release/atipicial-node --network testnet --data-dir ./data/testnet
```
...
```json
{
    ...
    "p2p": {
        "port": 20333,
        "seeds": ["seed1t5.atipicial.com:20333", "seed2t5.atipicial.com:20333", "seed3t5.atipicial.com:20333"]
    }
}
```
...
```bash
./target/release/atipicial-node --config config.testnet.json
```
...
```bash
# C# node
curl -X POST http://atipicial-testnet-node:20332 \
  -H 'Content-Type: application/json' \
  -d '{"jsonrpc":"2.0","method":"getblockcount","params":[],"id":1}'
```
