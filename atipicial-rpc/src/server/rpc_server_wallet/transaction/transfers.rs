...
use atipicial_payloads::transaction_attribute::TransactionAttribute;
#[cfg(test)]
use atipicial_primitives::WitnessScope;
use atipicial_primitives::{BigDecimal, UInt160};
use atipicial_vm::OpCode;
use atipicial_wallets::{AssetDescriptor, Aep6Wallet, TransferOutput};
...
impl RpcServerWallet {
    pub(in crate::server::rpc_server_wallet) fn send_from(
        server: &RpcServer,
        params: &[Value],
    ...
    pub(in crate::server::rpc_server_wallet) fn send_to_address(
        server: &RpcServer,
        params: &[Value],
    ) -> Result<Value, RpcException> {
        let _ = Self::require_wallet(server)?;
    ...
    pub(in crate::server::rpc_server_wallet) fn send_many(
        server: &RpcServer,
        params: &[Value],
    ) -> Result<Value, RpcException> {
        ...
        let descriptor_cache = |asset: &UInt160| {
            AssetDescriptor::new(Arc::clone(&descriptor_snapshot), &reader, *asset)
                .map_err(|err| atipicial_error::CoreError::other(err.to_string()))
        };
    ...
    pub(in crate::server::rpc_server_wallet) fn cancel_transaction(
        server: &RpcServer,
        params: &[Value],
    ) -> Result<Value, RpcException> {
        let request =
    ...
    #[cfg(test)]
    pub(in crate::server::rpc_server_wallet) fn parse_signers(
        server: &RpcServer,
        value: &Value,
    ...
    #[cfg(test)]
    pub(in crate::server::rpc_server_wallet) fn parse_signer_array(
        server: &RpcServer,
        array: &[Value],
    ...
    fn parse_send_many_output(
        server: &RpcServer,
        descriptor_cache: &impl Fn(&UInt160) -> atipicial_error::CoreResult<AssetDescriptor>,
        index: usize,
    ...
    pub(in crate::server::rpc_server_wallet) fn parse_positive_amount(
        text: &str,
        decimals: u8,
