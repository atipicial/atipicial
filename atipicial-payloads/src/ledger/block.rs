...
impl Block {
    ...
    pub fn verify_merkle_root(&self) -> bool {
        ...
        if let Some(root) = atipicial_crypto::MerkleTree::compute_root(&payload_hashes) {
...
impl atipicial_primitives::BlockLike for Block {
...
impl atipicial_primitives::SerializablePayload for Block {
...
atipicial_io::impl_default_via_new!(Block);

impl atipicial_primitives::Verifiable for Block {
...
impl Serializable for Block {
    ...
    fn deserialize(reader: &mut MemoryReader) -> IoResult<Self> {
        let header = <Header as atipicial_io::Serializable>::deserialize(reader)?;
        let tx_count = reader.read_var_int(usize::MAX as u64)? as usize;
        if tx_count > atipicial_primitives::constants::BLOCK_MAX_TX_WIRE_LIMIT {
        ...
        for _ in 0..tx_count {
            let tx = <Transaction as atipicial_io::Serializable>::deserialize(reader)?;
            let hash = tx
                .try_hash()
        ...
        let merkle_root = if hashes.is_empty() {
            ...
            atipicial_crypto::MerkleTree::compute_root(&hashes)
