...
pub enum WireError {
    ...
    /// (matches C# `Atipicial.Network.P2P.Message.PayloadMaxSize`).
    ...
    /// An underlying atipicial-io error occurred while serialising a payload.
    #[error("serialization error: {0}")]
    Serialization(String),
}

impl From<atipicial_io::IoError> for WireError {
    fn from(err: atipicial_io::IoError) -> Self {
        Self::Serialization(err.to_string())
    }
}

atipicial_error::impl_error_from_struct!(atipicial_error::CoreError, WireError => Network);
