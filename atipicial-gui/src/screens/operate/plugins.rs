...
//!
//! In atipicial-rs, "plugins" are compile-time Cargo features of `atipicial-node` (RPC
...
const PLUGINS: &[(&str, &str, &str)] = &[
    (
        "RpcServer",
        "API",
        "JSON-RPC server (atipicial-rpc/server) — always on in the default node build.",
    ),
    (
        "OracleService",
        "Core",
        "HTTPS + AtipicialFs oracle request fulfilment (atipicial-oracle-service).",
    ),
    (
        "StateService",
        "Core",
        "MPT state root, proofs (getproof/getstate) (atipicial-state-service).",
    ),
    (
        "ApplicationLogs",
        "Core",
    ...
    (
        "TokensTracker",
        "API",
        "AEP-11/AEP-17 balance + transfer indexing (merged into atipicial-rpc).",
    ),
    (
        "DBFTPlugin",
        "Consensus",
        "dBFT 2.0 consensus driver (atipicial-consensus).",
    ),
];

pub fn ui(app: &mut AtipicialGuiApp, ui: &mut Ui) {
    ui.heading("Plugins");
    ui.add_space(6.0);
    ui.label(
        egui::RichText::new(
            "Node plugins are compile-time features in atipicial-rs. Query the node for what it exposes.",
        )
