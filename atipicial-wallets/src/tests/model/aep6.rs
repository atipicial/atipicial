//! Tests for AEP-6 (de)serialization parity with C# `Atipicial.Wallets.AEP6`.
...
fn sample_contract_file_base64() -> Aep6ContractFile {
    Aep6ContractFile {
...
#[test]
fn aep6_contract_script_is_base64_not_hex_on_serialize() {
    ...
    let file = Aep6Contract::to_file(&contract, &["signature".to_string()], false);

    // C# AEP6Contract.ToJson emits Base64: `Convert.ToBase64String(Script)`.
    let expected_b64 = general_purpose::STANDARD.encode(SCRIPT_BYTES);
    assert_eq!(
        file.script, expected_b64,
        "AEP-6 contract.script must be Base64-encoded to interoperate with atipicial-cli/atipicial-gui"
    );

    // Guard against a regression back to hex.
    let hex = atipicial_primitives::hex_util::encode_hex(&SCRIPT_BYTES);
    assert_ne!(
        file.script, hex,
        "AEP-6 contract.script must NOT be hex-encoded (C# uses Base64)"
    );
}

#[test]
fn aep6_contract_round_trips_through_base64() {
    let original = Contract::create(
        vec![ContractParameterType::Signature],
        SCRIPT_BYTES.to_vec(),
    );

    let file = Aep6Contract::to_file(&original, &["signature".to_string()], false);
    let parsed = Aep6Contract::from_file(&file).expect("round-trip parse must succeed");
...
#[test]
fn aep6_contract_serializes_real_deployed_flag() {
    // C# AEP6Contract.ToJson writes the real `Deployed` (AEP6Contract.cs:47); a
    ...
    let file_deployed = Aep6Contract::to_file(&contract, &["signature".to_string()], true);
    assert!(
        file_deployed.deployed,
        "to_file must serialize the real deployed flag (true), not a hardcoded false"
    );
    assert!(
        Aep6Contract::from_file(&file_deployed)
            .expect("parse")
            .deployed,
    ...
    let file_local = Aep6Contract::to_file(&contract, &["signature".to_string()], false);
    assert!(!file_local.deployed, "deployed=false must stay false");
}

#[test]
fn aep6_contract_parses_csharp_shaped_base64_fixture() {
    // A minimal, hand-written AEP6Contract JSON exactly as C# atipicial-cli would write it:
    ...
    let file: Aep6ContractFile =
        serde_json::from_value(json).expect("C#-shaped contract JSON must deserialize");
    let parsed = Aep6Contract::from_file(&file).expect("C#-shaped contract must parse");
...
#[test]
fn aep6_contract_rejects_hex_script_as_base64() {
    // A hex-encoded script (the OLD, buggy atipicial-rs format) is not valid Base64 for these
    ...
    assert!(
        Aep6Contract::from_file(&file).is_err(),
