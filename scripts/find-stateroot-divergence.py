#!/usr/bin/env python3
"""
Binary-search for the first block where atipicial-rs and the C# reference node
...
Usage:
    ...
    python3 scripts/find-stateroot-divergence.py \
        ...
        --reference http://seed1.atipicial.com:10332 \
        --max-height 295097
"""
...
def start_local_node(repo_dir: str) -> subprocess.Popen | None:
    """Start the atipicial-node and return the Popen handle."""
    binary = os.path.join(repo_dir, "target/release/atipicial-node")
    ...
    env = os.environ.copy()
    env["ATC_LOG_LEVEL"] = "warn"

    print(f"Starting atipicial-node: {binary} --config {config} --enable-stateroot")
...
def main():
    parser = argparse.ArgumentParser(
        description="Find the first block where atipicial-rs state root diverges "
    ...
    parser.add_argument("--repo-dir", default="/home/atipicial/git/atipicial-rs",
                        help="Path to atipicial-rs repo")
