...
/// `atipicial_testnet_node.toml`) parses, derives the TestNet protocol
...
seed_nodes = ["seed1t5.atipicial.com:20333", "seed2t5.atipicial.com:20333"]
...
"#;
    ...
    let path = dir.join(format!("atipicial_node_cfg_test_{}.toml", std::process::id()));
...
fn load_config_rejects_builtin_chain_setting_overrides() {
    ...
    for (index, (section, expected_field)) in cases.into_iter().enumerate() {
        let path = dir.join(format!(
            "atipicial_node_protocol_override_{}_{}.toml",
            std::process::id(),
            index
        ));
...
fn mempool_capacity_is_validated_operator_policy() {
    assert_eq!(
        NodeConfig::default()
            ...
            .max_transactions(),
        atipicial_mempool::DEFAULT_MAX_TRANSACTIONS
    );
...
"#;
    ...
    assert_eq!(rpc.rpc_user, "atipicial");
...
/// The public-network presets should mirror the Atipicial v3.10.1
...
fn shipped_public_configs_match_v3101_p2p_channel_defaults() {
    ...
    let cases = [
        ...
        ("atipicial_mainnet_node.toml", Some(10333)),
        ("atipicial_production_node.toml", Some(10333)),
        ("config/testnet.toml", Some(20333)),
        ("config/testnet-service.toml", Some(20333)),
        ("atipicial_testnet_node.toml", Some(20333)),
    ];
...
fn shipped_mdbx_configs_pin_geometry_defaults() {
    ...
    let cases = [
        ...
        "atipicial_mainnet_node.toml",
        "atipicial_production_node.toml",
        "config/testnet.toml",
        "config/testnet-service.toml",
        "atipicial_testnet_node.toml",
    ];
...
fn load_config_missing_file_uses_defaults() {
    let path = PathBuf::from("/nonexistent/atipicial-node/definitely-missing.toml");
    let (chain_spec, config) = load_config(&path, None).expect("defaults");
    assert_eq!(
        chain_spec.identity().network_type(),
        atipicial_config::NetworkType::MainNet
    );
...
fn load_config_rejects_mismatched_magic_override() {
    let path = PathBuf::from("/nonexistent/atipicial-node/missing.toml");
...
fn node_cli_verified_chain_import_is_explicit_and_requires_an_archive() {
    ...
    NodeCli::try_parse_from(["atipicial-node", "--verify-import-chain"])
...
fn node_cli_state_root_is_default_off_and_has_explicit_overrides() {
    let default = NodeCli::try_parse_from(["atipicial-node"]).expect("parse defaults");
    assert_eq!(default.stateroot_override(), None);
    assert!(!default.stateroot_enabled());

    let enabled = NodeCli::try_parse_from(["atipicial-node", "--enable-stateroot"])
        .expect("parse StateRoot enable flag");
    assert_eq!(enabled.stateroot_override(), Some(true));
    assert!(enabled.stateroot_enabled());

    let explicit_true = NodeCli::try_parse_from(["atipicial-node", "--stateroot", "true"])
        .expect("parse explicit StateRoot true");
    assert_eq!(explicit_true.stateroot_override(), Some(true));

    let explicit_false = NodeCli::try_parse_from(["atipicial-node", "--stateroot", "false"])
    ...
    NodeCli::try_parse_from(["atipicial-node", "--stateroot", "maybe"])
        .expect_err("malformed StateRoot boolean must fail");
}

#[test]
fn node_cli_rejects_conflicting_state_root_overrides() {
    NodeCli::try_parse_from(["atipicial-node", "--enable-stateroot", "--stateroot", "false"])
...
fn node_cli_resolves_state_root_startup_mode_fail_closed() {
    ...
    let disabled = NodeCli::try_parse_from(["atipicial-node", "--stateroot", "false"])
