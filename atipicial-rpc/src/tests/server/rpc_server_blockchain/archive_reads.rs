...
use atipicial_blockchain::{BlockProvider, OptionalStaticLedgerProvider, StaticLedgerArchiveFactory};
...
impl ArchiveFixtureDir {
    fn new() -> Self {
        ...
        let path = std::env::temp_dir().join(format!(
            "atipicial-rpc-ledger-archive-{}-{id}",
            std::process::id()
        ));
...
async fn get_block_reads_configured_archive_after_hot_row_pruning() {
    ...
    let decoded =
        <atipicial_payloads::Block as Serializable>::deserialize(&mut reader).expect("decode block");
