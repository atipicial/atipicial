...
pub fn atipicialfs_json_header(header: &atipicialfs_v2::object::Header) -> Option<String> {
    ...
    if let Some(container) = header.container_id.as_ref() {
        if let Some(json) = atipicialfs_json_container_id(container) {
            push_json_field(&mut out, &mut first, "containerID", &json);
        }
    }

    if let Some(owner) = header.owner_id.as_ref() {
        if let Some(json) = atipicialfs_json_owner_id(owner) {
    ...
    if let Some(hash) = header.payload_hash.as_ref() {
        if let Some(json) = atipicialfs_json_checksum(hash) {
            push_json_field(&mut out, &mut first, "payloadHash", &json);
        }
    }

    let object_type = match atipicialfs_v2::object::ObjectType::try_from(header.object_type) {
        Ok(atipicialfs_v2::object::ObjectType::Regular) => "REGULAR",
        Ok(atipicialfs_v2::object::ObjectType::Lock) => "LOCK",
        Ok(atipicialfs_v2::object::ObjectType::Tombstone) => "TOMBSTONE",
        _ => "UNKNOWN",
    };
...
fn atipicialfs_json_container_id(id: &atipicialfs_v2::refs::ContainerId) -> Option<String> {
...
fn atipicialfs_json_checksum(checksum: &atipicialfs_v2::refs::Checksum) -> Option<String> {
    use base64::Engine as _;
    let sum = base64::engine::general_purpose::STANDARD.encode(&checksum.sum);
    let hash_type = match atipicialfs_v2::refs::ChecksumType::try_from(checksum.r#type) {
        Ok(atipicialfs_v2::refs::ChecksumType::Sha256) => "SHA256",
        _ => "UNKNOWN",
    };
    Some(format!(
