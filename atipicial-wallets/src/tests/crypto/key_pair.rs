...
#[test]
fn aep2_uses_canonical_checksig_verification_script() {
    // Regression: the AEP-2 salt script must be the canonical CheckSig
    ...
    // A standard '6P…' AEP-2 string round-trips at the N3 address version.
    let aep2 = kp.to_aep2("Satoshi", 0x09).unwrap();
    assert!(aep2.starts_with("6P"));
    let restored = KeyPair::from_aep2_string(&aep2, "Satoshi", 0x09).unwrap();
...
/// AEP-2 export must use Base58Check (standard "6P..." prefix), and AES-256-ECB,
...
#[test]
fn test_aep2_round_trip_uses_standard_base58check_format() {
    // N3 address version (0x09).
    const N3_ADDRESS_VERSION: u8 = 0x09;
    let key_pair = KeyPair::generate().unwrap();
    let password = "Satoshi";

    let aep2 = key_pair.to_aep2(password, N3_ADDRESS_VERSION).unwrap();
    // AEP-2 (prefix bytes 0x01 0x42) Base58Check-encodes to a "6P..." string.
    // A base64 encoding would never start with "6P".
    assert!(
        aep2.starts_with("6P"),
        "AEP-2 string must be Base58Check ('6P...'), got: {aep2}"
    );

    let restored = KeyPair::from_aep2_string(&aep2, password, N3_ADDRESS_VERSION).unwrap();
    assert_eq!(key_pair.private_key(), restored.private_key());

    // Wrong password must fail the address-hash verification.
    assert!(KeyPair::from_aep2_string(&aep2, "wrong", N3_ADDRESS_VERSION).is_err());
}

/// A AEP-6 wallet carries its own `ScryptParameters`; a key encrypted with
/// non-default parameters must round-trip only with the SAME parameters, and the
/// AEP-6 default (16384/8/8) must reject it — matching C# `AEP6Account`, which
/// threads `wallet.Scrypt.N/R/P` into encrypt/decrypt.
#[test]
fn aep2_honors_non_default_scrypt_parameters() {
    const N3_ADDRESS_VERSION: u8 = 0x09;
    let key_pair = KeyPair::generate().unwrap();
    let password = "Satoshi";
    ...
    let aep2 = key_pair
        .to_aep2_with_params(password, N3_ADDRESS_VERSION, n, r, p)
        .unwrap();
    assert!(aep2.starts_with("6P"));

    // Same parameters -> recovers the key.
    let restored =
        KeyPair::from_aep2_string_with_params(&aep2, password, N3_ADDRESS_VERSION, n, r, p)
            .unwrap();
    assert_eq!(key_pair.private_key(), restored.private_key());
    ...
    assert!(
        KeyPair::from_aep2_string(&aep2, password, N3_ADDRESS_VERSION).is_err(),
        "default scrypt params must reject a non-default-encrypted AEP-2 key"
    );
}
