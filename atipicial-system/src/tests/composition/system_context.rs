...
use atipicial_config::ChainSpecProvider;
use atipicial_execution::ExecutionArtifactLimits;
use atipicial_execution::specialization::SpecializationControl;
use atipicial_native_contracts::StandardNativeProvider;
use atipicial_storage::persistence::providers::MemoryStore;
use atipicial_storage::persistence::{StoreCache, TransactionalStore};
...
fn commit_to_store_flushes_the_canonical_snapshot() {
    ...
    let snapshot = Arc::new(store_cache.data_cache().clone());
    let context = NodeSystemContext::new(
        atipicial_config::AtipicialChainSpec::mainnet().expect("mainnet chain spec"),
        Arc::clone(&snapshot),
        store_cache,
        Arc::new(StandardNativeProvider::new()),
        Arc::new(NoopBlockCommitHooks),
...
fn context_reuses_composed_specialization_control_across_resource_requests() {
    ...
    let context = NodeSystemContext::new_with_ledger_provider_and_optional_native_persist_resources(
        ...
        provider,
        Some(resources),
        atipicial_blockchain::OptionalStaticLedgerProvider::default(),
        Arc::new(NoopBlockCommitHooks),
    );
...
fn replay_artifact_policy_is_owned_by_composed_commit_hooks() {
    let chain_spec = atipicial_config::AtipicialChainSpec::mainnet().expect("mainnet chain spec");
    let block = atipicial_blockchain::genesis_block(chain_spec.as_ref()).expect("genesis block");
    ...
    assert!(
        !observer_free
            .requires_replay_artifacts(&block, atipicial_blockchain::BlockPersistContext::live())
    );
    ...
    assert!(
        conservative.requires_replay_artifacts(&block, atipicial_blockchain::BlockPersistContext::live())
    );
}
