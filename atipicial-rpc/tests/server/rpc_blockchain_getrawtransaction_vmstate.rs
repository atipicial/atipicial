...
use atipicial_rpc::server::{RpcHandler, RpcServer, RpcServerBlockchain, RpcServerConfig};
use atipicial_storage::persistence::providers::RuntimeStore;
use atipicial_test_fixtures::{TestTransactionBuilder, try_make_ledger_block, try_store_block};
use serde_json::Value;
use std::sync::Arc;

fn node_to_context(node: &atipicial_system::Node) -> atipicial_rpc::server::NodeContext {
...
async fn get_raw_transaction_verbose_omits_vmstate() {
    let node = atipicial_system::Node::for_test(atipicial_test_fixtures::test_chain_spec(
        atipicial_config::ProtocolSettings::default(),
    ));
    let system: std::sync::Arc<atipicial_rpc::server::NodeContext> =
