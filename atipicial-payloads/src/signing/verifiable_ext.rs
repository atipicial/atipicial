...
//! Minimal extension trait for [`atipicial_primitives::Verifiable`] containers.
//!
//! This trait exposes only state-independent payload facts. Execution-backed
//! witness verification belongs to `atipicial-execution`; node services such as
...
/// Extension of [`atipicial_primitives::Verifiable`] for the payload layer.
///
/// Payload types implement this without depending on storage or the
/// application engine.
pub trait VerifiableExt: atipicial_primitives::Verifiable {
    /// Returns the script hashes that should be verified for this container.
    fn script_hashes_for_verifying(&self) -> Vec<atipicial_primitives::UInt160> {
        Vec::new()
    }
