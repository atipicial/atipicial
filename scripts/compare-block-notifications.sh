...
#   2. Imports blocks with ATC_TRACE_BLOCK to capture notification traces
...
#   NODE_BIN       Path to atipicial-node binary (default: auto-detect release/debug)
#   NODE_CONFIG    Node config file (default: atipicial_mainnet_node.toml)
#   ACC_FILE       Path to .acc block file (default: auto-detect)
#   DATA_DIR       MDBX data directory to clean (default: ./data/mainnet)
#   CSHARP_RPC     C# reference RPC (default: http://seed1.atipicial.com:10332)
#   TRACE_LOG      Output trace log path (default: /tmp/atipicial_trace_RANGE.log)
...
if [[ $# -lt 1 ]] || [[ "$1" == "-h" ]] || [[ "$1" == "--help" ]]; then
    ...
    echo "  NODE_BIN       Path to atipicial-node binary" >&2
    echo "  NODE_CONFIG    Node config file (default: atipicial_mainnet_node.toml)" >&2
    echo "  ACC_FILE       Path to .acc block file" >&2
    echo "  DATA_DIR       MDBX data directory to clean" >&2
    echo "  CSHARP_RPC     C# reference RPC (default: http://seed1.atipicial.com:10332)" >&2
...
NODE_CONFIG="${NODE_CONFIG:-atipicial_mainnet_node.toml}"
CSHARP_RPC="${CSHARP_RPC:-http://seed1.atipicial.com:10332}"
TRACE_LOG="${TRACE_LOG:-/tmp/atipicial_trace_${RANGE_START}-${RANGE_END}.log}"
SKIP_IMPORT="${SKIP_IMPORT:-0}"

# Auto-detect node binary
if [[ -n "${NODE_BIN:-}" ]]; then
    : # use provided
elif [[ -x "$ROOT/target/release/atipicial-node" ]]; then
    NODE_BIN="$ROOT/target/release/atipicial-node"
elif [[ -x "$ROOT/target/debug/atipicial-node" ]]; then
    NODE_BIN="$ROOT/target/debug/atipicial-node"
else
    echo "ERROR: atipicial-node binary not found. Build with: cargo build --release -p atipicial-node" >&2
    exit 1
fi
...
echo "======================================================================"
echo "  Atipicial-RS Transaction Notification Comparison"
echo "======================================================================"
...
if [[ "$SKIP_IMPORT" != "1" ]]; then
    ...
    echo "  ATC_TRACE_BLOCK=${RANGE_START}-${RANGE_END}"
    echo "  ATC_IMPORT_STOP_HEIGHT=${STOP_HEIGHT}"
    ...
    set +e
    ATC_TRACE_BLOCK="${RANGE_START}-${RANGE_END}" \
    ATC_IMPORT_STOP_HEIGHT="${STOP_HEIGHT}" \
    RUST_LOG="${RUST_LOG:-atipicial=warn}" \
    ...
    if [[ "$EXEC_LINES" -eq 0 ]]; then
        ...
        echo "  - The ATC_TRACE_BLOCK env var was not picked up"
        echo ""
    fi
else
