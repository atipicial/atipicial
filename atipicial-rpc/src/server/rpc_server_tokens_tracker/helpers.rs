...
use crate::plugins::tokens_tracker::{
    Aep11TransferKey, Aep17TransferKey, TokenTransfer, TokensTrackerService, find_range,
};
...
use super::response::{aep11_transfer_entry, transfer_entries, transfer_entry};
...
pub(super) fn collect_transfers(
    store: &impl atipicial_storage::persistence::Store,
    prefix: u8,
    script_hash: &UInt160,
    start: u64,
    end: u64,
    ...
    collect_transfer_entries::<Aep17TransferKey, _, _>(
        store,
        prefix,
        script_hash,
        start,
...
pub(super) fn collect_nep11_transfers(
    ...
    collect_transfer_entries::<Aep11TransferKey, _, _>(
        store,
        prefix,
        script_hash,
        start,
        ...
        max_results,
        |key, value| aep11_transfer_entry(key, value, address_version),
    )
}
    ...
    B: CacheRead,
{
    ...
    let mut engine =
        atipicial_execution::ApplicationEngine::new_with_shared_block_and_native_contract_provider(
