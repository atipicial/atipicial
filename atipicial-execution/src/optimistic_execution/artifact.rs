...
use atipicial_primitives::{TriggerType, UInt160, UInt256, Verifiable};
use atipicial_storage::{CacheRead, DataCache, Trackable};
...
impl SpeculativeEntryScriptIdentity {
    ...
    /// Exact AtipicialVM bytecode selected for the entry context.
    #[must_use]
    ...
    /// AtipicialVM return-value count configured for the entry context.
    #[must_use]
...
impl SpeculativeExecutionIdentity {
    ...
    /// AtipicialVM limits used by the execution.
    #[must_use]
...
pub enum SpeculativeIdentityComponent {
    ...
    Protocol,
    /// AtipicialVM execution limits differ.
    VmLimits,
...
pub struct SpeculativeStorageEffect {
    key: atipicial_storage::StorageKey,
    trackable: Trackable,
}
...
impl SpeculativeStorageEffect {
    /// Exact storage key.
    #[must_use]
    pub const fn key(&self) -> &atipicial_storage::StorageKey {
        &self.key
    }
...
pub struct SpeculativeExecutionArtifact<P, D, B = atipicial_storage::EmptyCacheBacking>
where
    P: NativeContractProvider + 'static,
    D: Diagnostic + 'static,
    B: CacheRead,
    ...
    B: CacheRead,
{
    ...
    /// the terminal AtipicialVM result and the immutable artifact snapshot.
    ...
    pub(super) fn vm_state(&self) -> atipicial_vm::VmState {
        self.engine.state()
    }
    ...
    ///
    /// Storage effects stay in the canonical [`atipicial_storage::Trackable`]
    ...
    pub(super) fn into_application_parts(
        ...
        Vec<(atipicial_storage::StorageKey, Trackable)>,
    ) {
        let effects = self
            .storage_effects
            .into_iter()
...
where
    ...
    B: CacheRead,
{
    // Loading the entry script intentionally leaves AtipicialVM in BREAK. That is
    ...
    let component = if !matches!(
        engine.state(),
        atipicial_vm::VmState::NONE | atipicial_vm::VmState::BREAK
    ) {
        Some("VM state")
...
mod tests {
    ...
    ) -> atipicial_vm::VmResult<()> {
        panic!("intentional optimistic capture panic");
    }

    fn panic_script() -> Vec<u8> {
        let mut script = vec![atipicial_vm::OpCode::SYSCALL.byte()];
        script.extend_from_slice(&atipicial_vm::interop_hash(PANIC_SYSCALL).to_le_bytes());
        script.push(atipicial_vm::OpCode::RET.byte());
        script
    }

    fn make_engine(
        cache: Arc<atipicial_storage::DataCache<atipicial_storage::EmptyCacheBacking>>,
    ) -> ApplicationEngine {
    ...
    fn identity_reports_first_stable_mismatch() {
        ...
        engine
            .load_script(
                vec![atipicial_vm::OpCode::RET.byte()],
                atipicial_primitives::CallFlags::ALL,
                None,
            )
            .expect("entry script");
    ...
    fn binding_rejects_second_engine_and_captures_host_observations() {
        ...
        let prefix = super::super::PinnedBlockPrefix::capture(
            super::super::BlockPrefixIdentity::new(atipicial_primitives::UInt256::default(), 1, 0),
            &base,
        );
        ...
        binding
            .record_native_cache(
                crate::host_access_audit::NativeCacheAccess::new(
                    atipicial_vm::NativeCacheDomain {
                        contract_hash: atipicial_primitives::UInt160::zero(),
                        contract_id: 1,
                        native_version: 1,
                        partition: 1,
                    },
        ...
        binding.observations.lock().record(|journal| {
            journal.record_call(
                crate::host_access_audit::ContractCallAccess::new(
                    ...
                    atipicial_vm::ContractResolutionIdentity::new(
                        atipicial_primitives::UInt160::zero(),
                        1,
                        0,
                        0,
                    ),
        ...
        binding.observations.lock().record(|journal| {
            journal.record_storage_range(
                crate::host_access_audit::StorageRangeAccess::prefix(
                    7,
                    b"range".to_vec(),
                    atipicial_vm::RangeDirection::Forward,
                    atipicial_primitives::FindOptions::None,
                    4,
                ),
                Vec::new(),
            )
    ...
    fn execution_panic_unbinds_and_seals_all_observation_state() {
        ...
        engine
            .register_host_service(
                PANIC_SYSCALL,
                0,
                atipicial_primitives::CallFlags::NONE,
        ...
        binding.bind(&mut engine).expect("binding");
        engine
            .load_script(panic_script(), atipicial_primitives::CallFlags::ALL, None)
            .expect("entry script");
