...
async fn canonical_admission_reports_already_exists_for_persisted_ledger_tx() {
    ...
    tx.set_signers(vec![atipicial_payloads::Signer::new(
        atipicial_primitives::UInt160::from_bytes(&[0x22; 20]).expect("signer"),
    ...
    let value = atipicial_native_contracts::LedgerContract::new()
        .serialize_persisted_transaction_state(7, atipicial_vm::VmState::HALT, &tx)
        .expect("transaction state");
    snapshot.add(
        atipicial_storage::StorageKey::new(atipicial_native_contracts::LedgerContract::ID, key),
        atipicial_storage::StorageItem::from_bytes(value),
    );
...
async fn canonical_admission_reports_traceable_ledger_conflict() {
    ...
    let signer = atipicial_primitives::UInt160::from_bytes(&[0x44; 20]).expect("signer");
