...
//!
//! The daemon owns polling and shutdown policy. `atipicial-network` owns correlated
//! range transport/retry, `atipicial-system` owns durable header/body stage
//! composition, and `atipicial-blockchain` remains the only canonical importer.

use std::collections::HashSet;
use std::sync::Arc;
use std::time::Duration;

use atipicial_network::{BlockDownloadPeer, HeaderRequest, PeerId};
use atipicial_runtime::{ServiceError, ServiceResult};
...
pub(super) async fn run_staged_sync<C, H>(
    blockchain: atipicial_blockchain::BlockchainHandle,
    pipeline: Arc<atipicial_system::StagedSyncPipeline<C, H>>,
    peer_registry: Arc<atipicial_network::PeerRegistry>,
    shutdown: CancellationToken,
    config: atipicial_network::BlockDownloadConfig,
) -> anyhow::Result<()>
where
    C: atipicial_runtime::SyncStageCheckpointStore + 'static,
    H: atipicial_runtime::VerifiedHeaderStore + 'static,
{
            ...
            _ = tick.tick() => {
                let local_height = match blockchain.get_height().await {
                    ...
                    Err(err) if shutdown.is_cancelled() => {
                        debug!(target: "atipicial::sync", error = %err, "blockchain height unavailable during shutdown");
                        return Ok(());
                    }
                ...
                let progress = match header_pipeline.prepare_window(peer_tip).await {
                    ...
                    Ok(None) => continue,
                    Err(err) if shutdown.is_cancelled() => {
                        debug!(target: "atipicial::sync", error = %err, "header-stage recovery stopped during shutdown");
                        return Ok(());
                    }
                    Err(err) => {
                        warn!(target: "atipicial::sync", error = %err, "header-stage recovery failed; will retry");
                        continue;
                    }
                };

                info!(
                    target: "atipicial::sync",
                    local_height,
                    peer_tip,
                ...
                .await
                {
                    ...
                    Err(err) if shutdown.is_cancelled() => {
                        debug!(target: "atipicial::sync", error = %err, "header download stopped during shutdown");
                        return Ok(());
                    }
                    Err(err) => {
                        warn!(target: "atipicial::sync", error = %err, "header stage failed; retaining durable prefix for retry");
                        continue;
                    }
                };
                ...
                let body_start = match blockchain.get_height().await {
                    ...
                    Err(err) => {
                        warn!(target: "atipicial::sync", error = %err, "canonical height unavailable before body stage");
                        continue;
                    }
                };
                if body_start >= target_height {
                    match header_pipeline.finish_imported_window(body_start).await {
                        Ok(_) => continue,
                        Err(err) => {
                            warn!(target: "atipicial::sync", error = %err, "failed to reconcile already-imported header window");
                            continue;
                        }
                    }
                }
                ...
                let downloader = atipicial_network::BlockDownloadCoordinator::new(
                    body_start,
                    target_height,
                    eligible,
                    config,
                ...
                let mut driver = atipicial_system::SyncDownloadImportDriver::new_at_chain_tip(
                    Arc::clone(&pipeline),
                    downloader,
                    body_start,
                );
                match driver.import_all().await {
                    ...
                    Err(err) if shutdown.is_cancelled() => {
                        debug!(target: "atipicial::sync", error = %err, "staged sync stopped during shutdown");
                        return Ok(());
                    }
                    Err(err) => {
                        warn!(
...
async fn download_verified_headers<H>(
    pipeline: Arc<atipicial_system::SyncHeaderPipeline<H>>,
    peer_registry: Arc<atipicial_network::PeerRegistry>,
    mut progress: atipicial_system::HeaderStageProgress,
    retry_limit: usize,
    shutdown: &CancellationToken,
) -> ServiceResult<atipicial_system::HeaderStageProgress>
where
    H: atipicial_runtime::VerifiedHeaderStore + 'static,
{
    ...
    while !progress.is_complete() {
        ...
        for peer in candidates {
            ...
            let batch = match downloaded {
                Ok(batch) => batch,
                Err(error) => {
                    atipicial_runtime::sync_metrics::record_header_fetch_failure();
                    rejected_peers.insert(peer.peer_id);
...
) -> atipicial_network::BlockDownloadConfig {
    let mut config = atipicial_network::BlockDownloadConfig::default();
