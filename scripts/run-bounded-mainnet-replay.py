#!/usr/bin/env python3
"""Run a bounded atipicial-node replay until a target height, then stop it."""
...
from mainnet_replay_proofs import (
    apply_chain_acc_import_speed_measurement,
    apply_import_speed_measurement,
    chain_acc_import_satisfies_speed_gate,
    chain_acc_import_speed_measurement,
    recover_chain_acc_status_from_import_proof,
    transaction_work_summary_from_chain_acc_import,
)
...
DEFAULT_METRIC_NAMES = [
    ...
    "atipicial_sync_blocks_persisted",
    ...
    "atipicial_storage_mdbx_commit_volume_avg",
]
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run atipicial-node until a target block height and emit a JSON replay summary."
    )
    parser.add_argument("--config", required=True, type=Path)
    parser.add_argument("--node-bin", default=Path("target/release/atipicial-node"), type=Path)
    parser.add_argument(
        "--import-chain",
        default=None,
        type=Path,
    ...
    parser.add_argument(
        "--fast-sync",
        action="store_true",
        help="Run atipicial-node's built-in fast-sync package import before live sync starts.",
    )
    parser.add_argument(
        "--fast-sync-cache",
        default=None,
    ...
    parser.add_argument("--probe-bin", default=Path("target/release/atipicial-db-probe"), type=Path)
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        default=None,
        type=Path,
        help="Append atipicial-node stdout/stderr to this file while replaying.",
    )
    parser.add_argument(
        "--report-output",
        default=None,
