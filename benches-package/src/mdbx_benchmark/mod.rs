...
fn require_durable_mdbx_sync_mode() -> Result<String> {
    let raw = std::env::var("ATC_MDBX_SYNC_MODE").unwrap_or_else(|_| "durable".to_owned());
...
fn validate_durable_mdbx_sync_mode(raw: &str) -> Result<String> {
    ...
    bail!(
        "durable MDBX benchmark requires ATC_MDBX_SYNC_MODE to be unset, 'durable', or 'default'; got {raw:?}"
    )
}
...
mod tests {
    ...
    fn durable_campaign_rejects_non_durable_or_ambiguous_sync_modes() {
        ...
        for mode in ["no-meta-sync", "safe_no_sync", "utterly-no-sync", "typo"] {
            ...
            assert!(error.to_string().contains("requires ATC_MDBX_SYNC_MODE"));
        }
    }
}
