...
/// A sequential binary writer for serializing Atipicial protocol data in little-endian format.
///
...
/// uses `bytes` for endian-aware primitive writes, and adds the Atipicial-specific
...
impl BinaryWriter {
    ...
    /// Writes a variable-length integer using the Atipicial compact encoding.
    ///
