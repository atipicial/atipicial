...
use atipicial_manifest::{
    AefFile, ContractAbi, ContractManifest, ContractMethodDescriptor, ContractParameterDefinition,
    ContractPermission, ManifestFeatures, WildCardContainer,
};
...
use atipicial_vm::{StackItem, script_builder::ScriptBuilder};
...
fn request_writes_record_id_list_counter_and_mints_response_gas() {
    ...
    assert_eq!(
        crate::AtipicialDollar::balance_of(&snapshot, &oracle_addr).unwrap(),
...
fn post_persist_mints_the_price_to_the_designated_oracle_node() {
    ...
    assert_eq!(
        crate::AtipicialDollar::balance_of(&snapshot, &node_account).unwrap(),
