use super::*;
use atipicial_native_contracts::{AtipicialDollar, LedgerContract, PolicyContract};
use atipicial_payloads::{Signer, Witness};
use atipicial_primitives::{UInt256, WitnessScope};
...
fn standard_native_provider() -> Arc<atipicial_native_contracts::StandardNativeProvider> {
...
fn mint_gas(snapshot: &DataCache, account: &UInt160, datoshi: i64) {
    ...
    snapshot.add(
        StorageKey::new(AtipicialDollar::ID, key),
        StorageItem::from_bytes(bytes),
    );
}
...
fn seed_notary_deposit(snapshot: &DataCache, account: &UInt160, amount: i64, till: u32) {
    ...
    key.extend_from_slice(&account.to_bytes());
    snapshot.add(
        StorageKey::new(atipicial_native_contracts::Notary::ID, key),
        StorageItem::from_bytes(bytes),
    );
}
...
fn notary_sponsored_fee_check_uses_payer_deposit_not_notary_gas() {
    ...
    assert_ne!(
        ...
        "Atipicial v3.10.1 TransactionVerificationContext checks Notary-sponsored fees against Signers[1]'s deposit, not Notary's ATD balance"
    );
}
...
fn native_reads_use_admission_provider_boundary() {
    ...
    assert!(!attributes.contains("AtipicialCoin::new()"));
    ...
    assert!(!provider.contains("atipicial: AtipicialCoin"));
