...
/// not erase stores behind callbacks or trait objects, so homogeatipicialus node
...
/// Represents a cache for the underlying storage of the ATC blockchain.
...
impl<B: CacheRead> DataCache<B> {
    ...
    fn apply_update(&self, key: &StorageKey, value: StorageItem) {
        ...
        // cycle (e.g. a ATD balance burned to exactly zero then
    ...
    /// for storage engines that beaefit from ordered, page-local writes.
