//! Signature and key adapters used by Atipicial cryptographic APIs.
//!
...
//! Atipicial-facing details: raw `r||s` byte shapes, prehash selection, low-s parity
//! with C# behavior, AtipicialFs signature framing, and crate-local error mapping.
...
/// ECDSA operations for secp256r1 (P-256, Atipicial's primary curve).
pub struct Secp256r1Crypto;

/// Signature prefix used by AtipicialFs ECDSA_SHA512 signatures.
pub const ATCFS_ECDSA_SHA512_PREFIX: u8 = 0x04;

/// Serialized AtipicialFs ECDSA_SHA512 signature length: one prefix byte plus raw P-256 ECDSA.
pub const ATCFS_ECDSA_SHA512_SIGNATURE_LEN: usize = 65;
...
impl Secp256r1Crypto {
    ...
    /// signer (HSM, hardware wallet) into Atipicial's 64-byte low-s `r‖s` form.
    ///
    ...
    /// Signs AtipicialFs data using P-256 over a SHA-512 prehash.
    ///
    /// AtipicialFs serializes this signature as `0x04 || raw_ecdsa_signature`.
    pub fn sign_atipicialfs_sha512(
        data: &[u8],
        private_key: &[u8; 32],
    ) -> CryptoResult<[u8; ATCFS_ECDSA_SHA512_SIGNATURE_LEN]> {
        let signing_key = SigningKey::try_from(private_key.as_slice())
            .map_err(|e| CryptoError::invalid_key(format!("Invalid private key: {e}")))?;
        ...
        let mut output = [0u8; ATCFS_ECDSA_SHA512_SIGNATURE_LEN];
        output[0] = ATCFS_ECDSA_SHA512_PREFIX;
        output[1..].copy_from_slice(&signature.to_bytes());
        Ok(output)
    }

    /// Verifies a AtipicialFs P-256 signature over a SHA-512 prehash.
    ///
    /// The verifier preserves current AtipicialFs behavior by requiring a 65-byte signature while
    /// ignoring the first byte instead of enforcing the `0x04` prefix.
    pub fn verify_atipicialfs_sha512(
        data: &[u8],
        signature: &[u8],
        public_key: &[u8],
    ) -> CryptoResult<bool> {
        if signature.len() != ATCFS_ECDSA_SHA512_SIGNATURE_LEN {
            return Err(CryptoError::invalid_signature(format!(
                "AtipicialFs signature must be {ATCFS_ECDSA_SHA512_SIGNATURE_LEN} bytes"
            )));
        }
...
impl Ed25519Crypto {
    ...
    /// Verifies an Ed25519 signature, matching the accept-set of C# Atipicial's
