...
const MAGIC: &[u8; 16] = b"ATC-PREFIX-IDX1\0";
...
mod tests {
    ...
    fn spec() -> PrefixOccupancySpec {
        PrefixOccupancySpec::new(Some("atipicial_state_service".to_string()), vec![0xf0], 33, 8).unwrap()
    }
    ...
    fn artifact_is_bound_to_checksum_environment_table_and_transaction() {
        ...
        let index = PrefixOccupancyIndex::load(&path, Some("atipicial_state_service"), environment_id)
            .unwrap()
            .unwrap();
        ...
        assert!(
            PrefixOccupancyIndex::load(&path, Some("atipicial_state_service"), [8; 16])
                .unwrap()
                .is_none()
        );
        ...
        let error = PrefixOccupancyIndex::load(&path, Some("atipicial_state_service"), environment_id)
            .unwrap_err();
