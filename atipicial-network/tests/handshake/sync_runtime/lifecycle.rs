...
async fn ping_is_answered_with_pong_carrying_local_height() {
    ...
    let mut reader = atipicial_io::MemoryReader::new(&pong.payload_raw);
    let payload =
        <PingPayload as atipicial_io::Serializable>::deserialize(&mut reader).expect("decode pong");
...
async fn relayed_block_is_forwarded_to_the_inventory_sink() {
    ...
    let block = atipicial_payloads::Block::new();
