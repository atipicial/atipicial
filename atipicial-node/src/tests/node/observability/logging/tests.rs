...
fn logging_filter_uses_toml_level_when_rust_log_is_unset() {
    let config = LoggingSection {
        level: Some("warn,atipicial=debug".to_string()),
        ..Default::default()
    };
    assert_eq!(logging_filter_directive(&config, None), "warn,atipicial=debug");
    assert_eq!(
        logging_filter_directive(&config, Some("error,atipicial_rpc=trace")),
        "error,atipicial_rpc=trace"
    );
}
...
fn size_rotating_writer_rolls_active_file_and_retains_archives() {
    ...
    let path = temp.path().join("atipicial-node.log");
