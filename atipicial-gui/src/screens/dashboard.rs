...
pub fn ui(app: &mut AtipicialGuiApp, ui: &mut Ui) {
    ...
    if !online {
        widgets::card(ui, |ui| {
            ...
            ui.add_space(4.0);
            ui.label(
                egui::RichText::new(last_err.unwrap_or_else(|| {
                    "Set the RPC endpoint in Settings and ensure the node is running.".into()
                }))
    ...
    ui.horizontal_wrapped(|ui| {
        widgets::stat_card(
            ui,
            "Block height",
            st.block_count.to_string(),
            theme::ACCENT,
        );
        widgets::stat_card(
            ui,
            "Headers",
            st.header_count.to_string(),
            theme::ACCENT_DIM,
        );
        ...
        widgets::stat_card(
            ui,
            "Network",
            st.version.protocol.network_name(),
            theme::ACCENT,
        );
        widgets::stat_card(
            ui,
            "Validators",
            st.version.protocol.validatorscount.to_string(),
            theme::ACCENT_DIM,
        );
    });
        ...
        ui.horizontal(|ui| {
            ...
            ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                let (txt, color) = if synced {
                    ...
                    (
                        format!("{} behind", st.header_count.saturating_sub(st.block_count)),
                        theme::WARN,
                    )
                };
    ...
    widgets::card(ui, |ui| {
        ...
        widgets::kv(
            ui,
            "Address version",
            st.version.protocol.addressversion.to_string(),
        );
