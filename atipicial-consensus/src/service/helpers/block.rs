...
impl ConsensusBlockFields {
    ...
    pub(in crate::service) fn bft_threshold(n: usize) -> usize {
        atipicial_vm::script_builder::RedeemScript::bft_threshold(n)
    }
    ...
    fn unchecked_multisig_verification_script(m: usize, keys: &[atipicial_crypto::ECPoint]) -> Vec<u8> {
    ...
    pub(in crate::service) fn compute_next_consensus_address(
        ...
        let keys: Vec<atipicial_crypto::ECPoint> =
    ...
    pub(in crate::service) fn compute_header_hash(
        ...
        UInt256::from(atipicial_crypto::Crypto::sha256(&unsigned))
    }
}
