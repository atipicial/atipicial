...
# atipicial-node processes may legitimately be running against unrelated data dirs.
...
echo "Run atipicial-db-probe to inspect the restored store:"
echo "  cargo run -p atipicial-node --bin atipicial-db-probe --release -- --db $CHAIN_DB --contract-id -4 --key-hex 0c"
