...
/// Default token tracker standards enabled by Atipicial's TokensTracker plugin.
pub const DEFAULT_ENABLED_TRACKER_STANDARDS: [&str; 2] = ["AEP-17", "AEP-11"];
...
pub struct TokensTrackerSettings {
    ...
    pub network: u32,
    /// List of enabled tracker standards (e.g., "AEP-11", "AEP-17").
...
impl TokensTrackerSettings {
    /// Returns the default AEP standards tracked when no explicit list is
    /// configured.
    ...
    /// AEP-17/AEP-11 pair when no non-empty tracker names are supplied.
    ...
    /// Returns true if AEP-11 tracking is enabled.
    pub fn enabled_aep11(&self) -> bool {
        self.enabled_trackers
            .iter()
            .any(|tracker| tracker.eq_ignore_ascii_case("AEP-11"))
    }

    /// Returns true if AEP-17 tracking is enabled.
    pub fn enabled_aep17(&self) -> bool {
        self.enabled_trackers
            .iter()
            .any(|tracker| tracker.eq_ignore_ascii_case("AEP-17"))
    }
