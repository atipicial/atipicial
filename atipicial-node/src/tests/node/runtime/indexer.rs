//! # atipicial-node::tests::node::runtime::indexer
//!
...
fn daemon_context_indexes_application_executed_notifications() {
    ...
    use atipicial_primitives::{TriggerType, UInt160, UInt256, WitnessScope};
...
fn persistent_indexer_write_failure_rejects_precommit_and_requests_shutdown() {
    ...
    let indexer = Arc::new(
        atipicial_indexer::IndexerService::open_store(read_only).expect("open persistent indexer"),
    );
...
fn persistent_indexer_arms_replay_marker_before_precommit_write() {
    ...
    let indexer = Arc::new(
        atipicial_indexer::IndexerService::open_store(indexer_store).expect("persistent indexer service"),
    );
