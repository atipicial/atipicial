...
fn reference_rpc_client(endpoint: &str) -> anyhow::Result<atipicial_rpc::RpcClient> {
    let url = url::Url::parse(endpoint)
        .with_context(|| format!("invalid fast-sync reference RPC endpoint {endpoint:?}"))?;
    atipicial_rpc::RpcClient::builder(url)
        .build()
...
pub(super) async fn verify_state_root_tip(
    ...
    let upstream = atipicial_rpc::StateApi::new(client)
