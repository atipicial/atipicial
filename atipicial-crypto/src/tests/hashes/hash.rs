...
#[test]
fn hash_algorithm_atipicial_protocol_bytes_match_csharp_v3_10() {
    assert_eq!(HashAlgorithm::Sha256.to_atipicial_byte(), Some(0x00));
    assert_eq!(HashAlgorithm::Keccak256.to_atipicial_byte(), Some(0x01));
    assert_eq!(HashAlgorithm::Sha512.to_atipicial_byte(), Some(0x02));

    assert_eq!(
        HashAlgorithm::from_atipicial_byte(0x00),
        Some(HashAlgorithm::Sha256)
    );
    assert_eq!(
        HashAlgorithm::from_atipicial_byte(0x01),
        Some(HashAlgorithm::Keccak256)
    );
    assert_eq!(
        HashAlgorithm::from_atipicial_byte(0x02),
        Some(HashAlgorithm::Sha512)
    );

    assert_eq!(HashAlgorithm::Ripemd160.to_atipicial_byte(), None);
    assert_eq!(HashAlgorithm::Blake2b.to_atipicial_byte(), None);
    assert_eq!(HashAlgorithm::Blake2s.to_atipicial_byte(), None);
    assert_eq!(HashAlgorithm::from_atipicial_byte(0x03), None);
}
