...
use atipicial_primitives::{TransactionAttributeType, UInt160};
...
impl PolicyContract {
    ...
    pub(super) fn read_required_i64_setting_key(
        snapshot: &DataCache<impl atipicial_storage::CacheRead>,
        key: StorageKey,
        setting: &str,
    ) -> CoreResult<i64> {
    ...
    ///
    /// Mirrors C# `AtipicialSystemExtensions.GetTimePerBlock(IReadOnlyStore, ProtocolSettings)`:
    ...
    /// Reads the max valid-until-block increment with C# `AtipicialSystemExtensions`
    ...
    ///
    /// Mirrors C# `AtipicialSystemExtensions.GetMaxTraceableBlocks`: before
    ...
    ///
    /// Exposed `pub(crate)` so `Notary::onAEP17Payment` can read the NotaryAssisted
    ...
    /// account's vote from HF_Faun (`ATC.VoteInternal(engine, account, null)`),
    ...
    >(
        ...
        if engine.is_hardfork_enabled(Hardfork::HfFaun) {
            ...
            // no ATC state / zero balance) but propagates faults.
            let _ = crate::AtipicialCoin::new().vote_internal(engine, account, None)?;
        }
