...
fn block_message(index: u32) -> Message {
    ...
    let block = atipicial_payloads::Block::from_parts(header, vec![]);
    Message::create(MessageCommand::Block, Some(&block), false).expect("encode block")
}

fn header(index: u32) -> atipicial_payloads::Header {
    let mut header = atipicial_payloads::Header::new();
    header.set_index(index);
    header
}
...
fn headers_message(indexes: &[u32]) -> Message {
    let payload =
        atipicial_payloads::HeadersPayload::create(indexes.iter().copied().map(header).collect());
...
async fn block_fetch_timeout_is_absolute_and_clears_pending_assignment() {
    ...
    match first_error {
        atipicial_network::NetworkError::RemoteUnavailable {
            peer_id: failed_peer,
            detail,
        } => {
...
async fn malformed_header_fetch_response_is_rejected_and_clears_assignment() {
    ...
    let fetch = tokio::spawn({
        ...
        async move {
            handle
                .fetch_headers_by_index(atipicial_network::HeaderRequest::new(11, 3))
                .await
        }
    });
    ...
    match error {
        atipicial_network::NetworkError::Protocol(detail) => {
    ...
    let misaligned = tokio::spawn({
        ...
        async move {
            handle
                .fetch_headers_by_index(atipicial_network::HeaderRequest::new(20, 2))
                .await
        }
    });
    ...
    let retry = tokio::spawn({
        ...
        async move {
            handle
                .fetch_headers_by_index(atipicial_network::HeaderRequest::new(30, 2))
                .await
        }
    });
    let retry_request = recv_getheaders(&mut fake).await;
...
async fn header_fetch_before_verack_is_rejected_without_queuing_stale_request() {
    ...
    let pre_ready_error = handle
        .fetch_headers_by_index(atipicial_network::HeaderRequest::new(7, 3))
        .await
        .expect_err("fetch before peer verack must be rejected");
    match pre_ready_error {
        atipicial_network::NetworkError::RemoteUnavailable {
    ...
    let ready_fetch = tokio::spawn({
        ...
        async move {
            handle
                .fetch_headers_by_index(atipicial_network::HeaderRequest::new(20, 1))
                .await
        }
    });
...
async fn header_fetch_timeout_is_absolute_and_clears_pending_assignment() {
    ...
    let first_fetch = tokio::spawn({
        ...
        async move {
            handle
                .fetch_headers_by_index(atipicial_network::HeaderRequest::new(30, 4))
                .await
        }
    });
    ...
    let second_fetch = tokio::spawn({
        ...
        async move {
            handle
                .fetch_headers_by_index(atipicial_network::HeaderRequest::new(40, 2))
                .await
        }
    });
...
async fn peer_registry_fetcher_collects_assigned_peer_range() {
    ...
    let fetch = tokio::spawn({
        ...
        async move {
            atipicial_network::BlockRangeFetcher::fetch_range(
                &registry,
    ...
    assert_eq!(
        batch
            .blocks
            .iter()
            .map(atipicial_payloads::Block::index)
            .collect::<Vec<_>>(),
        vec![11, 12]
    );
