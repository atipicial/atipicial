//! AEP-11 tracker implementation.
//!
//! Tracks AEP-11 (non-fungible token) ownership and transfer history.
...
use super::aep11_balance_key::Aep11BalanceKey;
use super::aep11_transfer_key::Aep11TransferKey;
...
use atipicial_payloads::{ApplicationExecuted, Block, VerifiableContainer};
use atipicial_primitives::CallFlags;
use atipicial_primitives::{LogLevel, TriggerType, UInt160};
...
const AEP11_BALANCE_PREFIX: u8 = 0xf8;
const AEP11_TRANSFER_SENT_PREFIX: u8 = 0xf9;
const AEP11_TRANSFER_RECEIVED_PREFIX: u8 = 0xfa;

/// AEP-11 token tracker.
pub struct Aep11Tracker<P = atipicial_native_contracts::StandardNativeProvider, S: Store = MemoryStore>
where
...
impl<P, S> Aep11Tracker<P, S>
where
    P: NativeContractProvider + 'static,
    S: Store + 'static,
{
    /// Creates a new AEP-11 tracker.
    pub fn new(
        db: Arc<S>,
        max_results: u32,
    ...
    fn record_transfer_history(
        ...
        token_id: &[u8],
        tx_hash: &atipicial_primitives::UInt256,
        transfer_index: &mut u32,
    ) {
        ...
        if record.from != UInt160::zero() {
            ...
            if let Err(e) = self.base.put(AEP11_TRANSFER_SENT_PREFIX, &key, &value) {
                TrackerBase::<P, S>::log(
                    self.track_name(),
                    &format!("Failed to store AEP-11 transfer sent: {e}"),
                    LogLevel::Error,
                );
            }
        }
        ...
        if record.to != UInt160::zero() {
            ...
            if let Err(e) = self.base.put(AEP11_TRANSFER_RECEIVED_PREFIX, &key, &value) {
                TrackerBase::<P, S>::log(
                    self.track_name(),
                    &format!("Failed to store AEP-11 transfer received: {e}"),
                    LogLevel::Error,
                );
            }
        }
    ...
    fn save_divisible_nft_balance<B: CacheRead>(
        ...
        let Some(token_id) = record.token_id.clone() else {
            TrackerBase::<P, S>::log(
                self.track_name(),
                "Divisible AEP-11 transfer missing tokenId",
                LogLevel::Warning,
            );
            return;
        };
            ...
            .is_err()
        {
            TrackerBase::<P, S>::log(
                self.track_name(),
                "Divisible AEP-11 balanceOf fault",
                LogLevel::Warning,
            );
            return;
        }
        ...
        if engine.state().contains(VMState::FAULT) || engine.result_stack().len() != 2 {
            TrackerBase::<P, S>::log(
                self.track_name(),
                "Divisible AEP-11 balanceOf returned unexpected stack size",
                LogLevel::Warning,
            );
            return;
        }
        ...
        let key_to = Aep11BalanceKey::new(record.to, record.asset, token_id.clone());
        let key_from = Aep11BalanceKey::new(record.from, record.asset, token_id);
        ...
        if let Err(e) = self.base.put(AEP11_BALANCE_PREFIX, &key_to, &value_to) {
            TrackerBase::<P, S>::log(
                self.track_name(),
                &format!("Failed to store AEP-11 balance (to): {e}"),
                LogLevel::Error,
            );
        }
        if let Err(e) = self.base.put(AEP11_BALANCE_PREFIX, &key_from, &value_from) {
            TrackerBase::<P, S>::log(
                self.track_name(),
                &format!("Failed to store AEP-11 balance (from): {e}"),
                LogLevel::Error,
            );
        }
    }
    ...
    fn save_nft_balance(&mut self, record: &TransferRecord) {
        let Some(token_id) = record.token_id.clone() else {
            TrackerBase::<P, S>::log(
                self.track_name(),
                "Indivisible AEP-11 transfer missing tokenId",
                LogLevel::Warning,
            );
            return;
        };
        ...
        if record.from != UInt160::zero() {
            let key_from = Aep11BalanceKey::new(record.from, record.asset, token_id.clone());
            if let Err(e) = self.base.delete(AEP11_BALANCE_PREFIX, &key_from) {
                TrackerBase::<P, S>::log(
                    self.track_name(),
                    &format!("Failed to delete AEP-11 balance (from): {e}"),
                    LogLevel::Error,
                );
            }
        }
        ...
        if record.to != UInt160::zero() {
            let key_to = Aep11BalanceKey::new(record.to, record.asset, token_id);
            let value = TokenBalance {
                balance: BigInt::one(),
                last_updated_block: self.current_height,
            };
            if let Err(e) = self.base.put(AEP11_BALANCE_PREFIX, &key_to, &value) {
                TrackerBase::<P, S>::log(
                    self.track_name(),
                    &format!("Failed to store AEP-11 balance (to): {e}"),
                    LogLevel::Error,
    ...
    pub fn rpc_prefixes() -> (u8, u8, u8) {
        (
            AEP11_BALANCE_PREFIX,
            AEP11_TRANSFER_SENT_PREFIX,
            AEP11_TRANSFER_RECEIVED_PREFIX,
        )
    }
}

impl<P, S> Tracker for Aep11Tracker<P, S>
where
    P: NativeContractProvider + 'static,
    S: Store + 'static,
{
    ...
    fn on_persist<B: CacheRead>(
        ...
        for app in executed_list {
            ...
            for notify in &app.notifications {
                ...
                if !contract.manifest.supports_standard("AEP-11") {
                    continue;
                }
        ...
        for record in &transfers {
            ...
            if let Entry::Vacant(entry) = divisibility.entry(record.asset) {
                ...
                if !has_balance1 && !has_balance2 {
                    TrackerBase::<P, S>::log(
                        self.track_name(),
                        "Contract does not expose balanceOf for AEP-11",
                        LogLevel::Warning,
                    );
                    continue;
                }
