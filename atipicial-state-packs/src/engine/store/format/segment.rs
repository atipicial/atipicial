...
use super::*;
use crate::engine::{failpoint, platform_io::O_CLOEXEC};
#[cfg(unix)]
...
mod tests {
    ...
    fn segment_header_binds_version_identity_and_checksum() {
        ...
        let mut header = [0u8; SEGMENT_HEADER_LEN];
        read_exact_at(&file, &mut header, 0).expect("read header");
        header[24] = 1;
        drop(file);
        ...
        write_all_at(&writable, &header, 0).expect("corrupt header");
    ...
    fn unknown_segment_version_fails_closed() {
        ...
        let writable = OpenOptions::new()
            ...
            .open(&path)
            .expect("open writable");
        write_all_at(&writable, &header, 0).expect("write version");
    ...
    fn duplicate_segment_create_does_not_replace_published_bytes() {
        ...
        write_all_at(&file, b"sentinel", SEGMENT_HEADER_LEN as u64)
