...
fn contract_script_hash_scan_returns_exact_versioned_contract_metadata() {
    let temp = tempfile::tempdir().expect("temporary MDBX directory");
    let script = vec![atipicial_vm::OpCode::PUSH1.byte(), atipicial_vm::OpCode::RET.byte()];
    let contract_hash = UInt160::from([0x55; 20]);
    let mut state = ContractState::new(
        42,
        contract_hash,
        atipicial_manifest::AefFile::new("probe-test".to_string(), script.clone()),
        atipicial_manifest::ContractManifest::new("ProbeContract".to_string()),
    );
    ...
    assert_eq!(output["matches"][0]["aef_checksum"], state.aef.checksum);
...
#[test]
fn decode_aep17_account_state_balance() {
    let bytes = base64_decode("QQEhBEGk/QI=").expect("base64");

    assert_eq!(
        decode_aep17_account_balance(&bytes).unwrap().to_string(),
        "50177089"
    );
}

#[test]
fn decode_atipicial_account_state_reads_reward_markers() {
    ...
    let state = decode_atipicial_account_state(&bytes).expect("decode ATC account state");
...
fn decode_transaction_state_reads_block_and_vm_state() {
    ...
    tx.set_signers(vec![atipicial_payloads::Signer::new(
        UInt160::zero(),
        atipicial_primitives::WitnessScope::CALLED_BY_ENTRY,
    )]);
    tx.set_script(vec![atipicial_vm::OpCode::PUSH1.byte()]);
    tx.set_witnesses(vec![atipicial_payloads::Witness::new()]);
...
fn cli_uses_mdbx_without_a_backend_switch() {
    let cli = Cli::try_parse_from(["atipicial-db-probe", "--db", "data/mainnet"]).expect("parse cli");
...
fn cli_requires_hot_database_unless_running_archive_scrub() {
    let error = Cli::try_parse_from(["atipicial-db-probe", "--contract-id", "-4", "--key-hex", "0c"])
...
fn cli_selects_testnet_chain_spec_for_transaction_replay() {
    ...
    assert_eq!(
        ...
        atipicial_primitives::constants::TESTNET_MAGIC
    );
}
...
fn offline_ledger_factory_reconstructs_archive_only_block_and_transaction() {
    ...
    use atipicial_payloads::TrimmedBlock;
    use atipicial_static_files::{
    ...
    transaction.set_witnesses(vec![atipicial_payloads::Witness::new()]);
    ...
    hot.add(
        ...
        atipicial_storage::StorageItem::from_bytes(block_hash.to_bytes()),
    );
    hot.add(
...
#[test]
fn cli_accepts_atipicial_account_decode_mode() {
    let cli = Cli::try_parse_from([
        "atipicial-db-probe",
        "--db",
        "data/mainnet",
    ...
    assert!(matches!(cli.decode, DecodeMode::AtipicialAccount));
}
...
fn cli_accepts_mpt_key_probe_with_contract_key() {
    ...
    assert!(matches!(cli.decode, DecodeMode::Aep17Account));
...
fn add_hot_rows<B: atipicial_storage::CacheRead>(hot: &DataCache<B>, rows: &[(StorageKey, Vec<u8>)]) {
    for (key, value) in rows {
        hot.add(
            key.clone(),
            atipicial_storage::StorageItem::from_bytes(value.clone()),
        );
    }
}

fn set_hot_tip<B: atipicial_storage::CacheRead>(hot: &DataCache<B>, hash: &UInt256, height: u32) {
    hot.add(
        StorageKey::new(LedgerContract::ID, vec![0x0c]),
        atipicial_storage::StorageItem::from_bytes(
            LedgerContract::new()
...
fn open_test_offline_ledger_factory<B: CacheRead>(
    ...
    let metadata = atipicial_storage::persistence::providers::memory_store::MemoryStore::new();
...
fn offline_ledger_factory_rejects_corrupt_hot_tip_without_touching_archive() {
    ...
    hot.add(
        StorageKey::new(LedgerContract::ID, vec![0x0c]),
        atipicial_storage::StorageItem::from_bytes(vec![0xff]),
    );
    let archive =
