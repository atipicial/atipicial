//! Error types for the Atipicial Core crate.
//!
//! This module provides comprehensive error handling for core Atipicial operations,
...
use atipicial_primitives::PrimitiveError;
use thiserror::Error;
...
impl CoreError {
    ...
    /// Create a new invalid AEP-2 key error
    pub fn invalid_aep2_key<S: Into<String>>(msg: S) -> Self {
        Self::Cryptographic {
            message: msg.into(),
        }
    }
...
impl From<atipicial_io::IoError> for CoreError {
    fn from(error: atipicial_io::IoError) -> Self {
        match error {
            atipicial_io::IoError::Io(source) => CoreError::io(source.to_string()),
...
// `atipicial_storage::StorageError`, `atipicial_vm::VmError`,
// `atipicial_script_builder::ScriptBuilderError`, and
// `atipicial_script_builder::RedeemScriptError`) used to live here when `CoreError`
// was owned by the `atipicial-core` monolith. They were removed when `CoreError`
// was extracted into the `atipicial-error` low infrastructure crate, because it must
...
// atipicial_error::CoreError` instead — see `atipicial-storage`, `atipicial-vm`, and
// `atipicial-script-builder`.

// The cross-crate `From<OtherError> for CoreError` impls now live in their
// owning crates (`atipicial-storage`, `atipicial-vm`, `atipicial-script-builder`), each
// implementing `From<LocalError> for
// atipicial_error::CoreError`. This keeps `atipicial-error` at the lower infrastructure
// boundary (it depends only on `atipicial-primitives` + `atipicial-io`), per the reth-style
