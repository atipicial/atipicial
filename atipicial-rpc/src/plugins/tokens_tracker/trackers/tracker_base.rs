...
//!
//! Provides common functionality for AEP-11 and AEP-17 trackers including
...
use atipicial_primitives::{LogLevel, UInt160};
...
pub struct TransferRecord {
    ...
    pub to: UInt160,
    /// Token ID for AEP-11 transfers.
    pub token_id: Option<Vec<u8>>,
    /// Transfer amount.
    pub amount: BigInt,
}

/// Common view over transfer-key types (TokenTransferKey, Aep17TransferKey, Aep11TransferKey).
...
/// Trait implemented by AEP-11/AEP-17 trackers.
...
/// Base tracker state shared by AEP-11 and AEP-17 trackers.
    ...
    S: Store,
{
    ...
    /// Base JSON conversion for a transfer pair (without AEP-11 tokenid).
    ...
    pub fn log(track: &str, message: &str, level: LogLevel) {
        match level {
            LogLevel::Debug => debug!(target: "atipicial::tokens_tracker", track, message),
            LogLevel::Info => info!(target: "atipicial::tokens_tracker", track, message),
            LogLevel::Warning => warn!(target: "atipicial::tokens_tracker", track, message),
            LogLevel::Error | LogLevel::Fatal => {
                error!(target: "atipicial::tokens_tracker", track, message)
            }
        }
    }
}
