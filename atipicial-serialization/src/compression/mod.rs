//! # atipicial-serialization::compression
//!
//! Compression codecs and deterministic envelope helpers.
//!
//! ## Boundary
//!
//! This module belongs to `atipicial-serialization`. This codec crate owns
...
/// Result type for compression operations (alias of [`atipicial_error::Result`]).
pub type CompressionResult<T> = atipicial_error::Result<T>;
...
impl Compression {
    ...
    pub fn compress(data: &[u8], algorithm: CompressionAlgorithm) -> atipicial_error::Result<Vec<u8>> {
        match algorithm {
    ...
    /// LZ4 compresses data using the canonical helper (delegates to `atipicial-io`).
    pub fn compress_lz4(data: &[u8]) -> atipicial_error::Result<Vec<u8>> {
        Lz4::compress_lz4(data).map_err(|e| CoreError::compression(e.to_string()))
    }

    /// LZ4 decompresses data using the canonical helper (delegates to `atipicial-io`).
    pub fn decompress_lz4(data: &[u8], max_size: usize) -> atipicial_error::Result<Vec<u8>> {
