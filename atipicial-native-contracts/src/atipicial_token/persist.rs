//! AtipicialCoin block-persist hooks.
//!
...
use super::{
    ATC_COMMITTEE_CHANGED_EVENT, AtipicialCoin, COMMITTEE_REWARD_RATIO, VOTE_FACTOR,
    VOTER_REWARD_RATIO, candidate_signature_account,
};
...
use atipicial_runtime::sync_metrics::{self, AtipicialCoinOnPersistStage};
use atipicial_storage::StorageItem;
use num_bigint::BigInt;
use std::time::Instant;

impl AtipicialCoin {
    /// C# `AtipicialCoin.OnPersistAsync`: on a committee-refresh block
    ...
    >(
        ...
        let block_index =
            crate::support::engine::require_persisting_block(engine, "AtipicialCoin::on_persist")?
                .index();
        let committee_count = engine.protocol_settings().committee_members_count();
        if committee_count == 0 {
            return Err(CoreError::invalid_operation(
                "AtipicialCoin::on_persist requires a non-empty standby committee",
            ));
        }
        if !Self::should_refresh_committee(block_index, committee_count) {
            sync_metrics::record_atipicial_coin_onpersist_stage(
        ...
        // Hardfork check for https://github.com/atipicial-project/atipicial/pull/3158.
        ...
        if engine.is_hardfork_enabled(Hardfork::HfCockatrice) {
            ...
            if committee_changed {
                ...
                engine
                    .send_notification(
                        Self::script_hash(),
                        ATC_COMMITTEE_CHANGED_EVENT.to_owned(),
                        vec![prev_key_item, new_key_item],
                    )
                    .map_err(|e| {
                        CoreError::invalid_operation(format!(
                            "AtipicialCoin::on_persist: {ATC_COMMITTEE_CHANGED_EVENT} notify: {e}"
                        ))
                    })?;
                sync_metrics::record_atipicial_coin_onpersist_stage(
        ...
        sync_metrics::record_atipicial_coin_onpersist_stage(
            ...
            atipicial_runtime::time::elapsed_us(refresh_start.elapsed()),
        );
        Ok(())
    }

    /// C# `AtipicialCoin.PostPersistAsync`: every block mints
    ...
    >(
        ...
        if committee_count == 0 {
            return Err(CoreError::invalid_operation(
                "AtipicialCoin::post_persist requires a non-empty standby committee",
            ));
        }
        let snapshot = engine.snapshot_cache();
        // C# `GetAtipicialDollarPerBlock(snapshot)` reads the record effective at
        ...
        // setAtipicialDollarPerBlock in this very block already applies).
            ...
            Some(committee) => committee
                ...
                .ok_or_else(|| {
                    CoreError::invalid_operation(
                        "AtipicialCoin::post_persist: committee cache too small",
                    )
                })?,
        ...
        crate::AtipicialDollar::new().gas_mint(engine, &account, &committee_reward, false)?;
        ...
        if let Some(committee) = committee {
            ...
            for (index, (member, cached_votes)) in committee.iter().enumerate() {
                ...
                if votes > BigInt::from(0) {
                    let reward_per_atipicial = factor * &voter_reward_of_each_committee / &votes;
                    let key = Self::voter_reward_per_committee_key(member);
                    // C# `GetAndChange(key, () => new StorageItem(0)).Add(...)`.
                    let accumulated =
                        self.voter_reward_per_committee(&snapshot, member) + reward_per_atipicial;
                    snapshot.update(
                        key,
