...
fn native_manifest_safe_flags_pin_csharp_attributes() {
    ...
    // AtipicialDollar (all from FungibleToken.cs): symbol/decimals `[ContractMethod]`
    // (RequiredCallFlags defaults to None), totalSupply/balanceOf ReadStates;
    // transfer is States|AllowCall|AllowNotify -> not safe.
    assert_eq!(
        manifest_safe_methods(&AtipicialDollar::new(), &settings, ALL_ACTIVE),
        vec![
            s("balanceOf", 1),
            s("decimals", 0),
            s("symbol", 0),
    ...
    // AtipicialCoin: every get*/unclaimedGas (incl. the getAllCandidates iterator)
    ...
    // onAEP17Payment) all carry States (± AllowNotify) -> not safe.
    assert_eq!(
        manifest_safe_methods(&AtipicialCoin::new(), &settings, ALL_ACTIVE),
        vec![
            s("balanceOf", 1),
            s("decimals", 0),
            s("getAccountState", 1),
            s("getAllCandidates", 0),
            s("getAtipicialDollarPerBlock", 0),
    ...
    // Notary.cs) are safe; onAEP17Payment/lockDepositUntil/
    ...
    // (Treasury.cs) -> SAFE, unlike Notary's States-flagged onAEP17Payment;
