...
use atipicial_crypto::BloomFilter;
use atipicial_payloads::{Signer, Transaction, Witness, WitnessScope};
use atipicial_primitives::UInt160;

struct StubBlockSource;
impl atipicial_network::BlockSource for StubBlockSource {
    fn block_by_index(&self, index: u32) -> Option<atipicial_payloads::Block> {
        (index == 0).then(atipicial_payloads::Block::new)
    }
    fn block_by_hash(&self, _hash: &UInt256) -> Option<atipicial_payloads::Block> {
        Some(atipicial_payloads::Block::new())
    }
}

struct EmptyBlockSource;
impl atipicial_network::BlockSource for EmptyBlockSource {
    fn block_by_index(&self, _index: u32) -> Option<atipicial_payloads::Block> {
        None
    }
}

struct ExtensibleStubSource {
    payload: atipicial_payloads::ExtensiblePayload,
    hash: UInt256,
}
...
impl atipicial_network::BlockSource for FilteredBlockSource {
    fn block_by_index(&self, index: u32) -> Option<atipicial_payloads::Block> {
...
impl atipicial_network::BlockSource for ExtensibleStubSource {
    fn block_by_index(&self, _index: u32) -> Option<atipicial_payloads::Block> {
        None
    }

    fn extensible_by_hash(&self, hash: &UInt256) -> Option<atipicial_payloads::ExtensiblePayload> {
        (*hash == self.hash).then(|| self.payload.clone())
    }
}

fn sample_extensible_payload() -> atipicial_payloads::ExtensiblePayload {
    let mut payload = atipicial_payloads::ExtensiblePayload::new();
...
async fn node_serves_getblockbyindex_from_the_block_source() {
    ...
    let mut reader = atipicial_io::MemoryReader::new(&block_frame.payload_raw);
    <atipicial_payloads::Block as atipicial_io::Serializable>::deserialize(&mut reader)
...
async fn node_serves_getheaders_from_the_block_source() {
    ...
    let mut reader = atipicial_io::MemoryReader::new(&headers_frame.payload_raw);
    let payload = <atipicial_payloads::HeadersPayload as atipicial_io::Serializable>::deserialize(&mut reader)
...
async fn node_serves_filtered_blocks_and_clears_filter() {
    ...
    let mut block =
        atipicial_payloads::Block::from_parts(atipicial_payloads::Header::new(), vec![transaction.clone()]);
    ...
    // account. This covers Atipicial's transaction-hash OR signer-account match.
    let empty_filter = BloomFilter::new(256, 3, 0).expect("empty bloom filter");
    let filter_load =
        atipicial_payloads::p2p_payloads::FilterLoadPayload::create_from_bloom_filter(&empty_filter);
    fake.send(
    ...
    let filter_add = atipicial_payloads::p2p_payloads::FilterAddPayload::new(account.to_bytes());
    fake.send(
    ...
    let mut reader = atipicial_io::MemoryReader::new(&merkle_frame.payload_raw);
    let merkle =
        <atipicial_payloads::MerkleBlockPayload as atipicial_io::Serializable>::deserialize(&mut reader)
    ...
    let filter_add =
        atipicial_payloads::p2p_payloads::FilterAddPayload::new(transaction.hash().to_bytes());
    fake.send(
        Message::create(MessageCommand::FilterAdd, Some(&filter_add), false)
...
async fn node_rejects_invalid_filterload_bounds() {
    ...
    let invalid = atipicial_payloads::p2p_payloads::FilterLoadPayload::new(Vec::new(), 0, 0);
    fake.send(
...
async fn node_replies_notfound_for_missing_getdata_block() {
    ...
    let mut reader = atipicial_io::MemoryReader::new(&notfound_frame.payload_raw);
    let payload =
        <InvPayload as atipicial_io::Serializable>::deserialize(&mut reader).expect("decode notfound");
...
async fn node_serves_getdata_extensible_from_the_block_source() {
    ...
    let mut reader = atipicial_io::MemoryReader::new(&extensible_frame.payload_raw);
    let served =
        <atipicial_payloads::ExtensiblePayload as atipicial_io::Serializable>::deserialize(&mut reader)
