...
| Database | `data/atipicial-v3101-staged-replay/pruning-stateroot-chain`, 22 GB |
...
- **The trie is really there.** `findstates` walks the local MPT and returns
  real AtipicialCoin storage entries; `getproof` on one of those keys yields a
...
- **The reference is not a single source.** 14 randomly sampled ladder heights
  from 614,000 to 10,662,000 were re-fetched from `mainnet1.atipicial.coz.io`, a
...
closed separately on a mixed atipicial-rs / atipicial-cli / atipicial-go private network, which
