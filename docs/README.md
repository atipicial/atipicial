...
# atipicial-rs Documentation

`atipicial-rs` is a Atipicial N3 blockchain node implemented from scratch in Rust, targeting
byte-for-byte compatibility with the official C# reference node through Atipicial
v3.10.1. The runnable program is a single daemon, `atipicial-node`: it syncs the chain
over a custom TCP P2P protocol, executes AtipicialVM bytecode and native contracts,
...
|-----|-------------------|
| [getting-started.md](./getting-started.md) | Install prerequisites, build `atipicial-node`, run a TestNet or MainNet node, point it at a data directory, and smoke-test it over JSON-RPC. Includes Docker and Makefile shortcuts. |
...
| [metrics-conventions.md](./metrics-conventions.md) | Metric naming conventions: the prefix-to-owning-crate table (`atipicial_node_`, `atipicial_sync_`, `atipicial_state_service_`, `atipicial_storage_mdbx_`, `atipicial_rpc_`), the `_total` counter rule, the `_us` duration suffix, and the two deliberately frozen names that keep `run-bounded-mainnet-replay.py` scraping. |
...
| [architecture.md](./architecture.md) | The 8-layer workspace design (Foundation → Infrastructure → Protocol → Domain service → Node service → Composition → Plugin/RPC boundary → Application), a crate reference table for all 28 production workspace members plus 3 development-only members, and the key design decisions (canonical AtipicialVM, supervised async services, block-import queue, provider factories, static archives, pipeline abstraction, MPT layering, C# parity). |
...
| [protocol-compatibility.md](./protocol-compatibility.md) | What "byte-for-byte C# parity" means, the 11 native contracts, the 8 hardforks with MainNet/TestNet activation heights, supported subsystems (consensus, VM, AEP standards, P2P), and the cryptography stack. |
