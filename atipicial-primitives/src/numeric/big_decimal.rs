...
//! BigDecimal - Fixed-point decimal arithmetic for Atipicial N3.
//!
//! This module provides `BigDecimal`, a fixed-point number type with arbitrary
//! precision for handling ATC and ATD token amounts accurately.
//!
//! ## Overview
//!
//! Atipicial N3 uses 8 decimal places for ATC and ATD token amounts. The `BigDecimal`
    ...
    fn change_decimals_to(&self, decimals: u32) -> PrimitiveResult<Self> {
        ...
        let value = if self.decimals < decimals {
            ...
            if !remainder.is_zero() {
                return Err(PrimitiveError::InvalidData {
...
impl Div for BigDecimal {
    ...
    fn div(self, other: Self) -> Self::Output {
        ...
        if other_normalized.value.is_zero() {
            // Division by zero: return zero (matches Atipicial C# behavior for BigDecimal)
            return Self::zero();
        }
...
#[cfg(test)]
#[path = "../tests/numeric/big_decimal.rs"]
mod tests;
