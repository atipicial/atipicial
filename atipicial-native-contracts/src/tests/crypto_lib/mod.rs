//! # atipicial-native-contracts::tests::crypto_lib
//!
//! Test module grouping Native CryptoLib interop surface and verification
//! helpers. coverage for atipicial-native-contracts.
//!
//! ## Boundary
//!
...
// The full byte-exact arithmetic is verified in atipicial_crypto::bls12381_point —
...
fn recover_secp256k1_returns_none_on_bad_input() {
    // The success path is round-trip-tested in atipicial-crypto
...
fn recover_secp256k1_rejects_64_byte_compact_signature_like_csharp() {
    ...
    let sk = [0x11u8; 32];
    let msg = b"atipicial ecrecover parity";
    let hash = atipicial_crypto::Crypto::sha256(msg);
    let expected = atipicial_crypto::Secp256k1Crypto::derive_public_key(&sk)
        .unwrap()
        .to_vec();
    let sig64 = atipicial_crypto::Secp256k1Crypto::sign(msg, &sk).unwrap();
...
fn verify_ecdsa_dispatch_gates_keccak_and_rejects_unknown_curve() {
    ...
    // mechanics themselves are covered by atipicial-crypto's verify_signature_with_hash
    ...
    let k1 = atipicial_crypto::Secp256k1Crypto::derive_public_key(&sk).unwrap();
    let r1 = atipicial_crypto::Secp256r1Crypto::derive_public_key(&sk).unwrap();
...
fn verify_ecdsa_v1_good_key_bad_signature_returns_false_not_fault() {
    ...
    let msg = b"message";
    let sk = [0x11u8; 32];
    let pubkey = atipicial_crypto::Secp256k1Crypto::derive_public_key(&sk).unwrap();
