...
//! BLS12-381 curve-point operations backing Atipicial's `CryptoLib` native methods
...
//!
//! C# parity reference: `Atipicial.SmartContract.Native.CryptoLib.BLS12_381` +
//! `Atipicial.Cryptography.BLS12_381` (a managed port of the zkcrypto `bls12_381`
...
/// deserialized coefficient is `>= p`, so the atipicial-rs Gt parse must reject the
...
/// so the atipicial-rs check compares the same LE value against `r`.
...
///
/// Mirrors the runtime objects Atipicial's `CryptoLib` wraps in an `InteropInterface`
...
impl Bls12381Point {
    ...
    /// operation is multiplication, matching zkcrypto / Atipicial).
    ///
    /// # Errors
    ///
