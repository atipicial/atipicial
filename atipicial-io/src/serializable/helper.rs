//! Helper utilities mirroring C# Atipicial serialization helpers.
...
impl SerializeHelper {
    /// Returns the number of bytes required to encode `value` using Atipicial variable-length encoding.
    #[inline]
    #[must_use]
    ...
    /// Serializes an array of `Serializable` items with a Atipicial length prefix.
    ...
    /// Serializes an array with a Atipicial length prefix and custom item writer.
    ...
    ///
    /// This is useful for Atipicial wire formats where one vector's length is defined by
