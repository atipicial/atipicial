#!/usr/bin/env python3
"""
Continuously validate atipicial-rs state roots against official Atipicial RPC nodes.

This validator compares the local atipicial-rs `getstateroot` output with one or more
...
Examples:
    ...
    python3 scripts/continuous-stateroot-validation.py \
        --local-config atipicial_mainnet_node.toml \
        --once

    python3 scripts/continuous-stateroot-validation.py \
        --local http://127.0.0.1:20332 \
        --reference http://seed1.atipicial.com:10332,http://seed2.atipicial.com:10332
"""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Continuously validate atipicial-rs state roots against official Atipicial RPC nodes"
    )
    parser.add_argument(
        "--local",
        default=None,
        help=f"Local atipicial-rs RPC URL (default: {DEFAULT_LOCAL_RPC} or --local-config)",
    )
    parser.add_argument(
        "--local-config",
        default=None,
        help="Optional atipicial-node TOML config used to infer local RPC bind/port/auth",
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "Defaults to official Atipicial mainnet seeds."
        ),
    )
    parser.add_argument(
        "--reference-user",
...
def resolve_local_endpoint(args: argparse.Namespace) -> RpcEndpoint:
    ...
    username = (
        args.local_user
        or os.getenv("ATC_RPC_USER")
        or config_values.get("username")
    )
    password = (
        args.local_pass
        or os.getenv("ATC_RPC_PASS")
...
def main() -> int:
    ...
    print("=== Atipicial-RS Continuous State Root Validator ===")
