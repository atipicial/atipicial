...
/// across `atipicial_dollar.rs`, `treasury.rs`, `policy_contract.rs`,
/// `atipicial_coin.rs`, `crypto_lib.rs`, and `role_management.rs`.
...
/// Three valid secp256r1 public keys (Atipicial N3 standby validators) used
...
/// lookup.
pub fn deploy_native(cache: &DataCache, state: &atipicial_execution::ContractState) {
    cache.add(
...
pub fn seed_committee(cache: &DataCache, points: &[ECPoint]) {
    ...
    cache.add(
        crate::AtipicialCoin::committee_key(),
