...
//!
//! Validates the architectural layering of the atipicial-rs workspace:
//!
//! ```text
//! Layer 0 (Foundation): atipicial-primitives
//! Layer 1 (Infrastructure): atipicial-io, atipicial-error, atipicial-crypto, atipicial-trie, atipicial-storage, atipicial-static-files, atipicial-state-packs, atipicial-checkpoint, atipicial-config, atipicial-vm, atipicial-serialization, atipicial-manifest
//! Layer 2 (Protocol): atipicial-payloads, atipicial-consensus, atipicial-hsm
//! Layer 3 (Domain services): atipicial-execution, atipicial-native-contracts, atipicial-mempool, atipicial-state-service, atipicial-runtime
//! Layer 4 (Node services): atipicial-blockchain, atipicial-network, atipicial-wallets, atipicial-indexer, atipicial-oracle-service
//! Layer 5 (Composition): atipicial-system
//! Layer 6 (Plugin/RPC boundary): atipicial-rpc
//! Layer 7 (Applications): atipicial-node, atipicial-gui
//! ```
...
/// Layer definitions for the atipicial-rs architecture
...
impl Layer {
    ...
    fn from_crate_name(name: &str) -> Option<Self> {
        match name {
            // Layer 0: Foundation (no atipicial-* dependencies allowed).
            "atipicial-primitives" => Some(Layer::Foundation),
            // Layer 1: Infrastructure and shared data tooling.
            "atipicial-io" | "atipicial-error" | "atipicial-crypto" | "atipicial-trie" | "atipicial-storage"
            | "atipicial-static-files" | "atipicial-state-packs" | "atipicial-checkpoint" | "atipicial-config"
            | "atipicial-vm" | "atipicial-serialization" | "atipicial-manifest" => Some(Layer::Infrastructure),
            // Layer 2: Protocol payloads and consensus message vocabulary.
            "atipicial-payloads" | "atipicial-consensus" | "atipicial-hsm" => Some(Layer::Protocol),
            // Layer 3: Domain logic with no node composition dependency.
            "atipicial-execution"
            | "atipicial-native-contracts"
            | "atipicial-mempool"
            | "atipicial-state-service"
            | "atipicial-runtime" => Some(Layer::DomainServices),
            // Layer 4: Long-running or queryable node services.
            "atipicial-blockchain" | "atipicial-network" | "atipicial-wallets" | "atipicial-indexer"
            | "atipicial-oracle-service" => Some(Layer::NodeServices),
            // Layer 5: Node composition root.
            "atipicial-system" => Some(Layer::Composition),
            // Layer 6: Optional plugin/RPC-facing service boundary.
            "atipicial-rpc" => Some(Layer::PluginBoundary),
            // Layer 7: Binaries, UI clients, and development-only tooling.
            "atipicial-node" | "atipicial-gui" | "atipicial-test-fixtures" => Some(Layer::Application),
            _ => None,
        }
    }
}
...
fn collect_atipicial_dependencies_from_table(table: Option<&toml::Value>, deps: &mut BTreeSet<String>) {
...
/// Parse Cargo.toml to extract runtime/build atipicial-* dependencies.
fn parse_atipicial_dependencies(cargo_toml_path: &Path) -> Vec<String> {
    let manifest = read_toml_manifest(cargo_toml_path);
    let mut deps = BTreeSet::new();

    collect_atipicial_dependencies_from_table(manifest.get("dependencies"), &mut deps);
    collect_atipicial_dependencies_from_table(manifest.get("build-dependencies"), &mut deps);

    if let Some(targets) = manifest.get("target").and_then(toml::Value::as_table) {
        for target in targets.values() {
            collect_atipicial_dependencies_from_table(target.get("dependencies"), &mut deps);
            collect_atipicial_dependencies_from_table(target.get("build-dependencies"), &mut deps);
        }
    }
