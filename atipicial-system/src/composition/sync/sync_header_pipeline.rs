...
//! durable [`atipicial_runtime::VerifiedHeaderStore`]. It owns recovery and the
//! bounded in-memory `HeaderCache` acceleration view, but it never writes
//! canonical Ledger rows or imports blocks.

use std::sync::Arc;

use atipicial_blockchain::{BlockchainHandle, HeaderCache};
use atipicial_network::{BlockDownloadBatch, HeaderDownloadBatch, PeerId};
...
impl<H: VerifiedHeaderStore> SyncHeaderPipeline<H> {
    ...
    /// Validate one correlated peer response through `atipicial-blockchain`, then
    ...
    pub async fn accept_downloaded_headers(
        ...
        atipicial_runtime::sync_metrics::record_headers_downloaded(
        ...
        if validation.accepted > 0 {
            atipicial_runtime::sync_metrics::record_headers_verified(
    ...
    pub fn verify_body_batch(&self, batch: &BlockDownloadBatch) -> ServiceResult<()> {
        ...
        for (offset, block) in batch.blocks.iter().enumerate() {
            ...
            if actual_hash != expected_hash {
                atipicial_runtime::sync_metrics::record_body_header_mismatch();
                return Err(ServiceError::invalid_input(format!(
    ...
    pub async fn finish_imported_window(
        ...
        atipicial_runtime::sync_metrics::record_bodies_checkpoint(u64::from(canonical_height));
        Ok(Some(checkpoint))
    }
