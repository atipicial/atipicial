...
impl Header {
    ...
    /// Back-compat constructor taking a witness vector (Atipicial N3 headers carry
    ...
    /// Creates a header from all of its parts (Atipicial N3 headers carry exactly one
    ...
    pub fn hash(&self) -> UInt256 {
        match self.try_hash() {
            ...
            Err(error) => {
                tracing::error!(
                    target: "atipicial::payloads::header",
                    error = %error,
    ...
    /// Serializes this header to its canonical Atipicial RPC JSON object, matching C#
    ...
    pub fn try_hash(&self) -> CoreResult<UInt256> {
        ...
        // Atipicial N3 block hashes use single SHA-256 over the unsigned header payload.
        let hash = UInt256::from(atipicial_crypto::Crypto::sha256(&hash_data));
...
atipicial_io::impl_default_via_new!(Header);
...
impl atipicial_primitives::SerializablePayload for Header {
...
impl Header {
    /// Serializes the full header using the canonical Atipicial wire format.
    ...
    /// Deserializes a full header from the canonical Atipicial wire format.
    pub fn from_bytes(bytes: &[u8]) -> IoResult<Self> {
        let mut reader = MemoryReader::new(bytes);
        let header = <Self as Serializable>::deserialize(&mut reader)?;
        if reader.remaining() != 0 {
            return Err(atipicial_io::IoError::invalid_data(format!(
...
impl atipicial_primitives::Verifiable for Header {
    fn hash(&self) -> atipicial_primitives::error::PrimitiveResult<atipicial_primitives::UInt256> {
        let data = self.try_get_hash_data().map_err(|e| {
            atipicial_primitives::error::PrimitiveError::invalid_data(format!(
                "header serialization failed: {e}"
            ))
        })?;
        Ok(atipicial_primitives::UInt256::from(atipicial_crypto::Crypto::sha256(
            &data,
        )))
    }
    fn hash_data(&self) -> Vec<u8> {
...
impl Serializable for Header {
    ...
    fn deserialize(reader: &mut MemoryReader) -> IoResult<Self> {
        ...
        if version > 0 {
            return Err(atipicial_io::IoError::invalid_data("Header version must be 0"));
        }
        ...
        if witness_count != 1 {
            return Err(atipicial_io::IoError::InvalidData {
