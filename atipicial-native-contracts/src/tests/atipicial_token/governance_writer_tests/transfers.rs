...
fn transfer_moves_balance_and_follows_vote_weight() {
    ...
    cache.update(
        ...
        StorageItem::from_bytes(
            AtipicialCoin::encode_candidate_state(true, &BigInt::from(100)).unwrap(),
        ),
    );
    let from_state = AtipicialAccountStateView {
    ...
    cache.update(
        AtipicialCoin::account_key(&from),
        StorageItem::from_bytes(
            AtipicialCoin::encode_atipicial_account_state(&from_state).unwrap(),
        ),
    );
    ...
    let from_after = AtipicialCoin::decode_atipicial_account_state(
        &AtipicialCoin::new()
    ...
    let to_after = AtipicialCoin::decode_atipicial_account_state(
        &AtipicialCoin::new()
            .read_account_state(&snapshot, &to)
            .unwrap(),
    )
    .unwrap();
