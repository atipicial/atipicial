...
impl RpcServer {
    /// Validate a Atipicial address against the node's configured address version.
    #[must_use]
    pub fn validate_address(&self, address: &str) -> Value {
        let address_version = self.system().settings().address_version;
        let is_valid =
            atipicial_wallets::wallet_helper::WalletAddress::to_script_hash(address, address_version)
                .is_ok();
