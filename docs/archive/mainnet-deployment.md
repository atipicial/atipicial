...
# Atipicial-RS Mainnet Deployment Guide
...
### 2. Build Atipicial-RS

```bash
cd /root/atipicial-rs
cargo build --release
```

Build time: ~9 minutes 26 seconds
Binary size: 24MB at `/root/atipicial-rs/target/release/atipicial-node`

### 3. Configuration

Create `/root/atipicial-rs/atipicial_mainnet.toml`:
...
[storage]
backend = "rocksdb"
path = "/root/atipicial-data/mainnet"
...
```bash
mkdir -p /root/atipicial-data/mainnet
cd /root/atipicial-rs
nohup ./target/release/atipicial-node --config atipicial_mainnet.toml > /root/atipicial-node.log 2>&1 &
```
...
```bash
# Check process
pgrep -f atipicial-node

# Check logs
tail -f /root/atipicial-node.log
...
```bash
tail -f /root/atipicial-node.log | grep -E "INFO|WARN|ERROR"
```
