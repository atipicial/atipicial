...
#[cfg(windows)]
mod backend {
    //! Windows fallback: no memory mapping, the same API over positioned
    //! reads. Correctness is identical (the module invariants only require
    //! an immutable shared byte view); sequential-read throughput differs,
    //! which is acceptable for a development/compat platform.

    use anyhow::{Context, Result};
    use std::fs::File;
    use std::path::Path;

    /// Byte-view stand-in for a read-only mapping of one immutable file.
    #[derive(Debug)]
    pub(crate) struct Mmap {
        bytes: Vec<u8>,
    }
    ...
    impl Mmap {
        /// Loads the first `len` bytes of `file`; `len == 0` yields an empty map.
        pub(crate) fn map(file: &File, len: u64, what: &Path) -> Result<Self> {
            let len = usize::try_from(len).context("mmap length does not fit usize")?;
            let mut bytes = Vec::with_capacity(len);
            bytes.resize(len, 0u8);
            crate::engine::platform_io::read_exact_at(file, &mut bytes, 0).with_context(|| {
                format!("read {} bytes of {} for pack view", len, what.display())
            })?;
            Ok(Self { bytes })
        }

        pub(crate) fn map_random(file: &File, len: u64, what: &Path) -> Result<Self> {
            Self::map(file, len, what)
        }
        ...
        /// No-op on Windows: the byte view is already materialized.
        pub(crate) fn advise_dontneed(&self, start: usize, _end: usize) -> Result<usize> {
            Ok(start)
        }
