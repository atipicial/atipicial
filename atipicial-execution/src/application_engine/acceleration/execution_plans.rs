...
/// Disabled-by-default configuration for immutable AtipicialVM execution plans.
///
...
pub enum ApplicationExecutionPlanConfig {
    /// Execute every script through the ordinary AtipicialVM path and allocate no
...
impl ApplicationExecutionPlanConfig {
    /// Enables plans with the conservative bounded AtipicialVM defaults.
