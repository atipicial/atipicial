//! # atipicial-storage::tests::mdbx
//!
//! Test module grouping the production-default MDBX provider and store adapter.
//!
//! ## Boundary
//!
//! This is test/benchmark-only code for atipicial-storage; it may assemble fixtures
...
fn named_table_views_isolate_identical_raw_keys() {
    ...
    let mut state_service = canonical
        .open_named_table("atipicial_state_service")
    ...
    assert_eq!(state_service.data_table_name(), Some("atipicial_state_service"));
...
fn named_table_validation_rejects_reserved_or_wrapper_panicking_names() {
    ...
    for invalid in ["", "atipicial_node_metadata", "state\0service"] {
        canonical
