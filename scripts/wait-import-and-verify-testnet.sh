...
# Wait for an import-only atipicial-node process to exit, then verify final height/hash
...
STORAGE_PATH="$2"
TARGET_HEIGHT="$3"
RPC_URL="${4:-http://seed1t5.atipicial.com:20332}"
...
if [[ -z "$LOCAL_HEIGHT" ]]; then
  echo "failed to read local current_index from atipicial-db-probe output" >&2
  printf '%s\n' "$LOCAL_HASH_OUT" >&2
  exit 1
fi
