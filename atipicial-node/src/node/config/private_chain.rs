...
//! [`AtipicialChainSpec::from_network_type`]. A private chain has no built-in
//! identity, so it must supply a complete [`AtipicialChainSpec`]. This module builds
//! one from the same `ProtocolConfiguration` document `atipicial-cli` consumes, so a
...
use anyhow::Context;
use atipicial_config::{
    GENESIS_NONCE, GENESIS_TIMESTAMP_MS, GenesisConfig, GenesisValidator, AtipicialChainSpec,
    ProtocolSettings,
};
use atipicial_primitives::UInt256;

/// Builds a private [`AtipicialChainSpec`] from a C# `ProtocolConfiguration` file.
///
...
pub(super) fn load_private_chain_spec(
...
) -> anyhow::Result<AtipicialChainSpec> {
    ...
    AtipicialChainSpec::private(name, settings, genesis, expected).with_context(|| {
        format!(
