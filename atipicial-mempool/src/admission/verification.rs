...
//!
//! Ports C# `Transaction.Verify` (`atipicial_csharp/src/Atipicial/Network/P2P/Payloads/
...
use atipicial_payloads::{MAX_TRANSACTION_SIZE, Transaction, TransactionAttribute};
use atipicial_primitives::{UInt160, VerifyResult};
...
pub fn verify_state_independent(tx: &Transaction, settings: &ProtocolSettings) -> VerifyResult {
    ...
    // BadScriptException).
    if atipicial_vm::validate_strict_script(tx.script()).is_err() {
    ...
    for (hash, witness) in hashes.iter().zip(witnesses.iter()) {
        ...
        if atipicial_vm::script_builder::redeem_script::RedeemScript::is_signature_contract(verification)
        {
            ...
            match atipicial_crypto::ecc::EcdsaVerify::verify_signature_secp256r1(
            ...
            atipicial_vm::script_builder::redeem_script::RedeemScript::parse_multi_sig_contract(
                verification,
            )
        {
            let Some(signatures) =
    ...
    P: NativeContractProvider + 'static,
{
    ...
    // Sender ATD balance (C# TransactionVerificationContext.CheckTransaction;
    ...
    for (hash, witness) in hashes.iter().zip(witnesses.iter()) {
        let verification = witness.verification_script();
        let invocation = witness.invocation_script();
        let is_single = atipicial_vm::script_builder::redeem_script::RedeemScript::is_signature_contract(
            verification,
        ) && single_signature_invocation(invocation).is_some();
        let multi = atipicial_vm::script_builder::redeem_script::RedeemScript::parse_multi_sig_contract(
            verification,
        )
        .and_then(|(m, points)| {
            atipicial_vm::script_builder::redeem_script::RedeemScript::parse_multi_sig_invocation(
