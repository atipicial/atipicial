use super::{ATCFS_ECDSA_SHA512_PREFIX, Secp256k1Crypto, Secp256r1Crypto};
...
#[test]
fn atipicialfs_p256_sha512_signs_and_verifies() {
    let private_key = Secp256r1Crypto::generate_private_key();
    let public_key = Secp256r1Crypto::derive_public_key(&private_key).unwrap();
    let message = b"atipicialfs bearer token";

    let signature = Secp256r1Crypto::sign_atipicialfs_sha512(message, &private_key).unwrap();

    assert_eq!(signature.len(), 65);
    assert_eq!(signature[0], ATCFS_ECDSA_SHA512_PREFIX);
    assert!(Secp256r1Crypto::verify_atipicialfs_sha512(message, &signature, &public_key).unwrap());
}

#[test]
fn atipicialfs_p256_sha512_rejects_mutated_inputs() {
    let private_key = Secp256r1Crypto::generate_private_key();
    let public_key = Secp256r1Crypto::derive_public_key(&private_key).unwrap();
    let message = b"atipicialfs bearer token";
    let signature = Secp256r1Crypto::sign_atipicialfs_sha512(message, &private_key).unwrap();
    ...
    assert!(
        !Secp256r1Crypto::verify_atipicialfs_sha512(b"different message", &signature, &public_key)
            .unwrap()
    );

    let mut mutated = signature;
    mutated[64] ^= 0x01;
    assert!(!Secp256r1Crypto::verify_atipicialfs_sha512(message, &mutated, &public_key).unwrap());

    assert!(Secp256r1Crypto::verify_atipicialfs_sha512(message, &signature[..64], &public_key).is_err());
    assert!(Secp256r1Crypto::verify_atipicialfs_sha512(message, &signature, &[0x02]).is_err());
}

#[test]
fn atipicialfs_p256_sha512_preserves_ignored_prefix_behavior() {
    let private_key = Secp256r1Crypto::generate_private_key();
    let public_key = Secp256r1Crypto::derive_public_key(&private_key).unwrap();
    let message = b"atipicialfs bearer token";
    let mut signature = Secp256r1Crypto::sign_atipicialfs_sha512(message, &private_key).unwrap();
    signature[0] = 0xff;
...
#[test]
fn atipicialfs_p256_sha512_rejects_regular_p256_signature() {
    let private_key = Secp256r1Crypto::generate_private_key();
    let public_key = Secp256r1Crypto::derive_public_key(&private_key).unwrap();
    let message = b"atipicialfs bearer token";
    let signature = Secp256r1Crypto::sign(message, &private_key).unwrap();
    let mut atipicialfs_shaped_signature = [0u8; 65];
    atipicialfs_shaped_signature[0] = ATCFS_ECDSA_SHA512_PREFIX;
    atipicialfs_shaped_signature[1..].copy_from_slice(&signature);

    assert!(
        !Secp256r1Crypto::verify_atipicialfs_sha512(message, &atipicialfs_shaped_signature, &public_key)
            .unwrap()
    );
}
