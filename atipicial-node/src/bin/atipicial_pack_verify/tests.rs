...
mod tests {
    ...
    use atipicial_storage::persistence::{StoreMaintenanceBatch, WriteStore};
    use serde_json::json;
    ...
    fn put_state_tip(store: &mut RuntimeStore, height: u32, root: [u8; 32]) {
        store
            .put(
                atipicial_state_service::Keys::CURRENT_LOCAL_ROOT_INDEX.to_vec(),
                height.to_le_bytes().to_vec(),
            )
            .expect("write current StateService index");
        let key = atipicial_state_service::Keys::state_root(height);
        let mut value = vec![0];
