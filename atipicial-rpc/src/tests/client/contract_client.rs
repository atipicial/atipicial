...
async fn test_invoke_reads_integer_from_bytestring_stack_item() {
    ...
    let script_hash = AtipicialDollar::new().hash();
...
async fn create_deploy_contract_tx_builds_signed_transaction() {
    ...
    let deploy_script =
        ContractClient::build_deploy_contract_script(&aef, &manifest, CallFlags::ALL)
    ...
    let tx = contract_client
        .create_deploy_contract_tx(&aef, &manifest, &key)
        .await
        .expect("deploy tx");
