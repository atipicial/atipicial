...
use atipicial_payloads::{Block, BlockHeader, VerifiableContainer};
...
fn block_account_internal_rejects_native_hashes() {
    ...
    let atipicial_hash = *crate::hashes::ATC_TOKEN_HASH;
    let err = crate::PolicyContract::new()
        .block_account_internal(&mut engine, &atipicial_hash)
        .unwrap_err();
    assert!(err.to_string().contains("native"));
    assert!(
        snapshot
            .get(&crate::PolicyContract::blocked_account_key(&atipicial_hash))
            .is_none()
    );
}

#[test]
fn block_account_internal_faun_runs_vote_transition_for_atipicial_holders() {
    // C# BlockAccountInternal post-Faun runs ATC.VoteInternal(account,
    // null): for a ATC-holding account the full vote transition executes
    ...
    // Seed a AtipicialCoin account state holding 100 ATC.
    let atipicial_key = crate::AtipicialCoin::account_key(&account);
    let atipicial_state = BinarySerializer::serialize(
        &StackItem::from_struct(vec![
    ...
    .unwrap();
    cache.add(atipicial_key, StorageItem::from_bytes(atipicial_state));
    let snapshot = Arc::new(cache);
    let mut engine = engine_for(
        Arc::clone(&snapshot),
        Some(Block::from_parts(header, vec![])),
