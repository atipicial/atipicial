...
use atipicial_payloads::TransactionAttribute;
use atipicial_primitives::{TransactionAttributeType, UInt160, UInt256};
use num_bigint::BigInt;

impl AtipicialDollar {
    /// C# `AtipicialDollar.OnPersistAsync` (AtipicialDollar.cs:39-58): for every transaction
    ...
    /// address of `ATC.GetNextBlockValidators(...)[block.PrimaryIndex]` - with
    /// `callOnPayment: false`.
    ///
    ...
    >(
        ...
        for (transaction_index, tx_hash, sender, system_fee, network_fee, notary_nkeys) in tx_data {
            ...
            let sender = sender.ok_or_else(|| {
                CoreError::invalid_operation(
                    "AtipicialDollar::on_persist: transaction has no sender",
                )
            })?;
            let fee = system_fee.checked_add(network_fee).ok_or_else(|| {
                CoreError::invalid_operation("AtipicialDollar::on_persist: fee overflow")
            })?;
            self.gas_burn(engine, &sender, &BigInt::from(fee)).map_err(|error| {
                CoreError::invalid_operation(format!(
                    "AtipicialDollar::on_persist transaction index={transaction_index} hash={tx_hash} sender={sender} system_fee={system_fee} network_fee={network_fee}: {error}"
                ))
            })?;
            total_network_fee = total_network_fee.checked_add(network_fee).ok_or_else(|| {
                CoreError::invalid_operation("AtipicialDollar::on_persist: network fee overflow")
            })?;
        ...
        // C# `ATC.GetNextBlockValidators(snapshot, settings.ValidatorsCount)`,
        ...
        let primary = crate::AtipicialCoin::new().next_block_validator_account(
            &snapshot,
            validators_count,
            primary_index,
        )?;
