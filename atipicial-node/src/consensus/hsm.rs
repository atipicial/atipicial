...
use atipicial_consensus::ValidatorInfo;
...
/// `pin_env` environment variable (default `ATC_HSM_CU_PASSWORD`). Requires
...
pub(super) fn build_hsm_consensus_setup(
    ...
    let provider = match cfg.provider.to_ascii_lowercase().as_str() {
        "aws" => atipicial_hsm::HsmProvider::Aws,
        "azure-cloud-hsm" | "azure" => atipicial_hsm::HsmProvider::AzureCloudHsm,
        "azure-dedicated-hsm" => atipicial_hsm::HsmProvider::AzureDedicatedHsm,
        "gcp-cloud-hsm" | "gcp" => atipicial_hsm::HsmProvider::GcpCloudHsm,
        "yubihsm2" | "yubihsm" => atipicial_hsm::HsmProvider::YubiHsm2,
        "nshield" => atipicial_hsm::HsmProvider::NShield,
        "softhsm2" | "softhsm" => atipicial_hsm::HsmProvider::SoftHsm2,
        "utimaco" => atipicial_hsm::HsmProvider::Utimaco,
        "generic" | "generic-pkcs11" => atipicial_hsm::HsmProvider::GenericPkcs11,
    ...
    let library_path = cfg
        ...
        .clone()
        .map(PathBuf::from)
        .unwrap_or_else(|| PathBuf::from(atipicial_hsm::profile(provider).default_library));
    ...
    let hsm_config = atipicial_hsm::HsmConfig {
        provider,
        library_path,
        slot: cfg.slot,
    ...
    let signer = atipicial_hsm::Pkcs11Signer::connect(&hsm_config)
...
pub(super) fn build_hsm_consensus_setup(
    ...
    anyhow::bail!(
        "[consensus.hsm] is configured but atipicial-node was built without the `hsm` feature; rebuild with --features hsm"
    )
}
