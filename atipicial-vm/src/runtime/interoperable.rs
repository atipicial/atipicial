...
//!
//! Atipicial native state types expose an `IInteroperable`-style projection to and
//! from the canonical VM [`StackItem`](crate::StackItem). This trait stays in
//! `atipicial-vm` because payload, manifest, and execution crates all project values
...
atipicial_error::impl_error_from_struct!(atipicial_error::CoreError, InteroperableError => InvalidOperation);

/// Host type that can project itself to and from a AtipicialVM stack item.
