...
fn serialize_json_string() {
    let data = "Hello, Atipicial!".to_string();
    let json = serialize_json(&data).unwrap();
    assert!(json.contains("Hello, Atipicial!"));
}

#[test]
fn deserialize_json_empty_returns_error() {
    let result: atipicial_error::Result<i32> = deserialize_json("");
...
fn compress_data_and_decompress_roundtrip() {
    let data = b"Hello, Atipicial blockchain! This is test data.".to_vec();
