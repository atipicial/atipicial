...
#[tokio::test]
async fn test_layer_0_has_no_atipicial_dependencies() {
    let workspace_root = Path::new(env!("CARGO_MANIFEST_DIR")).parent().unwrap();

    // The only crate with NO atipicial-* dependencies is atipicial-primitives. The other
    // foundation crates (atipicial-io -> primitives; atipicial-error -> primitives + io)
    // form a small acyclic base; atipicial-serialization is a serialization *layer*
    // (it depends on the VM stack-item types), not a zero-dependency foundation.
    let strict_layer_0 = ["atipicial-primitives"];
    ...
    for crate_name in &strict_layer_0 {
        ...
        let deps = parse_atipicial_dependencies(&cargo_toml);

        assert!(
            deps.is_empty(),
            "Strict Layer 0 crate '{}' should have NO atipicial-* dependencies, but found: {:?}",
            crate_name,
            deps
        );
    }
...
async fn test_storage_only_depends_on_primitives() {
    let workspace_root = Path::new(env!("CARGO_MANIFEST_DIR")).parent().unwrap();
    let cargo_toml = workspace_root.join("atipicial-storage").join("Cargo.toml");
    ...
    // atipicial-storage may depend on atipicial-primitives (UInt160/UInt256 key types) and
    // atipicial-error (it lifts StorageError / KeyBuilderError into atipicial_error::CoreError
    // in atipicial-storage/src/error.rs). Both are acyclic foundation crates — atipicial-error
    // depends only on atipicial-primitives + atipicial-io — so this keeps atipicial-storage at the
    // foundation layer without introducing a cycle.
    for dep in &deps {
        assert!(
            dep == "atipicial-primitives" || dep == "atipicial-error" || dep == "atipicial-io",
            "atipicial-storage should only depend on the foundation crates (atipicial-primitives, atipicial-error, atipicial-io), but found: {}",
            dep
        );
    }
}
...
#[tokio::test]
async fn test_atipicial_io_only_depends_on_primitives() {
    let workspace_root = Path::new(env!("CARGO_MANIFEST_DIR")).parent().unwrap();
    let cargo_toml = workspace_root.join("atipicial-io").join("Cargo.toml");
    ...
    // atipicial-io can depend on atipicial-primitives (for Serializable impls)
    for dep in &deps {
        assert_eq!(
            dep, "atipicial-primitives",
            "atipicial-io should only depend on atipicial-primitives, but found: {}",
            dep
        );
    }
}
...
async fn test_crypto_only_depends_on_layer_0() {
    let workspace_root = Path::new(env!("CARGO_MANIFEST_DIR")).parent().unwrap();
    let cargo_toml = workspace_root.join("atipicial-crypto").join("Cargo.toml");
    ...
    let deps = parse_atipicial_dependencies(&cargo_toml);
    // atipicial-error is part of the acyclic foundation; atipicial-crypto maps its typed
    // errors into CoreError but owns no codec or trie mechanics.
    let allowed = ["atipicial-primitives", "atipicial-error"];

    for dep in &deps {
        assert!(
            allowed.contains(&dep.as_str()),
            "atipicial-crypto (Layer 1) should only depend on Layer 0 crates, but found: {}",
            dep
        );
    }
}
...
fn test_trie_protocol_ownership_is_exclusive() {
    let workspace_root = Path::new(env!("CARGO_MANIFEST_DIR")).parent().unwrap();
    let crypto_root = workspace_root.join("atipicial-crypto");
    let trie_root = workspace_root.join("atipicial-trie");

    assert!(
        !crypto_root.join("src/mpt_trie").exists(),
        "Atipicial MPT mechanics belong exclusively to atipicial-trie"
    );
    let crypto_library =
        fs::read_to_string(crypto_root.join("src/lib.rs")).expect("read atipicial-crypto crate root");
    for forbidden in [
        "pub mod mpt_trie",
        "MptCache",
        "MptStoreSnapshot",
        ...
        assert!(
            ...
            "atipicial-crypto must not restore MPT facade `{forbidden}`"
        );
    }
    ...
    for required in [
        ...
        assert!(
            ...
            "atipicial-trie is missing canonical implementation file `{required}`"
        );
    }

    let trie_dependencies = parse_atipicial_dependencies(&trie_root.join("Cargo.toml"));
    assert!(
        trie_dependencies
            .iter()
            .any(|dependency| dependency == "atipicial-crypto"),
        "atipicial-trie must delegate Hash256 to atipicial-crypto"
    );
    for forbidden in ["atipicial-storage", "atipicial-state-service", "atipicial-rpc", "atipicial-node"] {
        assert!(
            !trie_dependencies
                .iter()
                .any(|dependency| dependency == forbidden),
            "backend-independent atipicial-trie must not depend on `{forbidden}`"
        );
    }

    let canonical_consumers = [
        "atipicial-state-service/src/storage/mpt_store.rs",
        "atipicial-state-service/src/providers/proof.rs",
        "atipicial-node/src/node/state_packs/authority.rs",
        "atipicial-rpc/src/tests/server/rpc_server_state/proof.rs",
    ];
    for relative_path in canonical_consumers {
        let source = fs::read_to_string(workspace_root.join(relative_path))
            .unwrap_or_else(|error| panic!("failed to read {relative_path}: {error}"));
        assert!(
            source.contains("atipicial_trie"),
            "MPT consumer `{relative_path}` must import the exclusive atipicial-trie owner"
        );
        assert!(
            !source.contains("atipicial_crypto::mpt_trie"),
...
fn test_rpc_exception_is_owned_exclusively_by_rpc() {
    ...
    let rpc_exception = workspace_root.join("atipicial-rpc/src/server/rpc_exception/mod.rs");

    assert!(
        !primitives_root.join("src/errors/rpc_exception.rs").exists(),
        "atipicial-primitives must not contain the RPC handler exception implementation"
    );
    ...
    assert!(
        !source.contains("pub use atipicial_primitives::RpcException"),
        "atipicial-rpc must own RpcException instead of restoring the primitives facade"
    );
}

#[tokio::test]
async fn test_all_atipicial_crates_are_classified() {
    let workspace_root = Path::new(env!("CARGO_MANIFEST_DIR")).parent().unwrap();
    ...
    assert!(
        ...
        "Every atipicial-* crate with a Cargo.toml should have an explicit architecture layer: {:?}",
        unclassified
    );
}
...
async fn test_development_workspace_crates_are_not_publishable() {
    let workspace_root = Path::new(env!("CARGO_MANIFEST_DIR")).parent().unwrap();
    let dev_crates = [
        ("tests/Cargo.toml", "atipicial-tests"),
        ("benches-package/Cargo.toml", "atipicial-benches"),
    ];
...
async fn test_workspace_dependencies_match_runtime_workspace_members() {
    ...
    for member in &workspace_members {
        ...
        if !package_name.starts_with("atipicial-") {
            continue;
        }
    ...
    for (dependency_name, dependency_spec) in workspace_dependencies {
        if !dependency_name.starts_with("atipicial-") {
            continue;
        }
    ...
    assert!(
        ...
        "Internal atipicial-* workspace dependencies should point at active workspace members: {:?}",
...
async fn test_development_workspace_crates_are_not_default_members() {
    ...
    // Only atipicial-node needs to be in default-members; atipicial-rpc and atipicial-indexer
    ...
    // `cargo build -p atipicial-rpc` or `cargo build -p atipicial-indexer`.
    let runtime_entrypoint = "atipicial-node";
    assert!(
        default_members
            .iter()
...
async fn test_standalone_directories_remain_excluded_from_workspace() {
    ...
    let required_excludes = ["atipicial-gui", "fuzz", "examples", "docs"];
...
fn test_same_layer_dependencies_are_explicit() {
    ...
    for crate_name in &crates {
        ...
        let cargo_toml = workspace_root.join(crate_name).join("Cargo.toml");
        for dependency in parse_atipicial_dependencies(&cargo_toml) {
...
async fn test_runtime_vocabulary_stays_below_composition_root() {
    ...
    let runtime_deps =
        parse_atipicial_dependencies(&workspace_root.join("atipicial-runtime").join("Cargo.toml"));
    assert!(
        runtime_deps
            .iter()
            .all(|dep| matches!(Layer::from_crate_name(dep), Some(layer) if layer < Layer::DomainServices)),
        "atipicial-runtime must stay a shared lower-layer vocabulary crate, but it depends on: {:?}",
        runtime_deps
    );

    let system_deps = parse_atipicial_dependencies(&workspace_root.join("atipicial-system").join("Cargo.toml"));
    assert!(
        system_deps.iter().any(|dep| dep == "atipicial-runtime"),
        "atipicial-system should consume atipicial-runtime service traits instead of owning that vocabulary"
    );

    for service_crate in ["atipicial-network", "atipicial-blockchain", "atipicial-mempool"] {
        let deps = parse_atipicial_dependencies(&workspace_root.join(service_crate).join("Cargo.toml"));
        assert!(
            !deps.iter().any(|dep| dep == "atipicial-system"),
            "{service_crate} must not depend on atipicial-system; shared service vocabulary belongs in atipicial-runtime"
        );
    }
}
...
fn test_runtime_does_not_reclaim_chain_config_or_node_aggregate_traits() {
    let workspace_root = Path::new(env!("CARGO_MANIFEST_DIR")).parent().unwrap();
    let runtime_root = workspace_root.join("atipicial-runtime");

    let runtime_deps = parse_atipicial_dependencies(&runtime_root.join("Cargo.toml"));
    assert!(
        !runtime_deps
            .iter()
            .any(|dependency| dependency == "atipicial-config"),
        "atipicial-runtime owns narrow service capabilities; immutable chain configuration belongs to atipicial-config"
    );

    assert!(
        !runtime_root.join("src/node/types.rs").exists(),
        "atipicial-runtime must not restore the removed node aggregate-trait module"
    );

    let checked_files = [
        "atipicial-runtime/src/lib.rs",
        "atipicial-runtime/src/node/mod.rs",
        "atipicial-runtime/src/node/providers.rs",
    ];
    let forbidden = [
        "ConfigProvider",
        "NodeTypes",
    ...
    assert!(
        violations.is_empty(),
        "atipicial-runtime must expose narrow capabilities instead of a duplicate node/config type family:\n{}",
        violations.join("\n")
    );
}
