...
//! The full mempool implementation remains in `atipicial-mempool`; this module keeps
...
/// attribute detection, and reverify queues stay in `atipicial-mempool` and are
...
pub trait MempoolLike: std::fmt::Debug + Send + Sync {
    ...
    fn add_transaction<B, L>(
        &self,
        origin: atipicial_mempool::TransactionOrigin,
        tx: &atipicial_payloads::Transaction,
        snapshot: &atipicial_storage::DataCache<B>,
        ledger_provider: &L,
    ) -> atipicial_mempool::TransactionAdmissionOutcome
    where
    ...
    fn block_persisted(&self, _block: &atipicial_payloads::Block) {}
...
impl<P> MempoolLike for atipicial_mempool::MemoryPool<P>
where
    P: NativeContractProvider + 'static,
{
    ...
    where
        ...
        L: atipicial_mempool::AdmissionLedgerProvider,
    {
        atipicial_mempool::MemoryPool::add_transaction(
            self,
            origin,
            tx.clone(),
            snapshot,
    ...
    fn block_persisted(&self, block: &atipicial_payloads::Block) {
    ...
    fn reverify_top_unverified<B: atipicial_storage::CacheRead>(
        ...
        atipicial_mempool::MemoryPool::reverify_top_unverified(self, snapshot, max_count)
    }
}
