...
use atipicial_payloads::{Block, ExtensiblePayload, HeadersPayload, MerkleBlockPayload, Transaction};
        ...
        match message.command {
            ...
            MessageCommand::GetBlocks => {
                ...
                if let Some(source) = self.block_source.clone() {
                    if let Some(start_index) = source.block_index_by_hash(&payload.hash_start) {
                        let count = if payload.count < 0 {
                            atipicial_payloads::inv_payload::MAX_HASHES_COUNT as u32
                        } else {
                            (payload.count as u32)
                                .min(atipicial_payloads::inv_payload::MAX_HASHES_COUNT as u32)
                        };
                ...
                if let Some(source) = self.block_source.clone() {
                    let unknown: Vec<UInt256> = payload
                        ...
                        .filter(|hash| match payload.inventory_type {
                            ...
                            // Atipicial N3 pulls ExtensiblePayload inventory by hash;
            ...
            MessageCommand::GetAddr => {
                let addrs = self.registry.listener_addresses(
                    self.peer_id,
                    atipicial_payloads::addr_payload::MAX_COUNT_TO_SEND,
                );
                if !addrs.is_empty() {
