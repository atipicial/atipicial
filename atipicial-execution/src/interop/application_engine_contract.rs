//! ApplicationEngine.Contract - ports Atipicial.SmartContract.ApplicationEngine.Contract.cs
...
use atipicial_crypto::bls12381_point::{G1_COMPRESSED_SIZE, G2_COMPRESSED_SIZE, GT_SIZE};
...
fn native_call_trace_tx_filter() -> Option<UInt256> {
    std::env::var("ATC_TRACE_CALL_NATIVE_TX")
        .ok()
    ...
    B: atipicial_storage::CacheRead,
{
    ...
    native_call_trace_filter_matches(
        ...
        *TRACE_ALL.get_or_init(|| env_flag_enabled("ATC_TRACE_CALL_NATIVE", false)),
...
fn contract_call_trace_tx_filter() -> Option<UInt256> {
    std::env::var("ATC_TRACE_CONTRACT_CALL_TX")
        .ok()
        .and_then(|raw| UInt256::from_str(raw.trim()).ok())
}
    ...
    B: atipicial_storage::CacheRead,
{
    contract_call_trace_filter_matches(
        ...
        env_flag_enabled("ATC_TRACE_CONTRACT_CALL", false),
        current_transaction_hash(app),
    )
}
...
where
    ...
    B: atipicial_storage::CacheRead,
{
    // C# parity (Atipicial.Extensions.ScriptBuilderExtensions.EmitDynamicCall):
...
where
    ...
    B: atipicial_storage::CacheRead,
{
    // C# parity (Atipicial.SmartContract.ApplicationEngine.Contract.cs):
    ...
    B: atipicial_storage::CacheRead,
{
    let result = (|| -> CoreResult<()> {
        ...
        // Load any queued calls requested by the native method (e.g. AEP-17 callbacks).
...
fn decode_native_result(
    ...
    match return_type {
        ...
        ContractParameterType::Array | ContractParameterType::Map => {
            if result.is_empty() {
                // Atipicial native methods use an empty payload to encode `null` results
