//! # atipicial-node::node::append_shadow
//!
...
//! ## Boundary
//!
//! This module belongs to `atipicial-node`. It wires configuration to the
//! `atipicial-state-packs` shadow adapter; it defines no pack format or storage
//! semantics (those live in `atipicial-state-packs`).
//!
//! ## Contents
//!
...
use atipicial_state_packs::shadow::{
    PreparedShadowFrame, SHADOW_DEGRADED_KEY, SHADOW_HIGH_WATER_KEY, ShadowDegradedRecord,
    ShadowHighWaterRecord, ShadowPackWriter,
};
use atipicial_state_packs::{PackFrameContext, PackStoreConfig};
use atipicial_state_service::keys::{STATE_ROOT_PREFIX, state_root_index};
...
impl AppendShadow {
    ...
    pub(in crate::node) fn canonical_commit_succeeded(&self) {
        ...
        if let Err(error) = writer.activate_prepared(prepared) {
            warn!(
                target: "atipicial::append_shadow",
...
fn load_canonical_tip(
    ...
    let state_service = canonical_store
        .open_coordinated_namespace(atipicial_state_service::MDBX_STATE_SERVICE_NAMESPACE)
...
where
    R: atipicial_storage::persistence::RawReadOnlyStore + ?Sized,
{
    ensure!(
...
mod tests {
    ...
    use atipicial_storage::persistence::{StoreMaintenanceBatch, WriteStore, storage::StorageConfig};
    ...
    fn state_root_entry(index: u32, root_byte: u8) -> (Vec<u8>, Option<Vec<u8>>) {
        let key = atipicial_state_service::Keys::state_root(index);
    ...
    fn state_root_span_rejects_short_values() {
        let key = atipicial_state_service::Keys::state_root(7);
    ...
    fn high_water_validation_binds_historical_previous_and_resulting_roots() {
        ...
        let record = ShadowHighWaterRecord {
            epoch: 0,
            frames_total: 1,
            segment_id: atipicial_state_packs::PackSegmentId::INITIAL,
            frame_end: atipicial_state_packs::PACK_SEGMENT_HEADER_LEN + 1,
            node_operations: 1,
        ...
        roots
            .put(
                atipicial_state_service::Keys::state_root(42),
                atipicial_state_service::StateRoot::new_current(
                    42,
                    atipicial_primitives::UInt256::from([0x44; 32]),
                )
                .to_array(),
            )
        ...
        roots
            .put(
                ...
                atipicial_state_service::StateRoot::new_current(
                    42,
                    atipicial_primitives::UInt256::from([0x22; 32]),
                )
                .to_array(),
            )
        ...
        roots
            .put(
                atipicial_state_service::Keys::state_root(40),
                atipicial_state_service::StateRoot::new_current(
                    40,
                    atipicial_primitives::UInt256::from([0x55; 32]),
                )
                .to_array(),
            )
    ...
    fn mirror_window_produces_marker_and_reopenable_exact_shadow() {
        ...
        let store = atipicial_state_packs::PackStore::open(&shadow_path, test_pack_config())
    ...
    fn mirror_window_is_invisible_until_canonical_success() {
        ...
        let record = ShadowHighWaterRecord::decode(&marker.value).expect("decode marker");
        drop(shadow);
        let store = atipicial_state_packs::PackStore::open_at_commit_horizon(
            &shadow_path,
            test_pack_config(),
            Some(atipicial_state_packs::PackCommitHorizon {
                epoch: record.epoch,
