...
use atipicial_vm::{CandidateContract, HardforkTableIdentity, VmError};
...
pub(in crate::application_engine) fn flamingo_cpu_fee_policy(
    ...
    for step in candidate.gas_steps() {
        match step.amount {
            atipicial_vm::GasAmount::Fixed(units) => {
                declarations.push(HostAccessDeclaration::CpuFeeCharge(units));
            }
            atipicial_vm::GasAmount::Decision {
                when_true,
                when_false,
                ..
            } => {
            ...
            atipicial_vm::GasAmount::ArgumentBytes { .. } => return None,
        }
    }
