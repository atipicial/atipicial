...
/// via ATC.VoteInternal (candidate weight drops, VoteTo cleared,
...
fn block_account_e2e_faun_clears_vote_and_stamps_time() {
    ...
    let candidate_key = crate::AtipicialCoin::candidate_key(&candidate);
    cache.add(
        candidate_key.clone(),
    ...
    let voter_key = crate::AtipicialCoin::account_key(&voter);
    cache.add(
        voter_key.clone(),
        StorageItem::from_bytes(
    ...
    let voters_count_key = crate::AtipicialCoin::voters_count_key();
    cache.add(
    ...
    // The candidate lost the voter's 100-ATC weight (still registered).
