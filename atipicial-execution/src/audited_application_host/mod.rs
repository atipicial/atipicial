//! # atipicial-execution::audited_application_host
//!
...
//! ordinary AtipicialVM execution as the semantic authority.
//!
//! ## Contents
//!
...
use atipicial_payloads::{NotifyEventArgs, VerifiableContainer};
use atipicial_primitives::CallFlags;
use atipicial_primitives::{Hardfork, TriggerType, UInt160};
use atipicial_storage::{CacheRead, SeekDirection, StorageKey};
    ...
    B: CacheRead,
{
    ...
    ///
    /// Atipicial's put fee depends on the old value, so both `StorageRead` and
    ...
    pub fn storage_find(
        ...
        if options.bits() & !atipicial_primitives::FindOptions::Backwards.bits() != 0 {
            return Err(CoreError::invalid_operation(
        ...
        let direction = match range.direction() {
            atipicial_vm::RangeDirection::Forward => SeekDirection::Forward,
            atipicial_vm::RangeDirection::Reverse => SeekDirection::Backward,
        };
    ...
    fn validate_contract_resolution(&self, access: &ContractCallAccess) -> Result<(), CoreError> {
        ...
        if contract.id != expected.contract_id()
            ...
            || contract.aef.checksum != expected.aef_checksum()
        {
            return Err(CoreError::invalid_operation(
        ...
        let result_count =
            usize::from(method.return_type != atipicial_primitives::ContractParameterType::Void);
