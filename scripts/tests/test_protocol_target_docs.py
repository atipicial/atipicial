...
class ProtocolTargetDocsTests(unittest.TestCase):
    ...
    def test_consistency_validator_handles_unavailable_rpc_selection_explicitly(self):
        ...
        self.assertRegex(
            text,
            re.compile(r'atipicial_rpc="\$\(select_rpc .*?\)" \|\| atipicial_rpc=""'),
        )
        self.assertIn(
    ...
    def test_release_guide_names_current_atipicial_n3_target(self):
    ...
    def test_csharp_compatibility_tests_do_not_treat_treasury_as_noncanonical(self):
        ...
        self.assertNotIn("not in C# Atipicial v3.9.1's standard set", text)

    def test_active_protocol_utility_scripts_name_current_atipicial_n3_target(self):
        paths = [
    ...
    def test_consistency_validator_uses_current_atipicial_n3_target(self):
        legacy = REPO_ROOT / "scripts" / "validate-v391-consistency.sh"
        canonical = REPO_ROOT / "scripts" / "validate-v310-consistency.sh"

        self.assertTrue(canonical.exists())
        canonical_text = canonical.read_text(encoding="utf-8")
        self.assertIn("Atipicial v3.10.1", canonical_text)
        self.assertIn("Atipicial:3.10.1", canonical_text)
        self.assertIn("compat-v310", canonical_text)
        self.assertNotIn("Atipicial v3.9.1", canonical_text)
        self.assertNotIn("Atipicial:3.9.1", canonical_text)
    ...
    def test_protocol_baseline_records_official_schedules_and_evidence_scope(self):
        ...
        self.assertNotIn("## Atipicial N3 v3.10.1 Parity", text)
    ...
    def test_active_architecture_docs_record_the_workspace_canonical_vm_boundary(self):
        ...
        node_readme = (REPO_ROOT / "atipicial-node" / "README.md").read_text(
            encoding="utf-8"
        )
        ...
        for document, text in (("architecture", architecture), ("design", design)):
            with self.subTest(document=document):
                prose = normalized(text)
                self.assertIn("workspace `atipicial-vm`", prose)
                self.assertIn("sole canonical", prose)
                self.assertNotIn("atipicial-vm-rs", prose)
    ...
    def test_adr_log_records_authority_semantics_and_evidence_limits(self):
        ...
        self.assertIn("### ADR-044: Workspace AtipicialVM authority and canonical local execution", text)
        self.assertIn(
    ...
    def test_active_regression_test_names_use_current_atipicial_n3_target(self):
        stale_markers = []
        search_roots = [
            REPO_ROOT / "atipicial-crypto" / "src" / "tests",
            REPO_ROOT / "atipicial-payloads" / "src" / "tests",
        ...
        self.assertEqual(
            [],
            stale_markers,
            "active regression test names should use v3101 for the current Atipicial N3 target",
        )

    def test_active_rust_sources_do_not_claim_v391_reference_target(self):
        stale_markers = []
        for root in REPO_ROOT.glob("atipicial-*/src"):
    ...
    def test_consistency_workflow_names_current_atipicial_n3_target(self):
        ...
        self.assertIn("Atipicial v3.10.1 Consistency", text)
        self.assertIn("validate-v310-consistency.sh", text)
        ...
        self.assertNotIn("Atipicial v3.9.1 Consistency", text)
