...
fn vote_assigns_weight_distributes_gas_and_records_target() {
    ...
    cache.update(
        ...
        StorageItem::from_bytes(
            AtipicialCoin::encode_candidate_state(true, &BigInt::from(0)).unwrap(),
        ),
    );
    let voter_state = AtipicialAccountStateView {
    ...
    cache.update(
        AtipicialCoin::account_key(&voter),
        StorageItem::from_bytes(
            AtipicialCoin::encode_atipicial_account_state(&voter_state).unwrap(),
        ),
    );
    ...
    let (_, cand_votes) = AtipicialCoin::decode_candidate_state(
        &snapshot
            .get(&AtipicialCoin::candidate_key(&candidate))
            .unwrap()
            .value_bytes(),
    )
    .unwrap();
    ...
    let acct = AtipicialCoin::decode_atipicial_account_state(
        &AtipicialCoin::new()
    ...
    let gas_key = crate::AtipicialDollar::account_key(&voter);
    let gas_item = snapshot.get(&gas_key).expect("voter ATD account written");
    ...
    // Reward scales with the 1B ATC supply: 100 * 5e8 * 10 / 100 / 1e9 * 100 = 500.
    assert_eq!(gas_balance, BigInt::from(500));
}
