...
use atipicial_consensus::messages::{ConsensusPayload, PrepareRequestMessage};
use atipicial_consensus::{ChangeViewReason, ConsensusMessageType, ValidatorInfo};
use atipicial_crypto::ECPoint;
use atipicial_execution::native_contract_provider::NativeContractProvider;
use atipicial_io::{Serializable, serializable::helper::SerializeHelper};
use atipicial_mempool::{MemoryPool, PoolItem, verify_transaction_with_native_provider};
use atipicial_payloads::{Transaction, TransactionAttribute, Witness};
use atipicial_primitives::{UInt160, UInt256, VerifyResult};
use atipicial_storage::persistence::DataCache;
use atipicial_vm::script_builder::{RedeemScript, ScriptBuilder};
    ...
    B: atipicial_storage::CacheRead,
{
    ...
    for hash in &request.transaction_hashes {
        match native.contains_transaction(snapshot, hash) {
            Ok(true) => {
                warn!(target: "atipicial", %hash, "rejected PrepareRequest: transaction already exists on-chain");
                return false;
            }
            Ok(false) => {}
            Err(error) => {
                warn!(target: "atipicial", %hash, %error, "failed to check PrepareRequest transaction existence");
                return false;
            }
        }
    }
    ...
    let max_traceable_blocks = match native.max_traceable_blocks(snapshot, settings) {
        Ok(value) => value,
        Err(error) => {
            warn!(target: "atipicial", %error, "failed to read MaxTraceableBlocks for PrepareRequest guard");
            return false;
        }
    };
    ...
    for hash in &request.transaction_hashes {
        ...
        match native.contains_conflict_hash(snapshot, hash, &signers, max_traceable_blocks) {
            Ok(true) => {
                warn!(target: "atipicial", %hash, "rejected PrepareRequest: transaction has on-chain conflict");
                return false;
            }
            Ok(false) => {}
            Err(error) => {
                warn!(target: "atipicial", %hash, %error, "failed to check PrepareRequest transaction conflict");
                return false;
            }
        }
    }
