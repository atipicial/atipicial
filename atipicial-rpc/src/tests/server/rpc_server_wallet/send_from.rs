...
fn wallet_methods_require_open_wallet() {
    ...
    let asset = AtipicialDollar::script_hash().to_string();
...
async fn send_from_multisig_collects_member_signatures() {
    ...
    // Build an in-memory AEP-6 wallet with two single-sig member accounts (each
    // holding its private key) plus a watch-only 2-of-2 multi-sig account.
    let wallet = Aep6Wallet::new(Some("multisig".to_string()), None, settings.clone());
