...
pub(in crate::node) async fn wait_for_shutdown_signal(
...
async fn wait_for_stop_height(
    mut events: tokio::sync::broadcast::Receiver<atipicial_blockchain::RuntimeEvent>,
    target_height: u32,
) -> std::io::Result<String> {
    ...
    loop {
        match events.recv().await {
            Ok(atipicial_blockchain::RuntimeEvent::Imported { height, .. })
                if height >= target_height =>
            {
                return Ok(format!("stop-at-height {target_height}"));
            }
            Ok(atipicial_blockchain::RuntimeEvent::Imported { .. })
            | Ok(atipicial_blockchain::RuntimeEvent::Reverted { .. })
            | Ok(atipicial_blockchain::RuntimeEvent::TipChanged { .. })
            | Ok(atipicial_blockchain::RuntimeEvent::RelayResult { .. }) => {}
            Ok(atipicial_blockchain::RuntimeEvent::Shutdown) => {
