...
use atipicial_wallets::{Aep6Wallet, Wallet, WalletResult};
...
impl RpcServerWallet {
    ...
    pub(super) fn save_wallet(wallet: &Arc<Aep6Wallet>) -> Result<(), RpcException> {
        let wallet_clone = Arc::clone(wallet);
        Self::await_wallet_future(async move { wallet_clone.save().await })
    }

    pub(super) fn require_wallet(server: &RpcServer) -> Result<Arc<Aep6Wallet>, RpcException> {
        server
            .wallet()
