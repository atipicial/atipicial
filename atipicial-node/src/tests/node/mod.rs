//! # atipicial-node::tests::node
//!
//! Test module grouping Daemon composition, CLI modes, and long-running node
//! startup. coverage for atipicial-node.
//!
//! ## Boundary
//!
//! This is test/benchmark-only code for atipicial-node; it may assemble fixtures but
...
use atipicial_network::{Message, MessageCodec, MessageCommand};
use atipicial_payloads::p2p_payloads::{GetBlockByIndexPayload, NodeCapability, VersionPayload};
...
fn decode_payload<T: atipicial_io::Serializable>(message: &Message) -> T {
    let mut reader = atipicial_io::MemoryReader::new(&message.payload_raw);
    <T as atipicial_io::Serializable>::deserialize(&mut reader).expect("decode payload")
}
...
fn empty_child_block(parent: &atipicial_payloads::Block, index: u32) -> atipicial_payloads::Block {
    ...
    atipicial_payloads::Block::from_parts(header, Vec::new())
}

fn signed_empty_child_block(
    parent: &atipicial_payloads::Block,
    index: u32,
    network: u32,
    private_key: &[u8; 32],
    public_key: &atipicial_crypto::ECPoint,
    ...
    let signature =
        atipicial_crypto::Secp256r1Crypto::sign(&sign_data, private_key).expect("sign header");
    let verification =
    ...
    block.header.witness = atipicial_payloads::Witness::new_with_scripts(invocation, verification);
    block
}
...
fn seed_store_tip(
    backend: &str,
    path: &Path,
    chain_spec: &atipicial_config::AtipicialChainSpec,
    tip: u32,
) -> anyhow::Result<()> {
    ...
    let resources = atipicial_blockchain::NativePersistResources::from_provider(Arc::new(
        atipicial_native_contracts::StandardNativeProvider::new(),
    ));

    let mut parent = Arc::new(atipicial_blockchain::genesis_block(chain_spec)?);
    ...
    let current_index = atipicial_native_contracts::LedgerContract::new()
...
fn remote_ledger_mode_rejects_local_import_sources() {
    let cli = NodeCli {
        config: PathBuf::from("atipicial_testnet_node.toml"),
        network_magic: None,
        storage_path: None,
        check_config: false,
        check_storage: false,
...
fn remote_ledger_preflight_skips_local_storage_validation() {
    let cli = NodeCli {
        config: PathBuf::from("atipicial_mainnet_node.toml"),
        network_magic: None,
        storage_path: Some(PathBuf::from("/tmp/atipicial-remote-ledger-unused")),
        check_config: false,
        check_storage: false,
        check_all: true,
        import_chain: None,
...
fn local_preflight_still_validates_local_storage() {
    let cli = NodeCli {
        config: PathBuf::from("atipicial_mainnet_node.toml"),
        network_magic: None,
        storage_path: Some(PathBuf::from("/tmp/atipicial-local-ledger")),
        check_config: false,
        check_storage: true,
        check_all: false,
        import_chain: None,
...
async fn inventory_block_batch_flush_sends_one_batch_command_and_clears_buffer() {
    let (handle, mut cmd_rx, _event_tx) = atipicial_blockchain::BlockchainHandle::channel(4, 4);
    let import_queue = Arc::new(atipicial_runtime::BlockImportQueue::new(
        Arc::new(handle.clone()),
        2,
    ));
    let live_import = atipicial_system::LiveBlockImportPipeline::new(handle, import_queue);
    let block = Arc::new(atipicial_payloads::Block::new());
    ...
    match cmd_rx.recv().await.expect("batch command") {
        atipicial_blockchain::BlockchainCommand::CheckedInventoryBlocks { checked, relay } => {
