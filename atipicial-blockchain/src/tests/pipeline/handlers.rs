...
use atipicial_config::{ChainSpecProvider, AtipicialChainSpec};
use atipicial_mempool::{TransactionAdmissionError, TransactionAdmissionOutcome, TransactionOrigin};
use atipicial_payloads::Block;
use atipicial_payloads::InventoryType;
use atipicial_payloads::Transaction;
...
type NativeProviderArc = Arc<atipicial_native_contracts::StandardNativeProvider>;

fn standard_native_provider() -> NativeProviderArc {
    Arc::new(atipicial_native_contracts::StandardNativeProvider::new())
}

fn default_chain_spec() -> Arc<AtipicialChainSpec> {
    test_chain_spec(atipicial_config::ProtocolSettings::default())
}

fn chain_spec_for_settings(settings: &atipicial_config::ProtocolSettings) -> Arc<AtipicialChainSpec> {
...
fn sign_header_for_test(
    ...
    atipicial_payloads::Witness::new_with_scripts(invocation, verification.to_vec())
}
...
impl MempoolLike for RecordingMempool {
    ...
    fn reverify_top_unverified<B: atipicial_storage::CacheRead>(
        &self,
        _snapshot: &atipicial_storage::DataCache<B>,
        _max_count: usize,
    ) -> bool {
...
fn extensible_verification_uses_system_native_provider() {
    ...
    assert!(
        !verifier.contains("AtipicialCoin::new()"),
        "extensible payload verification must not construct native ATC directly"
    );
    assert!(
    ...
    assert!(
        !provider.contains("AtipicialCoin::new()"),
        "extensible provider seam must resolve AtipicialCoin through the explicit native provider"
    );
    assert!(
...
fn transaction_admission_delegates_to_the_mempool_owned_validator() {
    ...
    assert!(
        ...
        "validation and native policy reads belong exclusively to atipicial-mempool"
    );
    assert!(
...
fn fixture_with_mempool_result(
    ...
    BlockchainHandle,
    Arc<atipicial_storage::DataCache>,
) {
...
struct StoreContext {
    ...
    state_service: Option<Arc<atipicial_state_service::commit_handlers::StateServiceCommitHandlers>>,
...
fn store_fixture() -> (
    ...
    store_fixture_with(atipicial_config::ProtocolSettings::default())
}

fn real_mempool_store_fixture() -> (
    BlockchainService<
        StoreContext,
        atipicial_mempool::MemoryPool<atipicial_native_contracts::StandardNativeProvider>,
    >,
    BlockchainHandle,
    Arc<atipicial_storage::DataCache>,
) {
    ...
    let ledger = Arc::new(LedgerContext::default());
    let header_cache = Arc::new(HeaderCache::default());
    let mempool = Arc::new(atipicial_mempool::MemoryPool::new_with_native_contract_provider(
        chain_spec,
...
fn optimistic_header_verification_publishes_only_the_verified_prefix() {
    let (mut service, _handle, _snapshot) =
        store_fixture_with(atipicial_config::ProtocolSettings::default());
    let valid_witness =
        atipicial_payloads::Witness::new_with_scripts(Vec::new(), vec![atipicial_vm::OpCode::PUSH1.byte()]);
...
fn optimistic_header_queue_contention_falls_back_without_truncating_valid_batch() {
    ...
    let public_key =
        atipicial_crypto::Secp256r1Crypto::derive_public_key(&private_key).expect("derive public key");
    let verification =
        atipicial_vm::script_builder::redeem_script::RedeemScript::signature_redeem_script(&public_key);
...
fn store_fixture_with_state_service() -> (
    ...
    BlockchainHandle,
    Arc<atipicial_storage::DataCache>,
    Arc<atipicial_state_service::StateStore>,
) {
    let snapshot = Arc::new(atipicial_storage::DataCache::new(false));
...
fn store_fixture_counting_snapshot_and_commits() -> (
    ...
    store_fixture_counting_snapshot_and_commits_with(atipicial_config::ProtocolSettings::default())
}
...
/// ATC total supply read (AEP-17 `Prefix_TotalSupply` = 11).
fn atipicial_total_supply(snapshot: &atipicial_storage::DataCache) -> Option<num_bigint::BigInt> {
    snapshot
...
fn transaction_with_nonce(nonce: u32) -> Transaction {
    ...
    tx.set_signers(vec![atipicial_payloads::Signer::new(
        atipicial_primitives::UInt160::from_bytes(&[0x33; 20]).expect("test signer"),
...
fn fund_test_signer_gas<B: atipicial_storage::CacheRead>(
    snapshot: &atipicial_storage::DataCache<B>,
    amount: i64,
) {
    let signer = atipicial_primitives::UInt160::from_bytes(&[0x33; 20]).expect("test signer");
    let mut gas_key = vec![20];
    gas_key.extend_from_slice(&signer.to_bytes());
    let account_state =
        atipicial_vm::StackItem::from_struct(vec![atipicial_vm::StackItem::from_int(BigInt::from(amount))]);
    let account_bytes =
        BinarySerializer::serialize(&account_state, &ExecutionEngineLimits::default())
            .expect("serialize ATD account");
    snapshot.add(
    ...
    let supply_key = StorageKey::new(atipicial_native_contracts::AtipicialDollar::ID, vec![11]);
    let supply = snapshot
        .get(&supply_key)
    ...
    snapshot.update(
        supply_key,
        atipicial_storage::StorageItem::from_bytes(supply.to_signed_bytes_le()),
    );
}

fn seed_current_ledger<B: atipicial_storage::CacheRead>(
    snapshot: &atipicial_storage::DataCache<B>,
    index: u32,
) {
    let hash = UInt256::from_bytes(&[0u8; 32]).expect("zero hash");
    let bytes = atipicial_native_contracts::LedgerContract::new()
    ...
    snapshot.add(
        ...
        atipicial_storage::StorageItem::from_bytes(bytes),
    );
}

fn seed_conflict_record<B: atipicial_storage::CacheRead>(
    snapshot: &atipicial_storage::DataCache<B>,
    hash: &UInt256,
    signer: &atipicial_primitives::UInt160,
    index: u32,
) {
    let stub = atipicial_native_contracts::LedgerContract::new()
    ...
    snapshot.add(
        atipicial_storage::StorageKey::new(atipicial_native_contracts::LedgerContract::ID, bare_key),
        atipicial_storage::StorageItem::from_bytes(stub.clone()),
    );
    ...
    snapshot.add(
        atipicial_storage::StorageKey::new(atipicial_native_contracts::LedgerContract::ID, signer_key),
        atipicial_storage::StorageItem::from_bytes(stub),
    );
}
...
fn dispatch_command_variants_is_exhaustive() {
    ...
    let _add_tx = BlockchainCommand::AddTransaction {
        transaction: atipicial_payloads::Transaction::new(),
