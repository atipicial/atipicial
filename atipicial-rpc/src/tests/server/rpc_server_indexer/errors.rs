...
use atipicial_indexer::IndexerService;
...
async fn indexer_rpc_reports_service_store_decode_errors() {
    ...
    assert!(
        err.to_string().contains("AtipicialIndexer service read failed"),
        "{err}"
    );
}
...
async fn indexer_methods_require_registered_service() {
    ...
    assert!(
        err.to_string().contains("AtipicialIndexer service not available"),
        "{err}"
    );
}
