...
/// canonical Atipicial N3 native-contract hash.
///
/// In C# Atipicial v3.10.1 each native contract sets
/// `Hash = Helper.GetContractHash(UInt160.Zero, 0, Name)`, where
/// `Name => GetType().Name` (the class name). `GetContractHash` emits
/// `ABORT, push(sender), push(aefCheckSum), push(name)` and takes the script
...
fn native_contract_hashes_match_csharp_derivation() {
    ...
    for spec in crate::standard_native_contract_specs() {
        ...
        assert_eq!(
            derived, spec.hash,
            "get_contract_hash(0, 0, {:?}) diverged from the canonical Atipicial N3 hash",
            spec.name
        );
    }
}
