...
use atipicial_storage::StorageResult;
...
/// ownership back from `atipicial-system`.
...
{
    type Overlay<'a>
        = &'a atipicial_storage::DataCache<StoreCacheBacking<S>>
    where
        Self: 'a;
    ...
    S: TransactionalStore,
{
    ...
    fn block_committing(
        &self,
        _block: &Block,
        _snapshot: &atipicial_storage::DataCache<StoreCacheBacking<S>>,
...
/// Provider-neutral context consumed by `atipicial-blockchain`.
    ...
    H: BlockCommitHooks<S> + 'static,
{
    ...
    fn block_committing(
        ...
        let accepted = self.hooks.block_committing(
            ...
            atipicial_runtime::sync_metrics::peer_live_tip(),
    ...
    async fn block_finalized(
        ...
        let result = self
            .hooks
            .block_finalized(finalized, atipicial_runtime::sync_metrics::peer_live_tip())
            .await;
        if let Err(error) = &result {
