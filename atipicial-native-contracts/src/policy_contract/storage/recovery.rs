...
impl PolicyContract {
    /// C# `ATC.GetCommittee(snapshot)`: decodes AtipicialCoin's `Prefix_Committee`
    ...
    /// indexer throw.
    pub(in crate::policy_contract) fn read_atipicial_committee_sorted(
        &self,
        snapshot: &DataCache<impl atipicial_storage::CacheRead>,
    ) -> CoreResult<Vec<ECPoint>> {
        let key = crate::AtipicialCoin::committee_key();
        ...
        let committee = crate::atipicial_coin::CachedCommittee::from_stack_item(&decoded)?;
    ...
    >(
        ...
        let committees = self.read_atipicial_committee_sorted(&snapshot)?;
        // C# AssertAlmostFullCommittee: m = max(max(1, committee majority), n - 2).
        // The `n - (n - 1) / 2` majority term is single-sourced in AtipicialCoin;
        // `n.max(1)` reproduces the original `max(1, …)` guard without underflow.
        let n = committees.len();
        let m = std::cmp::max(
            crate::AtipicialCoin::committee_threshold(n.max(1)),
            n.saturating_sub(2),
        );
        let script =
