#!/usr/bin/env python3
"""
State root validation script for atipicial-rs.

Compares state roots computed by the local atipicial-rs node against a reference
Atipicial N3 node (C# atipicial-cli or atipicial) for every block during sync.
...
def main():
    parser = argparse.ArgumentParser(description="Validate atipicial-rs state roots against reference node")
    parser.add_argument("--local", default="http://127.0.0.1:10332", help="Local atipicial-rs RPC URL")
    parser.add_argument("--reference", default="http://seed1.atipicial.com:10332", help="Reference node RPC URL")
    ...
    print("=== Atipicial-RS State Root Validator ===")
