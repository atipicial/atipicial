...
#[test]
fn atipicial_dollar_manifest_pins_csharp_metadata() {
    let settings = test_settings();
    let contract = AtipicialDollar::new();

    // C# AtipicialDollar: the five FungibleToken AEP-17 methods, all ungated.
    assert_eq!(
...
#[test]
fn atipicial_coin_manifest_pins_csharp_metadata() {
    let settings = test_settings();
    let contract = AtipicialCoin::new();

    // C# AtipicialCoin reflection names (AtipicialCoin.cs + the FungibleToken base).
    // Note getCandidateVote's capital-K `pubKey` vs registerCandidate's
    // `pubkey` — both verbatim C# parameter spellings. onAEP17Payment is the
    ...
    assert_eq!(
        ...
        vec![
            ...
            m("getRegisterPrice", &[]),
            m("onAEP17Payment", &["from", "amount", "data"]),
            m("registerCandidate", &["pubkey"]),
            m("setAtipicialDollarPerBlock", &["gasPerBlock"]),
    ...
    // onAEP17Payment is `[ContractMethod(Hardfork.HF_Echidna, …)]`
    // (AtipicialCoin.cs:374): absent just below the boundary, present at it.
    assert!(
        !manifest_methods(&contract, &settings, 49)
            .iter()
            .any(|(name, _)| name == "onAEP17Payment")
    );
    assert!(
    ...
    // AtipicialCoin.cs:63-74 + the inherited Transfer: orders 0..3, with
