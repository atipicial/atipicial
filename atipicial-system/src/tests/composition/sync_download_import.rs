...
use atipicial_blockchain::{BlockchainCommand, BlockchainHandle, HeaderCache, ImportMode};
use atipicial_network::{BlockDownloadBatch, BlockDownloadConfig, ChannelBlockDownloader, NetworkError};
...
fn block(index: u32) -> Block {
    let mut previous = atipicial_primitives::UInt256::zero();
        ...
        while let Some(command) = commands.recv().await {
            match command {
                BlockchainCommand::ImportBlocks { import, reply } => {
                    ...
                    reply
                        .send(atipicial_blockchain::ImportBlocksReply::ok(imported))
        ...
        while let Some(command) = commands.recv().await {
            match command {
                BlockchainCommand::ImportBlocks { import, reply } => {
                    ...
                    reply
                        .send(atipicial_blockchain::ImportBlocksReply::ok(import.blocks.len()))
                        .expect("send import reply");
                }
                BlockchainCommand::GetBlockByHeight { height, reply } => {
                    let _ = reply.send(Some(block(height)));
