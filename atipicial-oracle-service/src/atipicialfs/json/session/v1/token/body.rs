use super::super::super::super::super::proto::atipicialfs_v2;
use super::super::super::super::helpers::{json_string, json_u64_string, push_json_field};
use super::super::super::super::object::atipicialfs_json_owner_id;
use super::super::context::{
    atipicialfs_json_container_session_context, atipicialfs_json_object_session_context,
};
use base64::Engine as _;

pub(super) fn atipicialfs_json_session_token_body(
    body: &atipicialfs_v2::session::session_token::Body,
) -> Option<String> {
    ...
    if let Some(owner) = body.owner_id.as_ref().and_then(atipicialfs_json_owner_id) {
        push_json_field(&mut out, &mut first, "ownerID", &owner);
    }
    if let Some(lifetime) = body
        .lifetime
        .as_ref()
        .and_then(atipicialfs_json_session_token_lifetime)
    {
    ...
    if let Some(context) = body.context.as_ref() {
        match context {
            atipicialfs_v2::session::session_token::body::Context::Object(object) => {
                let json =
                    atipicialfs_json_object_session_context(object).unwrap_or_else(|| "{ }".to_string());
                push_json_field(&mut out, &mut first, "object", &json);
            }
            atipicialfs_v2::session::session_token::body::Context::Container(container) => {
                let json = atipicialfs_json_container_session_context(container)
...
fn atipicialfs_json_session_token_lifetime(
    lifetime: &atipicialfs_v2::session::session_token::body::TokenLifetime,
) -> Option<String> {
    let mut out = String::from("{ ");
    let mut first = true;
