...
//! concrete native contracts (in `atipicial-native-contracts`) both depend
...
//! `atipicial-native-contracts` directly.
//!
//! The concrete native-contract implementations live in
//! `atipicial-native-contracts`; the [`build_native_contract_state`] helper that
//! composes their on-disk `ContractState` (AEF + manifest) lives here with
...
use atipicial_manifest::{
    ...
    ContractParameterDefinition, AefFile,
};
    ...
    P: NativeContractProvider + 'static,
{
    ...
    /// which is how `FungibleToken.Transfer` reaches ATC and GAS). The
    ...
    fn lookup_contract_state<B: atipicial_storage::CacheRead>(
        &self,
        _snapshot: &atipicial_storage::DataCache<B>,
        _hash: &atipicial_primitives::UInt160,
    ...
    fn is_contract_blocked<B: atipicial_storage::CacheRead>(
        &self,
        _snapshot: &atipicial_storage::DataCache<B>,
        _contract_hash: &atipicial_primitives::UInt160,
    ) -> CoreResult<bool> {
        Ok(false)
    }

    /// Returns the committee multisig address (C# `ATC.GetCommitteeAddress`),
    /// used by `check_committee_witness` to authorize committee-gated operations.
    ///
    /// The default implementation returns `Ok(None)`; only `AtipicialCoin` (which
    /// owns the committee cache) overrides this.
    fn committee_address<B: atipicial_storage::CacheRead>(
        &self,
        _snapshot: &atipicial_storage::DataCache<B>,
    ) -> CoreResult<Option<atipicial_primitives::UInt160>> {
        Ok(None)
    }
    ...
    fn oracle_request_url_full<B: atipicial_storage::CacheRead>(
        &self,
        _snapshot: &atipicial_storage::DataCache<B>,
        _id: u64,
    ...
    fn transaction_state<B: atipicial_storage::CacheRead>(
        &self,
        _snapshot: &atipicial_storage::DataCache<B>,
        _tx_hash: &atipicial_primitives::UInt256,
    ) -> CoreResult<Option<atipicial_payloads::TransactionState>> {
        Ok(None)
    }

    /// Returns the trimmed block stored by hash.
    fn trimmed_block<B: atipicial_storage::CacheRead>(
        &self,
        _snapshot: &atipicial_storage::DataCache<B>,
        _block_hash: &atipicial_primitives::UInt256,
    ) -> CoreResult<Option<atipicial_payloads::TrimmedBlock>> {
        Ok(None)
    }
...
///
/// This lives in `atipicial-execution` (not next to `OracleContract` in
/// `atipicial-native-contracts`) because it is part of the [`NativeContract`]
...
/// creating a cycle (`atipicial-native-contracts` already depends on
/// `atipicial-execution`). The actual `OracleRequest` storage record — the
/// persisted state — is co-located with the contract in
/// `atipicial-native-contracts/src/oracle_contract.rs`.
#[derive(Clone, Debug)]
pub struct OracleRequestDetails {
    ...
    pub original_tx_id: atipicial_primitives::UInt256,
}

impl OracleRequestDetails {
    /// Creates a new request detail.
    pub fn new(url: impl Into<String>, original_tx_id: atipicial_primitives::UInt256) -> Self {
        Self {
            url: url.into(),
            original_tx_id,
        }
...
impl NativeMethod {
    ...
    pub fn with_required_call_flags(mut self, flags: atipicial_primitives::CallFlags) -> Self {
...
/// each native contract, including the AEF bytecode that the
...
where
    ...
    T: NativeContract<P>,
{
    let syscall_hash = atipicial_vm::interop_hash("System.Contract.CallNative");
    ...
    let aef = AefFile::new("atipicial-core-v3.0".to_string(), builder.to_array());
    ...
    ContractState::new(contract.id(), contract.hash(), aef, manifest)
}
...
fn native_static_event_descriptor(
    ...
    if name.is_empty() {
        tracing::error!(target: "atipicial", "native event name is empty");
    }
...
fn native_static_method_descriptor(
    ...
    if name.is_empty() {
        tracing::error!(target: "atipicial", "native method name is empty");
    }
    if offset < 0 {
        tracing::error!(
            target: "atipicial",
