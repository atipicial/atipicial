...
use atipicial_payloads::helper::get_sign_data_vec;
    ...
    P: NativeContractProvider + OracleContractReadProvider + 'static,
{
    ...
    pub(in super::super) fn add_response_tx_sign<B: atipicial_storage::CacheRead>(
        &self,
    ...
    pub(in super::super) fn check_tx_sign<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
        tx: &Transaction,
        ...
        let needed_threshold =
            atipicial_vm::script_builder::RedeemScript::bft_threshold(oracle_nodes.len());
        ...
        let outcome = self
            .runtime
            .submit_transaction(atipicial_mempool::TransactionOrigin::Local, tx_mut);
        if !outcome.is_accepted() {
            warn!(target: "atipicial::oracle", ?outcome, "failed to relay oracle response tx");
            return false;
        }

        debug!(target: "atipicial::oracle", tx = %tx.hash(), "oracle response tx relayed");
        true
    }
