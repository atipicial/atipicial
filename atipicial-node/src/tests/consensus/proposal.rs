...
fn proposal_resolution_rejects_unverified_conflicts_against_proposal_hashes() {
    ...
    let conflict = signed_tx_with_fees(
        ...
        vec![TransactionAttribute::Conflicts(
            atipicial_payloads::Conflicts::new(target_hash),
        )],
    );
...
fn primary_proposal_selection_stops_before_dbft_max_block_system_fee() {
    ...
    let hashes = select_primary_proposal_transactions(
        vec![
            PoolItem::new(first, atipicial_mempool::TransactionOrigin::Local),
            PoolItem::new(second, atipicial_mempool::TransactionOrigin::Local),
        ],
        2,
        &mut cache,
        &validators,
...
fn primary_proposal_skips_invalid_transactions_over_f() {
    ...
    let hashes = select_primary_proposal_transactions(
        vec![
            PoolItem::new(keep, atipicial_mempool::TransactionOrigin::Local),
            PoolItem::new(drop, atipicial_mempool::TransactionOrigin::Local),
        ],
        2,
        &mut cache,
        &validators,
...
async fn proposal_resolution_requests_change_view_for_invalid_unverified_transaction() {
    ...
    let mut driver = ConsensusDriver {
        ...
        store: Arc::new(atipicial_storage::persistence::providers::memory_store::MemoryStore::new()),
...
async fn primary_request_transactions_broadcasts_inv_of_proposal_hashes() {
    ...
    context.state = atipicial_consensus::ConsensusState::Primary;
    ...
    while let Ok(command) = network_rx.try_recv() {
        if let atipicial_network::NetworkCommand::BroadcastInv {
            inventory_type,
            hashes,
        } = command
        {
            assert_eq!(inventory_type, atipicial_primitives::InventoryType::Transaction);
...
fn expected_base_block_size_includes_commit_signature_invocation() {
    ...
    let mut sb = atipicial_vm::script_builder::ScriptBuilder::new();
    for _ in 0..m {
        sb.emit_push(&[0u8; 64]);
    }
    ...
    let expected = header_without_witness
        ...
        + atipicial_io::serializable::helper::SerializeHelper::get_var_size_usize(0);
    assert_eq!(
        base, expected,
...
fn invalid_validator_set_sizes_as_rejected_proposal() {
    ...
    let selected = select_primary_proposal_transactions(
        vec![PoolItem::new(tx, atipicial_mempool::TransactionOrigin::Local)],
        1,
        &mut cache,
        &validators,
        &settings,
