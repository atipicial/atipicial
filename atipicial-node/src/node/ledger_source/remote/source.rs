//! Remote JSON-RPC implementation of [`atipicial_network::BlockSource`].
...
impl RpcLedgerBlockSource {
    ...
    fn decode_block(raw_text: &str) -> anyhow::Result<atipicial_payloads::Block> {
        decode_remote_serialized_payload(raw_text, "block", |bytes| {
            atipicial_payloads::Block::deserialize(&mut MemoryReader::new(bytes))
                .map_err(|err| anyhow::anyhow!("deserializing remote ledger block: {err}"))
        })
    }

    fn decode_header(raw_text: &str) -> anyhow::Result<atipicial_payloads::Header> {
        decode_remote_serialized_payload(raw_text, "header", |bytes| {
            atipicial_payloads::Header::deserialize(&mut MemoryReader::new(bytes))
                .map_err(|err| anyhow::anyhow!("deserializing remote ledger header: {err}"))
        })
    }

    fn block_by_selector(&self, selector: Value) -> Option<atipicial_payloads::Block> {
        self.rpc_string("getblock", vec![selector])
            .and_then(|hex| Self::decode_block(&hex))
            .map_err(|err| {
                warn!(target: "atipicial::remote_ledger", error = %err, "remote getblock failed");
                err
            })
            .ok()
    }

    fn header_by_selector(&self, selector: Value) -> Option<atipicial_payloads::Header> {
        self.rpc_string("getblockheader", vec![selector])
            .and_then(|hex| Self::decode_header(&hex))
            .map_err(|err| {
                warn!(target: "atipicial::remote_ledger", error = %err, "remote getblockheader failed");
                err
            })
            .ok()
    }
...
impl atipicial_network::BlockSource for RpcLedgerBlockSource {
    ...
    fn block_hash_by_index(&self, index: u32) -> Option<UInt256> {
        self.rpc_string("getblockhash", vec![json!(index)])
            ...
            .map_err(|err| {
                warn!(target: "atipicial::remote_ledger", error = %err, "remote getblockhash failed");
                err
            })
            .ok()
    }

    fn block_by_hash(&self, hash: &UInt256) -> Option<atipicial_payloads::Block> {
    ...
    fn transaction_by_hash(&self, hash: &UInt256) -> Option<atipicial_payloads::Transaction> {
