...
use atipicial_test_fixtures::TestTransactionBuilder;
use atipicial_vm::{ExecutionEngineLimits, OpCode, StackItem, VmState as VMState};
...
fn make_ledger_block<S>(
    store: &atipicial_storage::persistence::StoreCache<S>,
    index: u32,
...
where
    S: atipicial_storage::persistence::Store,
{
    atipicial_test_fixtures::try_make_ledger_block(store, index, transactions)
        .expect("make ledger block fixture")
}

fn store_block<S>(store: &mut atipicial_storage::persistence::StoreCache<S>, block: &LedgerBlock)
where
    S: atipicial_storage::persistence::Store,
{
    atipicial_test_fixtures::try_store_block_with_vmstate(store, block, VMState::NONE)
...
fn store_committee<S>(
    store: &mut atipicial_storage::persistence::StoreCache<S>,
    committee: &[atipicial_crypto::ECPoint],
) where
    S: atipicial_storage::persistence::Store,
{
    const PREFIX_COMMITTEE: u8 = 0x0e;
    let atipicial_coin_id = crate::server::native_queries::NativeQueries::native_registry()
        .get_by_name("AtipicialCoin")
        .expect("atipicial token")
        .id();
    ...
    let key = StorageKey::create(atipicial_coin_id, PREFIX_COMMITTEE);
...
fn store_candidate_state<S>(
    store: &mut atipicial_storage::persistence::StoreCache<S>,
    candidate: &atipicial_crypto::ECPoint,
    registered: bool,
    votes: BigInt,
) where
    S: atipicial_storage::persistence::Store,
    ...
    S: atipicial_storage::persistence::Store,
{
    ...
    let key = StorageKey::new(atipicial_coin_id, key_bytes);
    store.add(key, StorageItem::from_bytes(bytes));
    store.try_commit().expect("commit test store");
}

fn store_blocked_account<S>(store: &mut atipicial_storage::persistence::StoreCache<S>, account: &UInt160)
where
    S: atipicial_storage::persistence::Store,
{
...
fn make_contract_state(id: i32, hash: UInt160, name: &str) -> ContractState {
    let aef = AefFile::new("test".to_string(), vec![OpCode::PUSH1.byte()]);
    let manifest = ContractManifest::new(name.to_string());
    ContractState::new(id, hash, aef, manifest)
}
