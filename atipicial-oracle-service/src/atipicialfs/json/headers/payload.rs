...
use super::attributes::{build_atipicialfs_attribute_json, build_atipicialfs_attribute_list};

pub(crate) fn build_atipicialfs_header_payload(headers: &reqwest::header::HeaderMap) -> String {
    ...
    if let Some(header_payload) = build_atipicialfs_header_proto(headers) {
...
fn build_atipicialfs_header_proto(headers: &reqwest::header::HeaderMap) -> Option<String> {
    ...
    if let Some(attributes) = build_atipicialfs_attribute_list(headers) {
        let attrs_json = build_atipicialfs_attribute_json(&attributes);
