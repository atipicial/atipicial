...
//!
//! Implements atipicial-execution's [`NativeContractProvider`] seam over the concrete
...
//! to a native contract without `atipicial-execution` depending on
//! `atipicial-native-contracts` (which would be a crate cycle).
//!
...
use atipicial_execution::{ContractState, NativeContract};
use atipicial_payloads::{TransactionState, TrimmedBlock};
use atipicial_primitives::{TransactionAttributeType, UInt160, UInt256};
...
use crate::{
    AtipicialCoin, AtipicialDollar, ContractManagement, LedgerContract, Notary, OracleContract,
    PolicyContract, Role, RoleManagement, StandardNativeContract,
};
...
atipicial_io::impl_default_via_new!(StandardNativeProvider);
...
impl NativeContractProvider for StandardNativeProvider {
    ...
    ) -> atipicial_error::CoreResult<u32> {
    ...
    fn gas_balance<B: CacheRead>(
        ...
        Ok(AtipicialDollar::balance_of(snapshot, account).unwrap_or_else(|_| BigInt::from(0)))
    }
    ...
    fn committee_address<B: CacheRead>(
        ...
        <AtipicialCoin as NativeContract<StandardNativeProvider>>::committee_address(
            &AtipicialCoin::new(),
            snapshot,
        )
    }
    ...
    fn next_block_validators<B: CacheRead>(
        ...
        AtipicialCoin::new().next_block_validators(
            snapshot,
    ...
    fn oracle_request_details<B: CacheRead>(
        ...
        snapshot: &DataCache<B>,
        request_id: u64,
    ) -> CoreResult<Option<atipicial_execution::native_contract::OracleRequestDetails>> {
    ...
    fn next_consensus_address_for_block<B: CacheRead>(
        ...
        AtipicialCoin::new().next_consensus_address_for_block(snapshot, settings, block_index)
    }
    ...
    fn fast_forward_empty_block_rewards<B: CacheRead>(
        ...
        AtipicialCoin::new().fast_forward_empty_block_rewards(snapshot, settings, start, end)
    }
}
