//! # atipicial-state-packs manifest generations
//!
//! ## Boundary
//!
...
const MANIFEST_HEADER_DIGEST_DOMAIN: &[u8] = b"atipicial-state-packs/manifest-header/v3\0";
const MANIFEST_EXTENTS_DIGEST_DOMAIN: &[u8] = b"atipicial-state-packs/manifest-extents/v3\0";
const MANIFEST_ENTRIES_DIGEST_DOMAIN: &[u8] = b"atipicial-state-packs/manifest-entries/v3\0";
const FRAME_CHAIN_SEED_DOMAIN: &[u8] = b"atipicial-state-packs/frame-chain-seed/v3\0";
const FRAME_CHAIN_STEP_DOMAIN: &[u8] = b"atipicial-state-packs/frame-chain-step/v3\0";
...
pub(crate) fn publish_manifest(root: &Path, manifest: &Manifest) -> Result<PathBuf> {
    ...
    // Windows cannot open a directory handle for fsync (see io::sync_directory);
    // file-level syncs already performed provide the crash guarantees there.
    #[cfg(unix)]
    {
        File::open(root)
