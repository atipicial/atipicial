...
use atipicial_trie::Trie;

#[tokio::test(flavor = "multi_thread")]
async fn proof_handlers_report_unsupported_state() {
    let (_system, _state_store, server) = make_server_with_state();
    let root = atipicial_primitives::UInt256::from([0x11u8; 32]).to_string();
    let contract = atipicial_primitives::UInt160::zero().to_string();
...
async fn verify_proof_rejects_invalid_payload() {
    let (_system, _state_store, server) = make_server_with_state();
    let root = atipicial_primitives::UInt256::from([0x22u8; 32]).to_string();
...
async fn verify_proof_rejects_wrong_root() {
    ...
    let (_system, _state_store, server) = make_server_with_state();
    let wrong_root = atipicial_primitives::UInt256::from([0x33u8; 32]).to_string();
    let err = call(
        &server,
        "verifyproof",
