    ...
    M: MempoolLike,
{
    ...
    /// block, which deploys/initializes the genesis-active natives (ATC committee
    ...
    pub(crate) async fn initialize(&self) -> Result<(), String> {
        let Some(snapshot) = self.system.store_snapshot() else {
            debug!(target: "atipicial", "blockchain service initialized without a store snapshot");
            return Ok(());
        };
