...
async fn get_nep17_balances_parses_rpc_payload() {
    ...
    let mut server = Server::new_async().await;
    let address = atipicial_primitives::UInt160::zero().to_address();
    let body = format!(
    ...
    assert_eq!(balances.user_script_hash, atipicial_primitives::UInt160::zero());
...
async fn get_nep17_transfers_parses_rpc_payload() {
    ...
    let mut server = Server::new_async().await;
    let address = atipicial_primitives::UInt160::zero().to_address();
    let tx_hash = atipicial_primitives::UInt256::zero();
    let body = format!(
    ...
    assert_eq!(transfers.user_script_hash, atipicial_primitives::UInt160::zero());
