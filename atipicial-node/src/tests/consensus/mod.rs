...
use atipicial_blockchain::ledger_provider::TransactionAdmissionLedger;
...
use atipicial_crypto::{ECPoint, signature::Secp256r1Crypto};
use atipicial_io::Serializable;
use atipicial_mempool::{MemoryPool, PoolItem, TransactionOrigin};
use atipicial_native_contracts::LedgerContract;
use atipicial_network::NetworkHandle;
use atipicial_payloads::{ExtensiblePayload, Signer, Transaction, TransactionAttribute, Witness};
use atipicial_primitives::{UInt160, UInt256, VerifyResult, WitnessScope};
use atipicial_serialization::BinarySerializer;
use atipicial_storage::{StorageItem, StorageKey, persistence::DataCache};
...
fn memory_pool(settings: &ProtocolSettings) -> MemoryPool {
    let chain_spec = atipicial_test_fixtures::test_chain_spec(settings.clone());
...
fn consensus_native_provider(
    pool: &MemoryPool,
) -> NativeConsensusProvider<atipicial_native_contracts::StandardNativeProvider> {
...
fn extensible_codec_round_trips() {
    ...
    let mut original = ConsensusPayload::new(
        ...
        validator_index,
        0, // view_number
        atipicial_consensus::ConsensusMessageType::Commit,
...
fn consensus_ledger_reads_use_provider_boundaries() {
    ...
    for (name, source) in sources {
        ...
        assert!(
            !source.contains("AtipicialCoin::new()"),
            "{name} must not construct native AtipicialCoin directly"
        );
        assert!(
    ...
    assert!(
        ...
        "consensus provider must resolve AtipicialCoin through the explicit native provider"
    );
    assert!(
...
fn prepare_request_ledger_guard_uses_dynamic_max_traceable_blocks() {
    ...
    settings.hardforks = atipicial_config::HardforkSchedule::new()
        .with_activation(atipicial_config::Hardfork::HfEchidna, 0)
...
/// `TimePerBlock = atipicialSystem.GetTimePerBlock()`.
fn effective_block_time_after_configure_round(
    ...
    let ms = atipicial_native_contracts::PolicyContract::new()
...
fn configure_round_uses_settings_default_before_echidna() {
    ...
    settings.hardforks = atipicial_config::HardforkSchedule::new()
        .with_activation(atipicial_config::Hardfork::HfEchidna, 1_000_000)
        .with_omitted_leading_at_genesis();
    let snapshot = DataCache::new(false);
    seed_current_block(&snapshot, 10);
...
fn set_zero_policy_fee(snapshot: &DataCache, prefix: u8) {
    snapshot.add(
        StorageKey::new(atipicial_native_contracts::PolicyContract::ID, vec![prefix]),
...
fn seed_persisted_transaction(snapshot: &DataCache, block_index: u32, tx: &Transaction) {
    ...
    snapshot.add(
        ...
        StorageItem::from_bytes(
            LedgerContract::new()
                .serialize_persisted_transaction_state(block_index, atipicial_vm::VmState::HALT, tx)
                .unwrap(),
        ),
    );
}
...
fn seed_gas_balance(snapshot: &DataCache, account: &UInt160, datoshi: i64) {
    ...
    snapshot.update(
        StorageKey::new(atipicial_native_contracts::AtipicialDollar::ID, key),
