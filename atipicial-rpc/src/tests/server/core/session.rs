...
use atipicial_primitives::FindOptions;
use atipicial_storage::{StorageItem, StorageKey};
use atipicial_vm::stack_item::StackItem;
use atipicial_vm::{OpCode, VmState};
...
#[test]
fn rpc_atipicial_hash_reads_use_native_query_facade() {
    let native_query = include_str!("../../../server/native_queries/atipicial.rs");
    assert!(
        native_query.contains("fn atipicial_script_hash"),
        "native_queries should expose the canonical ATC script hash"
    );
    assert!(
        native_query.contains("AtipicialCoin::script_hash()"),
        "only the native query facade should touch the concrete ATC native type"
    );
    ...
    for (name, source) in checked_handlers {
        assert!(
            source.contains("NativeQueries::atipicial_script_hash()"),
            "{name} should obtain the ATC hash through the native query facade"
        );
        assert!(
            !source.contains("AtipicialCoin::script_hash()"),
            "{name} should not reach into the concrete ATC native contract"
        );
        assert!(
            !source.contains("use atipicial_native_contracts::AtipicialCoin"),
            "{name} should not import the concrete ATC native contract"
        );
    }
}
...
const GENESIS_TIMESTAMP: u64 = 1_784_505_600_000;
...
async fn stateless_invoke_builds_dummy_persisting_block() {
    ...
    script.extend_from_slice(&atipicial_vm::interop_hash("System.Runtime.GetTime").to_le_bytes());
    ...
    assert_eq!(block.merkle_root(), &atipicial_primitives::UInt256::default());
...
async fn session_valid_until_block_uses_policy_aware_increment() {
    ...
    let signer = atipicial_payloads::Signer::new(
        atipicial_primitives::UInt160::default(),
        atipicial_payloads::WitnessScope::CALLED_BY_ENTRY,
    );
