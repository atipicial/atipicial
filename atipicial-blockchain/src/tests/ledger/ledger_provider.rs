...
use atipicial_payloads::{Block, Header, Signer, Transaction, TransactionAttribute, Witness};
...
use atipicial_storage::persistence::{RawReadOnlyStore, Store, StoreCache};
...
struct ColdLedgerProvider {
    ...
    states: HashMap<UInt256, atipicial_payloads::TransactionState>,
}
...
impl ColdLedgerProvider {
    fn with_block(mut self, block: Block) -> Self {
        ...
        for transaction in &block.transactions {
            ...
            self.states.insert(
                hash,
                atipicial_payloads::TransactionState::new(
                    block.index(),
    ...
    fn with_state(mut self, hash: UInt256, state: atipicial_payloads::TransactionState) -> Self {
...
fn storage_ledger_provider_exposes_conflict_stub_state_without_transaction() {
    ...
    tx.set_attributes(vec![TransactionAttribute::Conflicts(
        atipicial_payloads::Conflicts::new(conflict_hash),
    )]);
    let mut header = Header::new();
    header.set_index(41);
...
fn hot_cold_transaction_state_prefers_hot_conflict_stub_over_cold_transaction() {
    ...
    let cold = ColdLedgerProvider::default().with_state(
        conflict_hash,
        atipicial_payloads::TransactionState::new(7, Some(test_transaction(7)), VmState::NONE),
    );
    let provider = HotColdLedgerProvider::new(StorageLedgerProvider::new(&hot), cold);
...
fn hot_prune_watermark_rejects_malformed_typed_value() {
    ...
    use atipicial_storage::persistence::{StoreMaintenanceBatch, TransactionalStore};
    ...
    corruption.put_metadata(
        b"atipicial.ledger.hot-pruned-through.v1".to_vec(),
        vec![0x01, 0x02],
    );
    store
