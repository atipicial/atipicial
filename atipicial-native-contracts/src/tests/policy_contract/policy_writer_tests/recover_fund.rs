...
/// descriptor requires `AllowCall` for the nested AEP-17 calls.
#[test]
fn recover_fund_e2e_requires_call_flags_all_before_dispatch() {
    ...
    deploy_native(
        &cache,
        &build_native_contract_state(&crate::AtipicialDollar, &settings, 100),
    );
    ...
    assert_eq!(
        crate::AtipicialDollar::balance_of(&snapshot, &account).unwrap(),
        BigInt::from(BALANCE),
        "insufficient call flags must not move funds"
    );
    assert_eq!(
        crate::AtipicialDollar::balance_of(&snapshot, &crate::hashes::TREASURY_HASH).unwrap(),
        BigInt::from(0),
...
/// committee signer sweeps the account's full ATD balance to Treasury
...
/// `onAEP17Payment` callback included — and emits `Transfer` followed by
...
fn recover_fund_e2e_sweeps_balance_to_treasury_and_notifies() {
    ...
    // Treasury must be a deployed contract so the ATD transfer's
    // onAEP17Payment callback runs (C# PostTransferAsync calls it whenever
    ...
    // (an exact-balance AEP-17 transfer removes the from-record).
    assert_eq!(
        crate::AtipicialDollar::balance_of(&snapshot, &treasury).unwrap(),
        BigInt::from(SWEPT)
    );
    assert_eq!(
        crate::AtipicialDollar::balance_of(&snapshot, &account).unwrap(),
...
/// A deployed token that does not declare the AEP-17 standard faults (C#
/// "Contract {token} does not implement AEP-17 standard."). Treasury is a
/// deployed non-AEP-17 contract, so it doubles as the token here.
#[test]
fn recover_fund_e2e_requires_nep17_standard() {
    ...
    assert_eq!(state, VmState::FAULT, "a non-AEP-17 token must FAULT");
}
