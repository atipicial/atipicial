...
//! state assembly out of the contract root while preserving the exact Atipicial N3
...
impl RoleManagement {
    ...
    pub(crate) fn designation_key(role_byte: u8, index: u32) -> atipicial_storage::StorageKey {
        storage::designation_key(role_byte, index)
    }

    /// Parses the VM integer role argument through the shared Atipicial N3 role mapping.
