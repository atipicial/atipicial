...
use atipicial_primitives::{StorageValue, StorageValueResult};
...
pub enum StorageItemCache {
    /// Cached raw bytes.
    Bytes(Vec<u8>),
    /// Cached Atipicial `BigInteger` value.
    BigInteger(BigInt),
}
...
impl StorageItemCache {
    ...
    /// Creates a Atipicial `BigInteger` cache.
    #[must_use]
    pub fn big_integer(value: BigInt) -> Self {
        Self::BigInteger(value)
    }

    /// Materializes the cached value using Atipicial's storage byte representation.
    #[must_use]
...
/// Storage item for Atipicial blockchain.
///
/// Represents a value stored in contract storage, optionally backed by a typed
/// cache for lazy materialisation.
///
/// Mirrors C# `Atipicial.SmartContract.StorageItem` (v3.10): a plain value with no
...
impl StorageItem {
    ...
    ///
    /// This is used by higher-level crates (e.g. atipicial-core) when setting a
...
atipicial_io::impl_default_via_new!(StorageItem);
...
/// Implement `StorageValue` trait from atipicial-primitives.
///
...
/// between atipicial-storage and atipicial-vm.
///
