...
async fn daemon_task_metrics_use_bounded_task_kind_and_outcome_labels() {
    ...
    assert!(
        metrics.contains(
            r#"atipicial_node_daemon_task_spawned_total{task="telemetry_metrics",kind="normal"} 1"#
        ),
        "spawned metric should use bounded task/kind labels:\n{metrics}"
    );
    assert!(
        metrics.contains(
            r#"atipicial_node_daemon_task_events_total{task="telemetry_metrics",kind="normal",outcome="error"} 1"#
        ),
        "event metric should count task errors with bounded labels:\n{metrics}"
    );
    assert!(
        metrics.contains(
            r#"atipicial_node_daemon_task_events_total{task="telemetry_metrics",kind="normal",outcome="exit"} 0"#
        ),
