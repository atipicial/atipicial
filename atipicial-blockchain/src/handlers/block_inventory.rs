...
use atipicial_runtime::CheckedBlockBatch;
    ...
    M: MempoolLike,
{
    ...
    pub(crate) async fn handle_block_inventory(
        ...
        if drained > 0 {
            debug!(target: "atipicial", drained, "drained parked unverified blocks");
        }
        Ok(())
    }
    ...
    async fn handle_block_inventory_batch_with_integrity(
        ...
        for position in 0..blocks.len() {
                ...
                .await
            {
                ...
                Err(error) => {
                    ...
                    warn!(target: "atipicial", %error, "inventory block rejected in batch");
                    continue;
                }
            }
        ...
        let drained = self.handle_drain_unverified_blocks().await;
        if drained > 0 {
            debug!(target: "atipicial", drained, "drained parked unverified blocks after inventory batch");
            imported += drained;
        }
    ...
    async fn persist_next_expected_block_with_commit_policy_and_signature(
        ...
        // or commit (durable backend write). Enable with RUST_LOG=atipicial::sync=debug.
        let total_us = atipicial_runtime::time::elapsed_us(after_commit);
        let verify_us = atipicial_runtime::time::elapsed_us(after_verify - after_hash);
        let persist_us = atipicial_runtime::time::elapsed_us(after_persist - after_verify);
        let commit_us = atipicial_runtime::time::elapsed_us(after_commit - after_persist);
        debug!(
            target: "atipicial::sync",
            index,
            hash_us = atipicial_runtime::time::elapsed_us(after_hash),
            verify_us,
            persist_us,
            commit_us,
            total_us,
        ...
        atipicial_runtime::sync_metrics::record_block(
            index as u64,
            verify_us,
            persist_us,
            commit_us,
