//! ApplicationEngine.Storage - matches C# Atipicial.SmartContract.ApplicationEngine.Storage.cs exactly
...
use atipicial_vm::error::VmError;
use atipicial_vm::{StackItem, VmResult};
    ...
    B: atipicial_storage::CacheRead,
{
    ...
    let Ok(raw) = std::env::var("ATC_TRACE_STORAGE_TX") else {
        return false;
    };
...
fn storage_trace_key_enabled(key: &[u8]) -> bool {
    let Ok(raw) = std::env::var("ATC_TRACE_STORAGE_KEY_HEX") else {
        return false;
    };
    raw.split(',').any(|entry| {
    ...
    B: atipicial_storage::CacheRead,
{
    ...
    pub fn storage_put(
        ...
        // Check sizes
        if key.len() > atipicial_primitives::constants::MAX_STORAGE_KEY_SIZE {
            return Err(CoreError::other("Key too large"));
        }

        if value.len() > atipicial_primitives::constants::MAX_STORAGE_VALUE_SIZE {
    ...
    B: atipicial_storage::CacheRead,
{
    ...
    let trace_enabled =
        storage_trace_enabled(app, "ATC_TRACE_STORAGE_GET") || storage_trace_key_enabled(&key);
    ...
    B: atipicial_storage::CacheRead,
{
    ...
    if storage_trace_enabled(app, "ATC_TRACE_STORAGE_PUT") || storage_trace_key_enabled(&key) {
        let key_preview = key
            .iter()
            .take(32)
            .map(|byte| format!("{byte:02x}"))
    ...
    B: atipicial_storage::CacheRead,
{
    ...
    if storage_trace_enabled(app, "ATC_TRACE_STORAGE_DELETE") || storage_trace_key_enabled(&key) {
        let key_preview = key
            .iter()
            .take(32)
            .map(|byte| format!("{byte:02x}"))
    ...
    B: atipicial_storage::CacheRead,
{
    ...
    if storage_trace_enabled(app, "ATC_TRACE_STORAGE_FIND") || storage_trace_key_enabled(&prefix) {
