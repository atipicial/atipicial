...
use atipicial_indexer::{IndexerError, IndexerService};
...
impl RpcServerIndexer {
    pub(super) fn service(server: &RpcServer) -> Result<Arc<IndexerService>, RpcException> {
        server
            .system()
            .indexer_service()
            .ok_or_else(|| internal_error("AtipicialIndexer service not available"))
    }

    pub(super) fn indexer_error(error: IndexerError) -> RpcException {
        internal_error(format!("AtipicialIndexer service read failed: {error}"))
    }
}
