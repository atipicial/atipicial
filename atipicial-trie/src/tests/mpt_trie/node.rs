...
fn leaf_bytes_and_hash_match_csharp_v3101_vector() {
    ...
    assert_eq!(
        ...
        "LeafNode type, var-bytes value, and reference must retain Atipicial C# byte order"
    );
    assert_eq!(
...
fn sizing_embedded_child_does_not_compute_hash() {
    ...
    assert_eq!(node.byte_size_as_child(), 1 + atipicial_primitives::UINT256_SIZE);
    assert!(
        !node.hash_is_cached(),
...
fn deserialize_rejects_nesting_deeper_than_max_key_length_like_csharp_v3101() {
    ...
    assert!(
        ...
        "C# Atipicial.Cryptography.MPT v3.10.1 rejects MPT node nesting deeper than MaxKeyLength"
    );
}
