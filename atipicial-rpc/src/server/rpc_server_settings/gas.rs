...
fn parse_gas_value(value: Value) -> CoreResult<i64> {
    match value {
        ...
        Value::Null => Err(atipicial_error::CoreError::other("gas value cannot be null")),
        _ => Err(atipicial_error::CoreError::other(
...
fn parse_gas_number(number: &serde_json::Number) -> CoreResult<i64> {
    ...
    if let Some(uint_value) = number.as_u64() {
        let int_value = i64::try_from(uint_value)
            .map_err(|_| atipicial_error::CoreError::other("gas value exceeds i64"))?;
        return apply_gas_threshold(int_value);
    }
    let float_value = number
        .as_f64()
        .ok_or_else(|| atipicial_error::CoreError::other("gas value must be numeric"))?;
...
fn parse_gas_string(text: &str) -> CoreResult<i64> {
    ...
    if trimmed.is_empty() {
        return Err(atipicial_error::CoreError::other("gas value cannot be empty"));
    }
    ...
    let float_value = trimmed
        .parse::<f64>()
        .map_err(|_| atipicial_error::CoreError::other("gas value must be numeric"))?;
    convert_gas_units(float_value)
}
...
fn apply_gas_threshold(value: i64) -> CoreResult<i64> {
    if value.abs() <= GAS_UNIT_THRESHOLD {
        value
            ...
            .ok_or_else(|| atipicial_error::CoreError::other("gas value overflow"))
    } else {
        Ok(value)
    }
}
...
fn convert_gas_units(value: f64) -> CoreResult<i64> {
    if !value.is_finite() {
        return Err(atipicial_error::CoreError::other("gas value must be finite"));
    }
    let scaled = value * RpcServerConfig::gas_datoshi_factor() as f64;
    if scaled > i64::MAX as f64 || scaled < i64::MIN as f64 {
        return Err(atipicial_error::CoreError::other("gas value overflow"));
    }
