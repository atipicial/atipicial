...
//! lives in `atipicial-runtime` — the shared service-contract layer — so subsystems
//! can react without depending on the concrete `atipicial-blockchain` service
//! implementation.
//!
//! The concrete command channel and handle live in `atipicial-blockchain`
//! (`BlockchainCommand` / `BlockchainHandle`): that crate owns the command
//! loop and the full set of per-request commands. `atipicial-runtime` deliberately
...
use atipicial_primitives::{UInt256, VerifyResult};
...
pub enum BlockchainEvent {
    ...
    /// runtime crate independent of `atipicial-blockchain`'s concrete command
