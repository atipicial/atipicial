...
fn native_contract_surface() {
    ...
    for name in ["setRegisterPrice", "setAtipicialDollarPerBlock"] {
    ...
    // onAEP17Payment: Echidna-gated candidate registration via GAS payment
    // — States|AllowNotify -> not safe, Void, no manifest CpuFee.
    let pay = c
        .methods()
        .iter()
        .find(|m| m.name == "onAEP17Payment")
        .unwrap();
    assert!(!pay.safe);
...
fn storage_key_helpers_match_csharp_layout() {
    ...
    let voters_count = AtipicialCoin::voters_count_key();
    assert_eq!(voters_count.id(), AtipicialCoin::ID);
    assert_eq!(voters_count.suffix(), &[PREFIX_VOTERS_COUNT]);

    let register_price = AtipicialCoin::register_price_key();
    assert_eq!(register_price.id(), AtipicialCoin::ID);
    assert_eq!(register_price.suffix(), &[PREFIX_REGISTER_PRICE]);

    let committee = AtipicialCoin::committee_key();
    assert_eq!(committee.id(), AtipicialCoin::ID);
    assert_eq!(committee.suffix(), &[PREFIX_COMMITTEE]);

    let gas_prefix = AtipicialCoin::gas_per_block_prefix_key();
    assert_eq!(gas_prefix.id(), AtipicialCoin::ID);
    assert_eq!(gas_prefix.suffix(), &[PREFIX_GAS_PER_BLOCK]);

    let gas_record = AtipicialCoin::gas_per_block_key(0x0102_0304);
    assert_eq!(gas_record.id(), AtipicialCoin::ID);
    assert_eq!(gas_record.suffix(), &[PREFIX_GAS_PER_BLOCK, 1, 2, 3, 4]);
    assert!(gas_record.suffix().starts_with(gas_prefix.suffix()));

    let candidate_prefix = AtipicialCoin::candidate_prefix_key();
    assert_eq!(candidate_prefix.id(), AtipicialCoin::ID);
    assert_eq!(candidate_prefix.suffix(), &[PREFIX_CANDIDATE]);

    let candidate = AtipicialCoin::candidate_key(&pubkey);
    assert_eq!(candidate.id(), AtipicialCoin::ID);
    assert_eq!(candidate.suffix()[0], PREFIX_CANDIDATE);
    assert_eq!(&candidate.suffix()[1..], pubkey_bytes.as_slice());

    let reward = AtipicialCoin::voter_reward_per_committee_key(&pubkey);
    assert_eq!(reward.id(), AtipicialCoin::ID);
...
fn committee_threshold_is_majority() {
    // m = n - (n - 1) / 2.
    assert_eq!(AtipicialCoin::committee_threshold(1), 1);
    assert_eq!(AtipicialCoin::committee_threshold(3), 2);
    assert_eq!(AtipicialCoin::committee_threshold(4), 3);
    assert_eq!(AtipicialCoin::committee_threshold(7), 4);
    assert_eq!(AtipicialCoin::committee_threshold(21), 11);
}
...
fn committee_read_decodes_and_sorts() {
    ...
    let read = AtipicialCoin::new().read_committee_points(&cache).unwrap();
    ...
    expected.sort();
    assert_eq!(
        AtipicialCoin::new().committee_sorted(&cache).unwrap(),
        expected
    );
}
...
fn next_block_validators_takes_count_then_sorts() {
    ...
    let result = AtipicialCoin::new()
        .next_block_validators(&cache, 2)
        .unwrap();
    ...
    assert_eq!(
        AtipicialCoin::new()
            .next_block_validators(&cache, 10)
            .unwrap(),
        all_expected
    );
}
...
fn next_block_validator_account_matches_sorted_validator_account() {
    ...
    let validators = atipicial.next_block_validators(&cache, 2).unwrap();

    assert_eq!(
        atipicial
            .next_block_validator_account(&cache, 2, 1)
            .unwrap(),
        crate::atipicial_coin::candidate_signature_account(&validators[1])
    );
    assert!(
        atipicial
            .next_block_validator_account(&cache, 2, 2)
            .is_err()
    );
}
...
fn candidates_filters_registered_and_decodes_votes() {
    ...
    let candidates = AtipicialCoin::new()
        .read_registered_candidates(&cache)
        .unwrap();
...
#[test]
fn atipicial_public_array_return_encoders_use_stack_item_projection() {
    ...
    assert_eq!(
        AtipicialCoin::candidates_to_array_bytes(&candidates).unwrap(),
        expected_candidates
    );
    ...
    assert_eq!(
        AtipicialCoin::points_to_array_bytes(&points).unwrap(),
        expected_points
    );

    let candidate_storage_source = include_str!("../../atipicial_token/storage/candidates.rs");
    let points_storage_source = include_str!("../../atipicial_token/storage/points.rs");
    let persist_source = include_str!("../../atipicial_token/persist.rs");
...
fn zero_bigint_storage_writes_match_csharp_empty_bytes() {
    ...
    let cache = DataCache::new(false);
    AtipicialCoin::new().write_voters_count(&cache, &BigInt::from(0));
    let stored = cache
        .get(&AtipicialCoin::voters_count_key())
    ...
    assert_eq!(
        AtipicialCoin::new().read_voters_count(&cache),
        BigInt::from(0)
    );

    AtipicialCoin::new().put_gas_per_block(&cache, 7, &BigInt::from(0));
    let key = AtipicialCoin::gas_per_block_key(7);
    ...
    AtipicialCoin::new().write_voters_count(&cache, &BigInt::from(300));
    let stored = cache
        .get(&AtipicialCoin::voters_count_key())
        .expect("entry written");
    assert_eq!(
...
fn calculate_bonus_matches_csharp_testcalculatebonus() {
    // C# UT_AtipicialCoin.TestCalculateBonus "Normal 1": balance 100, no vote,
    // BalanceHeight 0, the genesis 5-GAS ATDPerBlock record at index 0, end
    // 100 -> 100 * (5e8 * 100) * 10 / 100 / 100_000_000 = 500 (with the 1B ATC supply the reward divides by TotalAmount).
    let cache = DataCache::new(false);
    AtipicialCoin::new().put_gas_per_block(&cache, 0, &BigInt::from(DEFAULT_GAS_PER_BLOCK));
    let holder = AtipicialAccountStateView {
    ...
    let zero = AtipicialAccountStateView {
        balance: BigInt::from(0),
        ..clone_view(&holder)
    };
    assert_eq!(
        AtipicialCoin::new()
            .calculate_bonus(&cache, &zero, 100)
            .unwrap(),
        BigInt::from(0)
    );
    let future = AtipicialAccountStateView {
        balance_height: 100,
        ..clone_view(&holder)
    };
    assert_eq!(
    ...
    let negative = AtipicialAccountStateView {
        balance: BigInt::from(-100),
        ..clone_view(&holder)
    };
    assert!(
...
fn clone_view(v: &AtipicialAccountStateView) -> AtipicialAccountStateView {
    AtipicialAccountStateView {
