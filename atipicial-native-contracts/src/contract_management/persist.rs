...
/// Ledger, ATC, GAS, Policy, RoleManagement, Oracle, Notary, Treasury), the
...
impl ContractManagement {
    ...
    ///   the AEF + manifest composed for this block height (id and hash
    ...
    >(
        ...
        for contract in native_contracts.iter() {
            ...
            // C# `contract.GetContractState(settings, index)`: the AEF +
            // manifest composed for this block height.
            let composed = atipicial_execution::native_contract::build_native_contract_state_for::<
                P,
                _,
            >(contract, &settings, block_index);
            ...
            match existing {
                ...
                Some(item) => {
                    // C#: UpdateCounter++ and the AEF/manifest swap on the
