...
use atipicial_state_packs::authority::{AUTHORITATIVE_HIGH_WATER_KEY, AuthoritativeHighWaterRecord};
use atipicial_state_packs::checkpoint::PackCheckpoint;
use atipicial_state_packs::{
    PACK_KEY_BYTES, PackFrameContext, PackStore, PackStoreConfig, PackStoreError, PackStoreOptions,
    Snapshot as PackSnapshot,
};
use atipicial_state_service::mpt_store::{PreparedMptCommit, PreparedMptMetadataOverlay};
use atipicial_state_service::{
...
use atipicial_storage::{StorageError, StorageResult};
...
impl PackMaintenance {
    fn spawn(
        ...
        let handle = thread::Builder::new()
            .name("atipicial-pack-compact".to_owned())
            .spawn(move || {
                if let Err(error) =
...
fn read_state_tip(store: &RuntimeStore) -> anyhow::Result<(u32, [u8; 32])> {
    let root = atipicial_state_service::read_current_local_root(store)
...
mod tests {
    use super::*;
    use atipicial_state_packs::checkpoint::{
    ...
    fn fixture() -> Fixture {
        ...
        let source =
            atipicial_state_service::MptStore::from_memory_store(Arc::clone(&source_backing), true)
        ...
        let root = source
            .apply_block_changes(
                0,
                None,
                &[atipicial_state_service::MptChange::Put {
                    key: storage_key,
        ...
        let mut state = RuntimeStore::Mdbx(
            canonical_mdbx
                .open_named_table(atipicial_state_service::MDBX_STATE_SERVICE_NAMESPACE)
    ...
    fn authoritative_frame_history_binds_result_and_previous_roots() {
        ...
        for (index, root) in [(4, previous_root), (5, resulting_root)] {
            roots
                .put(
                    atipicial_state_service::Keys::state_root(index),
                    atipicial_state_service::StateRoot::new_current(
                        index,
                        atipicial_primitives::UInt256::from(root),
                    )
                    .to_array(),
                )
        ...
        roots
            .put(
                atipicial_state_service::Keys::state_root(5),
                atipicial_state_service::StateRoot::new_current(
                    5,
                    atipicial_primitives::UInt256::from([0x66; 32]),
                )
                .to_array(),
            )
        ...
        roots
            .put(
                ...
                atipicial_state_service::StateRoot::new_current(
                    5,
                    atipicial_primitives::UInt256::from(resulting_root),
                )
                .to_array(),
            )
        ...
        roots
            .put(
                atipicial_state_service::Keys::state_root(4),
                atipicial_state_service::StateRoot::new_current(
                    4,
                    atipicial_primitives::UInt256::from([0x77; 32]),
                )
                .to_array(),
            )
    ...
    fn authoritative_checkpoint_context_is_an_explicit_snapshot_anchor() {
        ...
        roots
            .put(
                ...
                atipicial_state_service::StateRoot::new_current(
                    5,
                    atipicial_primitives::UInt256::from(checkpoint_root),
                )
                .to_array(),
            )
    ...
    fn authoritative_rewind_fails_closed_without_moving_marker_or_snapshot() {
        ...
        let state_store = Arc::new(
            atipicial_state_service::StateStore::with_mpt_store_and_node_snapshots(
                true, backing, factory,
            )
            .expect("open split StateService"),
        );
        let handlers =
            atipicial_state_service::commit_handlers::StateServiceCommitHandlers::try_new_coordinated(
    ...
    fn deferred_authority_materializes_two_windows_against_pinned_pack_generations() {
        ...
        let oracle =
            atipicial_state_service::MptStore::from_memory_store(Arc::clone(&fixture.oracle), true)
                .expect("open eager oracle");
        let classic_deferred = atipicial_state_service::MptStore::from_memory_store_with_options(
        ...
        let state_store = Arc::new(
            atipicial_state_service::StateStore::with_mpt_store_and_node_snapshot_options(
                true,
                true,
        ...
        let first_oracle_changes = first_values
            .iter()
            .map(|(key, value)| atipicial_state_service::MptChange::Put {
                key: key.to_array(),
                value: value.clone(),
            })
            .collect::<Vec<_>>();
        let first_oracle_root = oracle
            .apply_block_changes(
                1,
                Some(atipicial_primitives::UInt256::from(fixture.root)),
        ...
        assert!(
            exact_oracle_node_entries(&fixture.oracle)
                .iter()
                .any(
                    |(_, value)| atipicial_trie::Node::split_serialized_reference(value)
