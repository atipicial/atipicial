...
use anyhow::Context;
use atipicial_primitives::UnhandledExceptionPolicy;
use serde::Deserialize;

/// `[state_service]`: state-root/MPT support used by Atipicial's StateService plugin.
...
impl ApplicationLogsSection {
    pub(in crate::node) fn settings(
        &self,
        network: u32,
    ) -> atipicial_rpc::application_logs::ApplicationLogsSettings {
        let mut settings = atipicial_rpc::application_logs::ApplicationLogsSettings::default();
...
/// `[tokens_tracker]`: AEP-11/AEP-17 balance and transfer indexing.
#[derive(Debug, Default, Deserialize)]
pub(in crate::node) struct TokensTrackerSection {
    /// Whether to track AEP-11/AEP-17 balances and transfers.
    #[serde(default, alias = "Enabled")]
    pub(in crate::node) enabled: bool,
    /// Plugin database path. Accepts `{0}` as uppercase network magic.
    ...
    network: Option<u32>,
    /// Enabled standards, usually `AEP-17` and `AEP-11`.
...
impl TokensTrackerSection {
    pub(in crate::node) fn settings(
        ...
        let mut settings = atipicial_rpc::plugins::tokens_tracker::TokensTrackerSettings::default();
        ...
        settings.enabled_trackers =
            atipicial_rpc::plugins::tokens_tracker::TokensTrackerSettings::normalize_enabled_trackers(
...
pub(in crate::node) struct LoggingSection {
    ...
    pub(in crate::node) enabled: bool,
    /// Tracing filter directive, e.g. `info` or `info,atipicial=debug`.
