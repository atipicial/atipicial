...
impl RpcContractState {
    ...
    pub fn from_json(json: &JObject) -> CoreResult<Self> {
        ...
        let nef_json = json
            ...
            .ok_or_else(|| CoreError::other("Missing or invalid 'aef' field"))?;
        let aef = RpcAefFile::from_json(nef_json)?;
    ...
    pub fn to_json(&self) -> CoreResult<JObject> {
        ...
        json.insert(
            ...
            JToken::Object(
                RpcAefFile {
                    nef_file: self.contract_state.aef.clone(),
                }
                .to_json(),
            ),
        );
        ...
        let manifest_jtoken =
            atipicial_serialization::json::JToken::parse(&manifest_json_value.to_string(), 128)
                .map_err(|err| CoreError::other(err.to_string()))?;
...
#[cfg(test)]
#[path = "../tests/types/contract_state.rs"]
mod tests;
