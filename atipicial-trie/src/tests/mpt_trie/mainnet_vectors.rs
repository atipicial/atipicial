...
fn mainnet_height_5549_visible_storage_set_matches_reference_root() {
    ...
    assert_eq!(
        ...
        "rebuilding the verified height-5549 visible storage set must match the Atipicial reference root"
    );
}
...
fn mainnet_height_5549_missing_next_candidate_entry_reproduces_local_wrong_root() {
    ...
    assert_eq!(
        ...
        "omitting the NEXT/AtipicialLine candidate storage entry reproduces the known local height-5549 root"
    );
}
