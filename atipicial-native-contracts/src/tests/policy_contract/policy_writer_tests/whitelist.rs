...
/// the committee whitelists ATC.balanceOf (mirroring C# TestWhiteListFee),
...
fn whitelist_fee_contract_e2e_set_then_remove() {
    ...
    // The whitelist target: ATC's deployed state (its manifest carries the
    // balanceOf(1) descriptor whose offset keys the whitelist entry).
    let atipicial_state = build_native_contract_state(&crate::AtipicialCoin, &settings, 0);
    let balance_of_offset = atipicial_state
        .manifest
        .abi
        .methods
        .iter()
        .find(|m| m.name == "balanceOf" && m.parameters.len() == 1)
        .expect("ATC balanceOf in manifest")
        .offset;
    ...
    // setWhitelistFeeContract(ATC, "balanceOf", 1, 12345).
    let (state, _) = call_policy(
        ...
        |b| {
            ...
            b.emit_push(&atipicial_hash.to_array()); // contractHash (arg 0, top)
        },
    );
    assert_eq!(state, VmState::HALT, "setWhitelistFeeContract must HALT");
    let key = PolicyContract::whitelist_fee_key(&atipicial_hash, balance_of_offset);
    ...
    assert_eq!(view.contract_hash, atipicial_hash);
    ...
    assert_eq!(
        NativeContract::<atipicial_execution::native_contract_provider::NoNativeContractProvider>::whitelisted_fee(
            &PolicyContract::new(),
            &snapshot,
            &atipicial_hash,
            "balanceOf",
    ...
    // removeWhitelistFeeContract(ATC, "balanceOf", 1) deletes the entry.
    ...
    let (state3, _) = call_policy(
        ...
        |b| {
            ...
            b.emit_push(&atipicial_hash.to_array());
        },
    );
    assert_eq!(
        state3,
...
fn whitelist_fee_contract_e2e_validation_faults() {
    ...
    deploy_native(
        &cache,
        &build_native_contract_state(&crate::AtipicialCoin, &settings, 0),
    );
    let snapshot = Arc::new(cache);
    let signer = committee_address(&committee);
    let atipicial_hash = crate::AtipicialCoin::script_hash();
