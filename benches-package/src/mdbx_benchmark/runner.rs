...
use atipicial_crypto::Sha256Hasher;
use atipicial_storage::mdbx::{
    MdbxCommitMetrics, MdbxCommitMetricsSnapshot, MdbxStore, MdbxStoreProvider,
};
use atipicial_storage::persistence::{RawReadOnlyStore, Store, storage::StorageConfig};
...
pub fn run_mdbx_benchmark(config: &MdbxBenchmarkConfig) -> Result<MdbxBenchmarkReport> {
    ...
    let mut prefill_digest = OperationDigest::new(b"atipicial-mdbx-bench-prefill-v1");
    ...
    let mut campaign_digest = OperationDigest::new(b"atipicial-mdbx-bench-campaign-v1");
...
fn verify_reopened(store: &MdbxStore, queries: &[VerificationSample]) -> Result<(String, u64)> {
    ...
    digest.update(b"atipicial-mdbx-bench-reopen-v1");
    for query in queries {
        let actual = snapshot
            .try_get_bytes_result(&query.key)
...
fn build_report() -> Result<BenchmarkBuildReport> {
    ...
    Ok(BenchmarkBuildReport {
        ...
        profile: option_env!("ATC_BENCH_BUILD_PROFILE")
            .unwrap_or("unavailable")
            .to_owned(),
        opt_level: option_env!("ATC_BENCH_BUILD_OPT_LEVEL")
            .unwrap_or("unavailable")
            .to_owned(),
        debug_assertions: cfg!(debug_assertions),
        target: option_env!("ATC_BENCH_BUILD_TARGET")
            .unwrap_or("unavailable")
            .to_owned(),
        host: option_env!("ATC_BENCH_BUILD_HOST")
            .unwrap_or("unavailable")
            .to_owned(),
        rustc: option_env!("ATC_BENCH_BUILD_RUSTC")
            .unwrap_or("unavailable")
            .to_owned(),
        executable,
        git_revision,
...
fn mdbx_runtime_report(config: &ResolvedBenchmarkConfig) -> Result<MdbxRuntimeConfigReport> {
    ...
    .tuning();
    let batch_read_threads = configured_threads("ATC_MDBX_BATCH_READ_THREADS", 1);
    let write_intent_read_threads = match std::env::var("ATC_MDBX_WRITE_INTENT_READ_THREADS") {
        Ok(_) => configured_threads("ATC_MDBX_WRITE_INTENT_READ_THREADS", 1),
    ...
    Ok(MdbxRuntimeConfigReport {
        ...
        coalesce: configured_flag("ATC_MDBX_COALESCE"),
        no_meminit: configured_flag("ATC_MDBX_NO_MEMINIT"),
        prefix_index_path: std::env::var_os("ATC_MDBX_PREFIX_INDEX_PATH").map(Into::into),
        batch_read_threads,
...
fn configured_cursor_write_mode() -> String {
    let normalized = std::env::var("ATC_MDBX_CURSOR_WRITE_MODE")
        .unwrap_or_default()
        .trim()
        .to_ascii_lowercase()
        .replace(['-', '_'], "");
