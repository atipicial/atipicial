...
This API targets parity with the C# Atipicial `RpcServer` plugin: every method name
and its parameter order match the reference node, so existing Atipicial tooling and
SDKs work unchanged.

```mermaid
flowchart LR
  Client["RPC client<br/>(curl / SDK / dApp)"] -->|HTTP POST JSON-RPC| Server["atipicial-rpc server<br/>(jsonrpsee)"]
...
### Token tracker (AEP-11 / AEP-17)

| Method | Parameters | Returns | Notes |
|--------|------------|---------|-------|
| `getaep17balances` | `address` | AEP-17 balances | |
| `getaep17transfers` | `address`, `start?`, `end?` | AEP-17 transfer history | |
| `getaep11balances` | `address` | AEP-11 balances | |
| `getaep11transfers` | `address`, `start?`, `end?` | AEP-11 transfer history | |
| `getaep11properties` | `contract`, `tokenid` | AEP-11 token properties | |
...
For paged AtipicialIndexer methods, the default page size is 100 and the maximum page size is 1000; larger limits are capped server-side.
...
| `listservices` | none | Runtime service directory | Rust node operator view with `enabled`, `ready`, `interfaces`, `methods`, and lightweight `status` fields for RPC, StateService, ApplicationLogs, TokensTracker, AtipicialIndexer, and OracleService. |
