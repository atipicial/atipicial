use super::super::models::{RpcAccount, RpcTransferOut, RpcUnclaimedAtipicialDollar, RpcValidateAddressResult};
use super::super::{ClientRpcError, Aep17Api, RpcUtility};
use super::RpcClient;
use super::helpers::{
    ...
    O: RpcObserver,
{
    ...
    pub async fn get_wallet_balance(&self, asset_id: &str) -> Result<BigDecimal, ClientRpcError> {
        ...
        let asset_hash = RpcUtility::get_script_hash(asset_id, &self.protocol_settings)
            .map_err(|err| ClientRpcError::new(-32603, err.to_string()))?;
        let aep17 = Aep17Api::new(Arc::new(self.clone()));
        let decimals = aep17
            .decimals(&asset_hash)
            .await
            .map_err(|err| ClientRpcError::new(-32603, err.to_string()))?;
    ...
    pub async fn get_unclaimed_atipicial_dollar(
        &self,
        address: &str,
    ) -> Result<RpcUnclaimedAtipicialDollar, ClientRpcError> {
        let result = self
            .rpc_send_async("getunclaimedgas", vec![JToken::String(address.to_string())])
            .await?;
        let obj = token_as_object(result, "getunclaimedgas")?;
        RpcUnclaimedAtipicialDollar::from_json(&obj).map_err(|err| ClientRpcError::new(-32603, err.to_string()))
    }

    /// Gets the amount of unclaimed GAS in the wallet.
    /// Matches C# `GetWalletUnclaimedGasAsync`
    pub async fn get_wallet_unclaimed_atipicial_dollar(&self) -> Result<BigDecimal, ClientRpcError> {
        ...
        let amount = BigInt::from_str(&value).map_err(|_| {
            ClientRpcError::new(-32603, format!("Invalid unclaimed ATD value: {value}"))
        })?;
