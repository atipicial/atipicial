//! AtipicialCoin provider helpers used by consensus, GAS persistence, and node code.
//!
...
impl AtipicialCoin {
    ...
    /// `AtipicialDollar::on_persist` can resolve the primary validator the block's
    /// network fees are minted to (C# AtipicialDollar.cs:55) and the blockchain service
    /// can build the extensible-witness whitelist (C# `Blockchain.
    /// UpdateExtensibleWitnessWhiteList`).
    pub fn next_block_validators<B: atipicial_storage::CacheRead>(
        &self,
    ...
    /// C# `ATC.ComputeNextBlockValidators(snapshot, settings)`: recompute the next
    /// committee from live votes, take `ValidatorsCount`, then sort ascending.
    pub fn compute_next_block_validators<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
    ...
    pub fn next_consensus_address_for_block<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
        settings: &ProtocolSettings,
        block_index: u32,
