//! AEP account-balance RPC handlers.
//!
//! Balance handlers read tracker index records, enrich each asset through the
//! native contract snapshot, and build Atipicial-compatible AEP-11/AEP-17 response
...
use crate::plugins::tokens_tracker::{
    Aep11BalanceKey, Aep11Tracker, Aep17BalanceKey, Aep17Tracker, TokenBalance, find_prefix,
};
...
use atipicial_primitives::{UInt160, hex_util};
...
use super::response::{
    account_balances, aep11_balance_entry, aep11_token_entry, aep17_balance_entry,
};

impl RpcServerTokensTracker {
    pub(super) fn get_aep11_balances(
        server: &RpcServer,
        params: &[Value],
    ) -> Result<Value, RpcException> {
        let service = tracker_service(server)?;
        if !service.settings().enabled_aep11() {
            return Err(RpcException::from(RpcError::method_not_found()));
        }

        let address_version = server.system().settings().address_version;
        let request = AccountRequest::parse(params, "getaep11balances", address_version)?;

        let (balance_prefix, _, _) =
            Aep11Tracker::<atipicial_native_contracts::StandardNativeProvider>::rpc_prefixes();
        ...
        let balances =
            find_prefix::<_, Aep11BalanceKey, TokenBalance>(service.store().as_ref(), &prefix)
        ...
        for (asset, tokens) in grouped {
            ...
            let token_entries = tokens
                .into_iter()
                .map(|(token_id, balance)| aep11_token_entry(token_id, balance))
                .collect::<Vec<_>>();

            results.push(aep11_balance_entry(
                &asset,
    ...
    pub(super) fn get_aep17_balances(
        server: &RpcServer,
        params: &[Value],
    ) -> Result<Value, RpcException> {
        let service = tracker_service(server)?;
        if !service.settings().enabled_aep17() {
            return Err(RpcException::from(RpcError::method_not_found()));
        }

        let address_version = server.system().settings().address_version;
        let request = AccountRequest::parse(params, "getaep17balances", address_version)?;

        let (balance_prefix, _, _) =
            Aep17Tracker::<atipicial_native_contracts::StandardNativeProvider>::rpc_prefixes();
        let mut prefix = Vec::with_capacity(1 + UInt160::LENGTH);
        prefix.push(balance_prefix);
        prefix.extend_from_slice(&request.script_hash.to_bytes());

        let balances =
            find_prefix::<_, Aep17BalanceKey, TokenBalance>(service.store().as_ref(), &prefix)
                .map_err(internal_error)?;
        ...
        for (key, value) in balances {
            ...
            results.push(aep17_balance_entry(
                &key.asset_script_hash,
                &contract.manifest.name,
                &symbol,
                decimals,
