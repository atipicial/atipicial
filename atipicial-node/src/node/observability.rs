...
impl ObservabilityRuntime {
    pub(super) fn from_config(
        ...
        let runtime = Self {
            inner: Arc::new(ObservabilityInner {
                config: config.clone(),
                metadata: ObservabilityMetadata {
                    service_name: trimmed_or_default(config.service_name.as_deref(), "atipicial-node"),
        ...
        info!(
            target: "atipicial::observability",
    ...
    pub(super) fn install_panic_hook(&self) {
        ...
        std::panic::set_hook(Box::new(move |panic_info| {
            ...
            if let Err(err) = inner.report_error_blocking(&report) {
                eprintln!("atipicial-node observability panic report failed: {err}");
            }
        }));
    }
    ...
    where
        ...
        S: atipicial_storage::persistence::TransactionalStore + 'static,
    {
        self.inner
            .config
            .heartbeat_endpoints
...
impl ObservabilityInner {
    fn report_error_blocking(&self, report: &ErrorReport) -> anyhow::Result<()> {
        ...
        let reporter = std::thread::Builder::new()
            .name("atipicial-observability-error-report".to_string())
    ...
    async fn send_heartbeat_async<P, S>(
        ...
        node: &atipicial_system::Node<P, S>,
