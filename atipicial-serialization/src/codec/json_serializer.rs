...
//! JsonSerializer - mirrors `Atipicial.SmartContract.JsonSerializer`.

use atipicial_error::{CoreError, CoreResult};
use atipicial_vm::StackItem;
use atipicial_vm::StackItemType;
use atipicial_vm::stack_item::{Array, Map as MapItem, Struct};
...
impl JsonSerializer {
    ...
    /// Serializes a [`StackItem`] to a UTF-8 JSON byte vector, matching C# Atipicial's
    ...
    /// C# Atipicial's `System.Text.Json` default output (`JavaScriptEncoder.Default`).
    ///
    ...
    /// payload must match C# Atipicial v3.x.
        ...
        *remaining -= 1;
        ...
        match value {
            ...
            JsonValue::Number(n) => {
                ...
                // (Atipicial.Json `JNumber.Value` is a double parsed via GetDouble(), with
