...
use atipicial_crypto::{ECPoint, Secp256r1Crypto};
...
use atipicial_payloads::ExtensiblePayload;
use atipicial_primitives::time::now_millis;
use atipicial_state_service::{MessageType, StateRoot, StateStore, StateStoreLookup, Vote};
use atipicial_storage::persistence::providers::memory_store::MemoryStore;
use atipicial_storage::persistence::{Store, StoreCache, StoreDataCache};
    ...
    S: Store + 'static,
{
    ...
    /// StateValidator designation lookups (C# reads `AtipicialSystem.StoreView`).
    ...
    async fn fire_round(&mut self, round_index: u32) {
        ...
        let Ok(signature) = signature else {
            warn!(target: "atipicial::state_root", round_index, "state root vote signing failed");
            return;
        };
    ...
    async fn finalize_signed_root(&mut self, root_index: u32, signed: StateRoot, n: usize) {
        ...
        {
            let _ = self.network.broadcast_extensible(ext).await;
            info!(target: "atipicial::state_root", root_index, "relayed signed state root");
        }
        ...
        match message_type {
            ...
            MessageType::StateRoot => {
                ...
                if !verify_state_root_with_native_provider(
                    ...
                    warn!(target: "atipicial::state_root", root_index, "rejected unverifiable signed state root");
                    return;
                }
    ...
    async fn run(mut self) {
        ...
        info!(target: "atipicial::state_root", "state root driver loop exited");
    }
}
