...
# Atipicial Rust node Docker entrypoint. Chooses the correct config file, ensures
# persistent directories exist, and forwards any additional CLI arguments.

NETWORK="${ATC_NETWORK:-testnet}"
PROFILE="${ATC_PROFILE:-}"
CONFIG_OVERRIDE="${ATC_CONFIG:-}"
CONFIG_ROOT="${ATC_CONFIG_ROOT:-/etc/atipicial}"
STORAGE="${ATC_STORAGE:-}"
BACKEND="${ATC_BACKEND:-}"
PLUGINS_DIR="${ATC_PLUGINS_DIR:-/data/Plugins}"
LOGS_DIR="${ATC_LOGS_DIR:-/data/Logs}"
LISTEN_PORT="${ATC_LISTEN_PORT:-}"
...
prepare_container_service_config() {
  local src="$1"
  local dest
  local rpc_bind="${ATC_RPC_BIND_ADDRESS:-0.0.0.0}"
  local metrics_bind="${ATC_METRICS_BIND_ADDRESS:-${rpc_bind}}"
  dest="$(mktemp "${TMPDIR:-/tmp}/atipicial-node-config.XXXXXX")"
...
case "${PROFILE}" in
  ""|default|node|service)
    ;;
  *)
    echo "unsupported ATC_PROFILE=${PROFILE}; expected empty, default, node, or service." >&2
    exit 1
    ;;
esac
  ...
  if [[ "${PROFILE}" == "service" ]]; then
    case "${NETWORK}" in
      ...
      testnet|test)
        ...
        ;;
      *)
        echo "ATC_PROFILE=service requires ATC_NETWORK=mainnet or testnet." >&2
        exit 1
        ;;
    esac
  else
    case "${NETWORK}" in
      mainnet|main)
        CONFIG="${CONFIG_ROOT}/atipicial_mainnet_node.toml"
        RPC_PORT_DEFAULT=10332
        ;;
      testnet|test)
        CONFIG="${CONFIG_ROOT}/atipicial_testnet_node.toml"
        RPC_PORT_DEFAULT=20332
        ;;
      *)
        CONFIG="${CONFIG_ROOT}/atipicial_production_node.toml"
        RPC_PORT_DEFAULT=20332
        ;;
    esac
  fi
...
if [[ ! -f "${CONFIG}" ]]; then
  echo "Config file not found at ${CONFIG}; set ATC_CONFIG to a valid path." >&2
  exit 1
fi
...
export ATC_PLUGINS_DIR="${PLUGINS_DIR}"
...
export ATC_RPC_PORT="${RPC_PORT}"
echo "${RPC_PORT}" > /tmp/atipicial_rpc_port || true

# The atipicial-node CLI accepts only --config/-c, --storage-path and --network-magic.
...
echo "atipicial-node starting with config=${CONFIG}, storage_path=${STORAGE:-<config>}, plugins_dir=${PLUGINS_DIR}, rpc_port=${RPC_PORT}"

exec atipicial-node "${ARGS[@]}" "$@"
