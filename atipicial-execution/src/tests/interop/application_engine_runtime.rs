...
use atipicial_storage::{CacheRead, DataCache, StorageItem, StorageKey};
use atipicial_vm::OpCode;
use atipicial_vm::Script;
...
fn notification_parameter_type_check_matches_csharp() {
    ...
    assert!(matches_parameter_type(
        &StackItem::from_byte_string(b"atipicial".to_vec()),
        ContractParameterType::String
    ));
    assert!(!matches_parameter_type(
...
fn application_script_loading_is_relaxed_but_runtime_load_script_is_strict() {
    // Atipicial v3.10.1 RuntimeLoadScript constructs `new Script(script, true)` while
    ...
    for invalid_script in [
        ...
        vec![OpCode::CONVERT.byte(), atipicial_vm::StackItemType::Any.to_byte()],
    ] {
...
fn fault_clears_notifications_before_artifacts_are_exposed() {
    // Atipicial v3.10.1 ApplicationEngine.cs:567-571 sets notifications to null in OnFault.
    let mut engine =
        ApplicationEngine::<NoNativeContractProvider>::new_with_native_contract_provider(
            TriggerType::Application,
            None,
...
#[test]
fn pointer_result_halts_like_atipicial_vm_v3101() {
    // A pure `PUSHA <offset>; RET` script HALTs with a Pointer on the result
    // stack in AtipicialVM v3.10.1. Canonical execution must preserve that result.
    let script = vec![
        OpCode::PUSHA.byte(),
        0x00,
        0x00,
    ...
    assert!(matches!(
        ...
        Ok(atipicial_vm::stack_item::StackItem::Pointer(_))
    ));
}
...
#[test]
fn post_gorgon_zero_shift_coerces_boolean_result_to_integer_like_atipicial_vm_v3101() {
    ...
    assert!(
        matches!(result, atipicial_vm::stack_item::StackItem::Integer(_)),
...
fn pre_gorgon_zero_shift_preserves_boolean_like_csharp_v3101() {
    ...
    assert!(matches!(
        engine.result_stack().peek(0),
        Ok(atipicial_vm::stack_item::StackItem::Boolean(true))
    ));
}
