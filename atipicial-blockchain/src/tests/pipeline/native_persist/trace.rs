...
fn vm_profile_formatters_emit_stable_class_and_hot_opcode_summaries() {
    let mut engine = atipicial_vm::ExecutionEngine::<()>::new(None);
    engine.enable_execution_profiling();
    engine
        .load_script(
            atipicial_vm::Script::new_relaxed(vec![
                atipicial_vm::OpCode::PUSH1.byte(),
                atipicial_vm::OpCode::PUSH2.byte(),
                atipicial_vm::OpCode::ADD.byte(),
                atipicial_vm::OpCode::RET.byte(),
            ]),
            -1,
            0,
        )
        .expect("load profiled script");
    assert_eq!(engine.execute(), atipicial_vm::VmState::HALT);
...
fn vm_hardfork_context_records_activation_schedule_and_state() {
    ...
    settings.hardforks = atipicial_config::HardforkSchedule::new()
        .with_activation(atipicial_config::hardfork::Hardfork::HfBasilisk, 10)
        .with_activation(atipicial_config::hardfork::Hardfork::HfGorgon, 20);
...
fn tx_artifact_matches_application_log_shape() {
    ...
    let mut executed = atipicial_payloads::ApplicationExecuted::new(
        None,
        atipicial_primitives::TriggerType::APPLICATION,
        atipicial_vm::VmState::HALT,
        None,
        42,
        vec![atipicial_vm::StackItem::from_bool(true)],
    );
    executed
        .notifications
        .push(atipicial_payloads::NotifyEventArgs::new_with_optional_container(
            None,
            UInt160::from([0x66; 20]),
            "Transfer".to_string(),
            vec![atipicial_vm::StackItem::from_i64(7)],
        ));
