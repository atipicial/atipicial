...
use atipicial_storage::{DataCache, StorageItem, StorageKey, mdbx::MdbxStore, persistence::Store};
use std::sync::Arc;
...
fn make_mdbx_state_store() -> (StateStore<MdbxStore>, BenchTempDir) {
    let (backing, tempdir) = make_mdbx_store("atipicial-state-service-mdbx-bench");
    (
