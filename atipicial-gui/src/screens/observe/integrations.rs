...
pub fn ui(app: &mut AtipicialGuiApp, ui: &mut Ui) {
        ...
        for integ in app.integrations.iter_mut().filter(|i| i.kind == kind) {
            widgets::card(ui, |ui| {
                ui.horizontal(|ui| {
                    ...
                    ui.label(
                        egui::RichText::new(integ.name)
                            .strong()
                            .color(if integ.enabled {
                                theme::ACCENT
                            } else {
                                theme::TEXT
                            }),
                    ...
                    ui.with_layout(egui::Layout::right_to_left(egui::Align::Center), |ui| {
                        if ui
                            .add_enabled(
                                integ.enabled && !integ.value.is_empty(),
                ...
                ui.horizontal(|ui| {
                    ...
                    ui.add(
                        egui::TextEdit::singleline(&mut integ.value)
                            .desired_width(420.0)
                            .password(
                                integ.field.contains("token")
                                    || integ.field.contains("key")
                                    || integ.field.contains("DSN"),
                            ),
                    );
                });
            });
    ...
    if let Some(status) = lock(&app.integration_status, "Integration status").clone() {
        ui.add_space(8.0);
        widgets::card(ui, |ui| {
            ui.label(
...
fn test_ping(
    ...
    std::thread::spawn(move || {
        let result = if target.starts_with("http://") || target.starts_with("https://") {
            let body = serde_json::json!({ "text": format!("atipicial-gui test ping for {name}"), "content": format!("atipicial-gui test ping for {name}") });
        ...
        *lock(&sink, "Integration status") = Some(result);
        ctx.request_repaint();
    });
}
