...
use atipicial_primitives::constants::{MAX_BLOCK_SIZE, MAX_TRANSACTIONS_PER_BLOCK};
use atipicial_primitives::{TimeProvider, UInt256};
...
fn validate_import_integrity_does_not_enforce_production_tx_limit() {
    let mut block = atipicial_payloads::Block::new();
    ...
    assert!(
        ...
        "Atipicial C# treats MaxTransactionsPerBlock as a production limit, not a peer block validity rule"
    );
}
...
fn max_constants_are_correct() {
    ...
    assert_eq!(MIN_TIMESTAMP_MS, 1784505600000);
}
