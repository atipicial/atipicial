...
use atipicial_payloads::NotifyEventArgs;
use atipicial_vm::VmState;
use atipicial_vm::rpc_json::StackItemRpcJson;
...
pub(super) fn notification_to_json(
    ...
    // typed clients fail to decode the notification: atipicial-go rejects it with
    // `cannot unmarshal array into Go struct field invokeAux.notifications of
    // type stackitem.rawItem`, which aborts any atipicial-go transfer against this node.
...
pub(super) fn unclaimed_atipicial_dollar_to_json(address: String, unclaimed_datoshi: impl ToString) -> Value {
    json!({
        "address": address,
        // C# GetUnclaimedGas returns the raw datoshi BigInteger as a string
        // (ATC.UnclaimedGas(...).ToString()), e.g. "100000000" for 1 GAS - not
