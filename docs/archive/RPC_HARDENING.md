...
Recommendations for running the built-in `atipicial-node` JSON-RPC server securely.
...
- `max_concurrent_connections` — caps simultaatipicialus connections.
...
- Compatibility note: set `ATC_LISTPLUGINS_COMPAT=fixture` (and optionally `ATC_PLUGIN_VERSION=3.0.0.0`) to emulate legacy C# `listplugins` output when needed for fixture-based clients.
