...
//!
//! This deliberately has no dependency on `atipicial-state-packs`, so it can compare
...
impl Config {
    fn from_env() -> Self {
        Self {
            trials: env_usize("ATC_SEGMENT_SET_BENCH_TRIALS", DEFAULT_TRIALS),
            target_clone_slots: env_usize(
                "ATC_SEGMENT_SET_BENCH_TARGET_CLONE_SLOTS",
                DEFAULT_TARGET_CLONE_SLOTS,
            ),
            min_iterations: env_usize(
                "ATC_SEGMENT_SET_BENCH_MIN_ITERATIONS",
                DEFAULT_MIN_ITERATIONS,
            ),
            max_iterations: env_usize(
                "ATC_SEGMENT_SET_BENCH_MAX_ITERATIONS",
