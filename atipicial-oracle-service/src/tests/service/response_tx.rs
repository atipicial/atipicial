...
use atipicial_crypto::{ECCurve, ECPoint, Secp256r1Crypto};
use atipicial_payloads::{OracleResponse, OracleResponseCode, Signer, Transaction, Witness};
use atipicial_storage::{CacheRead, DataCache, StorageKey};

use atipicial_config::ProtocolSettings;
use atipicial_io::Serializable;
use atipicial_native_contracts::{LedgerContract, OracleRequest, StandardNativeProvider};
...
fn seed_oracle_contract<B: CacheRead>(snapshot: &DataCache<B>, settings: &ProtocolSettings) {
    ...
    use atipicial_native_contracts::{ContractManagement, OracleContract};
...
fn create_response_tx_matches_csharp_fee_math() {
    ...
    origin_tx.set_script(vec![atipicial_vm::OpCode::RET.byte()]);
    ...
    let system = std::sync::Arc::new(atipicial_system::Node::for_test(
        atipicial_test_fixtures::test_chain_spec(settings.as_ref().clone()),
    ));
