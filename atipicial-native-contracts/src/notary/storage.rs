...
atipicial_vm::impl_interoperable_via_stack_item!(DepositState);
...
impl Notary {
    ...
    pub(in crate::notary) fn read_deposit_field<B: atipicial_storage::CacheRead>(
        &self,
    ...
    pub(in crate::notary) fn read_deposit<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
        account: &UInt160,
    ...
    pub(in crate::notary) fn delete_deposit<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
        account: &UInt160,
    ) {
    ...
    pub(in crate::notary) fn write_deposit<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
        account: &UInt160,
        amount: &BigInt,
    ...
    /// Parses the `onAEP17Payment` `data` argument (an `Any` param that arrives
    ...
    pub(in crate::notary) fn parse_onnep17_data(
        ...
        let item = crate::support::codec::decode_stack_item(data, "Notary::onAEP17Payment data")?;
        let StackItem::Array(array) = item else {
            return Err(CoreError::invalid_operation(
                "Notary::onAEP17Payment data must be an array of 2 elements",
            ));
        };
        ...
        let to = if items[0].is_null() {
            *from
            ...
            let bytes = items[0].as_bytes().map_err(|_| {
                CoreError::invalid_operation("Notary::onAEP17Payment to: cannot convert to bytes")
            })?;
            crate::args::bytes_to_hash160(&bytes, "Notary::onAEP17Payment to: bad hash")?
        };
        let till = items[1]
            .as_int()
            .ok()
            ...
            .ok_or_else(|| {
                CoreError::invalid_operation("Notary::onAEP17Payment till out of uint range")
            })?;
        Ok((to, till))
    }
    ...
    /// correct primitive.
    pub(in crate::notary) fn put_max_not_valid_before_delta<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
        value: i64,
    ) {
    ...
    pub(in crate::notary) fn read_max_not_valid_before_delta<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
    ) -> CoreResult<i64> {
