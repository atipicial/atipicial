//! # atipicial-io::tests::io
//!
//! Test module grouping io behavior coverage for atipicial-io.
//!
//! ## Boundary
//!
//! This is test/benchmark-only code for atipicial-io; it may assemble fixtures but
//! must not introduce production behavior.
//!
//! ## Contents
//!
//! - `binary_reader_tests`: binary reader integration coverage.
//! - `binary_writer_tests`: binary writer integration coverage.
...
mod integration_tests {
    ...
    /// Test complete workflow matching C# Atipicial transaction serialization patterns
    #[test]
    fn test_transaction_like_serialization_workflow() {
        // Simulate a transaction-like structure as used in C# Atipicial
        ...
        impl Serializable for MockWitness {
            fn size(&self) -> usize {
                atipicial_io::serializable::helper::SerializeHelper::get_var_size(
            ...
            fn serialize(&self, writer: &mut BinaryWriter) -> atipicial_io::IoResult<()> {
                writer.write_var_bytes(&self.invocation_script)?;
                writer.write_var_bytes(&self.verification_script)?;
                Ok(())
            }

            fn deserialize(reader: &mut MemoryReader) -> atipicial_io::IoResult<Self> {
    ...
    /// Test block-like serialization workflow (matches C# Atipicial block patterns)
    #[test]
