...
use atipicial_manifest::{ContractAbi, ContractManifest, AefFile};
...
fn test_contract(script: Vec<u8>) -> ContractState {
    ContractState::new(
        ...
        AefFile::new("execution-plan-test".to_string(), script),
...
fn enabled_route_attaches_the_exact_v3101_plan_before_context_publication() {
    ...
    assert_eq!(key.protocol().version(), ProtocolVersion::ATC_N3_V3_10_1);
...
fn deployed_contract_route_includes_the_exact_resolution_identity() {
    ...
    assert_eq!(identity.aef_checksum(), contract.aef.checksum);
