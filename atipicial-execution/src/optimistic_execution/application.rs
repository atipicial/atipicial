...
/// Successfully validated speculative result and its original AtipicialVM engine.
pub struct AppliedSpeculativeExecution<P, D, B = atipicial_storage::EmptyCacheBacking>
where
    P: NativeContractProvider + 'static,
    D: Diagnostic + 'static,
    B: CacheRead,
...
where
    ...
    B: CacheRead,
{
    /// Original engine, including its AtipicialVM stacks, gas, fault, diagnostics,
...
pub struct RejectedSpeculativeExecution<P, D, B = atipicial_storage::EmptyCacheBacking>
where
    P: NativeContractProvider + 'static,
    D: Diagnostic + 'static,
    B: CacheRead,
...
mod tests {
    ...
    use atipicial_storage::{EmptyCacheBacking, StorageItem};
    use atipicial_vm::{NativeCacheDomain, OpCode};
        ...
        for mutation in mutations {
            match mutation {
                TestStorageMutation::Set(key, value) => engine
                    ...
                    .map_err(|error| atipicial_vm::VmError::invalid_operation_msg(error.to_string()))?,
    ...
    fn storage_effect_script(tail: &[u8]) -> Vec<u8> {
        ...
        script.extend_from_slice(&atipicial_vm::interop_hash(STORAGE_EFFECT_SYSCALL).to_le_bytes());
