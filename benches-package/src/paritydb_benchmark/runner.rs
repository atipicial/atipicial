...
pub fn run_paritydb_benchmark(config: &ParityDbBenchmarkConfig) -> Result<ParityDbBenchmarkReport> {
    ...
    let report = ParityDbBenchmarkReport {
        ...
        evidence_limitations: vec![
            ...
            "the dedicated column makes the 0xf0 namespace implicit and stores only the uniformly distributed 32-byte node hash; logical workload and digest accounting retain the exact 33-byte Atipicial key".to_owned(),
...
fn write_json_report(path: &Path, report: &ParityDbBenchmarkReport) -> Result<()> {
    ...
    let result = (|| -> Result<()> {
        ...
        // Windows cannot open a directory handle for fsync; the report file
        // was already synced before the rename.
        #[cfg(unix)]
        {
            File::open(parent)
