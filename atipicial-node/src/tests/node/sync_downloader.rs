...
use atipicial_blockchain::{BlockchainCommand, BlockchainHandle, HeaderCache, HeaderValidationOutcome};
use atipicial_network::{
...
use atipicial_payloads::{Header, HeadersPayload};
...
async fn recv_getheaders(fake: &mut FakeFramed) -> GetBlockByIndexPayload {
    loop {
        ...
        if frame.command == MessageCommand::GetHeaders {
            let mut reader = atipicial_io::MemoryReader::new(&frame.payload_raw);
            return <GetBlockByIndexPayload as atipicial_io::Serializable>::deserialize(&mut reader)
...
async fn spawn_ready_peer(
    ...
    peer_id: atipicial_network::PeerId,
    remote_nonce: u32,
    height: u32,
...
fn static_archive_bounds_downloaded_commit_batches() {
    ...
    assert_eq!(default, atipicial_network::BlockDownloadConfig::default());
...
fn header_peer_selection_requires_range_coverage_and_excludes_failed_peers() {
    let low = atipicial_network::PeerId::new();
    let failed = atipicial_network::PeerId::new();
    let best = atipicial_network::PeerId::new();
    let peers = vec![
        atipicial_network::BlockDownloadPeer::new(low, 10),
        atipicial_network::BlockDownloadPeer::new(failed, 20),
        atipicial_network::BlockDownloadPeer::new(best, 30),
    ];
...
async fn partial_header_prefix_retries_only_the_remaining_range_on_another_peer() {
    ...
    let progress = atipicial_system::HeaderStageProgress {
        window,
        checkpoint: store
    ...
    let pipeline = Arc::new(atipicial_system::SyncHeaderPipeline::new(
        blockchain.clone(),
        cache,
        Arc::clone(&store),
    ));
    ...
    let identity = Arc::new(LocalIdentity::new(
        atipicial_config::ProtocolSettings::default().network,
        7,
        "/atipicial-rs:staged-sync-test/".to_string(),
        true,
    ));
    let first_id = atipicial_network::PeerId::new();
    let second_id = atipicial_network::PeerId::new();
