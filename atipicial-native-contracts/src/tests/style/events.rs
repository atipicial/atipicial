...
#[test]
fn atipicial_coin_events_use_shared_event_names() {
    let source = standard_contract_sources()
        .into_iter()
        .find(|(name, _)| *name == "AtipicialCoin")
        .map(|(_, source)| source)
        .expect("AtipicialCoin source should be available");
    let production = source.split("#[cfg(test)]").next().unwrap_or(source);
    ...
    for event_const in [
        ...
        assert!(
            production.contains(event_const),
            "AtipicialCoin should share {event_const} between metadata and runtime notifications"
        );
    }
    ...
    for event_name in ["CandidateStateChanged", "Vote", "CommitteeChanged"] {
        assert!(
            !production.contains(&format!("\"{event_name}\".to_string()")),
            "AtipicialCoin runtime notifications should not duplicate the {event_name} event string"
        );
        assert!(
            ...
            "AtipicialCoin manifest events should not duplicate the {event_name} event string"
        );
    }
}
