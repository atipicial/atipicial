...
pub(in crate::node) fn render_prometheus() -> String {
    let mut out = String::from(
        "# HELP atipicial_node_daemon_task_spawned_total Total supervised daemon task spawns\n\
         # TYPE atipicial_node_daemon_task_spawned_total counter\n",
    );
    for (index, task) in TASKS.iter().enumerate() {
        let metrics = &METRICS[index];
        for kind in [TaskKind::Essential, TaskKind::Normal] {
            out.push_str(&format!(
                "atipicial_node_daemon_task_spawned_total{{task=\"{}\",kind=\"{}\"}} {}\n",
                task,
                kind.as_label(),
    ...
    out.push_str(
        "# HELP atipicial_node_daemon_task_events_total Total supervised daemon task terminal events\n\
         # TYPE atipicial_node_daemon_task_events_total counter\n",
    );
    for (index, task) in TASKS.iter().enumerate() {
        let metrics = &METRICS[index];
        for kind in [TaskKind::Essential, TaskKind::Normal] {
            ...
            for (outcome_index, outcome) in OUTCOMES.iter().enumerate() {
                out.push_str(&format!(
                    "atipicial_node_daemon_task_events_total{{task=\"{}\",kind=\"{}\",outcome=\"{}\"}} {}\n",
                    task,
                    kind.as_label(),
                    outcome,
