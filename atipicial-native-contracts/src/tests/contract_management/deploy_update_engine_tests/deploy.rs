...
use atipicial_manifest::{AefFile, ContractMethodDescriptor, ContractParameterDefinition};
...
fn deploy_writes_record_and_index_charges_fee_and_notifies() {
    ...
    // The record lands at GetContractHash(sender, aef.CheckSum, name) and
    // round-trips through the shared reader.
    let expected_hash = Helper::get_contract_hash(&sender, aef.checksum, "DeployFixture");
    ...
    assert_eq!(deployed.aef.checksum, aef.checksum);
...
fn deploy_hash_is_deterministic_and_duplicates_fault() {
    ...
    // Same sender + AEF checksum + name -> the same hash, so the second
    ...
    let hash_a = Helper::get_contract_hash(&sender, aef.checksum, "DeterministicFixture");
    let hash_b = Helper::get_contract_hash(&sender, aef.checksum, "DeterministicFixtureB");
...
fn deploy_runs_the_declared_deploy_callback_with_data() {
    ...
    script.emit_opcode(OpCode::RET);
    let aef = AefFile::new("e2e-test".to_string(), script.to_array());
    ...
    let (state, _) = run_deploy(
        ...
        &aef.to_bytes(),
        &manifest_json(&manifest),
        Some(&[0xAB]), // deploy(aef, manifest, data) overload
        CallFlags::ALL,
    );
    assert_eq!(
        state,
...
fn deploy_validation_failures_fault() {
    ...
    assert_eq!(state, VmState::FAULT, "empty AEF must fault");
    ...
    // A corrupted AEF checksum.
    let mut corrupted = aef.to_bytes();
    ...
    assert_eq!(state, VmState::FAULT, "bad AEF checksum must fault");

    // The target hash is Policy-blocked (C# "has been blocked").
    let snapshot = seeded_snapshot();
    let blocked_hash = Helper::get_contract_hash(&sender, aef.checksum, "FaultFixture");
    snapshot.add(
