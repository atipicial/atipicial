...
/// contract hash/ID/update/AEF identity. Hash160 is only a lookup hint; key
...
/// Value restriction for one concrete AtipicialVM stack-item type.
#[derive(Clone, Debug, Eq, Hash, PartialEq)]
pub enum StackItemConstraint {
    ...
    /// Exact Boolean value.
    ExactBoolean(bool),
    /// Inclusive signed range for a representable AtipicialVM integer.
    SignedIntegerRange {
...
impl StackItemShape {
    /// Creates and validates a concrete AtipicialVM stack-item shape.
    ///
    /// # Errors
    ///
    ...
    /// Returns the exact AtipicialVM type tag.
    #[must_use]
    ...
    /// Tests one ordinary `atipicial-vm` stack item without converting value models.
    #[must_use]
...
impl StackItemEligibility {
    ...
    /// Tests an ordinary `atipicial-vm` stack item against every accepted shape.
    #[must_use]
    pub fn matches(&self, item: &StackItem) -> bool {
...
impl ArgumentContract {
    ...
    /// Returns accepted AtipicialVM value shapes.
    #[must_use]
...
pub enum ContextDependency {
    ...
    /// Exact AtipicialVM `CALL` clone provenance: a parent frame exists, evaluation
...
impl InvocationEligibility {
    ...
    /// Checks exact arity and every normalized argument using `atipicial-vm` values.
    #[must_use]
...
pub enum ByteExpressionSegment {
    ...
    Literal(Arc<[u8]>),
    /// AtipicialVM byte conversion of one normalized method argument.
    Argument(u16),
    /// AtipicialVM byte conversion of one declared current-context slot.
    Slot {
        /// Slot domain.
        source: SlotSource,
