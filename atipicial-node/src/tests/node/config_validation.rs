...
project_id = "atipicial-production"
"#,
    )
    .expect("parse config");
...
[[observability.error_endpoints]]
kind = "google_error_reporting"
url = "https://clouderrorreporting.googleapis.com/v1beta1/projects/atipicial-production/events:report?key=example"
"#,
    )
    .expect("parse config");
...
[[observability.heartbeat_endpoints]]
url = "file:///tmp/atipicial-heartbeat"
"#,
    )
    .expect("parse config");
...
[[observability.heartbeat_endpoints]]
url = " https://uptime.example.com/atipicial-node "
"#,
    )
    .expect("parse config");
...
kind = "custom_json"
url = "https://errors.example.com/atipicial-node"
token_env = " ATC_OBSERVABILITY_TOKEN "
"#,
    )
    .expect("parse config");
...
fn validate_storage_rejects_local_replay_poison_marker() {
    ...
    let marker = chain_path.join(".atipicial-local-replay-poisoned");
...
fn validate_storage_rejects_coordinated_mdbx_state_height_mismatch() {
    ...
    assert!(error.to_string().contains("atipicial_state_service"), "{error}");
}
