//! Offline storage probe for Atipicial node databases.
...
use atipicial_config::{AtipicialChainSpec, NetworkType};
use atipicial_crypto::Crypto;
use atipicial_execution::native::native_contract_provider::NativeContractProvider;
use atipicial_execution::{ApplicationEngine, ContractState, Diagnostic};
use atipicial_io::Serializable;
use atipicial_native_contracts::{AtipicialDollar, LedgerContract, StandardNativeProvider};
use atipicial_node::NodeLifecycleLock;
use atipicial_payloads::{Block, Transaction, TransactionState, VerifiableContainer};
use atipicial_primitives::CallFlags;
use atipicial_primitives::{TriggerType, UInt160, UInt256};
use atipicial_state_service::{MDBX_STATE_SERVICE_NAMESPACE, MptStore};
use atipicial_static_files::StaticFileProvider;
...
use atipicial_storage::persistence::{
    ReadOnlyStoreGeneric, SeekDirection, StoreCache, StoreFactory, StoreSnapshot,
    TransactionalStore, WriteStore,
};
use atipicial_storage::{CacheRead, DataCache, StorageKey};
use atipicial_trie::MPT_NODE_PREFIX;
use atipicial_vm::{Instruction, StackItem, VmState as VMState};
use num_bigint::BigInt;
use parking_lot::Mutex;
use serde_json::{Value, json};

#[path = "atipicial_db_probe/stack_item_decode.rs"]
mod stack_item_decode;
use stack_item_decode::{deserialize_stack_item, stack_item_bytes, stack_item_u32};

#[derive(Debug, Parser)]
#[command(
    name = "atipicial-db-probe",
    about = "Inspect Atipicial storage and static archives without starting a node"
)]
struct Cli {
    ...
    /// Find deployed contracts whose exact AEF bytecode has this Hash160.
...
#[derive(Debug, PartialEq, Eq)]
struct AtipicialAccountStateProbe {
    balance: BigInt,
    balance_height: u32,
...
impl ReplayInstructionTracer {
    ...
    fn label_context<B: atipicial_storage::CacheRead>(
        context: &atipicial_execution::ApplicationExecutionContext<B>,
    ) -> TraceContextLabel {
...
impl Diagnostic for ReplayInstructionTracer {
    ...
    fn context_loaded<B: atipicial_storage::CacheRead>(
        &mut self,
        context: &atipicial_execution::ApplicationExecutionContext<B>,
    ) {
    ...
    fn context_unloaded<B: atipicial_storage::CacheRead>(
        &mut self,
        context: &atipicial_execution::ApplicationExecutionContext<B>,
    ) {
        let label = Self::label_context(context);
...
fn build_probe_request(cli: &Cli) -> Result<ProbeRequest> {
    ...
    if let Some(address) = cli.gas_address.as_deref() {
        ...
        return Ok(ProbeRequest {
            contract_id: AtipicialDollar::ID,
...
fn gas_account_key_from_address(address: &str) -> Result<Vec<u8>> {
    let script_hash =
        UInt160::from_address(address).map_err(|err| anyhow!("invalid Atipicial address: {err}"))?;
...
fn find_contracts_by_script_hash(db_path: &Path, requested: &[String]) -> Result<Value> {
    ...
    for (key, item) in store.find(Some(&prefix), SeekDirection::Forward) {
        ...
        let raw_script_hash = UInt160::from(Crypto::hash160(&state.aef.script));
...
fn load_replay_chain_spec(network: NetworkType) -> Result<Arc<AtipicialChainSpec>> {
    AtipicialChainSpec::from_network_type(network)
...
fn replay_transaction(
    db_path: &Path,
    static_files_dir: Option<&Path>,
    chain_spec: &AtipicialChainSpec,
    tx_hash: &str,
) -> Result<Value> {
    let tx_hash =
        UInt256::from_str(tx_hash).map_err(|err| anyhow!("invalid transaction hash: {err}"))?;
    let trace_instruction_limit = std::env::var("ATC_DB_PROBE_TRACE_INSTRUCTIONS")
        .ok()
...
fn decode_raw_transaction(raw_tx_base64: &str) -> Result<Transaction> {
    ...
    let mut reader = atipicial_io::MemoryReader::new(&bytes);
...
fn execute_transaction_probe<B: atipicial_storage::CacheRead>(
    db_path: &Path,
    snapshot: &DataCache<B>,
    chain_spec: &AtipicialChainSpec,
    block: Arc<Block>,
...
fn decode_value(mode: DecodeMode, value: &[u8]) -> Result<Value> {
    let decoded = match mode {
        ...
        DecodeMode::AtipicialAccount => {
            let state = decode_atipicial_account_state(value)?;
            json!({
                "format": "atipicial-account",
        ...
        DecodeMode::Aep17Account => json!({
            "format": "aep17-account",
            "balance": decode_aep17_account_balance(value)?.to_string(),
        }),
        ...
        DecodeMode::ContractState => {
            ...
            json!({
                ...
                "compiler": state.aef.compiler,
                "script_len": state.aef.script.len(),
                "script_base64": base64_encode(&state.aef.script),
                "methods": state.manifest.abi.methods.iter().map(|method| {
                    json!({
                        "name": method.name,
...
fn decode_atipicial_account_state(bytes: &[u8]) -> Result<AtipicialAccountStateProbe> {
    let value = deserialize_stack_item(bytes).context("deserialize ATC account state")?;
    let StackItem::Struct(structure) = value else {
        bail!("expected ATC account state struct");
    };
    let items = structure.items();
    ensure!(
        items.len() >= 4,
        "ATC account state struct is shorter than expected"
    );

    let balance = items[0]
        .as_int()
        .map_err(|err| anyhow!("decode ATC balance: {err}"))?;
    let balance_height = stack_item_u32(&items[1])
        .ok_or_else(|| anyhow!("ATC account balance_height is not u32"))?;
    let vote_to_hex = match &items[2] {
        StackItem::Null => None,
        item => {
            let bytes = stack_item_bytes(item)
                .ok_or_else(|| anyhow!("ATC account vote_to is not byte-like"))?;
            ensure!(
                bytes.len() == 33,
                "ATC account vote_to has {} bytes, expected 33",
                bytes.len()
            );
    ...
    let last_gas_per_vote = items[3]
        .as_int()
        .map_err(|err| anyhow!("decode ATC last_gas_per_vote: {err}"))?;

    Ok(AtipicialAccountStateProbe {
        balance,
        balance_height,
        vote_to_hex,
        last_gas_per_vote,
...
fn decode_aep17_account_balance(bytes: &[u8]) -> Result<BigInt> {
    if bytes.is_empty() {
        return Ok(BigInt::from(0));
    }
    let value = deserialize_stack_item(bytes).context("deserialize AEP-17 account state")?;

    let StackItem::Struct(structure) = value else {
        bail!("expected AEP-17 account state struct");
    };
    let items = structure.items();
    let balance = items
        .first()
        .ok_or_else(|| anyhow!("AEP-17 account state struct is empty"))?;
    balance
        .as_int()
        .map_err(|err| anyhow!("decode AEP-17 balance: {err}"))
}
...
#[cfg(test)]
#[path = "../tests/bin/atipicial_db_probe.rs"]
mod tests;
