...
pub(super) async fn flush_inventory_block_batch(
    live_import: &atipicial_system::LiveBlockImportPipeline,
    pending_blocks: &mut Vec<Arc<atipicial_payloads::Block>>,
) {
...
pub(super) async fn handle_inbound_inventory_item(
    item: atipicial_network::InboundInventory,
    blockchain: &atipicial_blockchain::BlockchainHandle,
    live_import: &atipicial_system::LiveBlockImportPipeline,
    relay: &atipicial_network::NetworkHandle,
    consensus_decode: &Option<(
        Arc<parking_lot::RwLock<Vec<atipicial_consensus::ValidatorInfo>>>,
        u32,
    )>,
    consensus_inbound_tx: &Option<
        tokio::sync::mpsc::Sender<atipicial_consensus::messages::ConsensusPayload>,
    >,
    consensus_tx_feed_tx: &Option<tokio::sync::mpsc::Sender<atipicial_primitives::UInt256>>,
    state_root_inbound_tx: &Option<tokio::sync::mpsc::Sender<atipicial_payloads::ExtensiblePayload>>,
    pending_blocks: &mut Vec<Arc<atipicial_payloads::Block>>,
) {
    use atipicial_network::InboundInventory;
    ...
    match item {
        ...
        InboundInventory::Transaction(tx) => {
            ...
            if let Ok(reply) = blockchain
                .add_transaction(atipicial_mempool::TransactionOrigin::External, (*tx).clone())
                .await
            {
                if reply.result.is_success() {
                    let _ = relay
                        .broadcast_inv(atipicial_primitives::InventoryType::Transaction, vec![reply.hash])
                        .await;
        ...
        InboundInventory::Extensible(payload) => {
            ...
            if let Some(tx) = state_root_inbound_tx {
                if payload.category == atipicial_state_service::STATE_SERVICE_CATEGORY {
