...
[observability]
enabled = true
service_name = "atipicial-node-testnet"
environment = "testnet"
node_id = "validator-1"
...
format = "json"
file_path = "./logs/atipicial-node-test.log"
console_output = true
max_file_size = "100MB"
max_files = 10
"#,
    ...
    assert_eq!(config.logging.level.as_deref(), Some("info,atipicial=debug"));
    assert_eq!(config.logging.format.as_deref(), Some("json"));
    assert_eq!(
        config.logging.file_path.as_deref(),
        Some(std::path::Path::new("./logs/atipicial-node-test.log"))
    );
