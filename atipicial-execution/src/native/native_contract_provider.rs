...
//! native contracts (ATC, GAS, Policy, ContractManagement, ...) by hash, but
//! the engine itself lives in `atipicial-execution` while the concrete
//! implementations live in `atipicial-native-contracts`. To avoid the resulting
//! crate-cycle, this module exposes a `NativeContractProvider` trait that:
//!
//! - is **defined** in `atipicial-execution` (the consumer);
//! - is **implemented** in `atipicial-native-contracts` (the provider); and
...
//!
//! ```ignore
//! // In atipicial-system / atipicial-node composition:
...
pub trait NativeContractProvider: Send + Sync + Sized + 'static {
    ...
    /// Returns the ATD balance of `account`.
    fn gas_balance<B: CacheRead>(
        ...
        let _ = (snapshot, account);
        Err(CoreError::invalid_operation(
            "native provider does not expose ATD balance",
        ))
    }
    ...
    /// Returns ATC's cached committee multisig address.
    ...
    /// Returns ATC's next block validator set in C# whitelist order.
    ...
    /// Returns ATC's `NextConsensus` account for `block_index`.
    ...
    /// Applies byte-equivalent ATC empty-block reward effects for `[start, end]`.
