...
//! converted from the C# Atipicial Wallet class (@atipicial-sharp/src/Atipicial/Wallets/Wallet.cs).
...
pub enum WalletError {
    ...
    /// Error propagated from shared Atipicial core functionality.
    #[error("Core error: {0}")]
    Core(#[from] atipicial_error::CoreError),

    /// Error propagated from filesystem or stream operations.
    #[error("IO error: {0}")]
    Io(#[from] std::io::Error),

    /// Miscellaatipicialus wallet error that does not fit a more specific variant.
    #[error("Other error: {0}")]
    Other(String),
}
...
pub trait Wallet: Send + Sync {
    ...
    /// Imports an account from a AEP-2 encrypted private key.
    fn import_aep2(
        &self,
        aep2_key: &str,
        password: &str,
