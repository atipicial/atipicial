...
pub fn attribute_from_json(json: &JObject) -> CoreResult<TransactionAttribute> {
    let attr_type = json
        .get("type")
        .and_then(atipicial_serialization::json::JToken::as_string)
        .ok_or_else(|| CoreError::other("Transaction attribute missing 'type' field"))?;
    ...
    match attr_type.as_str() {
        ...
        "Conflicts" => {
            let hash_str = json
                .get("hash")
                .and_then(atipicial_serialization::json::JToken::as_string)
                .ok_or_else(|| CoreError::other("Conflicts attribute missing 'hash' field"))?;
            let hash = UInt256::parse(&hash_str)
                .map_err(|err| CoreError::other(format!("Invalid conflicts hash: {err}")))?;
        ...
        "OracleResponse" => {
            let id_token = json
                .get("id")
                .ok_or_else(|| CoreError::other("OracleResponse attribute missing 'id' field"))?;
