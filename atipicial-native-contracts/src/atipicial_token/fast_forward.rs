//! Empty-block fast-forward reward accounting for ATC.
//!
...
use super::{
    AtipicialCoin, COMMITTEE_REWARD_RATIO, VOTE_FACTOR, VOTER_REWARD_RATIO,
...
impl AtipicialCoin {
    /// Applies the state-only ATC empty-block persist effects for a fast-forward
    /// run.
    ///
    ...
    pub fn fast_forward_empty_block_rewards<B: atipicial_storage::CacheRead>(
        &self,
        ...
        if committee_count == 0 {
            return Err(CoreError::invalid_operation(
                "AtipicialCoin::fast_forward_empty_block_rewards requires a non-empty standby committee",
            ));
        }
        if start > end {
            return Ok(());
        ...
        let gas = crate::AtipicialDollar::new();
    ...
    fn fast_forward_voter_reward_refreshes<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
        settings: &atipicial_config::ProtocolSettings,
        height: u32,
        ...
        // C# v3.10.1 `AtipicialCoin.PostPersistAsync`: from HF_Gorgon onward,
    ...
    fn fast_forward_refresh_reward<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
        settings: &atipicial_config::ProtocolSettings,
