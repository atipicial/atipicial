...
/// Seeds a ATD balance entry (`Struct[Balance]`) and a matching total supply.
fn seed_gas(cache: &DataCache, account: &UInt160, balance: &BigInt) {
    ...
    cache.add(
        crate::AtipicialDollar::account_key(account),
        StorageItem::from_bytes(bytes),
    );
    cache.add(
        crate::AtipicialDollar::total_supply_key(),
...
/// onAEP17Payment args `[from, amount, data]` as the dispatcher marshals
...
fn seed_registration_payment_snapshot(
    ...
    deploy_native(
        &cache,
        &build_native_contract_state(&AtipicialCoin, &settings, 0),
    );
    deploy_native(
        &cache,
        &build_native_contract_state(&crate::AtipicialDollar, &settings, 0),
    );
...
fn gas_transfer_register_candidate_script(sender: UInt160, pubkey: &ECPoint) -> Vec<u8> {
    ...
    b.emit_push(&AtipicialCoin::script_hash().to_array());
    b.emit_push(&sender.to_array());
    b.emit_push_int(4);
    b.emit_pack();
    b.emit_push_int(i64::from(CallFlags::ALL.bits()));
    b.emit_push("transfer".as_bytes());
    b.emit_push(&crate::AtipicialDollar::script_hash().to_array());
...
fn gas_transfer_register_then_get_candidates_script(sender: UInt160, pubkey: &ECPoint) -> Vec<u8> {
    ...
    b.emit_opcode(atipicial_vm::OpCode::DROP);
...
#[test]
fn on_aep17_payment_data_parser_uses_stack_item_projection() {
    let source = include_str!("../../../atipicial_token/invoke.rs");
    let start = source
        .find("fn invoke_on_aep17_payment(")
        .or_else(|| source.find("fn invoke_on_aep17_payment<"))
        .expect("onAEP17Payment handler exists");
...
#[test]
fn atipicial_payment_callback_runs_before_deferred_gas_distribution_mint() {
    let source = include_str!("../../../atipicial_token/transfers.rs");

    let post_start = source
        .find("pub(super) fn atipicial_post_transfer")
        .expect("atipicial_post_transfer exists");
    let transfer_start = source
        .find("pub(super) fn atipicial_transfer_core")
        .expect("atipicial_transfer_core exists");
    ...
    let distribution_mint = mint_body
        ...
        .expect("mint mints deferred ATD distributions");
    assert!(callback < distribution_mint);
}

/// Full Echidna flow (C# AtipicialCoin.OnNEP17Payment, AtipicialCoin.cs:374-389):
/// `GAS.transfer(sender -> ATC, registerPrice, data = pubkey)` registers
/// the candidate and burns the GAS from ATC's balance.
#[test]
fn on_aep17_payment_registers_candidate_and_burns_gas() {
    ...
    let item = snapshot
        .get(&AtipicialCoin::candidate_key(&pubkey))
        .expect("candidate entry written");
    let (registered, votes) = AtipicialCoin::decode_candidate_state(&item.value_bytes()).unwrap();
    assert!(registered, "candidate is Registered");
    assert_eq!(votes, BigInt::from(0));
    assert!(
        snapshot
            .get(&crate::AtipicialDollar::account_key(&sender))
            .is_none(),
        "sender spent all GAS"
    );
    assert!(
        snapshot
            .get(&crate::AtipicialDollar::account_key(
                &AtipicialCoin::script_hash()
            ))
            .is_none(),
        "ATC's received GAS is burned"
    );
    let supply = snapshot
        .get(&crate::AtipicialDollar::total_supply_key())
        .expect("supply entry");
    assert_eq!(
...
fn gas_payment_registration_is_visible_before_next_contract_call() {
    ...
    assert_eq!(
        engine.execute_allow_fault(),
        VmState::HALT,
        "GAS.transfer and immediate ATC.getCandidates must HALT: {:?}",
...
#[test]
fn gas_transfer_to_atipicial_faults_when_native_transfer_fee_exceeds_transaction_gas_limit() {
    let pubkey = candidate_pubkey();
    let candidate_account =
        UInt160::from_script(&Contract::create_signature_redeem_script(pubkey.clone()));
    let sender = UInt160::from_bytes(&[0x07; 20]).unwrap();
    ...
    assert_eq!(
        engine.execute_allow_fault(),
        VmState::FAULT,
        "GAS.transfer to ATC must fault when its native transfer fee exceeds the transaction gas limit",
    );
    let fault = engine
        .fault_exception()
...
#[test]
fn on_aep17_payment_rejects_bad_caller_amount_pubkey_and_witness() {
    let pubkey = candidate_pubkey();
    let candidate_account =
        UInt160::from_script(&Contract::create_signature_redeem_script(pubkey.clone()));
    let sender = UInt160::from_bytes(&[0x07; 20]).unwrap();
    ...
    .unwrap_err();
    assert!(err.to_string().contains("only the ATD contract"), "{err}");

    let gas_caller = Some(crate::AtipicialDollar::script_hash());
    ...
    .unwrap_err();
    assert!(err.to_string().contains("incorrect ATD amount"), "{err}");
