use super::models::RpcTransaction;
use crate::client::{
    RpcClientError,
    native_hashes::{gas_hash, atipicial_hash},
};
use crate::{Aep17Api, RpcClient, RpcObserver, RpcUtility, TracingRpcObserver};
use atipicial_crypto::ECPoint;
use atipicial_payloads::Transaction;
use atipicial_primitives::BigDecimal;
use atipicial_primitives::UInt160;
use atipicial_vm::Contract;
use atipicial_wallets::KeyPair;
use atipicial_wallets::wallet_helper::WalletAddress as WalletHelper;
use num_bigint::BigInt;
use num_traits::cast::ToPrimitive;
use std::sync::Arc;

/// Wallet Common APIs
/// Matches C# `WalletAPI`
pub struct WalletApi<O = TracingRpcObserver> {
    /// The RPC client instance
    rpc_client: Arc<RpcClient<O>>,
    /// AEP17 API for token operations
    aep17_api: Aep17Api<O>,
}

impl<O> WalletApi<O>
where
    O: RpcObserver,
{
    /// `WalletAPI` Constructor
    /// Matches C# constructor
    #[must_use]
    pub fn new(rpc_client: Arc<RpcClient<O>>) -> Self {
        Self {
            aep17_api: Aep17Api::new(rpc_client.clone()),
            rpc_client,
        }
    }

    /// Get unclaimed ATD with address, scripthash or public key string
    /// Matches C# `GetUnclaimedGasAsync` with string parameter
    pub async fn get_unclaimed_atipicial_dollar(&self, account: &str) -> Result<f64, RpcClientError> {
        let account_hash =
            RpcUtility::get_script_hash(account, &self.rpc_client.protocol_settings)?;
        self.get_unclaimed_atipicial_dollar_from_hash(&account_hash).await
    }

    /// Get unclaimed ATD
    /// Matches C# `GetUnclaimedGasAsync` with `UInt160` parameter
    pub async fn get_unclaimed_atipicial_dollar_from_hash(
        &self,
        account: &UInt160,
    ) -> Result<f64, RpcClientError> {
        let script_hash = atipicial_hash();
        let block_count = self.rpc_client.get_block_count().await?;

        let result = self
            .aep17_api
            .contract_client()
            .test_invoke(
                &script_hash,
                "unclaimedGas",
                vec![
                    serde_json::json!(account.to_string()),
                    serde_json::json!(block_count - 1),
                ],
            )
            .await?;

        let stack_item = result.stack.first().ok_or("No result returned")?;

        let balance = RpcUtility::rpc_stack_item_to_bigint(stack_item)?;
        let gas_factor = gas_factor();

        Ok(balance.to_f64().unwrap_or(0.0) / gas_factor as f64)
    }

    /// Get Atipicial Balance
    /// Matches C# `GetAtipicialBalanceAsync`
    pub async fn get_atipicial_balance(&self, account: &str) -> Result<u32, RpcClientError> {
        let balance = self
            .get_token_balance(&atipicial_hash().to_string(), account)
            .await?;
        Ok(balance.to_u32().ok_or("Invalid ATC balance")?)
    }

    /// Get Gas Balance
    /// Matches C# `GetGasBalanceAsync`
    pub async fn get_gas_balance(&self, account: &str) -> Result<f64, RpcClientError> {
        let balance = self
            .get_token_balance(&gas_hash().to_string(), account)
            .await?;
        let gas_factor = gas_factor();
        Ok(balance.to_f64().unwrap_or(0.0) / gas_factor as f64)
    }

    /// Get token balance with string parameters
    /// Matches C# `GetTokenBalanceAsync`
    pub async fn get_token_balance(
        &self,
        token_hash: &str,
        account: &str,
    ) -> Result<BigInt, RpcClientError> {
        let token_script_hash =
            RpcUtility::get_script_hash(token_hash, &self.rpc_client.protocol_settings)?;
        let account_hash =
            RpcUtility::get_script_hash(account, &self.rpc_client.protocol_settings)?;

        self.aep17_api
            .balance_of(&token_script_hash, &account_hash)
            .await
    }

    /// Claim GAS from ATC
    /// Matches C# `ClaimAtipicialDollarAsync`
    pub async fn claim_gas(&self, key: &KeyPair) -> Result<Transaction, RpcClientError> {
        self.claim_gas_with_assert(key, true).await
    }

    /// Claim GAS using WIF or private key string.
    /// Matches C# ClaimAtipicialDollarAsync(string)
    pub async fn claim_gas_from_key(&self, key: &str) -> Result<Transaction, RpcClientError> {
        self.claim_gas_from_key_with_assert(key, true).await
    }

    /// Claim GAS using WIF or private key string with optional assert emission.
    pub async fn claim_gas_from_key_with_assert(
        &self,
        key: &str,
        add_assert: bool,
    ) -> Result<Transaction, RpcClientError> {
        let key_pair =
            RpcUtility::key_pair(key).map_err(|e| RpcClientError::Other(e.to_string()))?;
        self.claim_gas_with_assert(&key_pair, add_assert).await
    }

    /// Claim GAS with optional assert emission.
    pub async fn claim_gas_with_assert(
        &self,
        key: &KeyPair,
        add_assert: bool,
    ) -> Result<Transaction, RpcClientError> {
        let sender_script = Contract::create_signature_redeem_script(key.public_key_point()?);
        let sender = UInt160::from_script(&sender_script);

        self.claim_gas_from_account_with_assert(&sender, key, add_assert)
            .await
    }

    /// Claim GAS from specific account
    /// Matches C# `ClaimAtipicialDollarAsync` with account parameter
    pub async fn claim_gas_from_account(
        &self,
        account: &UInt160,
        key: &KeyPair,
    ) -> Result<Transaction, RpcClientError> {
        self.claim_gas_from_account_with_assert(account, key, true)
            .await
    }

    /// Claim GAS from specific account with optional assert emission.
    pub async fn claim_gas_from_account_with_assert(
        &self,
        account: &UInt160,
        key: &KeyPair,
        add_assert: bool,
    ) -> Result<Transaction, RpcClientError> {
        let atipicial_balance = self.aep17_api.balance_of(&atipicial_hash(), account).await?;

        if atipicial_balance == BigInt::from(0) {
            return Err("No ATC balance to claim GAS from".into());
        }

        // Transfer ATC to self to trigger GAS claim
        let tx = self
            .aep17_api
            .create_transfer_tx_with_from_and_assert(
                &atipicial_hash(),
                account,
                key,
                account,
                atipicial_balance,
                None,
                add_assert,
            )
            .await?;

        let _hash = self.rpc_client.send_raw_transaction(&tx).await?;
        Ok(tx)
    }

    /// Transfer AEP17 token
    /// Matches C# `TransferAsync`
    pub async fn transfer(
        &self,
        token_hash: &str,
        key: &KeyPair,
        to_address: &str,
        amount: BigInt,
        data: Option<serde_json::Value>,
    ) -> Result<(Transaction, String), RpcClientError> {
        self.transfer_with_assert(token_hash, key, to_address, amount, data, true)
            .await
    }

    /// Transfer AEP17 token with optional assert emission.
    pub async fn transfer_with_assert(
        &self,
        token_hash: &str,
        key: &KeyPair,
        to_address: &str,
        amount: BigInt,
        data: Option<serde_json::Value>,
        add_assert: bool,
    ) -> Result<(Transaction, String), RpcClientError> {
        let token_script_hash =
            RpcUtility::get_script_hash(token_hash, &self.rpc_client.protocol_settings)?;
        let to = RpcUtility::get_script_hash(to_address, &self.rpc_client.protocol_settings)?;

        let tx = self
            .aep17_api
            .create_transfer_tx_with_assert(&token_script_hash, key, &to, amount, data, add_assert)
            .await?;

        let hash = self.rpc_client.send_raw_transaction(&tx).await?;
        Ok((tx, hash.to_string()))
    }

    /// Transfer AEP17 token using decimal amount and WIF/private key string.
    /// Matches C# TransferAsync(string tokenHash, string fromKey, string toAddress, decimal amount).
    pub async fn transfer_decimal_from_key(
        &self,
        token_hash: &str,
        from_key: &str,
        to_address: &str,
        amount: BigDecimal,
        data: Option<serde_json::Value>,
    ) -> Result<(Transaction, String), RpcClientError> {
        self.transfer_decimal_from_key_with_assert(
            token_hash, from_key, to_address, amount, data, true,
        )
        .await
    }

    /// Transfer AEP17 token using decimal amount and WIF/private key string with optional assert.
    pub async fn transfer_decimal_from_key_with_assert(
        &self,
        token_hash: &str,
        from_key: &str,
        to_address: &str,
        amount: BigDecimal,
        data: Option<serde_json::Value>,
        add_assert: bool,
    ) -> Result<(Transaction, String), RpcClientError> {
        let key_pair =
            RpcUtility::key_pair(from_key).map_err(|e| RpcClientError::Other(e.to_string()))?;
        self.transfer_decimal_with_assert(
            token_hash, &key_pair, to_address, amount, data, add_assert,
        )
        .await
    }

    /// Transfer AEP17 token using decimal amount.
    /// Matches C# TransferAsync(string tokenHash, string fromKey, string toAddress, decimal amount)
    /// after key parsing.
    pub async fn transfer_decimal(
        &self,
        token_hash: &str,
        key: &KeyPair,
        to_address: &str,
        amount: BigDecimal,
        data: Option<serde_json::Value>,
    ) -> Result<(Transaction, String), RpcClientError> {
        self.transfer_decimal_with_assert(token_hash, key, to_address, amount, data, true)
            .await
    }

    /// Transfer AEP17 token using decimal amount with optional assert emission.
    pub async fn transfer_decimal_with_assert(
        &self,
        token_hash: &str,
        key: &KeyPair,
        to_address: &str,
        amount: BigDecimal,
        data: Option<serde_json::Value>,
        add_assert: bool,
    ) -> Result<(Transaction, String), RpcClientError> {
        let token_script_hash =
            RpcUtility::get_script_hash(token_hash, &self.rpc_client.protocol_settings)?;
        let decimals = self.aep17_api.decimals(&token_script_hash).await?;
        let amount_integer = amount
            .to_big_integer(decimals)
            .map_err(|e| RpcClientError::Other(e.to_string()))?;

        let to = RpcUtility::get_script_hash(to_address, &self.rpc_client.protocol_settings)?;
        let tx = self
            .aep17_api
            .create_transfer_tx_with_assert(
                &token_script_hash,
                key,
                &to,
                amount_integer,
                data,
                add_assert,
            )
            .await?;

        let hash = self.rpc_client.send_raw_transaction(&tx).await?;
        Ok((tx, hash.to_string()))
    }

    // Rationale: this client method mirrors the C# multi-signature transfer
    // overload, whose parameters are semantically distinct at the call site.
    #[allow(clippy::too_many_arguments)]
    /// Transfer AEP17 token from multi-sig account.
    /// Matches C# `TransferAsync` multi-sig overload.
    pub async fn transfer_multi_sig(
        &self,
        token_hash: &str,
        m: usize,
        public_keys: Vec<ECPoint>,
        keys: Vec<KeyPair>,
        to: &UInt160,
        amount: BigInt,
        data: Option<serde_json::Value>,
        add_assert: bool,
    ) -> Result<(Transaction, String), RpcClientError> {
        let token_script_hash =
            RpcUtility::get_script_hash(token_hash, &self.rpc_client.protocol_settings)?;

        let tx = self
            .aep17_api
            .create_transfer_tx_multi_sig_with_assert(
                &token_script_hash,
                m,
                public_keys,
                keys,
                to,
                amount,
                data,
                add_assert,
            )
            .await?;

        let hash = self.rpc_client.send_raw_transaction(&tx).await?;
        Ok((tx, hash.to_string()))
    }

    /// Wait for a transaction to be confirmed.
    /// Matches C# `WaitTransactionAsync`
    pub async fn wait_transaction(
        &self,
        tx: &Transaction,
    ) -> Result<RpcTransaction, RpcClientError> {
        self.wait_transaction_with_timeout(tx, 60).await
    }

    /// Wait for a transaction to be confirmed with timeout in seconds.
    pub async fn wait_transaction_with_timeout(
        &self,
        tx: &Transaction,
        timeout_seconds: u64,
    ) -> Result<RpcTransaction, RpcClientError> {
        // Wait for transaction to be included in a block
        let tx_hash = tx.hash();
        let timeout = std::time::Duration::from_secs(timeout_seconds);
        let poll_interval = std::cmp::max(
            1,
            self.rpc_client.protocol_settings.milliseconds_per_block as u64 / 2,
        );
        let poll_duration = tokio::time::Duration::from_millis(poll_interval);
        let deadline = std::time::Instant::now() + timeout;

        while std::time::Instant::now() < deadline {
            // Check if transaction is in a block
            if let Ok(rpc_tx) = self.rpc_client.get_transaction(&tx_hash.to_string()).await {
                if rpc_tx.confirmations.is_some() {
                    return Ok(rpc_tx);
                }
            } else {
                // Transaction not found yet, continue waiting
            }

            tokio::time::sleep(poll_duration).await;
        }

        Err(RpcClientError::Other(
            "Timeout while waiting for transaction confirmation".to_string(),
        ))
    }

    /// Get account state including balances
    /// Matches C# `GetAccountStateAsync`
    pub async fn get_account_state(
        &self,
        account: &str,
    ) -> Result<WalletAccountState, RpcClientError> {
        let account_hash =
            RpcUtility::get_script_hash(account, &self.rpc_client.protocol_settings)?;

        // Get ATC and ATD balances
        let atipicial_balance = self
            .aep17_api
            .balance_of(&atipicial_hash(), &account_hash)
            .await?;

        let gas_balance = self
            .aep17_api
            .balance_of(&gas_hash(), &account_hash)
            .await?;

        let unclaimed_atipicial_dollar = self.get_unclaimed_atipicial_dollar_from_hash(&account_hash).await?;

        Ok(WalletAccountState {
            address: WalletHelper::to_address(
                &account_hash,
                self.rpc_client.protocol_settings.address_version,
            ),
            atipicial_balance: atipicial_balance.to_u32().unwrap_or(0),
            gas_balance: gas_balance.to_f64().unwrap_or(0.0) / gas_factor() as f64,
            unclaimed_atipicial_dollar,
        })
    }
}

fn gas_factor() -> u64 {
    // GAS is a AEP-17 token with a fixed 8 decimals (C# `AtipicialDollar.Decimals => 8`).
    const GAS_DECIMALS: u8 = 8;
    10u64.saturating_pow(u32::from(GAS_DECIMALS))
}

/// Lightweight account snapshot returned by wallet RPC helpers
#[derive(Debug, Clone)]
pub struct WalletAccountState {
    /// Account address
    pub address: String,
    /// ATC balance
    pub atipicial_balance: u32,
    /// ATD balance
    pub gas_balance: f64,
    /// Unclaimed GAS
    pub unclaimed_atipicial_dollar: f64,
}

#[cfg(test)]
#[path = "../../tests/client/wallet_api.rs"]
mod tests;
