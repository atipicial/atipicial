...
//!
//! Usage:
//!   atipicial-pack-verify --mdbx <canonical-store-dir> --pack <shadow-packs-dir>
//!     [--samples N] [--walk-cap N | --full-scan] [--maintain]
//!     [--random-point-mmap]
//!   atipicial-pack-verify --mode authority --network-magic <u32-or-hex>
...
use atipicial_state_packs::checkpoint::{PACK_CHECKPOINT_SOURCE_NAMESPACE, PackCheckpoint};
use atipicial_state_packs::shadow::{SHADOW_HIGH_WATER_KEY, ShadowHighWaterRecord};
use atipicial_state_packs::{PACK_KEY_BYTES, PackStore, PackStoreConfig, PackStoreOptions};
use atipicial_state_service::read_current_local_root;
use atipicial_storage::persistence::providers::RuntimeStore;
use atipicial_storage::persistence::storage::StorageConfig;
use atipicial_storage::persistence::{RawReadOnlyStore, StoreFactory, TransactionalStore};
use atipicial_trie::MPT_NODE_PREFIX;

#[path = "atipicial_pack_verify/evidence.rs"]
mod evidence;
use evidence::{
    gc_and_reopen_with_evidence, print_materialized_evidence, scrub_indexes_with_label,
    validate_authority_mutation_flags,
};
#[path = "atipicial_pack_verify/checkpoint_compare.rs"]
mod checkpoint_compare;
use checkpoint_compare::compare_checkpoint_nodes;

const BATCH: usize = 1_024;
// Atipicial MPT nodes are well below this defensive verifier ceiling. Keep a wider
...
include!("atipicial_pack_verify/tests.rs");
