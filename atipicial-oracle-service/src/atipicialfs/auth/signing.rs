...
use atipicial_io::var_int::VarInt;
use atipicial_primitives::hex_util;
use atipicial_wallets::KeyPair;
use rand::RngCore;
use rand::rngs::OsRng;

/// AtipicialFs bearer-token signing helpers.
pub(crate) struct AtipicialFsBearerSigner;

impl AtipicialFsBearerSigner {
    pub(crate) fn sign_atipicialfs_bearer(
        token: &str,
        key: &KeyPair,
        wallet_connect: bool,
        ...
        let signature = if wallet_connect {
            AtipicialFsBearerSigner::sign_atipicialfs_wallet_connect(&data, key).ok()?
        } else {
            AtipicialFsBearerSigner::sign_atipicialfs_sha512(&data, key).ok()?
        };
        Some((signature, key.compressed_public_key()))
    }

    pub(crate) fn sign_atipicialfs_sha512(data: &[u8], key: &KeyPair) -> CoreResult<Vec<u8>> {
        Secp256r1Crypto::sign_atipicialfs_sha512(data, key.private_key())
            .map(|signature| signature.to_vec())
            .map_err(|err| CoreError::other(format!("failed to sign bearer token: {err}")))
    }

    fn sign_atipicialfs_wallet_connect(data: &[u8], key: &KeyPair) -> CoreResult<Vec<u8>> {
        let b64 = base64::engine::general_purpose::STANDARD.encode(data);
        let mut salt = [0u8; 16];
        OsRng.fill_bytes(&mut salt);
        let message = AtipicialFsBearerSigner::salt_message_wallet_connect(b64.as_bytes(), &salt);
        let signature = Secp256r1Crypto::sign(&message, key.private_key())
            .map_err(|err| CoreError::other(format!("invalid atipicialfs key: {err}")))?;
...
#[cfg(test)]
#[path = "../../tests/atipicialfs/auth/signing.rs"]
mod tests;
