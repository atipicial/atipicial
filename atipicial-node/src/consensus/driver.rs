...
//!
//! This module owns node-side orchestration around `atipicial-consensus`: round
...
use atipicial_consensus::messages::ConsensusPayload;
use atipicial_consensus::{
...
fn recovery_log_path(data_dir: Option<&std::path::Path>) -> Option<std::path::PathBuf> {
    ...
    if let Err(err) = std::fs::create_dir_all(&dir) {
        warn!(target: "atipicial", %err, dir = %dir.display(), "could not create consensus recovery-log dir; persistence disabled");
        return None;
    }
    ...
    St: Store + 'static,
{
    ...
    fn configure_round<B: CacheRead>(
        ...
        *self.validators.write() = validators;
        ...
        // atipicialSystem.GetTimePerBlock()` once per block round. Post-Echidna this is
    ...
    async fn run(mut self) {
        ...
        let next_consensus = match self.configure_round(&round_snapshot, block_index) {
            ...
            Err(err) => {
                warn!(target: "atipicial", %err, "consensus round context unavailable");
                return;
            }
        };
            ...
            .await
        {
            Ok(resumed) => resumed,
            Err(err) => {
                warn!(target: "atipicial", %err, block_index, "consensus recovery-log resume failed; starting fresh");
                false
            }
        };
        if resumed {
            info!(target: "atipicial", block_index, "consensus resumed from recovery log");
        } else {
            match self.service.start_with_block_context(
                ...
                Ok(()) => info!(target: "atipicial", block_index, "consensus started"),
                Err(err) => {
                    info!(target: "atipicial", %err, block_index, "consensus not started; driver idle");
                }
            }
        }
            ...
            tokio::select! {
                ...
                maybe_msg = self.inbound_rx.recv() => {
                    ...
                    if let Err(err) = self.service.process_message(payload).await {
                        warn!(target: "atipicial", %err, "consensus rejected inbound payload");
                    }
                }
                    ...
                    match ev {
                        Ok(RuntimeEvent::Imported { hash, height, timestamp }) => {
                            ...
                            let next_consensus = match self.configure_round(&round_snapshot, block_index) {
                                Ok(next_consensus) => next_consensus,
                                Err(err) => {
                                    warn!(target: "atipicial", %err, block_index, "consensus round context unavailable");
                                    continue;
                                }
                            };
                            ...
                            match self.service.start_with_block_context(
                                ...
                                Ok(()) => info!(target: "atipicial", block_index, "consensus restarted"),
                                Err(err) => info!(target: "atipicial", %err, "consensus next round not started"),
                            }
                        }
                ...
                _ = ticker.tick() => {
                    if let Err(err) = self.service.on_timer_tick(now_millis()).await {
                        warn!(target: "atipicial", %err, "consensus timer tick failed");
                    }
                }
            }
        }
        info!(target: "atipicial", "consensus driver loop exited");
    }
    ...
    pub(super) async fn on_transaction_feed(&mut self, tx_hash: UInt256) {
        ...
        if let Err(err) = self.service.on_transaction(tx_hash).await {
            warn!(target: "atipicial", %err, "consensus on_transaction failed");
        }
    }
        ...
        match event {
            ...
            ConsensusEvent::RequestTransactions {
                ...
                if !hashes.is_empty() {
                    if let Err(err) = self
                        .network
                        .broadcast_inv(atipicial_primitives::InventoryType::Transaction, hashes.clone())
                        .await
                    {
                        warn!(target: "atipicial", %err, "consensus proposal Inv(TX) broadcast failed");
                    }
                }
                if let Err(err) = self.service.on_transactions_received(hashes).await {
                    warn!(target: "atipicial", %err, "consensus on_transactions_received failed");
                }
            }
            ConsensusEvent::RequestProposalTransactions {
                ...
                if let Some(reason) = availability.rejection_reason {
                    if let Err(err) = self.service.request_change_view(reason, now_millis()).await {
                        warn!(target: "atipicial", %err, ?reason, "consensus request_change_view failed");
                    }
                }
            }
            ConsensusEvent::BlockCommitted {
                ...
                let txs = match resolve_transactions(
                    ...
                    Some(txs) => txs,
                    None => {
                        error!(target: "atipicial", block_index, "missing transaction for committed block; cannot assemble");
                        return;
                    }
                };
                ...
                match block_data.assemble_block(txs) {
                    Ok(block) => {
                        ...
                        info!(target: "atipicial", block_index, "consensus produced + submitted block");
                        // The next round restarts off the RuntimeEvent::Imported.
                    }
                    Err(err) => {
                        error!(target: "atipicial", block_index, %err, "consensus block assembly failed")
                    }
                }
            }
            ConsensusEvent::ViewChanged {
                ...
                info!(target: "atipicial", block_index, old_view, new_view, "consensus view changed");
            }
        }
    }
}
    ...
    St: Store + 'static,
{
    ...
    if let Some(ref path) = state_path {
        info!(target: "atipicial", path = %path.display(), "dBFT recovery log enabled");
    }
