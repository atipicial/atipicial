...
pub(super) struct PeerSession<B: BlockSource> {
    ...
    /// Optional per-peer Bloom filter used by Atipicial SPV clients.
    ///
...
/// C# `Atipicial.Cryptography.Helper.Test(BloomFilter, Transaction)` semantics:
        ...
        for capability in &payload.capabilities {
            match capability {
                NodeCapability::FullNode { start_height } => {
                    ...
                    atipicial_runtime::sync_metrics::set_peer_live_tip(*start_height as u64);
                }
