#!/usr/bin/env python3
"""Compare GAS token storage between local atipicial-rs and C# reference at a specific block height."""
...
def decode_atipicial_integer(data, offset):
    """Decode a Atipicial BinaryFormatWriter integer from data at offset.
...
def decode_gas_account_state(raw_bytes):
    ...
    balance, offset = decode_atipicial_integer(raw_bytes, offset)
    return balance
...
def main():
    ...
    # In Atipicial, UInt160 in storage keys is stored in little-endian (raw byte order).
