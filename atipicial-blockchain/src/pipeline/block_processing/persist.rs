    ...
    B: CacheRead,
{
    ...
    pub(crate) chain_spec: Arc<AtipicialChainSpec>,
    ...
    M: MempoolLike,
{
    ...
    pub(crate) fn batch_persist_resources(
        &self,
        index: u32,
    ) -> atipicial_error::CoreResult<Option<BatchPersistResources<S::NativeProvider, S::CacheBacking>>>
    {
        ...
        let native_persist = self.system.native_persist_resources().ok_or_else(|| {
            atipicial_error::CoreError::invalid_operation(
    ...
    pub(crate) fn persist_block_sequence_with_resources(
        ...
        match crate::native_persist::stage_block_natives_with_resources(
            ...
            Err(err) => {
                error!(target: "atipicial", %err, "block persistence pipeline failed");
                Err(err.to_string())
            }
        }
    }
