...
/// at all: it reads the body and parses it as JSON regardless. atipicial-go's RPC
/// client relies on that leniency and sends no `Content-Type` whatsoever, so the
/// strict check makes every atipicial-go client fail against this node with
...
fn unauthorized_response() -> jsonrpsee::server::HttpResponse {
    static_response(
        ...
        &[
            ...
            ("www-authenticate", "Basic realm=\"atipicial-rpc\""),
        ],
    )
}
