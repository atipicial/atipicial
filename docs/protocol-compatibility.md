...
This node is a from-scratch Rust reimplementation of the Atipicial N3 protocol. It
targets **byte-for-byte compatibility** with the official C# reference node
(Atipicial v3.10.1). Phase 1 establishes a reproducible, hardfork-aware execution
...
```mermaid
flowchart LR
    A[C# Atipicial node v3.10.1] -- "target: same wire bytes" --> N[atipicial-rs node]
...
The v3.10.1 compatibility target is based on the official `atipicial-project/atipicial`
[`v3.10.1`](https://github.com/atipicial-project/atipicial/releases/tag/v3.10.1) release,
published on 2026-07-07 and pointing at commit
`d10e9ceecdabe3fcff719ee68ea5b76ba7e62c3d`. The C# release depends on
`atipicial-project/atipicial-vm` [`v3.10.1`](https://github.com/atipicial-project/atipicial-vm/releases/tag/v3.10.1),
commit `004cd6070a940405818d9357638277dd44407e2e`. The audited upstream
ranges are `atipicial` `v3.10.0...v3.10.1` and `atipicial-vm` `v3.10.0...v3.10.1`:

| Upstream commit / PR | Consensus or wire effect | atipicial-rs coverage |
|---|---|---|
| `df402675` / #4562, `d10e9cee` / #4575 | Release metadata: core package version `3.10.1` and final `Atipicial.VM` `3.10.1`. | `README.md`, protocol docs, release scripts, and protocol-target tests identify Atipicial v3.10.1 as the active C# parity target. |
| `9f4795ab` / #4571 | Adds `HF_Huyao` after `HF_Gorgon`. Empty `Hardforks: {}` now backfills every known hardfork through Huyao at height 0. | `atipicial-primitives::Hardfork::HfHuyao`, `atipicial-config::HardforkSchedule`, and config tests define Huyao and the C# empty-object backfill rule. Built-in MainNet/TestNet presets stay explicit operational schedules unless a loaded network config schedules later forks. |
| `f5ae5e82` / #4565 | Centralizes `ApplicationEngine.AddFee(gas, applyFactor)` so datoshi-vs-picoGAS conversion happens once, negative fees fault before whitelist bypass, and overflow cannot leave partial fee-consumed side effects. | `atipicial-execution` keeps `add_fee_datoshi`, `add_fee_pico`, `add_cpu_fee`, and native-method combined charging separate; fee tests cover negative-fee ordering, storage-fee units, and no-side-effect overflow. |
| `e66e4dfc` / #4563 | `StdLib.Itoa` formats integers with invariant culture. | `atipicial-native-contracts::StdLib` uses deterministic base-10/base-16 formatting; tests pin C# base-10 and two's-complement hex output independently of host locale. |
| `6b1c90c6` / #4566 plus `atipicial-vm` `7a8018e` / #581 and `004cd60` / #587 | Refreshes AtipicialVM to v3.10.1 reference-counter behavior. | `atipicial-vm` matches the recursive C# `ReferenceCounter`, including `CLEARITEMS` cycle and underflow behavior; runtime reference-counter tests pin those paths. |
| `55c14029` / #4569 | From `HF_Gorgon`, committee voter-reward refreshes use live candidate votes rather than stale committee-cache votes. | `atipicial-native-contracts::AtipicialCoin` reads live candidate votes for refresh-time voter rewards, including the empty-block fast-forward path; tests cover post-Gorgon normal and fast-forward rewards. |
| `abbc3a25` / #4570 | Notary-sponsored transactions reserve and persist fees against the secondary signer's Notary deposit; missing or overdrawn deposits fault instead of spending unrelated Notary GAS balance. | `atipicial-mempool` tracks v3.10.1 payer tuples `(Notary, Signers[1])`, verification reads Notary deposit balances, and `atipicial-native-contracts::Notary` faults on missing or overdrawn deposits during persist. |
| `7f8454f4` / #4572 | Hardening: invalid `ExtensiblePayload` relay results are returned to the sender but not published to the event stream; witnesses must hash to `Sender`. | `atipicial-payloads::ExtensiblePayload` rejects mismatched witness script hashes, `atipicial-blockchain` validates before cache/relay insertion, and failed extensible relay results are suppressed from runtime events. |
| `7bb91ff5` / #4574 | `StorageKey.ToString()` no longer indexes empty keys. | `atipicial-storage::StorageKey` formats empty keys as `StorageKey{Id=...}` and non-empty keys as `StorageKey{Id=...,Key=...}`. |

## Native Contracts

The node implements the standard Atipicial N3 native contracts. Each has a fixed, negative contract ID and a deterministic script hash derived the same way as in C#. IDs and names below are taken directly from the source (`atipicial-native-contracts/src`).
...
| AtipicialCoin | -5 | Governance token (AEP-17): voting, candidate registration, committee, GAS distribution |
| AtipicialDollar | -6 | Utility token (AEP-17): balances, transfers, fee burn/mint |
...
Atipicial N3 ships protocol upgrades as named hardforks (named after mythological
...
`HF_Gorgon` and `HF_Huyao`. The built-in atipicial-rs MainNet/TestNet presets instead
...
| HF_Cockatrice | 2 | Enables `CryptoLib` Keccak256 and adds hardfork-gated `AtipicialCoin` methods |
| HF_Domovoi | 3 | Adjusts executing-contract resolution in contract calls |
| HF_Echidna | 4 | Broad upgrade: additional `CryptoLib`/`StdLib`/`AtipicialCoin`/`PolicyContract` methods, Notary-related policy changes |
...
| HF_Huyao | 7 | Atipicial v3.10.1 protocol refinements; active when the loaded hardfork schedule enables it |
...
The built-in schedules are sourced from `atipicial-project/atipicial-node` tag `v3.10.1`,
commit `7313f8087724e1de4caa88edd2ada58c1fe54abc`. The `atipicial` and
`atipicial-vm` commits in the release-delta audit remain the semantic authorities.
...
| Area | Support |
|---|---|
| Consensus | dBFT 2.0 (`atipicial-consensus`): prepare/commit, view change, recovery, primary selection |
| Virtual machine | AtipicialVM with hardfork-gated jump table and gas accounting (`atipicial-vm`, `atipicial-execution`) |
| State | Merkle Patricia Trie state root, state store, atomic block-commit pipeline (`atipicial-state-service`) |
| Native contracts | The 11 standard native contracts listed above |
| AEP-17 | Fungible tokens (ATC, GAS) with transfer and `onNEP17Payment` callbacks |
| AEP-11 | Non-fungible token interface support |
| AEP-6 | Wallet format with BIP-32/BIP-39 key derivation (`atipicial-wallets`) |
| Oracle | HTTPS and AtipicialFs oracle request fulfilment (`atipicial-oracle-service`) |
| P2P | Atipicial N3 TCP wire protocol: version/verack handshake, inv/getdata, blocks, headers, addr, mempool relay (`atipicial-network`) |
| RPC | jsonrpsee JSON-RPC server and client (~55 methods); see the RPC reference |

```mermaid
flowchart TB
    P2P[P2P network] --> BC[Blockchain service]
    BC --> VM[AtipicialVM execution]
...
The node uses Atipicial N3's cryptographic scheme, built on mature, widely-used Rust crates rather than hand-rolled primitives (`atipicial-crypto`):
