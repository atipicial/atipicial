...
#   --data-dir <path>       atipicial-rs data root (default ./data)
...
#   ATC_CHECKPOINT_INTERVAL, ATC_CHECKPOINT_MAX, ATC_RPC_URL,
#   ATC_DATA_DIR, ATC_CHECKPOINT_ROOT
#
...
INTERVAL_BLOCKS="${ATC_CHECKPOINT_INTERVAL:-50000}"
MAX_CHECKPOINTS="${ATC_CHECKPOINT_MAX:-10}"
RPC_URL="${ATC_RPC_URL:-http://localhost:10332/}"
DATA_DIR="${ATC_DATA_DIR:-./data}"
CHECKPOINT_ROOT="${ATC_CHECKPOINT_ROOT:-}"
CHAIN_DB_OVERRIDE="${ATC_CHAIN_DB:-}"
ONCE=0
HEIGHT_OVERRIDE=""
CHAIN_ONLY=0
RESTORE_VERIFIED=0
...
take_snapshot() {
...

  {
    ...
    if [[ "$CHAIN_ONLY" -eq 0 ]]; then
      echo "stateroot_db=${CHAIN_DB}:atipicial_state_service"
