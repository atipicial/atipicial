...
fn state_service_metrics_from_parts(
    ...
    stages: &[atipicial_state_service::metrics::StateRootApplyStageStats],
    counts: &[atipicial_state_service::metrics::StateRootApplyCountStats],
    native_hooks: &[atipicial_runtime::sync_metrics::NativeContractHookStats],
    native_tx_stages: &[atipicial_runtime::sync_metrics::NativePersistTxStageStats],
    atipicialtoken_onpersist: &[atipicial_runtime::sync_metrics::AtipicialCoinOnPersistStageStats],
    atipicialtoken_committee: &[atipicial_runtime::sync_metrics::AtipicialCoinCommitteeComputeStageStats],
    atipicialtoken_candidate_counts: &[atipicial_runtime::sync_metrics::AtipicialCoinCommitteeCandidateCountStats],
    ...
    let native_tx_hot = native_tx_stages
        ...
        .filter(|stat| stat.avg_us > 0)
        .max_by_key(|stat| stat.avg_us);
    let atipicialtoken_onpersist_hot = atipicialtoken_onpersist
        .iter()
        .filter(|stat| stat.avg_us > 0)
        .max_by_key(|stat| stat.avg_us);
    let atipicialtoken_committee_hot = atipicialtoken_committee
        .iter()
        .filter(|stat| stat.avg_us > 0)
        .max_by_key(|stat| stat.avg_us);
    let atipicialtoken_candidate_hot = atipicialtoken_candidate_counts
        .iter()
    ...
    StateServiceMptImportMetrics::from_direct_hot_snapshot(
        sync,
        apply,
        atipicial_state_service::metrics::StateRootApplyHotStats {
        ...
        atipicialtoken_onpersist_hot.copied(),
        atipicialtoken_committee_hot.copied(),
        atipicialtoken_candidate_hot.copied(),
    )
}
...
fn native_tx_stage_import_metrics_project_split_load_execute_stages() {
    let stages = vec![
        atipicial_runtime::sync_metrics::NativePersistTxStageStats {
            stage: "load_execute",
            calls: 5,
            total_us: 2_100,
            avg_us: 420,
...
fn state_service_import_metrics_projects_hot_stage_fields() {
    ...
    let apply = atipicial_state_service::StateRootApplyStats {
        attempts: 12,
        failures: 1,
        latest_height: 42,
        avg_total_us: 9_000,
    ...
    let stages = vec![
        atipicial_state_service::metrics::StateRootApplyStageStats {
    ...
    let counts = vec![
        atipicial_state_service::metrics::StateRootApplyCountStats {
    ...
    let native_hooks = vec![
        atipicial_runtime::sync_metrics::NativeContractHookStats {
            trigger: "onpersist",
            contract_id: -5,
            contract: "AtipicialCoin",
            calls: 10,
            avg_us: 1_200,
        },
        atipicial_runtime::sync_metrics::NativeContractHookStats {
    ...
    let atipicialtoken_onpersist = vec![
        atipicial_runtime::sync_metrics::AtipicialCoinOnPersistStageStats {
    ...
    let atipicialtoken_committee = vec![
    ...
    let atipicialtoken_candidate_counts = vec![
    ...
    let metrics = state_service_metrics_from_parts(
        ...
        &native_hooks,
        &native_tx_stages,
        &atipicialtoken_onpersist,
        &atipicialtoken_committee,
        &atipicialtoken_candidate_counts,
    );
    ...
    assert_eq!(metrics.native_contract_hook_hot_contract, "AtipicialDollar");
    ...
    assert_eq!(metrics.atipicialtoken_onpersist_hot_stage, "compute_committee");
    assert_eq!(metrics.atipicialtoken_onpersist_hot_stage_avg_us, 3_300);
    assert_eq!(
        metrics.atipicialtoken_committee_compute_hot_stage,
        "candidate_state_decode"
    );
    assert_eq!(metrics.atipicialtoken_committee_compute_hot_stage_avg_us, 2_100);
    assert_eq!(
        metrics.atipicialtoken_committee_candidate_hot_kind,
        "eligible_candidates"
    );
    assert_eq!(metrics.atipicialtoken_committee_candidate_hot_avg, 42);
}
...
fn state_service_import_metrics_projects_direct_hot_snapshot_fields() {
    ...
    let apply_hot = atipicial_state_service::metrics::StateRootApplyHotStats {
    ...
    let native_hook_hot = Some(atipicial_runtime::sync_metrics::NativeContractHookStats {
        trigger: "onpersist",
        contract_id: -6,
        contract: "AtipicialDollar",
        calls: 10,
    ...
    let native_tx_hot = Some(atipicial_runtime::sync_metrics::NativePersistTxStageStats {
        stage: "execute",
        calls: 10,
        total_us: 81_000,
        avg_us: 8_100,
    });
    let atipicialtoken_onpersist_hot = Some(atipicial_runtime::sync_metrics::AtipicialCoinOnPersistStageStats {
        stage: "compute_committee",
        calls: 10,
        avg_us: 3_300,
    });
    let atipicialtoken_committee_hot = Some(
        atipicial_runtime::sync_metrics::AtipicialCoinCommitteeComputeStageStats {
            stage: "candidate_state_decode",
            calls: 10,
            avg_us: 2_100,
    ...
    let atipicialtoken_candidate_hot = Some(
        atipicial_runtime::sync_metrics::AtipicialCoinCommitteeCandidateCountStats {
            kind: "eligible_candidates",
            samples: 10,
            total: 420,
    ...
    let metrics = StateServiceMptImportMetrics::from_direct_hot_snapshot(
        ...
        native_hook_hot,
        native_tx_hot,
        atipicialtoken_onpersist_hot,
        atipicialtoken_committee_hot,
        atipicialtoken_candidate_hot,
    );
...
fn state_service_window_metrics_use_exact_cumulative_deltas() {
    let before = StateServiceMptCumulativeMetrics::from_stats(
        atipicial_state_service::StateRootApplyStats {
            attempts: 100,
            failures: 2,
            latest_height: 99,
            avg_total_us: 999,
        ...
        vec![atipicial_state_service::metrics::StateRootApplyStageStats {
            stage: "backing_commit",
            calls: 100,
            total_us: 1_000,
            avg_us: 999,
...
fn mdbx_commit_window_metrics_use_exact_cumulative_deltas() {
    let before = MdbxCommitCumulativeMetrics::from_stats(
        atipicial_storage::mdbx::MdbxCommitStats {
            attempts: 4,
            failures: 1,
            committed_transactions: 3,
        },
        vec![atipicial_storage::mdbx::MdbxCommitStageStats {
            stage: "cursor_write",
            calls: 6,
            total_us: 2_000,
            avg_us: 999,
        }],
        vec![atipicial_storage::mdbx::MdbxCommitCountStats {
            kind: "value_bytes",
            samples: 4,
            total: 8_000,
            avg: 999,
