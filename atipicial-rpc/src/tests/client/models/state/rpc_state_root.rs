...
fn rpc_state_root_parses_with_witness() {
    ...
    json.insert(
        "version".to_string(),
        atipicial_serialization::json::JToken::Number(0f64),
    );
    json.insert(
        "index".to_string(),
        atipicial_serialization::json::JToken::Number(1f64),
    );
    json.insert(
        "roothash".to_string(),
        atipicial_serialization::json::JToken::String(UInt256::zero().to_string()),
    );

    let mut witness_obj = JObject::new();
    witness_obj.insert(
        "invocation".to_string(),
        atipicial_serialization::json::JToken::String(general_purpose::STANDARD.encode(b"i")),
    );
    witness_obj.insert(
        "verification".to_string(),
        atipicial_serialization::json::JToken::String(general_purpose::STANDARD.encode(b"v")),
    );
    json.insert(
        "witnesses".to_string(),
        atipicial_serialization::json::JToken::Array(JArray::from(vec![
            atipicial_serialization::json::JToken::Object(witness_obj),
        ])),
    );
