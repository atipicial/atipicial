...
//!
//! Atipicial requires a 64-byte raw `r‖s` secp256r1 signature.  Post-processing:
//!
...
//! 3. `Signature::normalize_s()` → canonical low-s (Atipicial C# parity, step is
//!    cheap and provider-independent).
//! 4. `sig.to_bytes()` → 64-byte `r‖s`.

use crate::config::{HsmConfig, SigFormat, profile};
use crate::error::{HsmError, HsmResult};
use atipicial_consensus::ConsensusSigner;
use atipicial_consensus::error::ConsensusError;
use atipicial_crypto::{Crypto, Secp256r1Crypto};
...
pub struct Pkcs11Signer {
    ...
    public_key: [u8; 33],
    /// Atipicial script hash (`UInt160::from_script(single_sig_redeem_script(pubkey))`).
    script_hash: UInt160,
...
impl Pkcs11Signer {
    ...
    ///    33-byte secp256r1 pubkey, compute the Atipicial `UInt160` script hash.
    ///
    ...
    pub fn connect(cfg: &HsmConfig) -> HsmResult<Self> {
        ...
        let worker = thread::Builder::new()
            .name("atipicial-hsm-pkcs11-worker".to_string())
            .spawn(move || {
                // ── Init ──────────────────────────────────────────────────────
                ...
                info!(
                    ...
                    "atipicial-hsm: PKCS#11 worker ready"
                );
                for cmd in cmd_rx {
                    match cmd {
                ...
                debug!("atipicial-hsm: PKCS#11 worker shutting down");
            })
    ...
    /// Returns the Atipicial script hash derived from the public key.
    #[must_use]
    ...
    fn worker_init(
        ...
        if let Some(ref params) = ec_params_bytes {
            ...
            warn!("atipicial-hsm: CKA_EC_PARAMS not returned; skipping curve check");
        }
        ...
        // 8. Compute Atipicial UInt160 script hash from the compressed public key.
        // Atipicial single-sig redeem script: `PUSHDATA1(33) || pubkey || SYSCALL CheckSig`
        ...
        info!(
            ...
            "atipicial-hsm: key located on HSM"
        );
...
impl ConsensusSigner for Pkcs11Signer {
    ...
    async fn sign(&self, data: &[u8], script_hash: &UInt160) -> Result<Vec<u8>, ConsensusError> {
        ...
        tokio::task::spawn_blocking(move || {
            // Atipicial's consensus signs SHA-256(data); CKM_ECDSA expects the raw digest.
...
fn finalize_signature(raw: &[u8], format: SigFormat) -> HsmResult<Vec<u8>> {
    // Shared canonicalization (DER-decode if needed + low-s) lives in atipicial-crypto
...
/// Build the Atipicial single-sig verification/redeem script for a compressed pubkey.
///
...
/// `NextConsensus`).
///
/// Delegates to `atipicial_vm::script_builder::redeem_script::RedeemScript::signature_redeem_script`
/// to guarantee opcode values never drift from the canonical AtipicialVM definition.
fn signature_redeem_script(compressed_pubkey: &[u8; 33]) -> Vec<u8> {
    atipicial_vm::script_builder::redeem_script::RedeemScript::signature_redeem_script(compressed_pubkey)
}
