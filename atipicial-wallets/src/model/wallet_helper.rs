...
//! and the native contracts; it lives in `atipicial-core::wallets_runtime`
//! for now. The pure-crypto address helpers extracted here are
//! what `KeyPair`, `WalletAccount`, and `Aep6Wallet` actually need.

use atipicial_error::{CoreError, CoreResult};
use atipicial_primitives::UInt160;

use atipicial_primitives::base58_check::{AddressDecodeError, Base58Check, Base58CheckDecodeError};
...
impl WalletAddress {
    ...
    pub fn to_script_hash(address: &str, version: u8) -> CoreResult<UInt160> {
        let script_hash = Base58Check::decode_address_payload(address, version)
