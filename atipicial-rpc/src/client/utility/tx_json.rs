...
use atipicial_io::serializable::Serializable;
use atipicial_io::serializable::helper::SerializeHelper;
use atipicial_payloads::{Block, BlockHeader, Signer, Transaction};
...
pub fn block_to_json(block: &Block, protocol_settings: &ProtocolSettings) -> JObject {
    ...
    let block_size = header.size()
        ...
        + block
            .transactions
            .iter()
            .map(atipicial_io::Serializable::size)
            .sum::<usize>();
...
fn signer_to_json(signer: &atipicial_payloads::Signer, _protocol_settings: &ProtocolSettings) -> JObject {
    let mut json = JObject::new();
    json.insert(
        "account".to_string(),
...
fn attribute_to_json(attr: &atipicial_payloads::TransactionAttribute) -> JObject {
    let mut json = JObject::new();
    json.insert(
        "type".to_string(),
    ...
    use atipicial_payloads::TransactionAttribute as TA;
    match attr {
        TA::HighPriority => {}
...
fn rule_to_json(rule: &atipicial_payloads::WitnessRule) -> JObject {
    let mut json = JObject::new();
    json.insert(
        "action".to_string(),
...
fn condition_to_json(condition: &atipicial_payloads::WitnessCondition) -> JObject {
    use atipicial_payloads::WitnessCondition as WC;
