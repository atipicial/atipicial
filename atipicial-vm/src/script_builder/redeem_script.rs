//! Atipicial N3 redeem-script primitives.
//!
...
//! counterpart of the redeem-script helpers in C# `Atipicial.SmartContract.Contract`
//! / `Atipicial.SmartContract.Helper`.
//!
//! This module is layered on `atipicial-crypto` (ECPoint), [`ScriptBuilder`] and
//! `atipicial-vm` (OpCode / interop hashing), and sits *below* atipicial-core so the
...
//! therefore addresses, so the encoding must stay byte-identical to C# Atipicial
...
atipicial_error::impl_error_from_struct!(atipicial_error::CoreError, RedeemScriptError => InvalidOperation);

/// Atipicial N3 redeem-script primitives (construction and pattern recognition for
...
impl RedeemScript {
    ...
    /// keys)` when the script matches the canonical Atipicial multi-sig format. The
...
/// Computes a syscall hash via atipicial-vm interop hashing.
