#!/bin/bash
# Continuous monitoring script for atipicial-rs node

SERVER="root@89.167.120.122"
SSH_KEY="$HOME/.ssh/id_ed25519"

echo "=== Atipicial-RS Continuous Monitor ==="
...
while true; do
    ...
    # 1. Process status
    PROCESS=$(ssh -i "$SSH_KEY" "$SERVER" "ps aux | awk '/atipicial-node/ && !/awk/' | wc -l")
    if [ "$PROCESS" -gt 0 ]; then
        echo "✓ Process: Running"
        MEM=$(ssh -i "$SSH_KEY" "$SERVER" "ps aux | awk '/atipicial-node/ && !/awk/ {print \$6}' | head -1")
    ...
    RECENT_BLOCKS=$(ssh -i "$SSH_KEY" "$SERVER" "tail -100 /root/atipicial-node-optimized.log | grep 'inventory completed' | grep 'has_block=true' | wc -l")
    echo "  Recent blocks received: $RECENT_BLOCKS"

    # 5. Active tasks
    ACTIVE_TASKS=$(ssh -i "$SSH_KEY" "$SERVER" "tail -50 /root/atipicial-node-optimized.log | grep 'requesting tasks' | tail -1 | grep -o 'index_tasks=[0-9]*' | cut -d= -f2")
