...
//! by `atipicial_config::ChainSpecProvider`, its canonical owner.
...
pub trait StoreProvider: Send + Sync + Debug + 'static {
    ...
    type Store: atipicial_storage::persistence::store::Store + 'static;

    /// Returns the storage backend.
    fn store(&self) -> Arc<Self::Store>;

    /// Returns a snapshot of the current state.
    fn store_cache(&self) -> atipicial_storage::persistence::store_cache::StoreCache<Self::Store>;
}
