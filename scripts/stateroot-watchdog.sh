#!/bin/bash
# Watchdog script: monitors atipicial-node sync progress, restarts if stalled
# Each restart processes a burst of blocks before the P2P pipeline stalls

ROOT_DIR="${ROOT_DIR:-$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)}"
NODE_BIN="${NODE_BIN:-$ROOT_DIR/target/release/atipicial-node}"
NODE_CONFIG="${NODE_CONFIG:-$ROOT_DIR/atipicial_mainnet_node.toml}"
NODE_LOG="${NODE_LOG:-$ROOT_DIR/logs/atipicial-node-stateroot.log}"
LOCAL_RPC_URL="${LOCAL_RPC_URL:-http://127.0.0.1:10332}"
LOCAL_RPC_USER="${LOCAL_RPC_USER:-${ATC_RPC_USER:-}}"
LOCAL_RPC_PASS="${LOCAL_RPC_PASS:-${ATC_RPC_PASS:-}}"
