...
use atipicial_storage::persistence::SeekDirection;
use atipicial_storage::{DataCache, StorageKey};
...
/// `AtipicialCoin` native id (a fixed protocol constant, C# id -5).
pub(crate) const ATC_TOKEN_ID: i32 = -5;
/// C# `AtipicialCoin.Prefix_Committee` (14): the cached-committee record --
/// the first key genesis initialization writes.
pub(crate) const ATC_PREFIX_COMMITTEE_KEY: u8 = 14;
...
/// has been persisted. The point probe checks the `AtipicialCoin` committee
...
pub fn chain_state_initialized<B: atipicial_storage::CacheRead>(snapshot: &DataCache<B>) -> bool {
    if snapshot
        .get(&StorageKey::new(
            ATC_TOKEN_ID,
            vec![ATC_PREFIX_COMMITTEE_KEY],
        ))
        .is_some()
    {
        return true;
...
/// C# `AtipicialSystem.CreateGenesisBlock(settings)`: index 0, zero
...
///
/// Requiring the complete [`AtipicialChainSpec`] keeps genesis identity authoritative:
...
pub fn genesis_block(chain_spec: &AtipicialChainSpec) -> CoreResult<Block> {
...
pub(crate) fn bft_address(pubkeys: &[atipicial_crypto::ECPoint]) -> CoreResult<UInt160> {
