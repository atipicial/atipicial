...
use atipicial_runtime::sync_metrics::{
    self, AtipicialCoinCommitteeCandidateCount, AtipicialCoinCommitteeComputeStage,
};
...
impl CandidateScanCounts {
    pub(super) fn record(&self, top_candidates: u64) {
        sync_metrics::record_atipicial_coin_committee_candidate_count(
...
impl AtipicialCoin {
    /// C# `AtipicialCoin.CheckCandidate`: when a candidate is unregistered and has no
    /// remaining votes, delete its candidate + voter-reward entries.
    pub(in crate::atipicial_coin) fn check_candidate<B: atipicial_storage::CacheRead>(
        &self,
    ...
    /// C# `AtipicialCoin.ComputeCommitteeMembers(snapshot, settings)`: the next committee
    ...
    pub(in crate::atipicial_coin) fn compute_committee_members<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
        ...
        sync_metrics::record_atipicial_coin_committee_compute_stage(
            AtipicialCoinCommitteeComputeStage::ReadVotersCount,
            atipicial_runtime::time::elapsed_us(stage_start.elapsed()),
        );
        ...
        let turnout_reached = voters_count * 5 >= BigInt::from(ATC_TOTAL_AMOUNT);
        if !turnout_reached {
            return self
    ...
    fn standby_committee_with_registered_votes<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
    ...
    fn top_registered_candidates<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
        limit: usize,
        ...
        sync_metrics::record_atipicial_coin_committee_compute_stage(
            ...
            atipicial_runtime::time::elapsed_us(total_start.elapsed()),
        );
    ...
    pub(in crate::atipicial_coin) fn decode_candidate_state(
        value: &[u8],
    ) -> CoreResult<(bool, BigInt)> {
    ...
    pub(in crate::atipicial_coin) fn decode_canonical_candidate_state(
        value: &[u8],
    ) -> CoreResult<Option<(bool, BigInt)>> {
        const STRUCT: u8 = atipicial_vm::ATCVM_STACK_ITEM_TYPE_STRUCT;
        const BOOLEAN: u8 = atipicial_vm::ATCVM_STACK_ITEM_TYPE_BOOLEAN;
        const INTEGER: u8 = atipicial_vm::ATCVM_STACK_ITEM_TYPE_INTEGER;
        ...
        let Some((vote_len, vote_len_width)) =
            atipicial_io::var_int::VarInt::read_var_int_prefix(tail)
        else {
            return Ok(None);
        };
        if vote_len > 32 {
    ...
    pub(in crate::atipicial_coin) fn encode_candidate_state(
        registered: bool,
        votes: &BigInt,
    ) -> CoreResult<Vec<u8>> {
    ...
    pub(in crate::atipicial_coin) fn registered_candidate_entries<
    ...
    pub(in crate::atipicial_coin) fn read_registered_candidates<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
    ) -> CoreResult<Vec<(ECPoint, BigInt)>> {
        Ok(self
    ...
    /// C# `RegisterInternal` (AtipicialCoin.cs:411-423), shared by `registerCandidate`
    /// and the Echidna `onAEP17Payment` GAS-payment path: requires a witness from
    ...
    pub(in crate::atipicial_coin) fn register_internal<
    ...
    >(
        ...
        let authorized = engine.check_witness_hash(&account).map_err(|e| {
            CoreError::invalid_operation(format!("AtipicialCoin::{method}: witness: {e}"))
        })?;
        if !authorized {
            return Ok(false);
        }
        ...
        engine
            ...
            .map_err(|e| {
                CoreError::invalid_operation(format!("AtipicialCoin::{method}: notify: {e}"))
            })?;
        Ok(true)
    }
    ...
    pub(in crate::atipicial_coin) fn candidate_vote<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
        pubkey: &ECPoint,
    ) -> CoreResult<BigInt> {
    ...
    pub(in crate::atipicial_coin) fn candidates_to_array_bytes(
...
pub(super) fn candidate_is_blocked<B: atipicial_storage::CacheRead>(
    snapshot: &DataCache<B>,
    pubkey: &ECPoint,
) -> bool {
