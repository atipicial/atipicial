//! ExecutionContextState - matches C# Atipicial.SmartContract.ExecutionContextState exactly
...
/// AtipicialVM context specialized with the application engine's host state.
pub type ApplicationExecutionContext<B = EmptyCacheBacking> =
    atipicial_vm::ExecutionContext<ExecutionContextState<B>>;

/// AtipicialVM engine specialized with the application engine's host state.
pub type ApplicationExecutionEngine<B = EmptyCacheBacking> =
    atipicial_vm::ExecutionEngine<ExecutionContextState<B>>;

/// AtipicialVM opcode table specialized with the application engine's host state.
pub type ApplicationJumpTable<B = EmptyCacheBacking> = atipicial_vm::JumpTable<ExecutionContextState<B>>;
