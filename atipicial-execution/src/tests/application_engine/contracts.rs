...
use atipicial_crypto::{ECCurve, ECPoint};
use atipicial_manifest::{
    ContractAbi, ContractGroup, ContractManifest, ContractMethodDescriptor,
    ContractParameterDefinition, ContractPermission, ManifestFeatures, AefFile, WildCardContainer,
};
use atipicial_payloads::{
    OracleResponse, Signer, Transaction, TransactionAttribute, VerifiableContainer,
};
use atipicial_primitives::{ContractParameterType, OracleResponseCode, WitnessScope};
...
impl NativeContractProvider for CachedContractProvider {
    ...
    fn contract_exists<B: atipicial_storage::CacheRead>(
        &self,
        _snapshot: &DataCache<B>,
        hash: &UInt160,
    ) -> CoreResult<bool> {
...
impl NativeContractProvider for SingleNativeProvider {
    ...
    fn oracle_request_details<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
        request_id: u64,
    ...
    P: NativeContractProvider + 'static,
{
    ...
    fn hash(&self) -> UInt160 {
        UInt160::from_bytes(&[0xCE; 20]).expect("atipicial token hash")
    }
    ...
    {
        Err(CoreError::invalid_operation(
            "test atipicial token is metadata-only",
        ))
    }
...
fn build_mock_contract(hash: UInt160) -> ContractState {
    ...
    let aef = AefFile::new("test".to_string(), script);
    ...
    ContractState::new(1, hash, aef, manifest)
}
...
fn committee_witness_uses_provider_captured_at_engine_creation() {
    ...
    let err = engine
        ...
        .expect_err("captured AtipicialCoin provider should be used");
    assert!(
...
fn build_returning_mock(
    ...
    ContractState::new(2, hash, aef, manifest)
}
...
fn returning_call_yields_result_and_native_calling_hash() {
    ...
    script.extend_from_slice(
        &atipicial_vm::interop_hash("System.Runtime.GetCallingScriptHash").to_le_bytes(),
    );
    ...
    B: atipicial_storage::CacheRead,
{
    ...
    {
        Ok(_) => Err(atipicial_vm::VmError::invalid_operation_msg(
            "boundary test: callee unexpectedly returned",
        )),
        Err(err) => Err(atipicial_vm::VmError::invalid_operation_msg(err.to_string())),
    }
}
    ...
    B: atipicial_storage::CacheRead,
{
    ...
    app.process_pending_native_calls()
        .map_err(|err| atipicial_vm::VmError::invalid_operation_msg(err.to_string()))
}
...
fn returning_call_exception_cannot_be_caught_below_native_frame() {
    ...
    script.extend_from_slice(&atipicial_vm::interop_hash(BOUNDARY_TEST_SYSCALL).to_le_bytes());
    script.extend_from_slice(&[
        OpCode::ENDTRY.byte(),
        4,
        OpCode::PUSH2.byte(),
...
/// The queued native-call path used by AEP-17 `onAEP17Payment` must enforce
...
fn queued_native_call_exception_cannot_be_caught_below_native_frame() {
    ...
    // ip12: PUSH1; RET
    let mut script = vec![OpCode::TRY.byte(), 10, 0, OpCode::SYSCALL.byte()];
    script.extend_from_slice(&atipicial_vm::interop_hash(QUEUED_BOUNDARY_TEST_SYSCALL).to_le_bytes());
    script.extend_from_slice(&[
        OpCode::ENDTRY.byte(),
        4,
        OpCode::PUSH2.byte(),
