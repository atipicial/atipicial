...
pub(crate) use aep::{
    AepBalanceFieldRefs, AepTransferFieldRefs, balance_list_to_json, insert_nep_balance_fields,
...
use atipicial_native_contracts::{NativeRegistry, StandardNativeProvider};
use atipicial_payloads::{Block, BlockHeader, Transaction, Witness};
...
impl RpcUtility {
    ...
    pub fn get_script_hash(
        ...
        match account.len() {
            ...
            66 => {
                // Public key - Atipicial N3 uses secp256r1 (NIST P-256) curve
    ...
    /// Converts an RPC stack item using AtipicialVM integer rules.
    pub fn rpc_stack_item_to_bigint(value: &RpcStackItem) -> CoreResult<BigInt> {
        stack::rpc_stack_item_to_bigint(value)
    }

    /// Converts an RPC stack item using AtipicialVM truthiness rules.
    #[must_use]
    ...
    pub fn rule_from_json(
        json: &JObject,
        protocol_settings: &ProtocolSettings,
    ) -> CoreResult<atipicial_payloads::WitnessRule> {
