...
/// C# `FungibleToken.Transfer` is inherited by both ATC and GAS and is
/// annotated with `CpuFee = 1 << 17`, `StorageFee = 50`, and
/// `RequiredCallFlags = States | AllowCall | AllowNotify`
/// (`atipicial_csharp/.../Native/FungibleToken.cs`). The storage fee is charged by
...
fn fungible_token_transfer_fees_match_csharp_attribute() {
    ...
    assert_transfer_metadata("AtipicialCoin", &AtipicialCoin::new(), required);
    assert_transfer_metadata("AtipicialDollar", &AtipicialDollar::new(), required);
}
