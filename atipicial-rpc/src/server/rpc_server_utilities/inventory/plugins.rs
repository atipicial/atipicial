...
impl RpcServer {
    /// List built-in service plugins for Atipicial RPC compatibility.
    pub fn list_plugins(&self) -> Value {
        ...
        if self.system().tokens_tracker_service().is_some() {
            let name = match compat {
                ListPluginsCompat::Fixture => "RpcAep17Tracker",
        ...
        if self.system().indexer_service().is_some() {
            plugins.push(plugin_entry("AtipicialIndexer", &persistence_interfaces));
        }
...
fn list_plugins_compat() -> ListPluginsCompat {
    let Ok(raw) = std::env::var("ATC_LISTPLUGINS_COMPAT") else {
...
fn plugin_version(compat: ListPluginsCompat) -> String {
    if let Ok(value) = std::env::var("ATC_PLUGIN_VERSION") {
