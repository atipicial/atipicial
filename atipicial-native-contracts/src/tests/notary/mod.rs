//! # atipicial-native-contracts::tests::notary
//!
...
use atipicial_vm::{ExecutionEngineLimits, Interoperable, StackItem};
...
fn native_contract_surface() {
    ...
    // withdraw: not safe, CallFlags.All (re-entrant ATD transfer),
    ...
    // onAEP17Payment: not safe, States, (Hash160, Integer, Any) -> Void.
    let on_pay = c
        .methods()
        .iter()
        .find(|m| m.name == crate::AEP17_PAYMENT_METHOD)
        .unwrap();
    assert!(!on_pay.safe);
...
#[test]
fn compute_deposit_matches_csharp_onaep17_rules() {
...
#[test]
fn parse_onaep17_data_handles_null_and_explicit_to() {
