...
use atipicial_serialization::json::{JPathToken, JToken, OrderedDictionary};
...
fn test_json_path_simple_properties() {
    let json = make_object(vec![
        ("name".to_string(), Some(JToken::String("Atipicial".to_string()))),
    ...
    assert_eq!(results[0], &JToken::String("Atipicial".to_string()));
...
// ---------------------------------------------------------------------------
// Differential pinning tests against C# Atipicial.Json UT_JPath.cs.
//
// The Oracle filter (an on-chain, untrusted JSONPath string) is evaluated here,
// so any divergence from C# Atipicial.Json's JPathToken is a CONSENSUS FORK. These
// vectors are transcribed verbatim from
// atipicial_csharp/tests/Atipicial.Json.UnitTests/UT_JPath.cs and assert byte-identical
