...
"#,
    ...
    assert_eq!(
        ...
        vec!["AEP-17".to_string(), "AEP-11".to_string()]
    );
}
...
fn shipped_local_config_selects_canonical_testnet_identity() {
    ...
    assert_eq!(
        ...
        atipicial_config::NetworkType::TestNet
    );
...
/// Service-provider presets prepare the full public query stack a AtipicialFura-style
/// operator needs while retaining the explicit runtime gate for state proofs.
#[test]
fn shipped_service_provider_configs_prepare_atipicialfura_surface_with_explicit_stateroot_gate() {
    let workspace = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .expect("atipicial-node has a workspace parent");
    let cases = [
