...
use crate::{
    AtipicialCoin, AtipicialDollar, ContractManagement, CryptoLib, LedgerContract, Notary,
    OracleContract, PolicyContract, RoleManagement, StdLib, Treasury,
};
use atipicial_config::Hardfork;
use atipicial_execution::NativeContract;
...
fn standard_contract_handles_share_a_uniform_shape() {
    ...
    assert_handle::<AtipicialCoin>();
    assert_handle::<AtipicialDollar>();
...
fn standard_contract_handles_are_const_constructible() {
    ...
    const ATC_TOKEN: AtipicialCoin = AtipicialCoin::new();
    const GAS_TOKEN: AtipicialDollar = AtipicialDollar::new();
    ...
    assert_eq!(ATC_TOKEN.name(), "AtipicialCoin");
    assert_eq!(GAS_TOKEN.name(), "AtipicialDollar");
...
#[test]
fn standard_contract_activation_policy_matches_atipicial_n3_v3101() {
    ...
    for native in &contracts {
        match native.name() {
            ...
            name => assert_eq!(
                native.active_in(),
                None,
                "{name} should be genesis-active in Atipicial N3 v3.10.1"
            ),
        }
    }
    ...
    assert!(
        ...
        "AtipicialCoin should refresh at Echidna through method metadata"
    );
    assert_eq!(contract("AtipicialCoin").activations(), &[]);
    assert_eq!(
...
fn specs_keep_canonical_native_contract_order() {
    use crate::hashes::{
        ATC_TOKEN_HASH, CONTRACT_MANAGEMENT_HASH, CRYPTO_LIB_HASH, GAS_TOKEN_HASH,
        LEDGER_CONTRACT_HASH, NOTARY_HASH, ORACLE_CONTRACT_HASH, POLICY_CONTRACT_HASH,
    ...
    assert_eq!(
        ...
        [
            ...
            ("AtipicialCoin", -5, *ATC_TOKEN_HASH, None, &[][..], &[][..],),
            (
                "AtipicialDollar",
                -6,
                *GAS_TOKEN_HASH,
