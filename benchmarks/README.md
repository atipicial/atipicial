...
# Atipicial N3 cross-implementation benchmarks

A reproducible harness for comparing the performance of the three production
Atipicial N3 node implementations on **identical workloads, identical hardware**:

| Implementation | Language | Binary used here |
|----------------|----------|------------------|
| **atipicial-rs** (this repo) | Rust | `atipicial-node` |
| **atipicial-cli** | C# / .NET | `atipicial-cli` |
| **atipicial-go** | Go | `atipicial-go` |
...
1. **atipicial-rs** — build this repo: `cargo build --release -p atipicial-node`
   (binary at `target/release/atipicial-node`).
2. **atipicial-cli** — download a release from
   <https://github.com/atipicial-project/atipicial-node/releases> (or `atipicial-cli` from
   <https://github.com/atipicial-project/atipicial/releases> for current versions) and
   ensure `dotnet` is installed.
3. **atipicial-go** — download from <https://github.com/nspcc-dev/atipicial-go/releases>
   or `go install`.
...
benchmarks/scripts/sync-bench.sh \
    --name atipicial-rs \
    --cmd "target/release/atipicial-node --config config/mainnet.toml --import chain.acc" \
    --rpc http://127.0.0.1:10332 \
    --target-height 500000

benchmarks/scripts/sync-bench.sh --name atipicial-cli --cmd "..." --rpc ... --target-height 500000
benchmarks/scripts/sync-bench.sh --name atipicial-go  --cmd "..." --rpc ... --target-height 500000
...
benchmarks/scripts/rpc-bench.sh \
    ...
    atipicial-rs=http://127.0.0.1:10332 \
    atipicial-cli=http://127.0.0.1:20332 \
    atipicial-go=http://127.0.0.1:30332
```
...
- [ ] Storage engine noted (atipicial-rs: MDBX; atipicial-cli: LevelDB/RocksDB plugin;
      atipicial-go: LevelDB/BoltDB) — storage choice affects import speed, so record it.
