...
/// Decoded Atipicial address payload length: one version byte plus a 160-bit script hash.
...
/// Error returned when decoding a Atipicial address payload fails.
#[derive(Debug, Error, Clone, PartialEq, Eq)]
...
impl Base58Check {
    ...
    /// Encodes a script hash as a Atipicial address payload using the supplied version byte.
    #[must_use]
    ...
    /// Decodes a Atipicial address and returns the script hash bytes.
