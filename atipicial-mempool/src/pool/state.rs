...
use atipicial_native_contracts::Notary;
...
/// payer's Notary deposit instead of the Notary account's ATD balance.
...
pub(super) struct MemoryPoolInner {
    ...
    /// tuples charge the primary account's ATD balance; Notary-sponsored tuples
...
pub(super) fn conflict_target_hashes(tx: &Transaction) -> Vec<UInt256> {
    tx.attributes()
        ...
        .filter_map(|attr| match attr {
            atipicial_payloads::TransactionAttribute::Conflicts(c) => Some(c.hash),
            _ => None,
        })
        .collect()
}
...
pub(super) fn oracle_response_id(tx: &Transaction) -> Option<u64> {
    tx.attributes().iter().find_map(|attr| match attr {
        atipicial_payloads::TransactionAttribute::OracleResponse(resp) => Some(resp.id),
        _ => None,
    })
}
