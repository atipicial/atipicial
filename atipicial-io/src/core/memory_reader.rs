//! Borrowed-slice reader matching C# `Atipicial.IO.MemoryReader` protocol semantics.
//!
//! `std::io::Cursor<&[u8]>` is the right generic Rust tool for stream position
//! over memory. `MemoryReader` keeps a tiny Atipicial-specific facade because callers
//! need bounded Atipicial var-int/string reads, C#-compatible method names, zero-copy
...
/// Errors that can occur while reading Atipicial binary data.
#[derive(Debug, Error)]
pub enum IoError {
...
/// Result alias for IO operations within Atipicial.IO.
...
/// Memory reader matching C# `Atipicial.IO.MemoryReader`.
///
/// Prefer generic [`Read`] APIs for generic byte streams. Use this type at Atipicial
