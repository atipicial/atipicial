...
/// and just parses the body. atipicial-go's RPC client depends on that leniency and
/// sends no `Content-Type` at all, so without this normalisation every atipicial-go
/// client fails against a atipicial-rs node on its very first call
