...
use atipicial_manifest::{
    ...
    AefFile, WildCardContainer,
};
use atipicial_payloads::{Block, Header, OracleResponse, Signer, Transaction, TransactionAttribute};
use atipicial_primitives::{
...
fn build_verify_contract(hash: UInt160) -> ContractState {
    ...
    ContractState::new(7, hash, aef, manifest)
}
...
fn signed_standard_transaction(valid_signature: bool) -> (Transaction, Witness, Vec<u8>) {
    ...
    let sign_data = atipicial_payloads::get_sign_data_vec(&tx, settings.network)
...
#[test]
fn cache_aware_witness_helper_still_executes_full_atipicialvm_with_identical_fee() {
    ...
    .expect("ordinary AtipicialVM witness verification");
    ...
    .expect("cache-aware AtipicialVM witness verification");

    assert_eq!(
        cached, uncached,
        "cache must not change AtipicialVM fee accounting"
    );
...
#[test]
fn cache_aware_multisig_witness_matches_full_atipicialvm_fee_and_result() {
    ...
    .expect("ordinary multisig AtipicialVM witness verification");
    let cached = Helper::verify_witness_with_native_provider_and_signature_cache(
        &tx,
        &settings,
        &snapshot,
    ...
    .expect("cache-aware multisig AtipicialVM witness verification");
