...
use atipicial_io::serializable::helper;
use atipicial_io::{BinaryWriter, IoError, IoResult, MemoryReader, Serializable};
use atipicial_primitives::NodeCapabilityType;
...
/// Describes the advertised features of a Atipicial node.
...
impl NodeCapabilities {
    ...
    /// Serializes a capability vector using Atipicial's var-array encoding.
...
impl Serializable for NodeCapability {
    ...
    fn deserialize(reader: &mut MemoryReader) -> IoResult<Self> {
        ...
        let capability = match capability_type {
            ...
            NodeCapabilityType::DisableCompression => {
                // C# v3.10.x `NodeCapability.DeserializeFrom` does not map
                // this type back to `DisableCompressionCapability`; it falls
                // through to `UnknownCapability`, which reads a var-bytes
                // payload and is later filtered out of duplicate checks and
                // `VersionPayload.AllowCompression` calculation.
...
#[cfg(test)]
#[path = "../../tests/p2p_payloads/node_capability.rs"]
mod tests;
