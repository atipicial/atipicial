...
fn default_cache_dir_tracks_storage_path() {
    let config: NodeConfig = toml::from_str(
        r#"
[storage]
data_dir = "/var/lib/atipicial/mainnet"
"#,
    )
    .expect("config");
    ...
    assert_eq!(
        ...
        PathBuf::from("/var/lib/atipicial/mainnet/fast-sync")
    );
    assert_eq!(
...
fn memory_store_with_ledger_tip(tip: u32) -> Arc<MemoryStore> {
    ...
    let mut cache = atipicial_storage::persistence::StoreCache::new_from_store(Arc::clone(&store), false);
    let hash = atipicial_primitives::UInt256::from([tip as u8; 32]);
    let current = atipicial_native_contracts::LedgerContract::new()
        .serialize_hash_index_state(&hash, tip)
        .expect("serialize current ledger pointer");
    cache.data_cache().add(
        StorageKey::new(atipicial_native_contracts::LedgerContract::ID, vec![12]),
...
fn state_store_with_local_root(
    tip: u32,
) -> (Arc<atipicial_state_service::StateStore>, atipicial_primitives::UInt256) {
...
fn empty_block(index: u32) -> atipicial_payloads::Block {
...
fn stale_fast_sync_import_marker_blocks_retry() {
    ...
    let marker_path = temp.path().join(".atipicial-fast-sync-import-in-progress");
    std::fs::write(
        &marker_path,
...
fn empty_block_dominated_fast_sync_has_no_upper_bps_cap() {
    ...
    let import_tip = chain_acc::LocalLedgerTip {
        height: 99_999,
        hash: atipicial_primitives::UInt256::from([99; 32]),
    };
...
fn fast_sync_report_preserves_package_and_import_proof() {
    ...
    let import_tip = chain_acc::LocalLedgerTip {
        height: 100,
        hash: atipicial_primitives::UInt256::from([100; 32]),
    };
...
fn fast_sync_report_serializes_reference_verification_provenance() {
    ...
    let report = FastSyncReport::from_parts(
        ...
        Some(FastSyncReferenceReport {
            endpoint: "https://seed1.atipicial.com:10332".to_string(),
            block_height: 100,
            block_hash: import_tip.hash.to_string(),
            state_root_height: Some(100),
            state_root_hash: Some(atipicial_primitives::UInt256::from([7; 32]).to_string()),
        }),
    );
    let path = temp.path().join("proof.json");
    ...
    assert_eq!(
        ...
        "https://seed1.atipicial.com:10332"
    );
    ...
    assert_eq!(
        ...
        atipicial_primitives::UInt256::from([7; 32]).to_string()
    );
}
...
fn fast_sync_post_import_state_root_proof_accepts_matching_local_root() {
    ...
    let imported_tip = chain_acc::LocalLedgerTip {
        height: 100,
        hash: atipicial_primitives::UInt256::from([0xAB; 32]),
    };
...
async fn fast_sync_reference_state_root_proof_accepts_matching_upstream_root() {
    let root_hash = atipicial_primitives::UInt256::from([0x44; 32]);
...
async fn fast_sync_reference_state_root_proof_rejects_mismatched_upstream_root() {
    let local_root = LocalStateRootTip {
        index: 100,
        root_hash: atipicial_primitives::UInt256::from([0x44; 32]),
    };
    let upstream_root = atipicial_primitives::UInt256::from([0x55; 32]);
    let endpoint = serve_rpc_once(
        "getstateroot",
        serde_json::json!({
            "version": 0,
