...
//! remain in concrete implementation crates such as `atipicial-execution`,
//! `atipicial-blockchain`, and `atipicial-network`; this crate carries only the fields
...
//! Concrete `atipicial-network` code translates its richer internal events into
...
/// Result of executing a block via an `EngineApi` / `BlockExecutor`-style
/// service boundary.
///
...
/// `atipicial-execution` / `atipicial-blockchain`, while callers here receive the block
...
/// atipicial-rs it is the analogous blob returned by an `execute_block`-style
/// service call.
...
/// *execution* check (state transition). A `validate_block`-style call is the
/// cheap, consensus-shaped variant — anything beyond header / merkle belongs in
/// `execute_block`.
#[derive(Clone, Debug, Default, PartialEq, Eq, Serialize, Deserialize)]
...
/// types for the canonical `atipicial-network::wire` / `atipicial-payloads` representations.
