...
fn tx_admission_uses_ledger_provider_boundary() {
    ...
    assert!(
        ...
        "composition must adapt the routed provider to atipicial-mempool's canonical admission capability"
    );
    assert!(
...
fn test_constructor_uses_explicit_standard_components() {
    let node = Node::for_test(atipicial_config::AtipicialChainSpec::mainnet().expect("mainnet chain spec"));
