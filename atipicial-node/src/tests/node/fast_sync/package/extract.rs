...
fn command_preflight_reports_missing_binary_by_name() {
    let err = ensure_command_available("atipicial-rs-command-that-should-not-exist")
        .expect_err("missing extraction command must produce a deterministic error");

    assert!(
        err.to_string()
            .contains("required command `atipicial-rs-command-that-should-not-exist` is not available"),
...
fn cached_extracted_chain_acc_requires_matching_package_md5_marker() {
    ...
    std::fs::write(
        extract_dir.join(".atipicial-fast-sync-package-md5"),
