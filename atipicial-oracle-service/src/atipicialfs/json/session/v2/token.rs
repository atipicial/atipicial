// Rationale: AtipicialFs v2 token fields are protobuf-parity helpers; individual
...
use super::super::super::object::{atipicialfs_json_owner_id, atipicialfs_json_signature};
use super::context::atipicialfs_json_session_context_v2;

// Rationale: token projection is kept as the stable entry point for optional
// AtipicialFs v2 session support even when callers are feature-gated away.
#[allow(dead_code)]
pub(crate) fn atipicialfs_json_session_token_v2(
    token: &atipicialfs_v2::session::SessionTokenV2,
) -> Option<String> {
    ...
    if let Some(body) = token
        .body
        .as_ref()
        .and_then(atipicialfs_json_session_token_v2_body)
    {
        push_json_field(&mut out, &mut first, "body", &body);
    }
    if let Some(signature) = token.signature.as_ref().and_then(atipicialfs_json_signature) {
        push_json_field(&mut out, &mut first, "signature", &signature);
    }
    if let Some(origin) = token.origin.as_ref() {
        let json =
            atipicialfs_json_session_token_v2(origin.as_ref()).unwrap_or_else(|| "{ }".to_string());
...
fn atipicialfs_json_session_token_v2_body(
    body: &atipicialfs_v2::session::session_token_v2::Body,
) -> Option<String> {
    let mut out = String::from("{ ");
    let mut first = true;
    ...
    if let Some(issuer) = body.issuer.as_ref().and_then(atipicialfs_json_owner_id) {
    ...
    if !body.subjects.is_empty() {
        ...
        for (idx, subject) in body.subjects.iter().enumerate() {
            ...
            if let Some(subject_json) = atipicialfs_json_session_target(subject) {
    ...
    if let Some(lifetime) = body.lifetime.as_ref().and_then(atipicialfs_json_token_lifetime) {
    ...
    if !body.contexts.is_empty() {
        ...
        for (idx, context) in body.contexts.iter().enumerate() {
            ...
            if let Some(context_json) = atipicialfs_json_session_context_v2(context) {
...
fn atipicialfs_json_session_target(target: &atipicialfs_v2::session::Target) -> Option<String> {
    let mut out = String::from("{ ");
    let mut first = true;

    if let Some(identifier) = target.identifier.as_ref() {
        match identifier {
            atipicialfs_v2::session::target::Identifier::OwnerId(owner) => {
                if let Some(owner_json) = atipicialfs_json_owner_id(owner) {
                    push_json_field(&mut out, &mut first, "ownerID", &owner_json);
                }
            }
            atipicialfs_v2::session::target::Identifier::NnsName(name) => {
...
fn atipicialfs_json_token_lifetime(lifetime: &atipicialfs_v2::session::TokenLifetime) -> Option<String> {
    let mut out = String::from("{ ");
    let mut first = true;
