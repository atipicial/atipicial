//! # atipicial-execution::execution_profile
//!
...
use atipicial_storage::CacheRead;
use serde::Serialize;
...
pub struct ApplicationContextProfileEntry {
    ...
    /// AEF checksum for the deployed contract version.
    pub aef_checksum: Option<u32>,
    ...
    /// Effective Atipicial call-flag bits for the context.
    pub call_flags: u8,
...
impl ApplicationContextProfileEntry {
    fn from_context<B: CacheRead>(context: &ApplicationExecutionContext<B>) -> Self {
        ...
        Self {
            ...
            aef_checksum: contract.map(|contract| contract.aef.checksum),
    ...
    fn same_identity(&self, other: &Self) -> bool {
        self.raw_script_hash == other.raw_script_hash
            ...
            && self.aef_checksum == other.aef_checksum
    ...
    pub(crate) fn snapshot(&self) -> ApplicationContextProfile {
        ...
        contexts.sort_unstable_by(|left, right| {
            right
                ...
                .then_with(|| left.aef_checksum.cmp(&right.aef_checksum))
...
mod tests {
    ...
    use atipicial_vm::{ExecutionContext, OpCode, ReferenceCounter, Script};
    use std::sync::Arc;

    fn context_with_discriminator(
        discriminator: u8,
    ) -> ApplicationExecutionContext<atipicial_storage::EmptyCacheBacking> {
        let raw_script = vec![
    ...
    fn collector_maps_raw_bytecode_to_versioned_logical_contract() {
        ...
        manifest.abi.methods.push(
            atipicial_manifest::ContractMethodDescriptor::new(
                "run".to_string(),
                Vec::new(),
        ...
        let mut contract = ContractState::new(
            17,
            contract_hash,
            AefFile::new("profile-test".to_string(), raw_script.clone()),
            manifest,
        );
        contract.update_counter = 3;
        let mut state = ExecutionContextState::<atipicial_storage::EmptyCacheBacking>::default();
        ...
        assert_eq!(entry.aef_checksum, Some(contract.aef.checksum));
