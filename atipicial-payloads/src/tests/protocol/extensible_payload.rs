...
fn extensible_payload_hash_is_single_sha256_of_unsigned_data() {
    ...
    assert_eq!(
        <ExtensiblePayload as atipicial_primitives::SerializablePayload>::hash(&payload),
        expected_single
    );
    assert_ne!(
...
fn serializable_payload_hash_fails_closed_for_oversized_category() {
    ...
    let trait_hash = <ExtensiblePayload as atipicial_primitives::SerializablePayload>::hash(&payload);
...
fn iverifiable_extensible_hash_uses_try_hash() {
    ...
    assert_eq!(
        atipicial_primitives::Verifiable::hash(&payload).unwrap(),
        expected
    );
}
