...
fn node_composition_delegates_provider_neutral_core_assembly() {
    ...
    assert!(
        ...
        "the application composition should enter core assembly through atipicial-system"
    );
    ...
    for forbidden in [
        ...
        "atipicial_system::Node::builder",
    ] {
        assert!(
            !source.contains(forbidden),
            "provider-neutral core constructor `{forbidden}` belongs in atipicial-system"
        );
    }
}
