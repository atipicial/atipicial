...
//! The shared cross-crate event type ([`atipicial_runtime::BlockchainEvent`]) and
//! the default channel capacities live in [`atipicial_runtime`]; the command enum
//! and its handle are owned here.

use std::sync::Arc;
use std::time::Duration;

use atipicial_mempool::TransactionOrigin;
use atipicial_payloads::{Block, Transaction, extensible_payload::ExtensiblePayload, header::Header};
...
pub struct AddTransactionReply {
    ...
    pub result: atipicial_primitives::verify_result::VerifyResult,
    /// Hash of the transaction.
    pub hash: atipicial_primitives::UInt256,
}
