//! Storage value traits for Atipicial blockchain.
//!
...
//! between atipicial-storage and atipicial-vm (Chain 1: `StorageItem` → `Interoperable`).
//!
//! # Example
//!
//! ```rust
//! # fn main() -> Result<(), atipicial_primitives::StorageValueError> {
//! use atipicial_primitives::StorageValue;
//!
...
/// breaking the circular dependency with `Interoperable` (atipicial-vm).
///
/// # Design Rationale
///
/// The Atipicial C# implementation has `StorageItem` that can cache `Interoperable`
/// objects. This creates a dependency from storage to VM types. By using this
/// trait, we can:
///
/// 1. Define a simple `StorageItem<V: StorageValue>` in atipicial-storage
/// 2. Have atipicial-core implement `StorageValue` for types that need VM integration
/// 3. Avoid any dependency from atipicial-storage to atipicial-vm
///
/// # Performance
///
