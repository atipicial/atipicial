...
use atipicial_execution::application_engine::ApplicationEngine;
use atipicial_primitives::CallFlags;
use atipicial_primitives::{BigDecimal, TriggerType, UInt160};
use atipicial_vm::script_builder::ScriptBuilder;
use atipicial_vm::{OpCode, VmState as VMState};
use atipicial_wallets::{Wallet as CoreWallet, WalletAccount};
...
impl RpcServerWallet {
    pub(super) fn get_wallet_balance(
        ...
        // same native `balanceOf` / `decimals` methods for every AEP-17
        // asset, ATC and GAS included.
        let balance = Self::calculate_aep17_balance(server, wallet.as_ref(), &request.asset)?;
        Ok(wallet_balance_to_json(&balance))
    }

    pub(super) fn get_wallet_unclaimed_atipicial_dollar(
        server: &RpcServer,
        params: &[Value],
    ) -> Result<Value, RpcException> {
        ...
        for account in wallet.accounts() {
            // C# GetWalletUnclaimedGas sums NativeContract.ATC.UnclaimedGas
            // per account; the engine probe invokes the same native
            // `unclaimedGas(account, end)` method.
            let gas = crate::server::native_queries::NativeQueries::atipicial_unclaimed_atipicial_dollar(
                server,
                Arc::clone(&snapshot),
                &atipicial_hash,
                &account.script_hash(),
    ...
    pub(super) fn calculate_aep17_balance<W>(
        server: &RpcServer,
        wallet: &W,
        asset: &UInt160,
