...
use crate::{Aep17Api, RpcClient, RpcObserver, TracingRpcObserver, TransactionManagerFactory};
use atipicial_crypto::ECPoint;
use atipicial_execution::ContractParametersContext;
    ...
    O: RpcObserver,
{
    ...
    pub fn add_witness(&mut self, contract: Contract) -> Result<&mut Self, RpcClientError> {
    ...
    pub async fn sign(&mut self) -> Result<Transaction, RpcClientError> {
        ...
        let gas_balance = Aep17Api::new(self._rpc_client.clone())
    ...
    async fn get_contract_async(&self, script_hash: &UInt160) -> Result<Contract, RpcClientError> {
        let state = self
            ._rpc_client
