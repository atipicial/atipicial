...
use atipicial_storage::persistence::{MdbxEnvironmentInfo, TransactionalStore};
...
pub(super) struct MetricsExporter<
    P = atipicial_native_contracts::StandardNativeProvider,
    S = atipicial_storage::persistence::providers::MemoryStore,
> where
    P: NativeContractProvider,
    S: TransactionalStore,
{
    registry: Registry,
    node: Arc<atipicial_system::Node<P, S>>,
    ...
    S: TransactionalStore + 'static,
{
    pub(super) fn new(
        ...
        let up = IntGauge::new("atipicial_node_up", "Whether the node process is running")?;
        let info = IntGaugeVec::new(
            Opts::new("atipicial_node_info", "Static atipicial-node build and network labels"),
            &["version", "network"],
        )?;
        let uptime_seconds = Gauge::new(
            "atipicial_node_uptime_seconds",
            "Seconds since the metrics exporter was started",
        )?;
        let ledger_height = IntGauge::new(
            "atipicial_node_ledger_height",
            "Current persisted ledger height from the Ledger native contract",
        )?;
        let connected_peers = IntGauge::new(
            "atipicial_node_connected_peers",
            "Connected peers folded from network events",
        )?;
        let mempool_transactions = IntGauge::new(
            "atipicial_node_mempool_transactions",
            "Total transactions currently held in the memory pool",
        )?;
        let mempool_verified_transactions = IntGauge::new(
            "atipicial_node_mempool_verified_transactions",
            "Verified transactions currently held in the memory pool",
        )?;
        let mempool_unverified_transactions = IntGauge::new(
            "atipicial_node_mempool_unverified_transactions",
            "Unverified transactions currently held in the memory pool",
        )?;
        let header_cache_entries = IntGauge::new(
            "atipicial_node_header_cache_entries",
            "Headers currently retained ahead of the persisted tip",
        )?;
        let service_enabled = IntGaugeVec::new(
            Opts::new(
                "atipicial_node_service_enabled",
                "Whether an optional node service is registered in the runtime",
            ),
            &["service"],
        )?;
        let indexer_up = IntGauge::new(
            "atipicial_node_indexer_up",
            "Whether the registered AtipicialIndexer service can report status",
        )?;
        let indexer_indexed_height = IntGauge::new(
            "atipicial_node_indexer_indexed_height",
            "Highest block height indexed by AtipicialIndexer, or -1 when unavailable",
        )?;
        let indexer_indexed_blocks = IntGauge::new(
            "atipicial_node_indexer_indexed_blocks",
            "Number of canonical blocks indexed by AtipicialIndexer",
        )?;
        let indexer_indexed_transactions = IntGauge::new(
            "atipicial_node_indexer_indexed_transactions",
            "Number of transactions indexed by AtipicialIndexer",
        )?;
        let indexer_indexed_accounts = IntGauge::new(
            "atipicial_node_indexer_indexed_accounts",
            "Number of signer accounts indexed by AtipicialIndexer",
        )?;
        let indexer_indexed_notifications = IntGauge::new(
            "atipicial_node_indexer_indexed_notifications",
            "Number of smart-contract notifications indexed by AtipicialIndexer",
        )?;
        let indexer_indexed_notification_accounts = IntGauge::new(
            "atipicial_node_indexer_indexed_notification_accounts",
            "Number of notification participant accounts indexed by AtipicialIndexer",
        )?;
        let indexer_blocks_behind = IntGauge::new(
            "atipicial_node_indexer_blocks_behind",
            "Difference between current ledger height and AtipicialIndexer indexed height, or -1 when unavailable",
        )?;
        let indexer_synced = IntGauge::new(
            "atipicial_node_indexer_synced",
            "Whether AtipicialIndexer indexed height exactly matches the current ledger height",
        )?;
...
fn render_mdbx_metrics(info: atipicial_storage::StorageResult<MdbxEnvironmentInfo>) -> String {
    let info = match info {
        Ok(info) => info,
        Err(err) => {
            warn!(target: "atipicial::telemetry", error = %err, "failed to read MDBX environment info");
            return String::new();
        }
    };

    format!(
        "# HELP atipicial_storage_mdbx_map_size_bytes Current MDBX memory-map size in bytes\n\
         # TYPE atipicial_storage_mdbx_map_size_bytes gauge\n\
         atipicial_storage_mdbx_map_size_bytes {}\n\
         # HELP atipicial_storage_mdbx_last_page_number Last used MDBX page number\n\
         # TYPE atipicial_storage_mdbx_last_page_number gauge\n\
         atipicial_storage_mdbx_last_page_number {}\n\
         # HELP atipicial_storage_mdbx_last_transaction_id Last committed MDBX transaction id\n\
         # TYPE atipicial_storage_mdbx_last_transaction_id gauge\n\
         atipicial_storage_mdbx_last_transaction_id {}\n\
         # HELP atipicial_storage_mdbx_max_readers Configured MDBX reader slot capacity reported by the environment\n\
         # TYPE atipicial_storage_mdbx_max_readers gauge\n\
         atipicial_storage_mdbx_max_readers {}\n\
         # HELP atipicial_storage_mdbx_reader_slots_used MDBX reader slots currently used\n\
         # TYPE atipicial_storage_mdbx_reader_slots_used gauge\n\
         atipicial_storage_mdbx_reader_slots_used {}\n",
    ...
    S: TransactionalStore + 'static,
{
    ...
    fn indexer_status(&self) -> Option<Result<atipicial_indexer::IndexerStatus, String>> {
        self.services
            .indexer()
