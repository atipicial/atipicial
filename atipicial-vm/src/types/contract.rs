...
//!
//! This type was originally defined in `atipicial-execution` but has been moved here
//! because it is a pure data structure with no execution logic. Both
//! `atipicial-execution` (for witness verification) and `atipicial-wallets` (for account
//! contract management) need it, so it lives at the VM infrastructure layer
//! to avoid a circular dependency.

use crate::script_builder::ScriptBuilder;
use atipicial_crypto::{Crypto, ECPoint};
use atipicial_error::CoreError;
use atipicial_primitives::ContractParameterType;
use atipicial_primitives::UInt160;
use std::sync::OnceLock;

/// Represents a verification contract (matches C# `Atipicial.SmartContract.Contract`).
///
...
impl Contract {
    ...
    /// (e.g. from a AEP-6 wallet file that only stores the script hash).
    ...
    pub fn create_multi_sig_contract(m: usize, public_keys: &[ECPoint]) -> Self {
        match Self::try_create_multi_sig_contract(m, public_keys) {
            ...
            Err(error) => {
                tracing::error!(target: "atipicial_vm", %error, "multi-sig contract construction failed");
    ...
    pub fn create_multi_sig_redeem_script(m: usize, public_keys: &[ECPoint]) -> Vec<u8> {
        match Self::try_create_multi_sig_redeem_script(m, public_keys) {
            Ok(script) => script,
            Err(error) => {
                tracing::error!(target: "atipicial_vm", %error, "multi-sig redeem script construction failed");
                Vec::new()
            }
        }
    }
