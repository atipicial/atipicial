...
atipicial_error::impl_error_from_struct!(atipicial_error::CoreError, ProtocolConfigError => Configuration);

mod load;
mod parse;
mod presets;

/// Consensus and chain-execution fields from Atipicial C# `ProtocolSettings`.
///
/// Operator-owned resource policy is deliberately excluded. For example,
/// transaction-pool capacity is owned by `atipicial-mempool::TxPoolConfig` and cannot
/// mutate chain identity.
#[derive(Debug, Clone, PartialEq)]
pub struct ProtocolSettings {
    /// The magic number of the ATC network.
    /// Matches C# Network property
    pub network: u32,

    /// The address version of the ATC system.
    ...
    /// The number of the validators in ATC system.
...
impl ProtocolSettings {
    /// The number of members of the committee in ATC system.
    ...
    /// AEF/ABI even before their activation height.
