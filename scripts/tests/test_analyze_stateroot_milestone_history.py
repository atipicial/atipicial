...
class AnalyzeStateRootMilestoneHistoryTests(unittest.TestCase):
    ...
    def test_analyze_history_reports_perf_and_consistency_summary(self):
        ...
        self.assertEqual(
            report["latest_metrics_sample_summary"]["metrics"]["atipicial_sync_avg_persist_us"]["last"],
            4406.0,
        )
        self.assertTrue(
    ...
    def test_analyze_history_groups_performance_by_node_binary(self):
        ...
        self.assertEqual(
            ...
            ["target/release/atipicial-node", "unknown"],
        )
        release = report["performance_by_node_bin"][0]
        self.assertEqual(release["probe_bins"], ["target/release/atipicial-db-probe"])
        ...
        self.assertEqual(report["throughput_trend"][1]["node_bin"], "target/release/atipicial-node")
    ...
    def test_analyze_history_ranks_recurring_hot_latency_metrics(self):
        module = load_module()
        queue_wait = 'atipicial_state_service_mpt_apply_stage_avg_us{stage="queue_wait"}'
        load_execute = 'atipicial_sync_native_persist_tx_stage_avg_us{stage="load_execute"}'
        ignored = 'atipicial_sync_native_persist_tx_stage_avg_us{stage="ignored"}'
        records = [
            record(
                "2026-01-01T00:00:00+00:00",
                [
