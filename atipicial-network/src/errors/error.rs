...
//! `atipicial_runtime` is reused by the [`crate::handle::NetworkHandle`]
...
///
/// Distinct from [`atipicial_runtime::ServiceError`] (which is the
...
impl From<NetworkError> for atipicial_runtime::ServiceError {
    fn from(err: NetworkError) -> Self {
        match err {
            NetworkError::Io(e) => atipicial_runtime::ServiceError::Internal(e.to_string()),
            NetworkError::RemoteUnavailable { detail, .. } => {
                atipicial_runtime::ServiceError::ServiceUnavailable(format!("remote: {detail}"))
            }
            NetworkError::LocalShuttingDown => atipicial_runtime::ServiceError::ServiceUnavailable(
                "local node shutting down".to_string(),
            ),
            NetworkError::ChannelFull => atipicial_runtime::ServiceError::ServiceUnavailable(
                "local node command channel is full".to_string(),
            ),
            NetworkError::NotStarted => {
                atipicial_runtime::ServiceError::InvalidState("local node not started".to_string())
            }
            NetworkError::Protocol(msg) => {
                atipicial_runtime::ServiceError::Internal(format!("protocol: {msg}"))
            }
        }
    }
}
...
atipicial_error::impl_error_from_struct!(atipicial_error::CoreError, NetworkError => Network);
