//! Node: control a locally-spawned `atipicial-node` process and tail its logs.
...
pub fn ui(app: &mut AtipicialGuiApp, ui: &mut Ui) {
    ...
    ui.add_space(6.0);
    ui.label(
        egui::RichText::new("Run and supervise a local atipicial-node process.").color(theme::TEXT_MUTED),
    );
    ui.add_space(12.0);
            ...
            .show(ui, |ui| {
                ...
                ui.add(
                    egui::TextEdit::singleline(&mut app.local.binary)
                        .hint_text("./target/release/atipicial-node")
                        .desired_width(440.0),
                );
                ui.end_row();
