//! Wire-level [`Message`] - the on-the-wire representation of a Atipicial
//! P2P network frame, mirroring `Atipicial.Network.P2P.Message`.
//!
//! ```text
//! ┌──────────┬──────────┬──────────────┬──────────┐
...
use atipicial_io::{COMPRESSION_MIN_SIZE, COMPRESSION_THRESHOLD, Lz4};
use serde::{Deserialize, Serialize};

/// Maximum payload size (matches `Atipicial.Network.P2P.Message.PayloadMaxSize` = 32 MiB).
...
/// Atipicial wire-level network message (parity with `Atipicial.Network.P2P.Message`).
...
impl Message {
    ...
    pub fn wire_size(&self) -> usize {
        ...
        2 + atipicial_io::var_int::VarInt::encoded_len(payload_len as u64) + payload_len
    }
    ...
    pub fn to_bytes(&self) -> WireResult<Vec<u8>> {
        ...
        atipicial_io::var_int::VarInt::write_var_bytes(&self.payload_compressed, &mut buf);
        Ok(buf)
    }
