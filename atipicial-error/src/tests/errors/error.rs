...
#[test]
fn test_from_atipicial_io_errors_preserves_error_category() {
    let format_error = CoreError::from(atipicial_io::IoError::Format);
    assert!(matches!(format_error, CoreError::Codec { .. }));
    assert_eq!(format_error.category(), "serialization");

    let io_error = CoreError::from(atipicial_io::IoError::Io(std::io::Error::new(
