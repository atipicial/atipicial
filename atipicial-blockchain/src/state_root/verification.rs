//! Signed [`atipicial_state_service::StateRoot`] witness verification against the
...
//!
//! This lives in `atipicial-blockchain` (which already depends on `atipicial-native-contracts`,
//! `atipicial-execution`, and `atipicial-vm`) rather than in the light `atipicial-state-service`
//! crate, and wraps the [`atipicial_state_service::StateRoot`] in a `VerifiableExt`
//! newtype so the tested, engine-based witness verification
//! ([`atipicial_execution::Helper::verify_witnesses_with_native_provider`]) is reused
...
use atipicial_execution::Helper;
use atipicial_execution::native_contract_provider::NativeContractProvider;
use atipicial_payloads::{VerifiableExt, Witness};
use atipicial_primitives::error::PrimitiveResult;
use atipicial_primitives::{UInt160, UInt256, Verifiable};
use atipicial_state_service::StateRoot;
use atipicial_storage::{CacheRead, DataCache};
use atipicial_vm::script_builder::RedeemScript;
use std::sync::Arc;

/// Max ATD for state-root witness verification (C# `StateRoot.Verify`: 2 ATD).
