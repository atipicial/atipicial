...
use atipicial_execution::application_engine::{
...
use atipicial_execution::{ApplicationEngine, NativeContractsCache, NoDiagnostic};
use atipicial_payloads::{Block, VerifiableContainer};
    ...
    B: CacheRead,
{
    ...
    for (transaction_index, tx) in block.transactions.iter().enumerate() {
        ...
        record_tx_stage(
            atipicial_runtime::sync_metrics::NativePersistTxStage::Hash,
            stage_start,
        );
        ...
        record_tx_stage(
            atipicial_runtime::sync_metrics::NativePersistTxStage::CachePrepare,
            stage_start,
        );
        ...
        record_tx_stage(
            atipicial_runtime::sync_metrics::NativePersistTxStage::ContainerPrepare,
            stage_start,
        );
        ...
        record_tx_stage(
            atipicial_runtime::sync_metrics::NativePersistTxStage::EngineCreate,
            stage_start,
        );
        ...
        record_tx_stage(
            atipicial_runtime::sync_metrics::NativePersistTxStage::LoadScript,
            stage_start,
        );
        ...
        let (vm_state, load_error, execute_us) = match load_result {
            Ok(()) if shadow_requested => {
                let mut replay_writer = atipicial_io::BinaryWriter::new();
                atipicial_io::Serializable::serialize(tx, &mut replay_writer).map_err(|error| {
                ...
                match shadow {
                    Ok(shadow) => {
                        tracing::debug!(
                            target: "atipicial::specialization",
                            block_index,
                            transaction_index,
                            tx_hash = %tx_hash,
        ...
        record_tx_stage(
            atipicial_runtime::sync_metrics::NativePersistTxStage::LoadExecute,
            load_execute_start,
        );
        ...
        record_tx_stage(
            atipicial_runtime::sync_metrics::NativePersistTxStage::ApplicationExecuted,
            stage_start,
        );
        ...
        if should_trace_tx {
            if let Some(executed) = executed.as_ref() {
                match trace_tx_artifact(block_index, &tx_hash, executed) {
                    Ok(artifact) => eprintln!("ATC_TX_ARTIFACT {artifact}"),
                    Err(error) => eprintln!(
                        "ATC_TX_ARTIFACT_ERROR block={} hash={} error={}",
        ...
        if vm_state == VMState::HALT {
            ...
            record_tx_stage(
                atipicial_runtime::sync_metrics::NativePersistTxStage::TxCacheCommit,
                stage_start,
            );
        }
        let stage_start = std::time::Instant::now();
        ...
        record_tx_stage(
            atipicial_runtime::sync_metrics::NativePersistTxStage::LedgerVmState,
            stage_start,
        );
    }

    Ok(atipicial_runtime::time::elapsed_us(tx_start.elapsed()))
}
