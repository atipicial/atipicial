...
fn balance_roundtrip() {
    let entry = RpcAep17Balance {
        asset_hash: UInt160::zero(),
        amount: BigInt::from(42),
        last_updated_block: 10,
    };
    let json = entry.to_json();
    let parsed = RpcAep17Balance::from_json(&json, &ProtocolSettings::default_settings()).unwrap();
...
fn balances_roundtrip() {
    ...
    let balances = RpcAep17Balances {
        user_script_hash: UInt160::zero(),
        balances: vec![entry.clone()],
    };
    let json = balances.to_json(&ProtocolSettings::default_settings());
    let parsed = RpcAep17Balances::from_json(&json, &ProtocolSettings::default_settings()).unwrap();
...
fn balances_array_keeps_lossy_parse_behavior() {
    ...
    let parsed = RpcAep17Balances::from_json(&root, &settings).unwrap();
    assert_eq!(parsed.balances.len(), 1);
    assert_eq!(parsed.balances[0].amount, BigInt::from(5));
}

#[test]
fn aep17_balances_to_json_matches_rpc_test_case() {
    let Some(expected) = rpc_case_result("getnep17balancesasync") else {
        return;
    };
    let settings = ProtocolSettings::default_settings();
    let parsed = RpcAep17Balances::from_json(&expected, &settings).expect("parse");
