...
# atipicial-rs vs Reth vs Polkadot SDK — Architecture Comparison

> Comparative analysis of atipicial-rs (Atipicial N3 Rust node), Reth (Rust Ethereum),
> and Polkadot SDK (Substrate) across storage, sync, pipeline, task management,
> and networking dimensions. Each section identifies what atipicial-rs can learn.

---

## 1. Storage Architecture

### atipicial-rs (current)
...
emit tracked entries in byte-key order. This is the Atipicial adaptation of
...
The persistent indexer remains a separate durability domain, so atipicial-rs uses
...
`atipicial-static-files` supplies versioned append-only height frames, zstd
...
checks before adapting the node-composed native-contract provider for ATC and
Policy reads.
...
Offline `atipicial-db-probe` replay accepts `--static-files-dir` and composes the
...
consensus rows without authorizing compact or alternate Atipicial encodings. Backend
...
| Aspect | atipicial-rs | Reth | Polkadot SDK |
|--------|--------|------|-------------|
...
| Table encoding | Generic `Table` + GAT codecs over unchanged Atipicial/maintenance bytes; typed sync and prune metadata are live | Per-table `Encode`/`Decode` + `Compact` derive | Per-column encoding (parity-scale-codec) |
...
### Recommendations for atipicial-rs

| Priority | Change | Beaefit |
|----------|--------|---------|
...
| P1 | Compact derive for versioned node-local tables only | Fewer operational bytes without changing Atipicial consensus/storage codecs |
...
`atipicial_runtime::sync_pipeline` provides stage identifiers, Reth-style
...
backend's isolated maintenance namespace. `atipicial_system::StagedSyncPipeline`
...
`Import` is
followed by one honest production-consumed `Index` stage in `atipicial-node`. That
...
persistence, and state-root work remain inside `Import`; atipicial-rs does not invent
...
Live path: P2P Block -> `InboundInventory::Block` -> `atipicial-node` buffering ->
`atipicial_system::LiveBlockImportPipeline` -> shared
`BlockImportQueue::check_blocks` ->
`BlockchainHandle::submit_checked_inventory_blocks` -> atipicial-blockchain
...
`atipicial-blockchain`. The live adapter reuses the exact queue owned by staged sync,
...
| Composed / P2P Wired | Import queue boundary with bounded concurrent `check` | Reusable preverification surface; `BlockchainHandle::check` now shares live stateless import-integrity checks, `SyncImportPipeline` owns the queue inside `atipicial_system::StagedSyncPipeline`, and production P2P sync drains verified coordinator batches into it |
...
`atipicial_runtime::BlockImport` is the canonical import trait. `BlockchainHandle`
implements it and routes to the `atipicial-blockchain` service loop. The reusable
`atipicial_runtime::BlockImportQueue` runs bounded concurrent `check` calls. Strict
...
`atipicial-blockchain::VerifiedImportPipeline`, which composes the concrete
...
`atipicial_runtime::SyncPipelineDriver` consumes contiguous sync batches, rejects
...
keys in the normal Atipicial data table. `SyncCheckpointTable` and the verified-header
...
`atipicial_system::StagedSyncPipeline` is an explicit node field. It composes a
...
`atipicial_system::LiveBlockImportPipeline` is derived from that staged pipeline's
...
happen only inside `atipicial-blockchain`.
...
The same fixture now supports `ATC_BENCH_IMPORT_MODE=live` and
`ATC_BENCH_REPLAY_ARTIFACTS=1` for observer-artifact A/B checks. Short dense
...
`atipicial-network::BlockDownloadBatch` converts into `atipicial_runtime::SyncBlockBatch`,
preserving the single ordered import path. `BlockRequest` carries the protocol
cap and `HeaderRequest` carries Atipicial's 2,000-header cap. Correlated header fetches
...
The reth ExEx lesson is implemented as a Atipicial-specific three-channel outcome
model rather than one overloaded event bus. `atipicial-blockchain::FinalizedBlock<B>`
...
`atipicial_system::FinalizedBlockConsumer<B>` is bound to a concrete consumer by
...
and the lightweight reorg-aware `RuntimeEvent` broadcast. Atipicial's finalized
...
`atipicial-node` now has explicit daemon task supervision (genuinely done). Essential
...
```rust
// atipicial-node/src/node/tasks/supervision.rs
...
`atipicial_network::BlockDownloader` stream boundary and `BlockDownloadConfig` policy
records now exist, with a channel-backed adapter for tests/composition roots.
`BlockDownloadBatch` converts into `atipicial_runtime::SyncBlockBatch`, which the
runtime sync driver can feed into the import queue. `atipicial_system` provides
...
through the shared runtime error vocabulary. `HeaderRequest` owns the Atipicial
...
deleted. `atipicial-network` exclusively owns `MessageCommand`, its parse error, the
numeric serde mapping, and unknown-byte preservation; shared inventory and
verification values remain in `atipicial-primitives`.
...
Native Rust AtipicialVM — no WASM. `ApplicationEngine` with per-tx child caches.
`NativeContractProvider` remains the lower-level execution seam, but
`atipicial-system::NodeBuilder` now makes the provider an explicit composition-root
...
1. Keep native execution (AtipicialVM in Rust is already fast).
...
`atipicial-storage::persistence::Table` binds a logical name, physical namespace,
...
Reth's compact derive is not directly applicable to consensus-visible Atipicial
...
already established Atipicial codec byte-for-byte.
...
### atipicial-rs adaptation

`atipicial-mempool::MemoryPool<P>` is the sole owner of Atipicial admission and pool state.
It captures the immutable `Arc<AtipicialChainSpec>`, the composed native-contract
...
post-persist removal logic stay inside `atipicial-mempool`.

`atipicial-blockchain::MempoolLike` is deliberately a small consumer-side capability
...
same provider-versus-component split used by Reth, adapted to Atipicial's
...
composition without moving Atipicial verification policy into transport code.
...
`atipicial-execution::ApplicationEngine<P, D, B>` selects the native provider,
diagnostic policy, and cache backing as concrete generic collaborators while
`atipicial-vm` remains the only semantic VM. `atipicial-blockchain` owns the canonical
Atipicial block import and durable commit; `atipicial-state-service` owns state-root
mutation; `atipicial-system` supplies both through required builder inputs.

Reth's EVM engine traits are not a license to replace AtipicialVM or cache stateful
...
`atipicial-rpc` owns JSON-RPC wire models, transports, dispatch, and plugin method
groups. `NodeContext<P, S>` is constructed by `atipicial-system`/`atipicial-node` through
`from_parts`, so `atipicial-rpc` has no production dependency on the composition
...
atipicial-rs deliberately keeps the smaller equivalent ownership model rather than
adding an unused facade:

| Concern | Canonical atipicial-rs owner | Rule |
|---------|------------------------|------|
| `UInt160`, `UInt256`, inventory/witness value records | `atipicial-primitives` | No service crate defines another hash/address/value representation. |
| SHA/RIPEMD, Merkle roots, ECC, signatures, BLS | `atipicial-crypto` | Crypto algorithms and key/signature parsing stay below protocol services. |
| Block/transaction/witness wire records | `atipicial-payloads` | Payload encoding uses the canonical Atipicial serializer and never imports node configuration just to render an address. |
| Network identity, genesis, hardforks, protocol limits | `atipicial-config::AtipicialChainSpec` | One validated immutable `Arc` flows through composition; operator limits stay in service config. |
| AEF, manifest, method tokens, ABI | `atipicial-manifest` | Contract metadata is not reimplemented in execution, RPC, or wallets. |
| AtipicialVM stack/context/opcodes/faults | `atipicial-vm` | There is one semantic authority; no `StackValue` conversion graph or alternate VM crate. |
...
| Compact derive macro | ★★ | ★★★ | - | ★★ | Deferred to versioned node-local records; forbidden for incompatible Atipicial bytes |
...
1. **Essential task supervision + metrics** — implemented in `atipicial-node`.
2. **Typed table boundary** — implemented in `atipicial-storage`. Generic GAT codecs,
...
3. **State provider/factory boundary** — implemented in `atipicial-state-service`.
...
9. **Bounded finalized projection delivery** — implemented in `atipicial-system` and
   composed by `atipicial-node`. ApplicationLogs and TokensTracker consume exact
