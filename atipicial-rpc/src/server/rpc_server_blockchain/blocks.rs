...
impl RpcServerBlockchain {
    ...
    pub(super) fn get_block_hash(
        ...
        let hash = ledger_queries::block_hash_by_index(
            system.as_ref(),
            store.data_cache(),
            request.height,
        )
    ...
    pub(super) fn get_block(server: &RpcServer, params: &[Value]) -> Result<Value, RpcException> {
        ...
        let system = server.system();
        let store = system.store_cache();
        let block = Self::fetch_payload_block(system.as_ref(), &store, &request.identifier)?;
        if request.verbose {
    ...
    pub(super) fn get_block_sys_fee(
        ...
        let system_fee: i64 = block
            .transactions
            .iter()
            .map(atipicial_payloads::Transaction::system_fee)
            .sum();
