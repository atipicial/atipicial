...
pub const CHECKPOINT_ID_DOMAIN: &[u8] = b"atipicial-checkpoint-v1\0";
/// Domain used to derive supplementary certificate sign data.
pub const CHECKPOINT_CERTIFICATE_SIGN_DOMAIN: &[u8] = b"atipicial-checkpoint-certificate-v1\0";
...
impl CheckpointCertificateV1 {
    ...
    /// Returns Atipicial-style certificate sign data:
...
/// Computes Atipicial-style sign data for the supplementary certificate.
...
fn put_protocol(output: &mut Vec<u8>, protocol: ProtocolIdentityV1) {
    ...
    put_digest(output, protocol.atipicial_vm);
...
impl<'a> Reader<'a> {
    ...
    fn protocol(&mut self) -> CheckpointResult<ProtocolIdentityV1> {
        Ok(ProtocolIdentityV1 {
            ...
            atipicial_vm: self.digest("atipicial_vm_digest")?,
