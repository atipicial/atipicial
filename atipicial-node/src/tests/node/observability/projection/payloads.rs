...
fn generic_payload_includes_network_and_location() {
    let metadata = ObservabilityMetadata {
        service_name: "atipicial-node-mainnet".to_string(),
    ...
    assert_eq!(payload["service"]["name"], "atipicial-node-mainnet");
...
fn google_payload_uses_error_reporting_shape() {
    let metadata = ObservabilityMetadata {
        service_name: "atipicial-node".to_string(),
    ...
    assert_eq!(payload["serviceContext"]["service"], "atipicial-node");
    ...
    assert_eq!(payload["context"]["reportLocation"]["filePath"], "atipicial-node");
...
fn better_stack_payload_uses_log_event_shape() {
    ...
    assert_eq!(payload["service"], "atipicial-node-mainnet");
...
fn sentry_payload_uses_event_shape_and_tags() {
    ...
    assert_eq!(payload["logger"], "atipicial-node");
    assert_eq!(payload["transaction"], "panic");
    assert_eq!(payload["message"], "boom");
    assert_eq!(payload["release"], "atipicial-node@test");
    assert_eq!(payload["environment"], "production");
    assert_eq!(payload["server_name"], "validator-1");
    assert_eq!(payload["tags"]["service"], "atipicial-node-mainnet");
