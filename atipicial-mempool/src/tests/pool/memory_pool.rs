...
use atipicial_crypto::signature::Secp256r1Crypto;
use atipicial_payloads::{Signer, Transaction, TransactionAttribute, Witness};
use atipicial_primitives::{UInt160, UInt256, VerifyResult, WitnessScope};
use atipicial_serialization::BinarySerializer;
use atipicial_storage::DataCache;
...
fn memory_pool_with_config(settings: &ProtocolSettings, config: TxPoolConfig) -> MemoryPool {
    let chain_spec = Arc::new(
        atipicial_config::AtipicialChainSpec::private(
            "mempool-test",
            settings.clone(),
            atipicial_config::GenesisConfig::mainnet(),
            None,
        )
...
fn memory_pool_requires_explicit_native_provider_constructor() {
    ...
    assert!(
        source.contains("pub struct MemoryPool<P = atipicial_native_contracts::StandardNativeProvider>"),
...
fn keypair(seed: u8) -> ([u8; 32], Vec<u8>, UInt160) {
    ...
    let script =
        atipicial_vm::script_builder::redeem_script::RedeemScript::signature_redeem_script(&public);
    (private, public, UInt160::from_script(&script))
}

/// Writes a ATD AEP-17 account record (`Struct[balance]`, the C#
...
fn mint_gas(snapshot: &DataCache, account: &UInt160, datoshi: i64) {
    ...
    snapshot.add(
        atipicial_storage::StorageKey::new(atipicial_native_contracts::AtipicialDollar::ID, key),
...
fn seed_policy_fee_settings(snapshot: &DataCache, exec_fee_factor: i64) {
    snapshot.add(
        atipicial_storage::StorageKey::new(atipicial_native_contracts::PolicyContract::ID, vec![10]),
        atipicial_storage::StorageItem::from_bytes(num_bigint::BigInt::from(1_000).to_signed_bytes_le()),
    );
    snapshot.add(
        atipicial_storage::StorageKey::new(atipicial_native_contracts::PolicyContract::ID, vec![18]),
        atipicial_storage::StorageItem::from_bytes(
...
fn post_faun_mempool_divides_stored_exec_fee_factor_like_csharp() {
    ...
    seed_policy_fee_settings(
        &snapshot,
        30 * atipicial_execution::application_engine::FEE_FACTOR,
    );
    let pool = memory_pool(&settings);
...
fn duplicate_conflicts_attributes_with_same_hash_are_rejected_like_csharp_v3101() {
    ...
    let tx = signed_tx(
        ...
        1,
        vec![
            TransactionAttribute::Conflicts(atipicial_payloads::Conflicts::new(absent)),
            TransactionAttribute::Conflicts(atipicial_payloads::Conflicts::new(absent)),
        ],
    );
...
fn block_persist_removes_mined_tx_and_evicts_conflicts() {
    ...
    let conflicting = signed_tx(
        ...
        vec![TransactionAttribute::Conflicts(
            atipicial_payloads::Conflicts::new(target.hash()),
        )],
    );
    assert_eq!(
...
fn canonical_admission_conflict_eviction_reports_capacity_exceeded_like_csharp() {
    ...
    let replacement = signed_tx_with_fees(
        ...
        6_000_000,
        vec![TransactionAttribute::Conflicts(
            atipicial_payloads::Conflicts::new(old.hash()),
        )],
    );
...
fn oversize_transaction_reports_oversize() {
    ...
    tx.set_script(vec![
        OpCode::PUSH1.byte();
        atipicial_payloads::MAX_TRANSACTION_SIZE
    ]);
...
fn blocked_sender_reports_policy_fail() {
    ...
    snapshot.add(
        atipicial_native_contracts::PolicyContract::blocked_account_key(&account),
        atipicial_storage::StorageItem::from_bytes(Vec::new()),
    );
    let pool = memory_pool(&settings);
...
fn missing_balance_reports_insufficient_funds() {
    let settings = ProtocolSettings::default();
    let snapshot = DataCache::new(false); // no ATD minted
    seed_current_ledger(&snapshot, 0);
...
fn sender_fee_accumulates_until_balance_exhausted() {
    ...
    key.extend_from_slice(&account.to_bytes());
    snapshot.delete(&atipicial_storage::StorageKey::new(
