...
#![doc(html_root_url = "https://docs.rs/atipicial-blockchain/0.11.1")]
...
//
// The runtime crate (`atipicial-runtime`) owns trait-level service contracts and
// broadcast event defaults. `atipicial-blockchain` owns the concrete command loop,
...
pub mod blockchain {
    ...
    //! `atipicial-runtime` contributes only the broadcast event and the default
    //! channel capacities shared by both.
    pub use atipicial_runtime::{
...
pub use atipicial_runtime::BlockchainEvent;
...
pub use atipicial_runtime::{BlockchainEvent as RuntimeEvent, ServiceError};

pub use header_cache::HeaderCache;
pub use ledger::static_archive::{
    HotLedgerPruatipicialOutcome, StaticArchiveRecovery, StaticLedgerArchive, StaticLedgerArchiveFactory,
};
