...
class RepairBoundedReplayGasTests(unittest.TestCase):
    def test_builds_reference_repair_plan_from_latest_unique_failure(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            log = Path(tmp) / "atipicial-node.log"
            log.write_text(
                "native AtipicialDollar TriggerType(ON_PERSIST) hook failed at block 474702: "
                "Invalid operation: AtipicialDollar::burn: insufficient balance 804511 to burn 6694164\n"
                "native AtipicialDollar TriggerType(ON_PERSIST) hook failed at block 474702: "
                "Invalid operation: AtipicialDollar::burn: insufficient balance 804511 to burn 6694164\n"
                "native AtipicialDollar TriggerType(ON_PERSIST) hook failed at block 595391: "
                "Invalid operation: AtipicialDollar::burn: insufficient balance 364 to burn 424613\n",
                encoding="utf-8",
            )
            calls = []
            ...
            plan = module.build_repair_plan(
                log_path=log,
                reference_rpc="http://seed1.atipicial.com:10332",
    ...
    def test_ambiguous_fee_matches_are_disambiguated_by_local_balance(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            log = Path(tmp) / "atipicial-node.log"
            log.write_text(
                "native AtipicialDollar TriggerType(ON_PERSIST) hook failed at block 596898: "
                "Invalid operation: AtipicialDollar::burn: insufficient balance 0 to burn 7010070\n",
                encoding="utf-8",
            )
    ...
    def test_repeated_fee_matches_from_one_sender_are_disambiguated_by_sender(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            log = Path(tmp) / "atipicial-node.log"
            log.write_text(
                "native AtipicialDollar TriggerType(ON_PERSIST) hook failed at block 682485: "
                "Invalid operation: AtipicialDollar::burn: insufficient balance 5024789 to burn 7110219\n",
                encoding="utf-8",
            )
    ...
    def test_repeated_block_sender_is_disambiguated_by_projected_in_block_balance(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            log = Path(tmp) / "atipicial-node.log"
            log.write_text(
                "native AtipicialDollar TriggerType(ON_PERSIST) hook failed at block 682490: "
                "Invalid operation: AtipicialDollar::burn: insufficient balance 1868519 to burn 12208081\n",
                encoding="utf-8",
            )
