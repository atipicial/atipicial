...
use atipicial_wallets::{KeyPair, Aep6Wallet, Wallet as CoreWallet, WalletAccount, WalletError};
...
impl RpcServerWallet {
    ...
    pub(super) fn open_wallet(server: &RpcServer, params: &[Value]) -> Result<Value, RpcException> {
        ...
        let wallet = Aep6Wallet::from_file(&request.path, &request.password, settings);
