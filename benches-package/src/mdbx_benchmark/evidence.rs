...
/// Windows fallback: `GlobalMemoryStatusEx` reports the process' current
/// working set only via `PROCESS_MEMORY_COUNTERS`; for a dependency-light
/// implementation we query the total-modified/working-set through the
/// `GetProcessMemoryInfo`-equivalent exposed by `windows_sys` (already in
/// the dependency graph via `libmdbx`). When unavailable, sampling yields
/// no samples and the report's RSS fields stay `None` — benchmarks still
/// run, just without a memory column.
#[cfg(windows)]
fn read_process_status() -> Option<ProcessStatus> {
    // Parsing /proc is impossible on Windows; return a status with no RSS
    // so callers degrade gracefully instead of failing.
    None
}
...
mod tests {
    ...
    fn rss_sampler_shutdown_interrupts_the_sampling_interval() {
        ...
        // On Linux /proc/self/status is sampled at least once. On Windows the
        // sampler degrades to zero samples (no RSS column in the report);
        // the shutdown-timeliness invariant is what this test owns.
        #[cfg(unix)]
