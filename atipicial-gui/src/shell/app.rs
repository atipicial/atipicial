...
impl AtipicialGuiApp {
    ...
    pub fn new(cc: &eframe::CreationContext<'_>) -> Self {
        ...
        let url_edit = lock(&cfg, "PollerCfg").url.clone();
    ...
    pub fn run_rpc_to(
        ...
        std::thread::spawn(move || {
            ...
            *lock(&sink, "RPC output") = Some(text);
            ctx.request_repaint();
        });
    }
    ...
    pub fn rpc_url(&self) -> String {
        lock(&self.cfg, "PollerCfg").url.clone()
    }
...
impl eframe::App for AtipicialGuiApp {
        ...
        egui::TopBottomPanel::top("top")
            ...
            .show(ctx, |ui| {
                ui.horizontal_centered(|ui| {
                    ui.label(
                        RichText::new("◆ Atipicial Node Manager")
                            .size(18.0)
                            .strong()
                            .color(theme::ACCENT),
                    );
                    ...
                    let (online, height, network) = {
                        let s = lock(&self.state, "NodeState");
                        (
                            s.online,
                            s.status.as_ref().map(|st| st.block_count).unwrap_or(0),
                            s.status
                                .as_ref()
                                .map(|st| st.version.protocol.network_name())
                                .unwrap_or("—"),
                        )
                    };
                    ...
                    .show(ui, |ui| {
                        ...
                        for (group, items) in crate::screens::NAV {
                            ...
                            for item in *items {
                                ...
                                let label =
                                    RichText::new(format!("  {}  {}", item.icon(), item.title()))
                                        .size(14.0)
                        ...
                        ui.add_space(10.0);
                        ui.label(
                            RichText::new("atipicial-gui v0.1")
                                .size(11.0)
                                .color(theme::TEXT_MUTED),
                        );
                    });
