...
#
# Compares our atipicial-rs node (syncing) against a live C# MainNet reference node
# at the highest height BOTH have, checking:
#   1. Block hashes (byte-exact consensus)
#   2. Native contract state (ATC totalSupply, GAS totalSupply, Policy values)
#   3. ATC balance of the genesis BFT address (execution + storage + AEP-17)
#
# Usage:
#   scripts/mainnet-parity-check.sh [--continuous] [--interval 60]
#
# Requires our node running on 127.0.0.1:10332 with RPC auth (atipicial:change-me-mainnet-rpc-password).
set -euo pipefail

OUR_RPC="http://127.0.0.1:10332"
LIVE_RPC="${LIVE_RPC:-http://seed1.atipicial.com:10332}"
AUTH="$(printf 'atipicial:change-me-mainnet-rpc-password' | base64)"
CONTINUOUS=false
INTERVAL=60
...
run_check() {
  ...
  for entry in "$ATC_HASH:totalSupply" "$GAS_HASH:totalSupply" "$POLICY_HASH:getFeePerByte" "$POLICY_HASH:getExecFeeFactor" "$POLICY_HASH:getStoragePrice"; do
  ...
  # --- 4. ATC committee (consensus-critical state) ---
  echo "--- Committee ---"
  local our_comm live_comm
  our_comm=$(rpc "$OUR_RPC" invokefunction "[\"$ATC_HASH\",\"getCommittee\"]" | python3 -c "
import json,sys
d=json.load(sys.stdin)
...
if s and isinstance(s[0],dict) and s[0].get('type')=='Array':
    ...
    print('parse-fail')
" 2>/dev/null)
  live_comm=$(rpc_live invokefunction "[\"$ATC_HASH\",\"getCommittee\"]" | python3 -c "
import json,sys
d=json.load(sys.stdin)
s=d.get('result',{}).get('stack',[])
if s and isinstance(s[0],dict) and s[0].get('type')=='Array':
