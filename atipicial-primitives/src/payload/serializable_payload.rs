...
/// Verification logic lives in a separate `Verifiable` trait in atipicial-core.
///
/// # Design Rationale
///
/// Separating serialization from verification allows:
/// - atipicial-p2p to handle message framing without depending on atipicial-core
...
pub trait SerializablePayload: Send + Sync {
    ...
    ///
    /// Default implementation: `SHA256(hash_data())`. Atipicial N3 payload hashing is a
    /// single SHA-256 over the unsigned serialization (not Bitcoin's double hash).
    fn hash(&self) -> UInt256 {
        use sha2::{Digest, Sha256};
        let digest = Sha256::digest(self.hash_data());
        let mut hash = [0u8; UInt256::LENGTH];
...
#[cfg(test)]
#[path = "../tests/payload/serializable_payload.rs"]
mod tests;
