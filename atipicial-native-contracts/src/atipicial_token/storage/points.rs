...
impl AtipicialCoin {
    ...
    pub(in crate::atipicial_coin) fn points_to_array_bytes(
        points: &[ECPoint],
    ) -> CoreResult<Vec<u8>> {
        let array = Self::build_points_stack_item(points.iter());
        BinarySerializer::serialize_default(&array)
            .map_err(|e| CoreError::invalid_operation(format!("AtipicialCoin point array: {e}")))
    }

    pub(in crate::atipicial_coin) fn points_to_stack_item<'a, I>(points: I) -> CoreResult<StackItem>
    where
        I: IntoIterator<Item = &'a ECPoint>,
    {
