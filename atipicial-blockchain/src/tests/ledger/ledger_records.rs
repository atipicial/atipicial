...
fn optimized_trimmed_block_serializer_matches_canonical_trimmed_block_bytes() {
    ...
    let expected_empty = {
        let trimmed = atipicial_payloads::TrimmedBlock::new(empty_block.header.clone(), Vec::new());
        let mut writer = atipicial_io::BinaryWriter::with_capacity(trimmed.size());
    ...
    let expected_tx = {
        let trimmed = atipicial_payloads::TrimmedBlock::new(tx_block.header.clone(), vec![tx_hash]);
        let mut writer = atipicial_io::BinaryWriter::with_capacity(trimmed.size());
...
fn persisted_records_pin_csharp_key_and_value_bytes() {
    ...
    let mut header_writer = atipicial_io::BinaryWriter::new();
    ...
    let mut tx_writer = atipicial_io::BinaryWriter::new();
