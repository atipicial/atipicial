...
use atipicial_manifest::{AefFile, ContractManifest, ContractMethodDescriptor};
...
use atipicial_primitives::{CallFlags, ContractParameterType, TriggerType, UInt160, WitnessScope};
...
/// The smallest AEF that parses: a single RET at offset 0.
pub(super) fn minimal_nef() -> AefFile {
    AefFile::new("e2e-test".to_string(), vec![OpCode::RET.byte()])
}
...
/// Runs `System.Contract.Call(CM, "deploy", [aef, manifest(, data)])` and
...
/// `System.Contract.Call(CM, "update", [aef?, manifest?])`; `None` pushes
/// the C# `null` argument.
pub(super) fn update_script(
    ...
    let mut builder = ScriptBuilder::new();
    // arg1 (manifest) deepest, then arg0 (aef) on top, then PACK 2.
...
/// Builds the self-update entry script
/// `System.Contract.Call(CM, "update", [aef?, manifest?, data])`.
