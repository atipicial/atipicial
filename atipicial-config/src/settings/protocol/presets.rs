//! Built-in Atipicial N3 protocol setting presets.
//!
...
use atipicial_primitives::constants;

use super::{ProtocolConfigError, ProtocolSettings, parse::CommitteeParser};

const MAINNET_STANDBY_COMMITTEE: &[&str] = &[
...
const TESTNET_STANDBY_COMMITTEE: &[&str] = &[
...
impl ProtocolSettings {
    ...
    ///
    /// Atipicial's MainNet and TestNet values are supplied by `config.mainnet.json`
    ...
    /// Returns built-in ProtocolSettings for Atipicial MainNet.
    pub fn mainnet() -> Self {
        Self::try_mainnet().expect("embedded MainNet committee keys must be valid")
    }

    /// Returns built-in ProtocolSettings for Atipicial MainNet, surfacing embedded
    /// committee parse errors to callers that want startup diagnostics.
    pub fn try_mainnet() -> Result<Self, ProtocolConfigError> {
        let committee = CommitteeParser::parse_committee_slice(MAINNET_STANDBY_COMMITTEE)?;
        Ok(Self::mainnet_with_committee(committee))
    }

    fn mainnet_with_committee(standby_committee: Vec<ECPoint>) -> Self {
        Self {
            ...
            validators_count: 7,
            seed_list: vec![
                "seed1.atipicial.com:10333".to_string(),
                "seed2.atipicial.com:10333".to_string(),
                "seed3.atipicial.com:10333".to_string(),
                "seed4.atipicial.com:10333".to_string(),
                "seed5.atipicial.com:10333".to_string(),
            ],
            milliseconds_per_block: 15_000,
    ...
    /// Returns built-in ProtocolSettings for Atipicial TestNet.
    pub fn testnet() -> Self {
        Self::try_testnet().expect("embedded TestNet committee keys must be valid")
    }

    /// Returns built-in ProtocolSettings for Atipicial TestNet, surfacing embedded
    /// committee parse errors to callers that want startup diagnostics.
    pub fn try_testnet() -> Result<Self, ProtocolConfigError> {
        let committee = CommitteeParser::parse_committee_slice(TESTNET_STANDBY_COMMITTEE)?;
        Ok(Self::testnet_with_committee(committee))
    }

    fn testnet_with_committee(standby_committee: Vec<ECPoint>) -> Self {
        Self {
            ...
            validators_count: 7,
            seed_list: vec![
                "seed1.atipicial.com:20333".to_string(),
                "seed2.atipicial.com:20333".to_string(),
                "seed3.atipicial.com:20333".to_string(),
                "seed4.atipicial.com:20333".to_string(),
                "seed5.atipicial.com:20333".to_string(),
            ],
    ...
    /// The workspace's operational default protocol settings (Atipicial N3 MainNet).
    ///
