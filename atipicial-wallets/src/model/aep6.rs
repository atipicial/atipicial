//! AEP-6 wallet implementation.
//!
//! This module provides the AEP-6 wallet standard as implemented in the
//! C# Atipicial node.  It mirrors the behaviour of `Atipicial.Wallets.AEP6` exactly,
//! enabling interoperability between the Rust and C# wallets.

use crate::key_pair::KeyPair;
use crate::version::Version;
use crate::wallet::{Wallet, WalletError, WalletResult};
use crate::wallet_account::{StandardWalletAccount, WalletAccount};
use crate::wallet_helper::WalletAddress;
use base64::{Engine as _, engine::general_purpose};
use atipicial_config::ProtocolSettings;
use atipicial_payloads::{Transaction, Witness};
use atipicial_primitives::ContractParameterType;
use atipicial_primitives::{UInt160, UInt256};
use atipicial_vm::Contract;
use parking_lot::RwLock;
use serde::{Deserialize, Serialize};
use serde_json::Value;
use std::collections::HashMap;
use std::fs;
use std::sync::Arc;

/// AEP-6 wallet representation matching C# `AEP6Wallet`.
#[derive(Debug, Clone)]
pub struct Aep6Wallet {
    name: Option<String>,
    path: Option<String>,
    version: Version,
    scrypt: ScryptParameters,
    accounts: Arc<RwLock<HashMap<UInt160, Arc<Aep6Account>>>>,
    extra: Option<Value>,
    protocol_settings: Arc<ProtocolSettings>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct Aep6WalletFile {
    name: Option<String>,
    version: String,
    scrypt: ScryptParameters,
    accounts: Vec<Aep6AccountFile>,
    extra: Option<Value>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct Aep6AccountFile {
    address: String,
    label: Option<String>,
    #[serde(rename = "isDefault")]
    is_default: bool,
    lock: bool,
    key: Option<String>,
    contract: Option<Aep6ContractFile>,
    extra: Option<Value>,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct Aep6ContractParameterFile {
    name: String,
    #[serde(rename = "type")]
    param_type: String,
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct Aep6ContractFile {
    script: String,
    parameters: Vec<Aep6ContractParameterFile>,
    deployed: bool,
}

/// AEP-6 account implementation (C# `AEP6Account`).
#[derive(Debug, Clone)]
pub struct Aep6Account {
    inner: StandardWalletAccount,
    is_default: bool,
    lock: bool,
    extra: Option<Value>,
    parameter_names: Vec<String>,
    /// Whether the account's contract is already deployed on chain
    /// (C# `AEP6Contract.Deployed`). Preserved across load/save.
    deployed: bool,
    _wallet: Arc<Aep6Wallet>,
}

/// AEP-6 contract wrapper (C# `AEP6Contract`).
#[derive(Debug, Clone)]
pub struct Aep6Contract {
    /// Verification contract associated with the AEP-6 account.
    pub contract: Contract,
    /// Contract parameter names in verification parameter order.
    pub parameter_names: Vec<String>,
    /// Whether the contract is already deployed on chain.
    pub deployed: bool,
}

/// Scrypt parameters used for AEP-2 encryption (C# `ScryptParameters`).
#[derive(Debug, Clone, Serialize, Deserialize)]
pub struct ScryptParameters {
    /// CPU/memory cost parameter.
    pub n: u32,
    /// Block-size parameter.
    pub r: u32,
    /// Parallelization parameter.
    pub p: u32,
}

impl ScryptParameters {
    /// Return the AEP-6 default scrypt parameters.
    pub fn default_nep6() -> Self {
        Self {
            n: 16384,
            r: 8,
            p: 8,
        }
    }
}

impl Default for ScryptParameters {
    fn default() -> Self {
        Self::default_nep6()
    }
}

impl Aep6Wallet {
    /// Create an empty AEP-6 wallet bound to the given protocol settings.
    pub fn new(
        name: Option<String>,
        path: Option<String>,
        settings: Arc<ProtocolSettings>,
    ) -> Self {
        Self {
            name,
            path,
            version: Version::new(1, 0, 0),
            scrypt: ScryptParameters::default_nep6(),
            accounts: Arc::new(RwLock::new(HashMap::new())),
            extra: None,
            protocol_settings: settings,
        }
    }

    /// Load a AEP-6 wallet file and unlock encrypted accounts with `password`.
    pub fn from_file(
        path: &str,
        password: &str,
        settings: Arc<ProtocolSettings>,
    ) -> WalletResult<Self> {
        Self::from_file_with_password(path, Some(password), settings)
    }

    /// Load a AEP-6 wallet file, optionally leaving encrypted accounts locked.
    pub fn from_file_with_password(
        path: &str,
        password: Option<&str>,
        settings: Arc<ProtocolSettings>,
    ) -> WalletResult<Self> {
        let content = fs::read_to_string(path)?;
        let wallet_file: Aep6WalletFile = serde_json::from_str(&content)
            .map_err(|e| WalletError::Other(format!("Invalid wallet format: {e}")))?;
        let version =
            Version::parse(&wallet_file.version).map_err(|e| WalletError::Other(e.to_string()))?;

        let wallet = Self {
            name: wallet_file.name.clone(),
            path: Some(path.to_string()),
            version,
            scrypt: wallet_file.scrypt.clone(),
            accounts: Arc::new(RwLock::new(HashMap::new())),
            extra: wallet_file.extra.clone(),
            protocol_settings: settings.clone(),
        };

        let mut account_map = HashMap::new();
        for account_file in wallet_file.accounts {
            let account = Aep6Account::from_file(account_file, &wallet, password)?;
            account_map.insert(account.script_hash(), Arc::new(account));
        }

        *wallet.accounts.write() = account_map;
        Ok(wallet)
    }

    fn to_file(&self) -> WalletResult<Aep6WalletFile> {
        let accounts: Vec<_> = self.accounts.read().values().cloned().collect();
        let account_files = accounts
            .iter()
            .map(|account| account.to_file())
            .collect::<WalletResult<Vec<_>>>()?;

        Ok(Aep6WalletFile {
            name: self.name.clone(),
            version: self.version.to_string(),
            scrypt: self.scrypt.clone(),
            accounts: account_files,
            extra: self.extra.clone(),
        })
    }

    /// Persist the wallet back to its configured file path.
    pub fn persist(&self) -> WalletResult<()> {
        let path = self
            .path
            .as_ref()
            .ok_or_else(|| WalletError::Other("wallet has no path".to_string()))?;
        let wallet_file = self.to_file()?;
        let json = serde_json::to_string_pretty(&wallet_file)
            .map_err(|e| WalletError::Other(e.to_string()))?;
        fs::write(path, json)?;
        Ok(())
    }

    fn protocol_settings(&self) -> &Arc<ProtocolSettings> {
        &self.protocol_settings
    }

    fn add_account(&self, account: Aep6Account) {
        self.accounts
            .write()
            .insert(account.script_hash(), Arc::new(account));
    }
}

impl Wallet for Aep6Wallet {
    type Account = Aep6Account;

    fn name(&self) -> &str {
        self.name.as_deref().unwrap_or("")
    }

    fn path(&self) -> Option<&str> {
        self.path.as_deref()
    }

    fn version(&self) -> &Version {
        &self.version
    }

    async fn change_password(&self, old_password: &str, new_password: &str) -> WalletResult<bool> {
        if old_password == new_password {
            return Ok(true);
        }

        let mut updated_accounts = Vec::new();

        let accounts: Vec<_> = self
            .accounts
            .read()
            .iter()
            .map(|(hash, account)| (*hash, Arc::clone(account)))
            .collect();

        for (hash, account_arc) in accounts {
            let mut account = (*account_arc).clone();
            if account.inner.aep2_key().is_some() {
                if !account
                    .unlock(old_password)
                    .map_err(|e| WalletError::Other(e.to_string()))?
                {
                    return Err(WalletError::InvalidPassword);
                }

                let key = account.inner.key().ok_or(WalletError::AccountLocked)?;
                let version = self.protocol_settings.address_version;
                let new_aep2 = key
                    .to_aep2(new_password, version)
                    .map_err(|e| WalletError::Other(e.to_string()))?;
                account.inner.set_aep2_key(Some(new_aep2));
                account.inner.lock();
            }

            updated_accounts.push((hash, Arc::new(account)));
        }

        *self.accounts.write() = updated_accounts.into_iter().collect();

        self.persist()?;
        Ok(true)
    }

    fn contains(&self, script_hash: &UInt160) -> bool {
        self.accounts.read().contains_key(script_hash)
    }

    async fn create_account(&self, private_key: &[u8]) -> WalletResult<Arc<Self::Account>> {
        let key_pair = KeyPair::from_private_key(private_key)
            .map_err(|e| WalletError::Other(e.to_string()))?;
        let contract = Contract::create_signature_contract(
            key_pair
                .public_key_point()
                .map_err(|e| WalletError::Other(e.to_string()))?,
        );
        let account = Aep6Account::with_key(self.clone(), key_pair, contract, None);
        let account_arc = Arc::new(account.clone());
        self.add_account(account);
        Ok(account_arc)
    }

    async fn create_account_with_contract(
        &self,
        contract: Contract,
        key_pair: Option<KeyPair>,
    ) -> WalletResult<Arc<Self::Account>> {
        let account = if let Some(key_pair) = key_pair {
            Aep6Account::with_key(self.clone(), key_pair, contract, None)
        } else {
            Aep6Account::watch_only(self.clone(), contract.script_hash(), Some(contract))
        };

        let account_arc = Arc::new(account.clone());
        self.add_account(account);
        Ok(account_arc)
    }

    async fn create_account_watch_only(
        &self,
        script_hash: UInt160,
    ) -> WalletResult<Arc<Self::Account>> {
        let account = Aep6Account::watch_only(self.clone(), script_hash, None);
        let account_arc = Arc::new(account.clone());
        self.add_account(account);
        Ok(account_arc)
    }

    async fn delete_account(&self, script_hash: &UInt160) -> WalletResult<bool> {
        Ok(self.accounts.write().remove(script_hash).is_some())
    }

    async fn export(&self, path: &str, _password: &str) -> WalletResult<()> {
        let wallet_file = self.to_file()?;
        let json = serde_json::to_string_pretty(&wallet_file)
            .map_err(|e| WalletError::Other(e.to_string()))?;
        fs::write(path, json)?;
        Ok(())
    }

    fn account(&self, script_hash: &UInt160) -> Option<Arc<Self::Account>> {
        self.accounts.read().get(script_hash).cloned()
    }

    fn accounts(&self) -> Vec<Arc<Self::Account>> {
        self.accounts.read().values().cloned().collect()
    }

    async fn available_balance(&self, _asset_id: &UInt256) -> WalletResult<i64> {
        Ok(0)
    }

    async fn unclaimed_gas(&self) -> WalletResult<i64> {
        Ok(0)
    }

    async fn import_wif(&self, wif: &str) -> WalletResult<Arc<Self::Account>> {
        let key_pair = KeyPair::from_wif(wif).map_err(|e| WalletError::Other(e.to_string()))?;
        let contract = Contract::create_signature_contract(
            key_pair
                .public_key_point()
                .map_err(|e| WalletError::Other(e.to_string()))?,
        );
        let account = Aep6Account::with_key(self.clone(), key_pair, contract, None);
        let account_arc = Arc::new(account.clone());
        self.add_account(account);
        Ok(account_arc)
    }

    async fn import_aep2(
        &self,
        aep2_key: &str,
        password: &str,
    ) -> WalletResult<Arc<Self::Account>> {
        let version = self.protocol_settings.address_version;
        let key_pair = KeyPair::from_aep2_string(aep2_key, password, version)
            .map_err(|e| WalletError::Other(e.to_string()))?;
        let contract = Contract::create_signature_contract(
            key_pair
                .public_key_point()
                .map_err(|e| WalletError::Other(e.to_string()))?,
        );
        let account =
            Aep6Account::with_key(self.clone(), key_pair, contract, Some(aep2_key.to_string()));
        let account_arc = Arc::new(account.clone());
        self.add_account(account);
        Ok(account_arc)
    }

    async fn sign(&self, data: &[u8], script_hash: &UInt160) -> WalletResult<Vec<u8>> {
        let account = {
            let accounts = self.accounts.read();
            accounts
                .get(script_hash)
                .cloned()
                .ok_or(WalletError::AccountNotFound(*script_hash))?
        };

        if account.inner.is_locked() {
            return Err(WalletError::AccountLocked);
        }

        let key = account.inner.key().ok_or(WalletError::AccountLocked)?;

        key.sign(data)
            .map_err(|e| WalletError::SigningFailed(e.to_string()))
    }

    async fn sign_transaction(&self, transaction: &mut Transaction) -> WalletResult<()> {
        let signer_hashes: Vec<UInt160> = transaction
            .signers()
            .iter()
            .map(|signer| signer.account)
            .collect();

        let relevant_accounts: HashMap<UInt160, Arc<Aep6Account>> = {
            let accounts = self.accounts.read();
            signer_hashes
                .iter()
                .filter_map(|hash| {
                    accounts
                        .get(hash)
                        .map(|account| (*hash, Arc::clone(account)))
                })
                .collect()
        };

        for hash in signer_hashes {
            if let Some(account) = relevant_accounts.get(&hash) {
                if account.inner.is_locked() {
                    return Err(WalletError::AccountLocked);
                }

                let witness = account.inner.create_witness(transaction)?;
                transaction.add_witness(witness);
            }
        }

        Ok(())
    }

    async fn unlock(&self, password: &str) -> WalletResult<bool> {
        let mut updated_accounts = Vec::new();

        let accounts: Vec<_> = self
            .accounts
            .read()
            .iter()
            .map(|(hash, account)| (*hash, Arc::clone(account)))
            .collect();

        for (hash, account_arc) in accounts {
            let mut account = (*account_arc).clone();
            if account.inner.aep2_key().is_some() {
                if !account.inner.verify_password(password)? {
                    return Err(WalletError::InvalidPassword);
                }
                if account.inner.is_locked() {
                    let unlocked = account
                        .unlock(password)
                        .map_err(|e| WalletError::Other(e.to_string()))?;
                    if !unlocked {
                        return Err(WalletError::InvalidPassword);
                    }
                }
            }
            updated_accounts.push((hash, Arc::new(account)));
        }

        *self.accounts.write() = updated_accounts.into_iter().collect();

        Ok(true)
    }

    fn lock(&self) {
        let mut updated_accounts = Vec::new();

        let accounts: Vec<_> = self
            .accounts
            .read()
            .iter()
            .map(|(hash, account)| (*hash, Arc::clone(account)))
            .collect();

        for (hash, account_arc) in accounts {
            let mut account = (*account_arc).clone();
            account.inner.lock();
            updated_accounts.push((hash, Arc::new(account)));
        }

        *self.accounts.write() = updated_accounts.into_iter().collect();
    }

    async fn verify_password(&self, password: &str) -> WalletResult<bool> {
        let accounts: Vec<_> = self.accounts.read().values().cloned().collect();
        for account in accounts {
            if account.inner.aep2_key().is_some() && !account.inner.verify_password(password)? {
                return Ok(false);
            }
        }
        Ok(true)
    }

    async fn save(&self) -> WalletResult<()> {
        self.persist()
    }

    fn default_account(&self) -> Option<Arc<Self::Account>> {
        self.accounts
            .read()
            .values()
            .find(|account| account.is_default)
            .cloned()
    }

    async fn set_default_account(&self, script_hash: &UInt160) -> WalletResult<()> {
        let mut updated_accounts = Vec::new();
        let mut found = false;

        let accounts: Vec<_> = self
            .accounts
            .read()
            .iter()
            .map(|(hash, account)| (*hash, Arc::clone(account)))
            .collect();

        for (hash, account_arc) in accounts {
            let mut account = (*account_arc).clone();
            let is_target = &hash == script_hash;
            account.is_default = is_target;
            if is_target {
                found = true;
            }
            updated_accounts.push((hash, Arc::new(account)));
        }

        if !found {
            return Err(WalletError::AccountNotFound(*script_hash));
        }

        *self.accounts.write() = updated_accounts.into_iter().collect();

        self.persist()?;
        Ok(())
    }
}

impl Aep6Account {
    fn script_hash(&self) -> UInt160 {
        self.inner.script_hash()
    }

    fn from_file(
        file: Aep6AccountFile,
        wallet: &Aep6Wallet,
        password: Option<&str>,
    ) -> WalletResult<Self> {
        let script_hash =
            WalletAddress::to_script_hash(&file.address, wallet.protocol_settings.address_version)
                .map_err(|e| WalletError::Other(e.to_string()))?;

        let parsed_contract = match &file.contract {
            Some(contract_file) => Some(Aep6Contract::from_file(contract_file)?),
            None => None,
        };

        let contract_data = parsed_contract
            .as_ref()
            .map(|contract| contract.contract.clone());

        let mut inner = if let Some(ref aep2_key) = file.key {
            StandardWalletAccount::new_from_encrypted(
                script_hash,
                aep2_key.clone(),
                contract_data.clone(),
                Arc::clone(&wallet.protocol_settings),
            )
        } else {
            StandardWalletAccount::new_watch_only(
                script_hash,
                contract_data.clone(),
                Arc::clone(&wallet.protocol_settings),
            )
        };
        // C# `AEP6Account` decrypts with `wallet.Scrypt.N/R/P`; thread the wallet's
        // scrypt parameters so a wallet created with non-default values unlocks.
        inner.set_scrypt_parameters(wallet.scrypt.n, wallet.scrypt.r, wallet.scrypt.p);

        let deployed = parsed_contract
            .as_ref()
            .map(|contract| contract.deployed)
            .unwrap_or(false);
        let parameter_names = parsed_contract
            .map(|contract| contract.parameter_names)
            .or_else(|| contract_data.as_ref().map(default_parameter_names))
            .unwrap_or_default();

        let mut account = Self {
            inner,
            is_default: file.is_default,
            lock: file.lock,
            extra: file.extra,
            parameter_names,
            deployed,
            _wallet: Arc::new(wallet.clone()),
        };

        if account.inner.aep2_key().is_some() {
            if let Some(password) = password {
                let unlocked = account
                    .inner
                    .unlock(password)
                    .map_err(|e| WalletError::Other(e.to_string()))?;
                if !unlocked {
                    return Err(WalletError::InvalidPassword);
                }
                if account.lock {
                    account.inner.lock();
                }
            }
        }

        Ok(account)
    }

    fn to_file(&self) -> WalletResult<Aep6AccountFile> {
        let address = self.inner.address();
        let contract = self
            .inner
            .contract()
            .map(|contract| Aep6Contract::to_file(contract, &self.parameter_names, self.deployed));

        Ok(Aep6AccountFile {
            address,
            label: self.inner.label().map(|s| s.to_string()),
            is_default: self.is_default,
            lock: self.lock,
            key: self.inner.aep2_key().map(|s| s.to_string()),
            contract,
            extra: self.extra.clone(),
        })
    }

    fn with_key(
        wallet: Aep6Wallet,
        key_pair: KeyPair,
        contract: Contract,
        aep2_key: Option<String>,
    ) -> Self {
        let parameter_names = default_parameter_names(&contract);
        let mut inner = StandardWalletAccount::new_with_key(
            key_pair,
            Some(contract.clone()),
            Arc::clone(wallet.protocol_settings()),
            aep2_key,
        );
        inner.set_scrypt_parameters(wallet.scrypt.n, wallet.scrypt.r, wallet.scrypt.p);

        Self {
            inner,
            is_default: false,
            lock: false,
            extra: None,
            parameter_names,
            // A locally key-generated account's contract is a signature contract,
            // not an on-chain deployed contract (C# AEP6Account default).
            deployed: false,
            _wallet: Arc::new(wallet),
        }
    }

    fn watch_only(wallet: Aep6Wallet, script_hash: UInt160, contract: Option<Contract>) -> Self {
        let parameter_names = contract
            .as_ref()
            .map(default_parameter_names)
            .unwrap_or_default();
        let inner = StandardWalletAccount::new_watch_only(
            script_hash,
            contract,
            Arc::clone(wallet.protocol_settings()),
        );

        Self {
            inner,
            is_default: false,
            lock: false,
            extra: None,
            parameter_names,
            // A watch-only account added by script hash has no known deployed
            // flag; default false (C# AEP6Account default).
            deployed: false,
            _wallet: Arc::new(wallet),
        }
    }
}

impl WalletAccount for Aep6Account {
    fn script_hash(&self) -> UInt160 {
        self.inner.script_hash()
    }

    fn address(&self) -> String {
        self.inner.address()
    }

    fn label(&self) -> Option<&str> {
        self.inner.label()
    }

    fn set_label(&mut self, label: Option<String>) {
        self.inner.set_label(label);
    }

    fn is_default(&self) -> bool {
        self.is_default
    }

    fn set_is_default(&mut self, is_default: bool) {
        self.is_default = is_default;
        self.inner.set_is_default(is_default);
    }

    fn is_locked(&self) -> bool {
        self.inner.is_locked()
    }

    fn has_key(&self) -> bool {
        self.inner.has_key()
    }

    fn key(&self) -> Option<KeyPair> {
        self.inner.key()
    }

    fn contract(&self) -> Option<&Contract> {
        self.inner.contract()
    }

    fn set_contract(&mut self, contract: Option<Contract>) {
        if let Some(contract) = contract {
            self.parameter_names = default_parameter_names(&contract);
            self.inner.set_contract(Some(contract));
        } else {
            self.parameter_names.clear();
            self.inner.set_contract(None);
        }
    }

    fn protocol_settings(&self) -> &Arc<ProtocolSettings> {
        self.inner.protocol_settings()
    }

    fn unlock(&mut self, password: &str) -> WalletResult<bool> {
        self.inner.unlock(password)
    }

    fn lock(&mut self) {
        self.inner.lock();
    }

    fn verify_password(&self, password: &str) -> WalletResult<bool> {
        self.inner.verify_password(password)
    }

    fn export_wif(&self) -> WalletResult<String> {
        self.inner.export_wif()
    }

    fn export_aep2(&self, password: &str) -> WalletResult<String> {
        self.inner.export_aep2(password)
    }

    fn create_witness(&self, transaction: &Transaction) -> WalletResult<Witness> {
        self.inner.create_witness(transaction)
    }
}

impl Aep6Contract {
    fn from_file(file: &Aep6ContractFile) -> WalletResult<Self> {
        // C# `AEP6Contract.FromJson` decodes `script` via `Convert.FromBase64String`
        // (AEP6Contract.cs:29); it must be Base64, not hex, for atipicial-cli/atipicial-gui interop.
        let script = general_purpose::STANDARD
            .decode(&file.script)
            .map_err(|e| WalletError::Other(e.to_string()))?;
        let mut parameter_names = Vec::with_capacity(file.parameters.len());
        let parameter_types = file
            .parameters
            .iter()
            .map(|parameter| {
                parameter_names.push(parameter.name.clone());
                parse_parameter_type(&parameter.param_type)
            })
            .collect::<WalletResult<Vec<_>>>()?;
        let contract = Contract::create(parameter_types, script);
        Ok(Self {
            contract,
            parameter_names,
            deployed: file.deployed,
        })
    }

    fn to_file(
        contract: &Contract,
        parameter_names: &[String],
        deployed: bool,
    ) -> Aep6ContractFile {
        let parameters = contract
            .parameter_list
            .iter()
            .enumerate()
            .map(|(index, param)| Aep6ContractParameterFile {
                name: parameter_names
                    .get(index)
                    .cloned()
                    .unwrap_or_else(|| format!("parameter{index}")),
                param_type: param.as_str().to_string(),
            })
            .collect();

        Aep6ContractFile {
            // C# `AEP6Contract.ToJson` encodes `script` via `Convert.ToBase64String`
            // (AEP6Contract.cs:39); it must be Base64, not hex, for atipicial-cli/atipicial-gui interop.
            script: general_purpose::STANDARD.encode(&contract.script),
            parameters,
            // C# `AEP6Contract.ToJson` writes the real `Deployed` value
            // (AEP6Contract.cs:47); hardcoding false made a deployed-contract
            // account round-trip as non-deployed.
            deployed,
        }
    }
}

fn parse_parameter_type(name: &str) -> WalletResult<ContractParameterType> {
    let kind = match name {
        "Any" => ContractParameterType::Any,
        "Boolean" => ContractParameterType::Boolean,
        "Integer" => ContractParameterType::Integer,
        "ByteArray" => ContractParameterType::ByteArray,
        "String" => ContractParameterType::String,
        "Hash160" => ContractParameterType::Hash160,
        "Hash256" => ContractParameterType::Hash256,
        "PublicKey" => ContractParameterType::PublicKey,
        "Signature" => ContractParameterType::Signature,
        "Array" => ContractParameterType::Array,
        "Map" => ContractParameterType::Map,
        "InteropInterface" => ContractParameterType::InteropInterface,
        "Void" => ContractParameterType::Void,
        other => {
            return Err(WalletError::Other(format!(
                "Unsupported contract parameter type: {other}"
            )));
        }
    };

    Ok(kind)
}

fn default_parameter_names(contract: &Contract) -> Vec<String> {
    if contract.parameter_list.len() == 1
        && contract.parameter_list[0] == ContractParameterType::Signature
    {
        vec!["signature".to_string()]
    } else {
        contract
            .parameter_list
            .iter()
            .enumerate()
            .map(|(index, _)| format!("parameter{index}"))
            .collect()
    }
}

#[cfg(test)]
#[path = "../tests/model/aep6.rs"]
mod tests;
