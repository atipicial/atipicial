...
//!
//! These handlers read native ATC/GAS contract state through the RPC native
...
impl RpcServerBlockchain {
    pub(super) fn get_native_contracts(
        ...
        for contract in registry.contracts() {
            let state = deployed_contracts
                ...
                .or_else(|| {
                    <atipicial_native_contracts::StandardNativeContract as NativeContract<
    ...
    pub(super) fn get_next_block_validators(
        ...
        let validators = native_queries::NativeQueries::atipicial_next_block_validators(
            server,
            Arc::clone(&snapshot),
            &atipicial_hash,
        )
        ...
        for point in validators {
            let votes = native_queries::NativeQueries::atipicial_candidate_vote(
                server,
                Arc::clone(&snapshot),
                &atipicial_hash,
                &point,
    ...
    pub(super) fn get_candidates(
        ...
        let atipicial_hash = native_queries::NativeQueries::atipicial_script_hash();
        let candidates =
            native_queries::NativeQueries::atipicial_candidates(server, Arc::clone(&snapshot), &atipicial_hash)
                .map_err(|_| {
                    RpcException::from(
    ...
    pub(super) fn get_committee(
        ...
        let snapshot = Arc::new(store.data_cache().clone());
        let atipicial_hash = native_queries::NativeQueries::atipicial_script_hash();
        let committee = native_queries::NativeQueries::atipicial_committee(server, snapshot, &atipicial_hash)
            .map_err(|err| {
