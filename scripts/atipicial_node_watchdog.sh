...
NODE_LOG="${NODE_LOG:-logs/atipicial-node-watchdog-node.log}"
WATCHDOG_LOG="${WATCHDOG_LOG:-logs/atipicial-node-watchdog.log}"
RUST_LOG_LEVEL="${RUST_LOG_LEVEL:-warn}"
NODE_CONFIG="${NODE_CONFIG:-atipicial_testnet_node.toml}"
NODE_STORAGE="${NODE_STORAGE:-}"

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
NODE_BIN="${NODE_BIN:-$ROOT/target/release/atipicial-node}"
if [[ ! -x "$NODE_BIN" && -x "$ROOT/target/debug/atipicial-node" ]]; then
  NODE_BIN="$ROOT/target/debug/atipicial-node"
fi
if [[ ! -x "$NODE_BIN" ]]; then
  echo "error: atipicial-node binary not found at $ROOT/target/{release,debug}/atipicial-node" >&2
  exit 1
fi
...
start_node() {
  ...
  pkill -f "atipicial-node --config $NODE_CONFIG" >/dev/null 2>&1 || true
  env \
