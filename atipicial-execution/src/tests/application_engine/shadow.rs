...
use atipicial_manifest::{
    ...
    ManifestFeatures, AefFile, WildCardContainer,
};
use atipicial_payloads::Block;
use atipicial_primitives::{CallFlags, ContractParameterType, TriggerType, UInt160};
use atipicial_storage::{DataCache, StorageItem, StorageKey};
use atipicial_vm::{HardforkTableIdentity, OpCode, Script, Slot, SpecializationMode, StackItem, VmState};
...
fn prepare_exact_candidate(
    ...
    const CONTRACT_AEF_CHECKSUM: u32 = 2_962_741_568;
    ...
    let mut aef = AefFile::new("shadow-pair-test".to_string(), script_bytes.clone());
    aef.checksum = CONTRACT_AEF_CHECKSUM;
...
fn alias_cycle_contract(hash: UInt160) -> ContractState {
    ...
    ContractState::new(
        27,
        hash,
        AefFile::new("observed-echo".to_string(), vec![OpCode::RET.byte()]),
        manifest,
    )
}
...
impl crate::Diagnostic for ToggleDiagnostic {
    ...
    fn pre_execute_instruction(&mut self, _instruction: &atipicial_vm::Instruction) {}

    fn post_execute_instruction(&mut self, _instruction: &atipicial_vm::Instruction) {}
}
