...
impl MdbxSnapshot {
        ...
        let (data_table, read_tx, initialization_error) = match store.read_txn() {
            Ok(read_tx) => match read_tx.open_table(store.data_table_name()) {
                ...
                Err(err) => {
                    ...
                    error!(target: "atipicial", error = %error, "MDBX snapshot table open failed");
            ...
            Err(err) => {
                ...
                error!(target: "atipicial", error = %error, "MDBX snapshot open failed");
                (None, None, Some(error))
            }
        };
        ...
        K: AsRef<[u8]>,
    {
        ...
        self.read_entries_parallel(read_tx.id(), &borrowed_keys, parallelism, false)
            .or_else(|error| {
                error!(target: "atipicial", error = %error, "parallel MDBX batch read failed; preserving correctness with the frozen serial snapshot");
...
impl ReadOnlyStoreGeneric<Vec<u8>, Vec<u8>> for MdbxSnapshot {
    ...
    fn try_get(&self, key: &Vec<u8>) -> Option<Vec<u8>> {
        match self.try_get_result(key) {
            Ok(value) => value,
            Err(err) => {
                error!(target: "atipicial", error = %err, "MDBX snapshot get failed - this is a critical error that may cause incorrect state");
                None
            }
        }
    }
    ...
    fn find(
        ...
        match self.collect_entries(key_prefix.map(Vec::as_slice), direction) {
            ...
            Err(err) => {
                error!(target: "atipicial", error = %err, "MDBX snapshot find failed - this may cause incorrect state");
                Vec::new().into_iter()
            }
        }
    }
