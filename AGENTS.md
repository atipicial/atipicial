...
# atipicial-rs Development Guide for Agents

atipicial-rs is a production Atipicial N3 full node in Rust. Correctness is defined by Atipicial
N3 v3.10.1 protocol behavior, Atipicial C# parity, and deterministic MainNet replay.
...
1. Deterministic Atipicial N3 v3.10.1 protocol correctness.
...
- Foundation: `atipicial-primitives` owns small domain value types.
- Infrastructure: `atipicial-config`, `atipicial-crypto`, `atipicial-trie`, `atipicial-io`,
  `atipicial-error`, `atipicial-storage`, `atipicial-static-files`, `atipicial-state-packs`,
  `atipicial-checkpoint`, `atipicial-vm`, `atipicial-serialization`, and `atipicial-manifest` own
  mechanical concerns.
- Protocol: `atipicial-payloads`, `atipicial-consensus`, and `atipicial-hsm` own wire payloads,
  dBFT, and consensus signing.
- Domain services: `atipicial-runtime`, `atipicial-execution`, `atipicial-native-contracts`,
  `atipicial-state-service`, and `atipicial-mempool` own typed service contracts and state
  transitions.
- Node services: `atipicial-blockchain`, `atipicial-network`, `atipicial-wallets`, `atipicial-indexer`,
  and `atipicial-oracle-service` own concrete node capabilities.
- Composition: `atipicial-system` wires concrete implementations into an embeddable
  node without owning protocol policy.
- RPC boundary: `atipicial-rpc` owns RPC models, transport, handlers, and RPC plugin
  adapters. Node-level plugin lifecycle and policy are composed in `atipicial-node`.
- Application: `atipicial-node` owns CLI, configuration loading, process lifecycle,
...
- **Types and cryptography.** `atipicial-primitives` owns protocol-neutral Atipicial value
  types (`UInt160`, `UInt256`, inventory and witness records). `atipicial-crypto`
  owns hashes, Merkle construction, curves, keys, and signatures. Payload
  encoding stays in `atipicial-payloads`/`atipicial-serialization`; do not add a second
  hash, key, address, or stack-value representation to a service crate.
- **Chain specification.** `atipicial-config::AtipicialChainSpec` is immutable and shared
...
- **Storage and providers.** `atipicial-storage` owns `Store`,
  `TransactionalStore`, typed table codecs, `StoreCache`, and durable commit
  fences. `atipicial-blockchain` and `atipicial-state-service` own Ledger/State provider
...
- **Execution and engine.** `atipicial-execution::ApplicationEngine<P, D, B>` is
  composed from a native provider, diagnostic policy, and cache backing. The
  caller chooses concrete generic collaborators; `atipicial-vm` remains the only VM
...
- **Pool and import engine.** `atipicial-mempool::MemoryPool<P>` owns admission,
  ...
  the boundary. `atipicial-runtime::BlockImport`/`ImportQueue` own the generic
  import contract and bounded preflight; `atipicial-blockchain` owns the canonical
  execution and durable publication implementation.
- **P2P.** `atipicial-network::PeerRegistry` owns live peers and pending connection
...
- **RPC and application.** `atipicial-rpc` owns wire models, codecs, client/server
  transport, and plugin method groups. `atipicial-node` owns daemon lifecycle and
...
- Workspace `atipicial-vm` is the sole VM semantic authority. Do not introduce
  `atipicial-vm-rs`, `StackValue`, parallel VM object graphs, or stack-item conversion
  bridges.
- `atipicial-execution` owns the Atipicial host/application engine around `atipicial-vm`; native
  contract behavior belongs in `atipicial-native-contracts`.
- Preserve byte-for-byte Atipicial C# behavior for codecs, hashes, witnesses, AEF,
  manifests, native state, storage keys, MPT nodes, and network payloads.
- `atipicial-network` owns `MessageCommand`, `MessageCommandParseError`, and
  `NetworkError`; shared values such as `InventoryType` remain in
  `atipicial-primitives` and must not be re-exported through network facades.
...
`AtipicialChainSpec` is the immutable source of truth for a Atipicial network. It owns or
...
- Pass a shared `Arc<AtipicialChainSpec>` from application configuration through
...
- When migrating chain rules into `AtipicialChainSpec`, update all callers and delete
...
typed composition and provider patterns while retaining Atipicial dBFT, AtipicialVM,
native-contract, and StateService semantics.

## Rust Architecture

- Top-level functions should read as ordered Atipicial domain operations. Move codec,
...
- Atipicial P2P framing has one canonical command-to-payload decode and encode path.
...
- Parallelize only work with an explicit ordering and conflict contract.
  ...
  height order. `atipicial_execution::optimistic_execution` currently owns isolation,
...
- Signature preverification may overlap tentative execution, but tickets and
  caches are advisory exact-input artifacts. Canonical AtipicialVM witness validation
...
cargo test -p atipicial-tests --test layer_boundary_tests
git diff --check
```
