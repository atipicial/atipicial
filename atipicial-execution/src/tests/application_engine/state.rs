...
impl SnapshotPolicyProvider {
    ...
    fn read_u32<B: atipicial_storage::CacheRead>(
...
fn echidna_without_gorgon_selects_not_gorgon_table_like_csharp_v3101() {
    ...
    // to their pre-atipicial-vm#543 handlers and SHL/SHR reverted to pre-atipicial-vm#567.
    let mut settings = ProtocolSettings::default();
    settings.hardforks = atipicial_config::HardforkSchedule::new();
    settings.hardforks = settings.hardforks.with_activation(Hardfork::HfEchidna, 0);
    let selected = ApplicationEngine::<NoNativeContractProvider>::select_jump_table(&settings, 0);
...
fn prepare_next_transaction_refreshes_policy_values_without_resetting_snapshot_changes() {
    // Atipicial v3.10.1 Blockchain.Persist creates each transaction engine from the
...
/// Consensus-parity: C# Atipicial (v3.10.1) has **no** instruction-count cap on the
/// execution path — bounding is done purely by gas. atipicial-rs previously enforced
