...
use anyhow::Context;
use atipicial_state_packs::authority::AUTHORITATIVE_HIGH_WATER_KEY;
...
pub(super) struct StateServiceRuntime {
    pub(super) state_store: Option<Arc<atipicial_state_service::StateStore<RuntimeStore>>>,
    pub(super) state_service:
        Option<Arc<atipicial_state_service::commit_handlers::StateServiceCommitHandlers<RuntimeStore>>>,
...
pub(super) fn build_state_service_runtime(
    ...
    let state_service = match (state_store.as_ref(), coordinated) {
        ...
        (Some(state_store), false) if use_async_state_pipeline => {
            let handlers =
                atipicial_state_service::commit_handlers::StateServiceCommitHandlers::try_new_async_with_limits(
                    Arc::clone(state_store),
...
fn build_state_store(
    ...
    Option<Arc<atipicial_state_service::StateStore<RuntimeStore>>>,
    Option<ServiceStore>,
    bool,
    ...
    let state_store = if coordinated {
        ...
        if config.storage.state_packs.enabled {
            ...
            let pack_options = atipicial_state_packs::PackStoreOptions {
            ...
            let factory: Arc<dyn atipicial_state_service::MptNodeSnapshotFactory> = authority.clone();
            let state_store =
            ...
            Arc::new(
                atipicial_state_service::StateStore::with_mpt_store_options(
                    config.state_service.full_state,
                    config.state_service.defer_full_state_finalization,
                    backing,
                )
