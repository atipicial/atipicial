...
DEFAULT_PROBE_BIN = "target/debug/atipicial-db-probe"
...
def storage_provider_from_config(config_path: Path) -> str:
    with config_path.open("rb") as handle:
        config = tomllib.load(handle)
    storage = config.get("storage") or {}
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--node-config",
        default=None,
        help="Optional atipicial-node TOML used to infer --data-dir and --chain-db.",
    )
    parser.add_argument(
        "--data-dir",
        default=None,
        help=f"atipicial-rs data root passed to checkpoint-on-height (default: {DEFAULT_DATA_DIR})",
    )
    parser.add_argument(
        "--chain-db",
        default=None,
    ...
    parser.add_argument(
        "--writer-pid",
        default="none",
        help="atipicial-node writer PID, or 'none' for stopped/offline data dirs",
    )
    parser.add_argument(
        "--script",
    ...
    parser.add_argument(
        ...
        help=f"atipicial-db-probe binary used for restore verification (default: {DEFAULT_PROBE_BIN})",
    )
    parser.add_argument(
        "--execute",
        action="store_true",
