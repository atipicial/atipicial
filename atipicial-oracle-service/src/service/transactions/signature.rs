    ...
    pub(in super::super) fn send_response_signature(
        ...
        tokio::spawn(async move {
            let public_key = match key.public_key_point() {
                Ok(pk) => pk,
                Err(e) => {
                    warn!(target: "atipicial::oracle", %e, "failed to get public key point for oracle signature");
                    return;
                }
            };
            ...
            let sign = match key.sign(&message) {
                Ok(sign) => sign,
                Err(e) => {
                    warn!(target: "atipicial::oracle", %e, "failed to sign oracle response message");
                    return;
                }
            };
            ...
            for node in settings.nodes.iter() {
                let url = match url::Url::parse(node) {
                    Ok(url) => url,
                    Err(_) => {
                        warn!(target: "atipicial::oracle", node = %node, "invalid oracle node endpoint");
                        continue;
                    }
                };
                let res = https
                ...
                match res {
                    ...
                    Err(err) => {
                        warn!(target: "atipicial::oracle", node = %node, %err, "failed to send oracle signature");
                    }
                }
            }
        })
