...
fn block_backed_transaction_container_borrows_payload_without_cloning() {
    ...
    assert_eq!(
        atipicial_primitives::Verifiable::hash(&container).expect("container hash"),
...
fn serializable_payload_hash_is_single_sha256_of_unsigned_transaction() {
    ...
    assert_eq!(
        <Transaction as atipicial_primitives::SerializablePayload>::hash(&tx),
        expected_single
    );
...
fn verifiable_hash_uses_transaction_hash_cache() {
    ...
    let expected_hash = <Transaction as atipicial_primitives::Verifiable>::hash(&tx)
...
fn serializable_payload_hash_fails_closed_for_oversized_script() {
    let tx = transaction_with_script(vec![OpCode::NOP.byte(); u16::MAX as usize + 1]);
    let trait_hash = <Transaction as atipicial_primitives::SerializablePayload>::hash(&tx);
...
fn verifiable_hash_rejects_oversized_script() {
    ...
    assert!(<Transaction as atipicial_primitives::Verifiable>::hash(&tx).is_err());
}
