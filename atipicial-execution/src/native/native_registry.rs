...
//! `atipicial-native-contracts::populate_standard_native_contracts` or
//! similar). This keeps the dependency direction:
//!
//! ```text
//! atipicial-execution         ──> defines NativeContract / NativeRegistry
//!       ▲
//!       │ implements
//! atipicial-native-contracts  ──> provides BaseNativeContract + the 11 standard contracts
//! ```
    ...
    P: NativeContractProvider + 'static,
{
    ...
    /// `atipicial-native-contracts` to break the crate-level dependency
    /// cycle between the engine (consumer) and the native contracts
    /// (provider). Callers that need the standard contracts should
    /// use `atipicial_native_contracts::populate_standard_native_contracts`
