//! Common dBFT extensible-payload envelope helpers.
//!
//! Atipicial N3 stores consensus messages in `ExtensiblePayload.Data` as the
//! `DBFTPlugin` wire envelope: common message metadata followed by the
//! per-message body. Individual message modules own their body codecs; this
//! module owns only the shared envelope.
...
impl ConsensusPayload {
    ...
    /// Serializes this consensus message using the Atipicial N3 `DBFTPlugin` on-wire format:
    ...
    /// Sets the witness invocation script.
