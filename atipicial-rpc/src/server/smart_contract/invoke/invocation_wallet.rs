...
use atipicial_wallets::{Aep6Account, Aep6Wallet, Wallet, WalletAccount, WalletError};
...
const TRANSACTION_TYPE_NAME: &str = "Atipicial.Network.P2P.Payloads.Transaction";
...
struct PendingSignatureItem {
    ...
    parameter_types: Vec<atipicial_primitives::ContractParameterType>,
}
...
fn build_and_sign_transaction<B: CacheRead>(
    ...
    let account_script = |hash: &atipicial_primitives::UInt160| -> Option<Vec<u8>> {
        wallet
            .account(hash)
    ...
    if available < required_fee {
        return Err(CoreError::other(
            "Insufficient ATD balance to pay system and network fees.",
        ));
    }
...
fn build_account_witness(
    ...
    invocation.push(atipicial_vm::OpCode::PUSHDATA1.byte());
...
fn build_pending_item(
    account: UInt160,
    wallet_account: Option<Arc<Aep6Account>>,
