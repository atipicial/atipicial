//! Atipicial Merkle tree construction for block and MerkleBlock payloads.
//!
//! The root algorithm is protocol data, not an interchangeable generic tree:
//! odd leaves are duplicated and parent hashes are Atipicial `Hash256(left || right)`
//! values over little-endian `UInt256` bytes. Generic Merkle crates can provide
//! useful proof machinery, but this module keeps the Atipicial/C# byte semantics at
//! the boundary.
...
/// Atipicial-compatible Merkle tree used by ledger validation and network payloads.
#[derive(Debug)]
...
#[cfg(test)]
#[path = "../tests/hashes/merkle_tree.rs"]
mod tests;
