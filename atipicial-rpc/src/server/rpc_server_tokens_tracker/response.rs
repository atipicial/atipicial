...
use crate::plugins::tokens_tracker::{Aep11TransferKey, TokenBalance, TokenTransfer};
...
pub(super) fn account_balances(
    ...
    json!({
        "address": atipicial_wallets::wallet_helper::WalletAddress::to_address(script_hash, address_version),
        "balance": balances
    })
}
    ...
    K: TokenTransferKeyView,
{
    let transfer_address = if value.user_script_hash == UInt160::zero() {
        ...
        Value::String(atipicial_wallets::wallet_helper::WalletAddress::to_address(
...
pub(super) fn aep11_transfer_entry(
    key: &Aep11TransferKey,
    value: &TokenTransfer,
    address_version: u8,
) -> Value {
...
pub(super) fn aep17_balance_entry(
    asset: &UInt160,
    name: &str,
    symbol: &str,
    decimals: u32,
...
pub(super) fn aep11_balance_entry(
    asset: &UInt160,
    name: &str,
    symbol: &str,
    decimals: u32,
...
pub(super) fn aep11_token_entry(token_id: String, balance: TokenBalance) -> Value {
    json!({
        "tokenid": token_id,
        "amount": balance.balance.to_string(),
        "lastupdatedblock": balance.last_updated_block
