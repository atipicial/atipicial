...
class RpcApiDocsTests(unittest.TestCase):
    def test_atipicial_indexer_documents_page_defaults_and_caps(self):
        text = RPC_API_DOC.read_text(encoding="utf-8")
        section = text.split("### AtipicialIndexer", maxsplit=1)[1].split(
            "### Oracle", maxsplit=1
        )[0]
    ...
    def test_atipicial_indexer_documents_exact_sync_semantics(self):
        text = RPC_API_DOC.read_text(encoding="utf-8")
        section = text.split("### AtipicialIndexer", maxsplit=1)[1].split(
            "### Oracle", maxsplit=1
        )[0]
