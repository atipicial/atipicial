...
fn transfer_roundtrip() {
    ...
    let parsed = RpcAep17Transfer::from_json(&json, &ProtocolSettings::default_settings()).unwrap();
...
fn transfers_roundtrip() {
    ...
    let transfers = RpcAep17Transfers {
    ...
    let parsed =
        RpcAep17Transfers::from_json(&json, &ProtocolSettings::default_settings()).unwrap();
...
#[test]
fn aep17_transfers_to_json_matches_rpc_test_case() {
    let Some(expected) = rpc_case_result("getnep17transfersasync") else {
        return;
    };
    let settings = ProtocolSettings::default_settings();
    let parsed = RpcAep17Transfers::from_json(&expected, &settings).expect("parse");
    let actual = parsed.to_json(&settings);
    assert_eq!(expected.to_string(), actual.to_string());
}

#[test]
fn aep17_transfers_null_address_matches_rpc_test_case() {
