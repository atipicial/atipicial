...
pub const DEFAULT_SETTINGS_PATH: &str = "atipicial_testnet_node.toml";

/// Command-line arguments for the `atipicial-node` daemon.
#[derive(Debug, Parser)]
#[command(name = "atipicial-node", version, about = "Atipicial N3 node daemon")]
pub struct NodeCli {
    ...
    /// The file is the C# Atipicial block-dump format (u32 count, then repeated
    /// i32-size + serialized-Block). Blocks use trusted `verify=false` replay
    /// by default, matching Atipicial C#; combine with `--verify-import-chain` for a
    ...
    ///
    /// Trusted archive replay skips verification by default, matching Atipicial C#.
