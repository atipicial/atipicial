...
use atipicial_native_contracts::{AtipicialDollar, LedgerContract, AtipicialCoin};
...
use atipicial_wallets::wallet_helper::WalletAddress as wallet_helper;
use atipicial_wallets::{KeyPair, Aep6Wallet, Wallet, WalletError};
...
async fn create_wallet_file(password: &str) -> (String, KeyPair, String) {
    ...
    let aep2 = keypair
        .to_aep2(password, settings.address_version)
        .expect("aep2");
    wallet
        .import_aep2(&aep2, password)
        .await
        .expect("import aep2");
