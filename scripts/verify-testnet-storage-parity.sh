...
Example (GAS totalSupply key at height 128955):
  scripts/verify-testnet-storage-parity.sh \
    ...
    -6 0b http://seed1t5.atipicial.com:20332
...
- Override the live-writer guard only for debugging with `ATC_VERIFY_ALLOW_LIVE_DB=1`.
USAGE
}
...
RPC_URL="${6:-http://seed1t5.atipicial.com:20332}"
...
if [[ "${ATC_VERIFY_ALLOW_LIVE_DB:-0}" != "1" ]]; then
  LIVE_WRITER_PID="$(
    ps -eo pid=,args= | awk -v raw="$STORAGE_PATH" -v canon="$STORAGE_PATH_CANON" '
      /atipicial-node/ && (index($0, "--storage " raw) || index($0, "--storage=" raw) || index($0, "--storage " canon) || index($0, "--storage=" canon)) {
        print $1
        exit
      }
    '
  ...
  if [[ -n "$LIVE_WRITER_PID" ]]; then
    echo "detected active atipicial-node writer on this storage path (pid=$LIVE_WRITER_PID)." >&2
    echo "stop the writer before parity checks, or set ATC_VERIFY_ALLOW_LIVE_DB=1 to override." >&2
    exit 1
  fi
fi
...
probe() {
  cargo run -q -p atipicial-node --bin atipicial-db-probe -- --db "$STORAGE_PATH" "$@"
}
...
if [[ -z "$LOCAL_HEIGHT" || -z "$LOCAL_HASH" || "$LOCAL_HASH" == "<none>" ]]; then
  echo "failed to read local height/hash from atipicial-db-probe output:" >&2
  printf '%s\n' "$LOCAL_HEIGHT_OUT" >&2
  printf '%s\n' "$LOCAL_HASH_OUT" >&2
  exit 1
fi
