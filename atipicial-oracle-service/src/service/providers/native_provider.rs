...
use atipicial_native_contracts::{OracleContract, OracleRequest};
...
///
/// This remains local to `atipicial-oracle-service` because `OracleRequest` is owned
/// by `atipicial-native-contracts`; moving it into `atipicial-execution` would reverse the
...
impl OracleContractReadProvider for atipicial_native_contracts::StandardNativeProvider {
    fn oracle_request<B: CacheRead>(
        &self,
        snapshot: &DataCache<B>,
        request_id: u64,
...
pub(in crate::service) trait OracleServiceNativeProvider {
...
#[derive(Clone)]
pub(in crate::service) struct NativeOracleServiceProvider<P>
where
    ...
    P: NativeContractProvider + OracleContractReadProvider,
{
    ...
    #[must_use]
    pub(in crate::service) fn new(native_contract_provider: Arc<P>) -> Self {
        Self {
...
{
    pub(in crate::service) fn native_provider(&self) -> NativeOracleServiceProvider<P> {
