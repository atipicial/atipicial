//! # atipicial-node::node::indexer_runtime
//!
...
use atipicial_blockchain::{BlockchainHandle, RuntimeEvent};
use atipicial_indexer::{IndexerService, NotificationIndexRecord};
use atipicial_rpc::application_logs::ApplicationLogsService;
...
{
    let start_mode =
        indexer_runtime_start_mode(startup_height, atipicial_runtime::sync_metrics::peer_live_tip());
...
async fn wait_until_indexer_runtime_ready(
    ...
    let initial_peer_tip = atipicial_runtime::sync_metrics::peer_live_tip();
    if start_mode == IndexerRuntimeStartMode::StartNow {
        info!(
    ...
    loop {
        ...
        let peer_tip = atipicial_runtime::sync_metrics::peer_live_tip();
...
where
    ...
    N: Fn(&atipicial_payloads::Block) -> Vec<NotificationIndexRecord> + Send + Sync,
{
