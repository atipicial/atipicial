...
//!
//! This module provides the canonical hex helpers for the atipicial-rs workspace.
...
//! - The Atipicial **reversed-hex** (little-endian) format is clearly distinguished
//!   from straight (big-endian) hex.
//!
//! # Straight vs Reversed Hex
//!
//! Atipicial uses **reversed hex** (little-endian byte order) for hash display:
//! `UInt256` Display produces `0x<reversed>`. This matches the C# Atipicial
//! convention where hashes are displayed in reversed byte order.
//!
//! Use [`encode_reversed_hex`] / [`decode_reversed_hex`] for Atipicial hash format,
...
/// written in their natural order. For Atipicial hash display format (reversed),
...
///
/// This is **straight hex** (big-endian byte order) — the bytes are
/// produced in their natural order. For Atipicial hash format (reversed),
...
/// ```
/// # use atipicial_primitives::hex_util;
/// # fn main() -> atipicial_primitives::PrimitiveResult<()> {
...
///
/// This is the **Atipicial hash display format** — bytes are reversed (little-endian)
...
///
/// This is the **Atipicial hash format** — after stripping the prefix and decoding,
