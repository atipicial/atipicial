...
impl Table for ContractStorageTable {
    ...
    const NAME: &'static str = "AtipicialContractStorage";
...
#[test]
fn typed_data_table_preserves_atipicial_storage_key_and_item_bytes() {
    let store = MemoryStore::new();
    ...
    batch
        ...
        .expect("encode Atipicial row");
    store.commit_maintenance(&batch).expect("commit Atipicial row");
