...
use atipicial_primitives::constants::ADDRESS_SIZE;
...
// Removed atipicial_cryptography dependency - using external crypto crates directly
...
impl ContractGroup {
    ...
    pub fn verify_signature(&self, contract_hash: &[u8]) -> CoreResult<bool> {
        ...
        match atipicial_crypto::Secp256r1Crypto::verify(
            contract_hash,
            &signature_array,
            &public_key_bytes,
        ) {
    ...
    /// Builds a contract group from a atipicial-vm stack item.
    ///
    /// # Errors
    ///
    ...
    /// Converts to a atipicial-vm stack item.
