//! Hash function implementations for Atipicial blockchain.
//!
...
/// Hash algorithms supported by Atipicial.
...
impl HashAlgorithm {
    /// Returns the C# `Atipicial.Cryptography.HashAlgorithm` byte, if this algorithm is
    /// part of that protocol enum.
    #[must_use]
    pub const fn to_atipicial_byte(self) -> Option<u8> {
        match self {
    ...
    /// Decodes a C# `Atipicial.Cryptography.HashAlgorithm` byte.
    #[must_use]
    pub const fn from_atipicial_byte(value: u8) -> Option<Self> {
        match value {
...
/// Cryptographic hash functions for Atipicial blockchain.
///
/// This struct provides static methods for all hash functions used in Atipicial.
pub struct Crypto;

/// Incremental SHA-256 hasher for streaming inputs.
///
/// This is a thin newtype wrapper around `sha2::Sha256` that keeps direct
/// `sha2` usage contained to `atipicial-crypto` while exposing a small
...
impl Crypto {
    ...
    /// let hash = Crypto::sha256(b"Hello, Atipicial!");
    ...
    ///
    /// This is commonly used for Atipicial script hashes and addresses.
    ...
    ///
    /// This is commonly used for Atipicial transaction and block hashes.
...
impl CtCompare {
    ...
    /// ```
    /// use atipicial_crypto::Crypto;
    /// use atipicial_crypto::hash::CtCompare;
    ///
