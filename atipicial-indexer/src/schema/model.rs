//! Public index inputs and records exposed by the Atipicial indexer service.
...
pub struct BlockIndexRecord {
    ...
    /// Block height.
    pub height: u32,
    /// Block timestamp in milliseconds since Unix epoch, matching Atipicial headers.
    pub timestamp: u64,
...
pub struct NotificationIndexRecord {
    ...
    /// Notification state payload rendered as Atipicial JSON-RPC stack item envelopes.
    #[serde(default)]
    pub state: Vec<Value>,
    /// Transfer participant accounts extracted from the notification payload.
    ///
    /// Populated for AEP-style `Transfer` events whose first two state items
