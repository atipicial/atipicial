...
fn flush_state_service<S: Store>(
    state_service: &atipicial_state_service::commit_handlers::StateServiceCommitHandlers<S>,
) -> anyhow::Result<()> {
    state_service
        .flush_result()
...
where
    S: atipicial_storage::persistence::store::Store,
    ServiceS: atipicial_storage::persistence::store::Store,
{
    chain_store
        .flush()
...
pub(in crate::node) fn abort_startup_after_import_failure<S, ServiceS>(
    node: &atipicial_system::Node<atipicial_native_contracts::StandardNativeProvider, S>,
    services: &NodeServiceHandles<S>,
