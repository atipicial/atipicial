...
fn contract_state_roundtrip_matches_signed_id() {
    let aef = AefFile::new("compiler".to_string(), vec![1, 2, 3]);
    let manifest = ContractManifest::new("test".to_string());
    let state = ContractState::new(-1, UInt160::zero(), aef.clone(), manifest.clone());
    ...
    assert_eq!(parsed.aef.script, aef.script);
...
fn contract_state_projects_to_stack_item() {
    ...
    let aef = AefFile::new("compiler".to_string(), vec![1, 2, 3]);
    let manifest = ContractManifest::new("test".to_string());
    let mut state = ContractState::new(-7, hash, aef.clone(), manifest.clone());
    ...
    let right = array(vec![
        ...
        StackItem::from_byte_string(aef.to_bytes()),
...
fn contract_state_reads_stack_item() {
    ...
    let aef = AefFile::new("compiler".to_string(), vec![4, 5, 6]);
    ...
    assert_eq!(state.aef.script, aef.script);
...
fn contract_state_trait_projection_matches_inherent_projection() {
    ...
    let aef = AefFile::new("compiler".to_string(), vec![7, 8, 9]);
    let manifest = ContractManifest::new("adapter".to_string());
    let state = ContractState::new(4, hash, aef, manifest);
    let expected = state.to_stack_item();

    let projected = <ContractState as atipicial_vm::Interoperable>::to_stack_item(&state).unwrap();
...
fn contract_record_pins_the_interoperable_stack_item_encoding() {
    ...
    // ByteString(Hash), ByteString(Aef.ToArray()), Manifest.ToStackItem()]
    // — verified against atipicial_csharp ContractState.cs / StorageItem.cs.
    let hash = UInt160::from_bytes(&[0x11u8; 20]).expect("hash");
    let aef = AefFile::new("compiler".to_string(), vec![0x40]);
    let mut manifest = ContractManifest::new("Fixture".to_string());
    manifest.supported_standards = vec!["AEP-17".to_string()];
    let mut state = ContractState::new(7, hash, aef.clone(), manifest);
    state.update_counter = 9;
    ...
    let expected_value = array(vec![
        ...
        structure(vec![
            ...
            array(vec![StackItem::from_byte_string(b"AEP-17".to_vec())]),
            structure(vec![
    ...
    let expected = atipicial_serialization::BinarySerializer::serialize(
        &expected_value,
    ...
    // so a regression to the raw atipicial_io field encoding (which would start
    ...
    let aef_bytes = aef.to_bytes();
    assert!(
        aef_bytes.len() < 0xFD,
        "fixture AEF stays in 1-byte var-int range"
    );
    assert_eq!(record[30], 0x28, "AEF is a ByteString of Aef.ToArray()");
    assert_eq!(record[31] as usize, aef_bytes.len());
    assert_eq!(&record[32..32 + aef_bytes.len()], aef_bytes.as_slice());
    let manifest_offset = 32 + aef_bytes.len();
    assert_eq!(record[manifest_offset], 0x41, "manifest is a Struct");
    assert_eq!(record[manifest_offset + 1], 8, "manifest has 8 fields");

    // And the record must NOT be the legacy raw atipicial_io encoding.
    let mut writer = BinaryWriter::new();
...
fn contract_record_roundtrips_with_nested_manifest() {
    ...
    let mut state = ContractState::new(-3, hash, aef, manifest);
...
fn contract_record_rejects_top_level_struct_like_csharp() {
    ...
    // Atipicial.VM.Types.Array. The nested manifest is a Struct, but a Struct at
    ...
    let record = atipicial_serialization::BinarySerializer::serialize(
        &malformed,
        &atipicial_vm::ExecutionEngineLimits::default(),
    )
...
fn contract_record_rejects_legacy_raw_encoding() {
    // A legacy raw atipicial_io record (i32 id first) must NOT decode as an
    ...
    let state = ContractState::new(7, hash, aef, manifest);
