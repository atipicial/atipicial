...
use atipicial_payloads::{Block, Header, Transaction};
use atipicial_storage::SeekDirection;
...
fn empty_block_with_merkle_root(index: u32, byte: u8) -> Arc<Block> {
    let mut header = Header::new();
    header.set_index(index);
    header.set_merkle_root(atipicial_primitives::UInt256::from([byte; 32]));
    Arc::new(Block::from_parts(header, Vec::new()))
}
...
) -> atipicial_error::CoreResult<crate::native_persist::NativePersistOutcome> {
...
fn visible_store_dump(snapshot: &atipicial_storage::DataCache) -> Vec<(Vec<u8>, Vec<u8>)> {
    snapshot
...
fn atipicial_dollar_per_block_key(index: u32) -> atipicial_storage::StorageKey {
    let mut key = vec![29];
    key.extend_from_slice(&index.to_be_bytes());
    atipicial_storage::StorageKey::new(atipicial_native_contracts::AtipicialCoin::ID, key)
}
...
fn planner_rejects_empty_blocks_with_non_zero_merkle_root() {
    let resources = standard_resources();
    let settings = ProtocolSettings::default();
    let merkle_root = atipicial_primitives::UInt256::from([0x42; 32]);
...
fn standard_natives_explicitly_opt_in_to_empty_block_fast_forward() {
    ...
    assert_eq!(
        names.len(),
        atipicial_native_contracts::STANDARD_NATIVE_CONTRACT_COUNT
    );
    assert!(
...
fn stage_empty_block_fast_forward_uses_composed_native_provider() {
    ...
    assert!(
        !stage.contains("AtipicialCoin::new()"),
        "stage must not construct AtipicialCoin directly"
    );
    ...
    assert!(
        ...
        "empty-block fast-forward provider must resolve AtipicialCoin through the explicit native provider"
    );
    assert!(
...
fn fast_forward_empty_blocks_matches_normal_persist_store_dump_between_refreshes() {
    ...
    let normal = Arc::new(atipicial_storage::DataCache::new(false));
    let fast = Arc::new(atipicial_storage::DataCache::new(false));
    let genesis = Arc::new(
    ...
    // ATDPerBlock record effective inside an otherwise empty run. The fast path
    // must keep the normal single-block integer-division boundary for each
    // height before aggregating rewards.
    let gas_change_key = atipicial_dollar_per_block_key(3);
    let gas_change_value = atipicial_storage::StorageItem::from_bytes(
...
fn fast_forward_empty_blocks_matches_normal_persist_across_multiple_gas_changes() {
    ...
    for (index, gas_per_block) in [(3, 7), (5, 11), (7, 13)] {
        let key = atipicial_dollar_per_block_key(index);
        let value = atipicial_storage::StorageItem::from_bytes(
