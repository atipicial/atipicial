...
#[test]
fn string_wildcard_projects_to_atipicial_vm_null() {
    assert_eq!(
...
#[test]
fn string_list_projects_to_atipicial_vm_byte_string_array() {
...
fn string_interoperable_projection_matches_inherent_projection() {
    ...
    let projected = atipicial_vm::Interoperable::to_stack_item(&container).unwrap();

    assert!(stack_item_struct_eq(&projected, &expected));
}

#[test]
fn string_wildcard_reads_from_atipicial_vm_null() {
    assert_eq!(
...
#[test]
fn string_list_reads_from_atipicial_vm_array() {
    assert_eq!(
...
fn string_list_rejects_struct_and_invalid_strings_like_csharp() {
    assert!(
        WildCardContainer::<String>::from_stack_item(&StackItem::from_struct(vec![
            ...
            StackItem::from_byte_string(b"onAEP17Payment".to_vec()),
        ]))
        .is_err()
    );
    assert!(
