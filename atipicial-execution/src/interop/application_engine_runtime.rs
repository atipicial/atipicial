//! ApplicationEngine.Runtime - matches C# Atipicial.SmartContract.ApplicationEngine.Runtime.cs exactly
...
use atipicial_crypto::murmur;
use atipicial_error::{CoreError, CoreResult};
use atipicial_manifest::ContractParameterDefinition;
use atipicial_payloads::LogEventArgs;
use atipicial_primitives::CallFlags;
use atipicial_primitives::ContractParameterType;
use atipicial_primitives::UInt160;
use atipicial_primitives::constants::{ADDRESS_SIZE, HASH_SIZE};
use atipicial_vm::StackItemType;
use atipicial_vm::{Script, StackItem, VmError, VmResult};
    ...
    B: atipicial_storage::CacheRead,
{
    ...
    pub fn runtime_platform(&mut self) -> CoreResult<()> {
        self.push_string("ATC".to_string())
    }
    ...
    pub fn runtime_get_script_container(&mut self) -> CoreResult<()> {
        ...
        if let Some(transaction) = container.as_transaction() {
            let item =
                <atipicial_payloads::Transaction as atipicial_vm::Interoperable>::to_stack_item(transaction)
    ...
    pub fn runtime_load_script(
        ...
        // Atipicial v3.10.1 always constructs Runtime.LoadScript payloads in strict
    ...
    pub fn runtime_get_random(&mut self) -> CoreResult<()> {
        ...
        let buffer = if aspid_enabled {
            ...
            murmur::murmur128(self.nonce_bytes(), seed)
        } else {
            let bytes = murmur::murmur128(self.nonce_bytes(), network);
    ...
    pub fn runtime_burn_gas(&mut self, amount: i64) -> CoreResult<()> {
        ...
        let datoshi = u64::try_from(amount).map_err(|_| CoreError::other("ATD amount overflow"))?;
    ...
    pub fn runtime_current_signers(&mut self) -> CoreResult<()> {
        ...
        let items = tx
            ...
            .iter()
            .map(|s| {
                <atipicial_payloads::Signer as atipicial_vm::Interoperable>::to_stack_item(s)
                    .map_err(|e| CoreError::other(e.to_string()))
            })
...
#[cfg(test)]
#[path = "../tests/interop/application_engine_runtime.rs"]
mod tests;
