...
//!   helpers.
//! - `tx_builder`: Transaction builder helpers for constructing Atipicial payloads.

#![doc(html_root_url = "https://docs.rs/atipicial-payloads/0.11.1")]

// ── P2P wire payload types (relocated from atipicial-p2p) ───────────────────

/// Simple P2P wire payload types (serialization-only, no atipicial-core dependencies).
...
pub mod tx_builder;
/// Witness scope flags (re-exported from `atipicial-primitives`).
pub mod witness_scope {
    pub use atipicial_primitives::{InvalidWitnessScopeError, WitnessScope};
}
...
pub use atipicial_primitives::{InventoryType, OracleResponseCode, TransactionAttributeType};
