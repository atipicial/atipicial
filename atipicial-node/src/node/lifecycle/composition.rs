...
use atipicial_config::AtipicialChainSpec;
...
enum NodeLedgerBlockSource {
    Local(LedgerBlockSource<atipicial_storage::persistence::StoreCacheBacking<RuntimeStore>>),
    Remote(RpcLedgerBlockSource),
}

impl atipicial_network::BlockSource for NodeLedgerBlockSource {
...
/// Constructs the [`atipicial_system::Node`] with a live blockchain service
/// and a spawned [`atipicial_network::LocalNodeService`].
pub(in crate::node) async fn build_node(
    ...
    let store: Arc<RuntimeStore> = match ledger_mode {
        ...
        LedgerMode::RemoteRpc { .. } => {
            info!(
                target: "atipicial::remote_ledger",
    ...
    let cold_ledger_provider = atipicial_blockchain::OptionalStaticLedgerProvider::from_option(
        static_archive
            .as_ref()
            .map(atipicial_blockchain::StaticLedgerArchive::provider),
    );
    ...
    let tokens_tracker = tokens_tracker_runtime.map(|(tracker_settings, tracker_store)| {
        Arc::new(atipicial_rpc::plugins::tokens_tracker::TokensTracker::new(
            tracker_settings,
            tracker_store,
            Arc::clone(&settings),
    ...
    let mut core_builder = atipicial_system::NodeCoreBuilder::new(
    ...
    let (consensus_inbound_tx, consensus_inbound_rx) = if consensus_configured {
        let (tx, rx) =
            tokio::sync::mpsc::channel::<atipicial_consensus::messages::ConsensusPayload>(1024);
        (Some(tx), Some(rx))
    } else {
        (None, None)
    };
    ...
    let (consensus_tx_feed_tx, consensus_tx_feed_rx) = if consensus_configured {
        let (tx, rx) = tokio::sync::mpsc::channel::<atipicial_primitives::UInt256>(1024);
        (Some(tx), Some(rx))
    } else {
        (None, None)
    };
    ...
    let (state_root_inbound_tx, state_root_inbound_rx) = if state_root_setup.is_some() {
        let (tx, rx) = tokio::sync::mpsc::channel::<atipicial_payloads::ExtensiblePayload>(1024);
        (Some(tx), Some(rx))
    } else {
        (None, None)
    };
    ...
    // The network layer is decoupled from the blockchain (C# `AtipicialSystem`
    ...
    let (inv_tx, mut inv_rx) =
        tokio::sync::mpsc::channel::<atipicial_network::InboundInventory>(FAST_SYNC_BURST_CAPACITY);

    // ----- P2P service -----
    let channels_config = config.p2p.channels_config()?;
    let peer_registry = Arc::new(atipicial_network::PeerRegistry::from_config(&channels_config));
    let (net_service, network) = atipicial_network::LocalNodeService::with_config_and_registry(
        Arc::clone(&chain_spec),
        channels_config,
    ...
    if ledger_mode.uses_local_replay_services() {
        ...
        spawn_daemon_task(
            ...
            async move {
                ...
                let mut pending_blocks: Vec<Arc<atipicial_payloads::Block>> =
    ...
    if let (Some(setup), Some(validators), Some(inbound_rx), Some(tx_feed_rx)) = (
        ...
        if let Some(task) = crate::consensus::consensus_driver_task(
            ...
            info!(target: "atipicial", "dBFT consensus driver started (validator node)");
            spawn_daemon_task(
                &mut handles,
                observability.as_ref(),
                &shutdown,
    ...
    info!(target: "atipicial", height = advertised_tip, "advertised ledger tip to peers");
    ...
    // `Ledger.CurrentIndex`). Range planning remains coordinator-owned.
    {
        ...
        spawn_daemon_task(
            ...
            async move {
                use atipicial_blockchain::RuntimeEvent;
