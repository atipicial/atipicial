...
fn seed_candidate(cache: &DataCache, pubkey: &ECPoint, votes: i64) {
    cache.add(
        AtipicialCoin::candidate_key(pubkey),
...
fn should_refresh_committee_matches_csharp_modulo() {
    ...
    assert!(AtipicialCoin::should_refresh_committee(0, 21));
    assert!(!AtipicialCoin::should_refresh_committee(1, 21));
    assert!(!AtipicialCoin::should_refresh_committee(20, 21));
    assert!(AtipicialCoin::should_refresh_committee(21, 21));
    assert!(AtipicialCoin::should_refresh_committee(42, 21));
    // A single-member committee refreshes every block.
    assert!(AtipicialCoin::should_refresh_committee(5, 1));
}

#[test]
fn standby_fallback_below_turnout_zips_registered_votes() {
    // Turnout one ATC short of the 20% boundary: votersCount * 5 =
    ...
    let cache = DataCache::new(false);
    seed_voters_count(&cache, 199_999_999);
...
fn standby_fallback_when_fewer_candidates_than_committee() {
    ...
    let settings = settings_with_committee(standby.clone());
    let cache = DataCache::new(false);
    seed_voters_count(&cache, 200_000_000);
    seed_candidate(&cache, &all[3], 1000);
    seed_candidate(&cache, &all[4], 900);
...
fn bft_address_uses_the_bft_multisig_threshold() {
    ...
    let script =
        ...
        .unwrap();
    assert_eq!(
        AtipicialCoin::bft_address(&validators).unwrap(),
...
fn next_consensus_address_recomputes_validators_only_on_refresh_height() {
    ...
    assert_eq!(
        AtipicialCoin::new()
            ...
            .unwrap(),
        AtipicialCoin::bft_address(&cached_validator).unwrap(),
    ...
    assert_eq!(
        AtipicialCoin::new()
            ...
            .unwrap(),
        AtipicialCoin::bft_address(&elected_validator).unwrap(),
