...
//!
//! `AtipicialValidateStage` owns structural block validation and stateful parent
...
//! header witness through the AtipicialVM helper with the caller's explicit native
//! contract provider.
...
/// ATD limit used for consensus header witness verification.
///
...
pub struct AtipicialConsensusWitnessStage<C>
where
...
impl<C> fmt::Debug for AtipicialConsensusWitnessStage<C>
where
    C: ConsensusWitnessContext,
{
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        f.debug_struct("AtipicialConsensusWitnessStage")
            .field("ctx", &self.ctx)
            .finish_non_exhaustive()
    }
}

impl<C> AtipicialConsensusWitnessStage<C>
where
    C: ConsensusWitnessContext,
{
...
impl<C> ConsensusWitnessStage for AtipicialConsensusWitnessStage<C>
where
    C: ConsensusWitnessContext,
{
...
impl<C> PipelineStage for AtipicialConsensusWitnessStage<C>
where
    C: ConsensusWitnessContext,
{
    fn id(&self) -> StageId {
    ...
    fn execute(&self, ctx: &StageContext, block: &Block) -> EngineResult<StageOutput> {
        ...
        Ok(StageOutput::performed(atipicial_runtime::time::elapsed_us(
            start.elapsed(),
        )))
    }
}
