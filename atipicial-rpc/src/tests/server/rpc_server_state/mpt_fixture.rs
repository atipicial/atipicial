...
use atipicial_state_service::mpt_store::MptChange;
...
pub(super) struct MptFixture {
    pub(super) contract_hash: atipicial_primitives::UInt160,
...
pub(super) fn make_server_with_mpt(full_state: bool) -> MptFixture {
    ...
    let contract_hash =
        atipicial_primitives::UInt160::from_bytes(&[0x42u8; 20]).expect("fixture contract hash");
    let contract = ContractState::new(
        FIXTURE_CONTRACT_ID,
        contract_hash,
        AefFile::new("test".to_string(), vec![OpCode::PUSH1.byte()]),
