//! Elliptic Curve Cryptography for Atipicial blockchain.
//!
...
pub enum ECCurve {
    /// NIST P-256 curve (secp256r1) - Atipicial's primary curve
    Secp256r1,
...
impl ECCurve {
    /// Returns the secp256r1 curve (Atipicial's default).
    #[must_use]
...
///
/// This is the primary type for public keys in Atipicial.
///
/// # Security
...
impl ECPoint {
    ...
    ///
    /// Atipicial's Rust codebase historically used an `ECPoint` wrapper that exposed `to_bytes()`.
...
impl fmt::Debug for ECPoint {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(
            ...
            "ECPoint({:?}, {})",
            self.curve,
            atipicial_primitives::hex_util::encode_hex(&self.data)
        )
    }
}

atipicial_primitives::impl_display_hex!(ECPoint, data);
...
// Ordering mirrors the C# `Atipicial.Cryptography.ECC.ECPoint.CompareTo` implementation:
...
impl Ord for ECPoint {
    fn cmp(&self, other: &Self) -> Ordering {
        ...
        match self.curve {
            ...
            // Ed25519 isn't used by Atipicial N3 consensus/committee keys, but keep a deterministic order.
