//! ApplicationEngine.Crypto - matches C# Atipicial.SmartContract.ApplicationEngine.Crypto.cs
...
use atipicial_crypto::{CryptoError, ECCurve, ECPoint, Secp256r1Crypto};
use atipicial_error::{CoreError, CoreResult};
use atipicial_primitives::CallFlags;
use atipicial_vm::VmResult;
    ...
    B: atipicial_storage::CacheRead,
{
    ...
    pub fn crypto_check_sig(&mut self) -> CoreResult<bool> {
        // Atipicial VM calling convention: the first parameter is on top of the stack.
    ...
    fn get_sign_data(&self) -> CoreResult<Vec<u8>> {
        ...
        atipicial_payloads::get_sign_data_vec(container.as_ref(), network)
    }
    ...
    fn pop_sig_elements(&mut self) -> CoreResult<Vec<Vec<u8>>> {
        ...
        match &item {
            atipicial_vm::stack_item::StackItem::Array(arr) => {
    ...
    B: atipicial_storage::CacheRead,
{
    match app.crypto_check_sig() {
        Ok(result) => app
            .push_boolean(result)
            .map_err(|e| atipicial_vm::VmError::InteropService {
                service: "System.Crypto.CheckSig".to_string(),
                error: e.to_string(),
            }),
        Err(e) => Err(atipicial_vm::VmError::InteropService {
            service: "System.Crypto.CheckSig".to_string(),
            error: e.to_string(),
        }),
    }
