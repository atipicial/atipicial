...
use atipicial_rpc::server::{RpcServerBlockchain, RpcServerIndexer, RpcServerWallet};

fn handler_names(handlers: &[atipicial_rpc::server::RpcHandler]) -> Vec<&str> {
    handlers
        .iter()
...
fn indexer_rpc_docs_match_registered_handlers() {
    ...
    let indexer_section = docs
        .split("### AtipicialIndexer")
        .nth(1)
        .expect("rpc-api.md should document the AtipicialIndexer section")
        .split("\n### ")
        .next()
        .expect("AtipicialIndexer section should have content");
