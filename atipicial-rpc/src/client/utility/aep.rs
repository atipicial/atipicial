...
/// Builds the shared AEP balance container shape.
...
/// Parses the shared AEP balance container shape.
...
/// Shared AEP balance item fields.
...
/// Borrowed AEP balance item fields used when building RPC JSON.
#[derive(Debug, Clone, Copy)]
pub(crate) struct AepBalanceFieldRefs<'a> {
    /// Balance amount.
    pub amount: &'a BigInt,
    /// Last updated block height.
    pub last_updated_block: u32,
...
/// Appends shared AEP balance item fields in RPC wire order.
pub(crate) fn insert_nep_balance_fields(json: &mut JObject, fields: AepBalanceFieldRefs<'_>) {
    json.insert(
        "amount".to_string(),
...
/// Parses shared AEP balance item fields.
...
/// Builds the shared AEP transfer container shape.
...
/// Parses the shared AEP transfer container shape.
...
/// Shared AEP transfer entry fields used by AEP-11 and AEP-17 RPC payloads.
#[derive(Debug, Clone, PartialEq, Eq)]
...
/// Borrowed AEP transfer entry fields used when building RPC JSON.
#[derive(Debug, Clone, Copy)]
pub(crate) struct AepTransferFieldRefs<'a> {
    /// Timestamp in milliseconds.
    pub timestamp_ms: u64,
    /// Asset hash.
    pub asset_hash: UInt160,
...
/// Appends the shared AEP transfer entry fields in RPC wire order.
pub(crate) fn insert_nep_transfer_fields(
    json: &mut JObject,
    fields: AepTransferFieldRefs<'_>,
    protocol_settings: &ProtocolSettings,
) {
    json.insert(
        "timestamp".to_string(),
...
/// Parses the shared AEP transfer entry fields while preserving legacy field semantics.
...
#[cfg(test)]
#[path = "../../tests/client/utility/aep.rs"]
mod tests;
