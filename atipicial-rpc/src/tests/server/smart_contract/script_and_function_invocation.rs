...
async fn invokefunction_total_supply_matches_csharp() {
    ...
    let atipicial_hash = AtipicialCoin::new().hash().to_string();
    let params = [
        Value::String(atipicial_hash),
...
async fn invokescript_with_diagnostics_reports_invoked_contract_and_storage_shape() {
    ...
    assert!(hashes.contains(&AtipicialCoin::new().hash().to_string()));
...
///
/// atipicial-rs used to emit the bare array, which typed clients cannot decode: on a
/// mixed private network every atipicial-go transfer against a atipicial-rs node aborted with
/// `cannot unmarshal array into Go struct field invokeAux.notifications of type
/// stackitem.rawItem`, because atipicial-go decodes `state` into a single stack item.
...
fn notification_state_is_a_stack_item_not_a_bare_array() {
    let notification = atipicial_payloads::NotifyEventArgs::new_with_optional_container(
        None,
        UInt160::zero(),
        "Transfer".to_string(),
        vec![
            atipicial_vm::stack_item::StackItem::from_byte_string(vec![0x01, 0x02]),
            atipicial_vm::stack_item::StackItem::from_int(BigInt::from(1_250_000_000u64)),
        ],
    );
