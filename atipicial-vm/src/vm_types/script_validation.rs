//! Shared AtipicialVM script parsing and structural validation.
...
/// Validated AtipicialVM script metadata.
#[derive(Debug, Clone)]
...
/// Validates AtipicialVM bytecode and rejects invalid control-flow/type operands.
pub fn validate_strict_script(script: &[u8]) -> ValidationResult<()> {
    validate_script(script, true).map(|_| ())
}

/// Validates AtipicialVM bytecode and returns instruction offsets for ABI checks.
...
/// Parses all instructions in a AtipicialVM bytecode script.
