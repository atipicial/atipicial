...
impl RecoveryRequestMessage {
    ...
    ///
    /// Atipicial N3 `DBFTPlugin` format: `timestamp (8)`.
    #[must_use]
...
impl Serializable for CommitPayloadCompact {
    ...
    fn serialize(&self, writer: &mut BinaryWriter) -> atipicial_io::IoResult<()> {
        ...
        if self.signature.len() != 64 {
            return Err(atipicial_io::IoError::invalid_data(
...
///
/// This struct models the Atipicial N3 `DBFTPlugin` on-wire format (message body only).
#[derive(Debug, Clone, Serialize, Deserialize)]
...
/// Maps a `atipicial_io::IoError` from the in-memory `BinaryWriter` into a
...
fn writer_error(err: atipicial_io::IoError) -> crate::ConsensusError {
