...
use super::CryptoLib;
use atipicial_crypto::{ECPoint, HashAlgorithm, NamedCurveHash, Secp256k1Crypto};
...
impl CryptoLib {
    ...
    pub(super) fn verify_ed25519_method(message: &[u8], pubkey: &[u8], signature: &[u8]) -> bool {
        signature.len() == 64
            && pubkey.len() == 32
            && atipicial_crypto::ecc::EcdsaVerify::verify_ed25519(pubkey, message, signature)
                .unwrap_or(false)
    }
    ...
    pub(super) fn verify_ecdsa_method(
        ...
        if gorgon {
            ...
            return atipicial_crypto::ecc::EcdsaVerify::verify_signature_with_hash(
                named.curve(),
                pubkey,
                message,
                signature,
        ...
        Ok(
            atipicial_crypto::ecc::EcdsaVerify::verify_signature_with_hash(
                named.curve(),
                pubkey,
                message,
                signature,
    ...
    /// without an [`atipicial_execution::ApplicationEngine`].
