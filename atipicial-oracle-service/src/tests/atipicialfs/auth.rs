use super::super::auth::{AtipicialFsBearerSigner, strip_bearer_prefix};
use super::super::json::normalize_atipicialfs_hex_header;

#[test]
fn normalize_atipicialfs_hex_header_strips_prefix() {
    assert_eq!(normalize_atipicialfs_hex_header("0xdeadbeef"), "deadbeef");
}

#[test]
fn normalize_atipicialfs_hex_header_converts_base64() {
    let input = "aGVsbG8=";
    assert_eq!(normalize_atipicialfs_hex_header(input), "68656c6c6f");
}

#[test]
fn normalize_atipicialfs_hex_header_preserves_hex() {
    let input = "cafebabe";
    assert_eq!(normalize_atipicialfs_hex_header(input), "cafebabe");
}
...
fn salt_message_wallet_connect_includes_salt_and_suffix() {
    let data = b"payload";
    let salt = [0x11u8; 16];
    let message = AtipicialFsBearerSigner::salt_message_wallet_connect(data, &salt);
