...
//!
//! The Rust `atipicial_native_contracts::LedgerContract` is read-only (its
...
use atipicial_payloads::{Block, Transaction, TransactionAttribute};
...
use atipicial_native_contracts::ledger_contract::storage::{
...
fn upsert<B: atipicial_storage::CacheRead>(cache: &DataCache<B>, key: StorageKey, value: Vec<u8>) {
...
impl LedgerRecords {
    ...
    fn serialize_trimmed_block(block: &Block, hashes: &[UInt256]) -> CoreResult<Vec<u8>> {
        let mut writer = atipicial_io::BinaryWriter::with_capacity(
            <atipicial_payloads::Header as Serializable>::size(&block.header)
                + atipicial_io::serializable::helper::SerializeHelper::get_var_size_serializable_slice(
                    hashes,
                ),
        );
        <atipicial_payloads::Header as Serializable>::serialize(&block.header, &mut writer)
    ...
    pub(crate) fn write_on_persist_records<B: atipicial_storage::CacheRead>(
        cache: &DataCache<B>,
        block: &Block,
        block_hash: &UInt256,
    ) -> CoreResult<()> {
    ...
    pub(crate) fn update_transaction_vm_state<B: atipicial_storage::CacheRead>(
        cache: &DataCache<B>,
        block_index: u32,
        tx: &Transaction,
        tx_hash: &UInt256,
    ...
    pub(crate) fn write_post_persist_record<B: atipicial_storage::CacheRead>(
        cache: &DataCache<B>,
        block_hash: &UInt256,
        index: u32,
    ) -> CoreResult<()> {
