//! Shared execution limits used by AtipicialVM consumers.

use alloc::{format, string::String};

/// Maximum AtipicialVM script size accepted by local execution and proof input code.
...
/// (Atipicial.VM/ExecutionEngineLimits.cs). Distinct from [`MAX_SCRIPT_SIZE`] (1 MiB):
/// NEWBUFFER/CAT results are capped at the item size, not the script size.
pub const MAX_ITEM_SIZE: usize = (u16::MAX as usize) * 2;

/// Restrictions applied by the AtipicialVM execution engine.
...
impl ExecutionEngineLimits {
    /// Default execution limits matching the Atipicial C# reference node.
