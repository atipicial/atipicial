...
async fn node_pulls_unknown_inv_with_getdata() {
    ...
    let mut reader = atipicial_io::MemoryReader::new(&getdata.payload_raw);
    let payload =
        <InvPayload as atipicial_io::Serializable>::deserialize(&mut reader).expect("decode getdata");
...
/// Atipicial N3 v3.10.1 still treats extensible payload hashes as fetchable
...
/// A [`atipicial_network::BlockSource`] that reports a single verified mempool tx.
struct MempoolStubSource(UInt256);
impl atipicial_network::BlockSource for MempoolStubSource {
...
async fn node_answers_mempool_request_with_inv() {
    ...
    let mut reader = atipicial_io::MemoryReader::new(&inv.payload_raw);
    let payload =
        <InvPayload as atipicial_io::Serializable>::deserialize(&mut reader).expect("decode inv");
    assert_eq!(payload.inventory_type, InventoryType::Transaction);
