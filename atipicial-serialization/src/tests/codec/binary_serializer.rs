...
use atipicial_vm::stack_item::Map as MapItem;
...
fn deserialize_preserves_map_entry_order_for_roundtrip_bytes() {
    ...
    let mut map_items = atipicial_vm::VmOrderedDictionary::new();
...
fn deserialize_rejects_invalid_map_keys_like_csharp() {
    ...
    for key in invalid_keys {
        ...
        assert!(
            ...
            "Atipicial rejects non-primitive and oversized map keys during reconstruction"
        );
    }
...
fn deserialize_with_limits_enforces_item_limits() {
    let payload = vec![atipicial_vm::ATCVM_STACK_ITEM_TYPE_ARRAY, 3, 0, 0, 0];
