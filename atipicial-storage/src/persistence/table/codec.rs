...
/// Byte-identical codec for Atipicial contract-storage keys.
#[derive(Debug)]
    ...
    fn decode(bytes: &[u8]) -> StorageResult<StorageKey> {
        if bytes.len() < std::mem::size_of::<i32>() {
            return Err(StorageError::invalid_data(format!(
                "Atipicial storage key must include a 4-byte contract id, found {} bytes",
                bytes.len()
            )));
        }
...
/// Byte-identical codec for Atipicial contract-storage values.
#[derive(Debug)]
