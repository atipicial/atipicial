...
impl OracleContract {
    ...
    >(
        ...
        let price =
            crate::args::raw_i64_arg(args, 0, "OracleContract::setPrice").map_err(|_| {
    ...
    >(
        ...
        // size/shape validations, fees + response-ATD mint, id
        ...
        let callback = crate::args::raw_string_arg(args, 2, "OracleContract::request", "callback")?;
        ...
        let gas_for_response = crate::args::raw_i64_arg(args, 4, "OracleContract::request")
            .map_err(|_| {
                CoreError::invalid_operation("OracleContract::request requires a gasForResponse")
            })?;
        if gas_for_response < MIN_GAS_FOR_RESPONSE {
            return Err(CoreError::invalid_operation(format!(
                "gasForResponse {gas_for_response} must be at least 0.1 ATD.",
            )));
        }
        ...
        crate::AtipicialDollar::new().gas_mint(
            engine,
            &Self::script_hash(),
        ...
        let calling = engine.get_calling_script_hash().ok_or_else(|| {
            CoreError::invalid_operation("OracleContract::request requires a calling contract")
        })?;
    ...
    >(
        ...
        .map_err(|e| CoreError::deserialization(format!("OracleContract::finish userData: {e}")))?;
