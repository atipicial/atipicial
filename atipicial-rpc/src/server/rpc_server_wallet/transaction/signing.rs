...
use atipicial_execution::contract_parameters_context::ContractParametersContext;
...
use atipicial_wallets::{Aep6Account, Aep6Wallet, Wallet as CoreWallet, WalletAccount};
...
pub(in crate::server::rpc_server_wallet) fn sign_and_relay<B: CacheRead>(
    server: &RpcServer,
    ...
    let mut context = ContractParametersContext::new_with_type(
        ...
        Some("Atipicial.Network.P2P.Payloads.Transaction".to_string()),
    );
                    ...
                    if let Some(contract) = contract_opt {
                        if let Some(key) = key_opt {
                            if account.has_key() {
                                ...
                                .map_err(internal_error)?;
                                // Atipicial N3 uses secp256r1 (NIST P-256) curve.
                                let pub_key =
                                    ECPoint::new(ECCurve::Secp256r1, key.compressed_public_key())
                                        .map_err(|e| internal_error(e.to_string()))?;
                            ...
                            let sign_data = if let Some(data) = sign_data.as_ref() {
                                ...
                                let data = atipicial_payloads::get_sign_data_vec(
                                    &tx,
                                    server.system().settings().network,
                                )
...
fn account_for_point(wallet: &Arc<Aep6Wallet>, point: &ECPoint) -> Option<Arc<Aep6Account>> {
