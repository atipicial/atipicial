...
[logging]
file_path = "./logs/atipicial-node-validate.log"
...
class PrepareCleanStateRootValidationTests(unittest.TestCase):
    def test_prepare_workspace_writes_isolated_config_and_commands(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            base_config = root / "atipicial_mainnet_validate.toml"
            ...
            config = (work_root / "atipicial_mainnet_validate.toml").read_text(encoding="utf-8")
            ...
            self.assertEqual(plan["commands"]["preflight"][0], "target/debug/atipicial-node")
            self.assertIn(str(work_root / "atipicial_mainnet_validate.toml"), plan["commands"]["preflight"])
            ...
            self.assertIn("http://seed1.atipicial.com:10332", reference_arg)
