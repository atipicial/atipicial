...
pub(in crate::node) struct LiveServiceGuards {
    _rpc_keepalive: Option<Arc<parking_lot::RwLock<atipicial_rpc::server::RpcServer>>>,
}
...
pub(in crate::node) async fn start_live_services(
    node: &Arc<atipicial_system::Node<atipicial_native_contracts::StandardNativeProvider, RuntimeStore>>,
    services: &Arc<NodeServiceHandles<RuntimeStore>>,
    network: &atipicial_network::NetworkHandle,
...
async fn start_p2p_listener(
    ...
    match format!("{p2p_bind}:{p2p_port}").parse::<SocketAddr>() {
        Ok(bind_addr) => match network.start(bind_addr).await {
            Ok(()) => info!(target: "atipicial", %bind_addr, "P2P listener started"),
            Err(err) => {
                warn!(target: "atipicial", %bind_addr, error = %err, "failed to start P2P listener");
                if let Some(observability) = observability {
        ...
        Err(err) => {
            warn!(target: "atipicial", addr = %format!("{p2p_bind}:{p2p_port}"), error = %err, "invalid P2P bind address");
            if let Some(observability) = observability {
...
fn start_rpc(
    ...
    handles: &mut Vec<tokio::task::JoinHandle<()>>,
    observability: Option<&ObservabilityRuntime>,
) -> anyhow::Result<Option<Arc<parking_lot::RwLock<atipicial_rpc::server::RpcServer>>>> {
    if !config.rpc.enabled {
        info!(target: "atipicial", "RPC server disabled in config");
        return Ok(None);
    }
