    ...
    pub fn to_json(&self, protocol_settings: &ProtocolSettings) -> JObject {
        ...
        if let Some(confirmations) = self.confirmations {
            if let Some(ref block_hash) = self.block_hash {
                json.insert(
                    "blockhash".to_string(),
                    atipicial_serialization::json::JToken::String(block_hash.to_string()),
                );
            }
            json.insert(
                "confirmations".to_string(),
                atipicial_serialization::json::JToken::Number(f64::from(confirmations)),
            );

            if let Some(block_time) = self.block_time {
                json.insert(
                    "blocktime".to_string(),
                    atipicial_serialization::json::JToken::Number(block_time as f64),
                );
            }
            ...
            if let Some(ref vm_state) = self.vm_state {
                json.insert(
                    "vmstate".to_string(),
                    atipicial_serialization::json::JToken::String(vm_state_to_string(*vm_state)),
                );
            }
        }
...
#[cfg(test)]
#[path = "../../../tests/client/models/ledger/rpc_transaction.rs"]
mod tests;
