...
pub(super) type WalletHandle = Arc<RwLock<Option<Arc<Aep6Wallet>>>>;
...
impl RpcServer {
    ...
    pub fn set_wallet(&self, wallet: Option<Arc<Aep6Wallet>>) {
        *self.wallet.write() = wallet;
    }

    /// Return the wallet currently exposed to wallet RPC methods.
    #[must_use]
    pub fn wallet(&self) -> Option<Arc<Aep6Wallet>> {
