...
use atipicial_state_service::{StateProof, StateProviderFactory, StateView, verify_state_proof};
...
impl RpcServerState {
    ...
    /// Converts a `atipicial_io::IoError` from the in-memory `BinaryWriter` into
    ...
    /// streaming writer.
    fn writer_error_to_rpc(err: atipicial_io::IoError) -> RpcException {
        RpcException::from(
