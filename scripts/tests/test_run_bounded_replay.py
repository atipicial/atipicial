...
class RunBoundedReplayTests(unittest.TestCase):
    def test_node_command_passes_stop_height_to_node(self):
        ...
        command = module.node_command(
            Path("target/release/atipicial-node"),
            Path("bounded.toml"),
            665603,
        )
    ...
    def test_parse_prometheus_metrics_keeps_hotspot_metrics(self):
        ...
        metrics = module.parse_prometheus_metrics(
            "\n".join(
                [
                    ...
                    "atipicial_sync_avg_total_us 4200",
                    "atipicial_sync_native_persist_avg_tx_us 1200",
                    'atipicial_sync_native_contract_hook_avg_us{trigger="onpersist",contract="AtipicialDollar",id="-6"} 7100',
                    'atipicial_sync_native_persist_tx_stage_avg_us{stage="load_execute"} 8100',
                    'atipicial_sync_atipicialtoken_onpersist_stage_avg_us{stage="compute_committee"} 3300',
                    'atipicial_sync_atipicialtoken_committee_compute_stage_avg_us{stage="candidate_state_decode"} 2100',
                    'atipicial_sync_atipicialtoken_committee_candidate_scan_avg_items{kind="eligible_candidates"} 42',
                    'atipicial_state_service_mpt_apply_stage_avg_us{stage="queue_wait"} 900',
                    'atipicial_state_service_mpt_apply_stage_avg_us{stage="trie_commit"} 120',
                    'atipicial_state_service_mpt_apply_avg_items{kind="overlay_entries"} 19',
                    'atipicial_node_service_enabled{service="state_service"} 1',
                    "atipicial_state_service_mpt_apply_avg_changes 17",
                    "bad_line nope",
                ]
            )
        )

        self.assertEqual(metrics["atipicial_sync_avg_total_us"], 4200.0)
        self.assertEqual(metrics["atipicial_sync_native_persist_avg_tx_us"], 1200.0)
        self.assertEqual(
            metrics[
        ...
        self.assertEqual(
            metrics[
                'atipicial_sync_native_persist_tx_stage_avg_us{stage="load_execute"}'
            ],
            8100.0,
        )
        self.assertEqual(
            metrics['atipicial_sync_atipicialtoken_onpersist_stage_avg_us{stage="compute_committee"}'],
            3300.0,
        )
        self.assertEqual(
            metrics[
        ...
        self.assertEqual(
            metrics['atipicial_state_service_mpt_apply_stage_avg_us{stage="trie_commit"}'],
            120.0,
        )
        self.assertEqual(
            metrics['atipicial_state_service_mpt_apply_stage_avg_us{stage="queue_wait"}'],
            900.0,
        )
        self.assertEqual(
            metrics['atipicial_state_service_mpt_apply_avg_items{kind="overlay_entries"}'],
            19.0,
        )
        self.assertEqual(metrics["atipicial_state_service_mpt_apply_avg_changes"], 17.0)
        self.assertNotIn("atipicial_node_service_enabled", metrics)
    ...
    def test_summarize_metric_samples_reports_hotspots(self):
        ...
        summary = module.summarize_metric_samples(
            [
                {
                    "metrics": {
                        "atipicial_sync_avg_total_us": 4000.0,
                        'atipicial_sync_native_persist_tx_stage_avg_us{stage="load_execute"}': 9000.0,
                        'atipicial_state_service_mpt_apply_stage_avg_us{stage="trie_commit"}': 7000.0,
                        'atipicial_state_service_mpt_apply_avg_items{kind="overlay_entries"}': 20.0,
                    }
                },
                {
                    "metrics": {
                        "atipicial_sync_avg_total_us": 6000.0,
                        'atipicial_sync_native_persist_tx_stage_avg_us{stage="load_execute"}': 11000.0,
                        'atipicial_state_service_mpt_apply_stage_avg_us{stage="trie_commit"}': 3000.0,
                        'atipicial_state_service_mpt_apply_avg_items{kind="overlay_entries"}': 30.0,
                    }
                },
        ...
        self.assertEqual(summary["metrics"]["atipicial_sync_avg_total_us"]["average"], 5000.0)
        self.assertEqual(
        ...
        self.assertEqual(
            ...
            'atipicial_state_service_mpt_apply_avg_items{kind="overlay_entries"}',
        )
    ...
    def test_localhost_metrics_bypasses_environment_proxy(self):
        ...
        class FakeNoProxyOpener:
            def open(self, request, timeout=None):
                captured["url"] = request.full_url
                captured["timeout"] = timeout
                return FakeHttpResponse("atipicial_sync_avg_total_us 4200\n")
    ...
    def test_stops_node_when_target_height_is_reached_and_reports_rate(self):
        ...
        report = module.run_until_target(
            command=["atipicial-node", "--config", "bounded.toml"],
        ...
        self.assertEqual(spawned[0][0], ["atipicial-node", "--config", "bounded.toml"])
    ...
    def test_run_until_target_can_require_sync_speed_floor(self):
        ...
        report = module.run_until_target(
            ...
            metrics_fetcher=lambda _url: {
                'atipicial_sync_native_persist_tx_stage_avg_us{stage="load_execute"}': 8100.0
            },
        )
    ...
    def test_run_until_target_accepts_sync_speed_inside_band(self):
        ...
        report = module.run_until_target(
            ...
            metrics_url="http://127.0.0.1:21990/metrics",
            metrics_fetcher=lambda _url: {"atipicial_sync_native_persist_avg_tx_count": 1.0},
        )
    ...
    def test_run_until_target_rejects_speed_proof_without_transaction_work(self):
        ...
        report = module.run_until_target(
            ...
            min_blocks_per_second=0.5,
            metrics_url="http://127.0.0.1:21990/metrics",
            metrics_fetcher=lambda _url: {"atipicial_sync_native_persist_avg_tx_count": 0.0},
        )
    ...
    def test_run_until_target_attaches_metrics_to_height_samples(self):
        ...
        report = module.run_until_target(
            ...
            clock=clock,
            metrics_url="http://127.0.0.1:21990/metrics",
            metrics_fetcher=lambda url: {"atipicial_sync_avg_total_us": 4200.0},
        )

        self.assertEqual(report["status"], "target-reached")
        self.assertEqual(
            report["height_samples"][0]["metrics"]["atipicial_sync_avg_total_us"],
            4200.0,
        )
        self.assertEqual(
            report["metrics_summary"]["metrics"]["atipicial_sync_avg_total_us"]["last"],
            4200.0,
        )
    ...
    def test_clean_process_exit_counts_as_target_reached_for_node_stop_height(self):
        ...
        report = module.run_until_target(
            command=["atipicial-node", "--stop-at-height", "665603"],
            rpc_url="http://127.0.0.1:21332",
    ...
    def test_final_fallback_at_target_waits_for_required_target_readiness(self):
        ...
        report = module.run_until_target(
            command=["atipicial-node", "--import-chain", "chain.0.100k.acc"],
            rpc_url="http://127.0.0.1:21332",
    ...
    def test_process_exit_after_fast_sync_uses_fallback_samples_for_rate_and_source(self):
        ...
        report = module.run_until_target(
            command=["atipicial-node", "--fast-sync", "--stop-at-height", "100000"],
            rpc_url="http://127.0.0.1:21332",
            target_height=100000,
            poll_interval=10,
            max_seconds=100,
    ...
    def test_attach_post_probe_report_updates_sync_proof(self):
        ...
        try:
            ...
            updated = module.attach_post_probe_report(
                report,
                db=Path("clean/chain"),
                probe_bin=Path("target/release/atipicial-db-probe"),
    ...
    def test_attach_fast_sync_report_merges_package_and_import_window_proof(self):
                ...
                json.dumps(
                    {
                        ...
                        "reference": {
                            "endpoint": "http://seed1.atipicial.com:10332",
                            "block_height": 100000,
                            "block_hash": "0x01",
        ...
        self.assertEqual(reference["endpoint"], "http://seed1.atipicial.com:10332")
    ...
    def test_read_probe_ledger_height_decodes_hash_index_probe_output(self):
        ...
        try:
            ...
            height = module.read_probe_ledger_height(
                Path("bounded/data"),
                Path("target/release/atipicial-db-probe"),
            )
        finally:
            module.subprocess.run = original_run

        self.assertEqual(height, 677297)
        self.assertEqual(captured["command"][0], "target/release/atipicial-db-probe")
    ...
    def test_collect_post_probe_can_compare_reference_stateroots(self):
        ...
        try:
            ...
            post_probe = module.collect_post_probe(
                db=Path("bounded/chain"),
                probe_bin=Path("target/release/atipicial-db-probe"),
                reference_urls=["http://seed1.atipicial.com:10332,http://seed2.atipicial.com:10332"],
                rpc=fake_rpc,
            )
        finally:
            module.read_probe_ledger_height = original_chain_reader
    ...
    def test_required_reference_match_marks_target_run_failed_on_mismatch(self):
        ...
        try:
            ...
            updated = module.attach_post_probe_report(
                ...
                probe_bin=Path("target/release/atipicial-db-probe"),
                require_stateroot_height_match=True,
                reference_urls=["http://seed1.atipicial.com:10332"],
        ...
        try:
            ...
            updated = module.attach_post_probe_report(
                ...
                reference_urls=[
                    ...
                    "http://seed3.atipicial.com:10332",
                    "http://seed4.atipicial.com:10332",
                    "http://seed5.atipicial.com:10332",
                ],
                require_reference_stateroot_match=True,
            )
        finally:
