...
//! the `atipicial-node` binary actually constructs at startup. It owns:
//!
//! - the typed blockchain handle from `atipicial-blockchain`,
//! - the typed network handle from `atipicial-network`,
...
//! - the native contract provider owned for AtipicialVM host calls,
...
/// The composed Atipicial node runtime.
///
/// Cheap to clone — every field is either `Clone` (handles) or
/// `Arc<T>` (shared state).
#[derive(Clone)]
pub struct Node<P = atipicial_native_contracts::StandardNativeProvider, S = MemoryStore>
where
    P: NativeContractProvider,
    S: TransactionalStore,
{
    /// Immutable chain specification selected at startup.
    pub(super) chain_spec: Arc<AtipicialChainSpec>,
    ...
    /// Native-contract provider captured for AtipicialVM host calls.
    ///
    ...
    /// `atipicial-execution` lookup seam.
    ...
    S: TransactionalStore + 'static,
{
    ...
    ///
    /// This is the replacement for the legacy `AtipicialSystem::store_cache`:
...
impl Node<atipicial_native_contracts::StandardNativeProvider, MemoryStore> {
    /// Construct an in-memory node for test-only RPC/service fixtures.
    pub fn for_test(chain_spec: std::sync::Arc<AtipicialChainSpec>) -> Self {
        ...
        let (network, _nrx, _etx) = atipicial_network::NetworkHandle::channel(8, 8);
