...
use atipicial_payloads::{NotaryAssisted, OracleResponse, Signer, TransactionAttribute, Witness};
use atipicial_primitives::{OracleResponseCode, WitnessScope};
use atipicial_storage::StorageItem;

const NOTARY_ASSISTED_FEE_PER_KEY: i64 = 1000_0000;
const ORACLE_PREFIX_REQUEST: u8 = 7;

fn transaction_block(index: u32, transaction: atipicial_payloads::Transaction) -> Arc<Block> {
...
fn oracle_response_block_runs_oracle_post_persist_gate() {
    ...
    snapshot.add(
        StorageKey::new(atipicial_native_contracts::OracleContract::ID, request_key),
