...
use anyhow::Context;
use atipicial_storage::persistence::{StoreCache, providers::RuntimeStore};
use tracing::info;
...
pub(super) async fn open_static_ledger_archive(
...
) -> anyhow::Result<Option<atipicial_blockchain::StaticLedgerArchive>> {
    ...
    let (archive, recovery, hot_pruned_through, pruned_rows) =
        tokio::task::spawn_blocking(move || {
            let archive = atipicial_blockchain::StaticLedgerArchiveFactory::new(static_config)
                .open(&worker_path)
                .with_context(|| {
