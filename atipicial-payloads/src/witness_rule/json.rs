...
fn parse_json_group(value: &str) -> CoreResult<Vec<u8>> {
    ...
    atipicial_crypto::ECPoint::from_bytes_with_curve(atipicial_crypto::ECCurve::Secp256r1, &bytes)
        .and_then(|point| point.encode_compressed())
        .map_err(|e| CoreError::other(format!("Invalid ECPoint group: {e}")))
}

impl WitnessCondition {
    /// Converts the condition to Atipicial's JSON witness-condition shape.
    ...
    /// Parses a witness condition from Atipicial's JSON representation.
...
impl WitnessRule {
    /// Converts the witness rule to Atipicial's JSON representation.
    pub fn to_json(&self) -> Value {
        json!({
    ...
    /// Parses a witness rule from Atipicial's JSON representation.
