...
//! (`atipicial_csharp/src/Atipicial/Ledger/Blockchain.cs:410`): every persisted
...
//! `atipicial-execution` grows a registry-population seam, this can switch to the
//! engine functions without reintroducing ambient provider lookup.
//!
//! In C# the native *deployment + initialization* (committee cache,
//! genesis ATC/ATD mints, Oracle price, …) happens inside
...
//!
//! `AtipicialDollar::on_persist` lives in `atipicial-native-contracts` and is invoked by
...
//! network-fee total to the primary validator, matching C# `AtipicialDollar.OnPersist`.
...
use atipicial_execution::{ApplicationEngine, NativeContract, NoDiagnostic};
...
#[cfg(test)]
pub(crate) use genesis::{LEDGER_PREFIX_BLOCK, ATC_PREFIX_COMMITTEE_KEY, ATC_TOKEN_ID};
    ...
    B: CacheRead,
{
    ...
    drop(engine);
    let onpersist_us = atipicial_runtime::time::elapsed_us(onpersist_start.elapsed());
    ...
    drop(engine);
    let postpersist_us = atipicial_runtime::time::elapsed_us(postpersist_start.elapsed());
