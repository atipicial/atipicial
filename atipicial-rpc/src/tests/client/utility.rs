...
use atipicial_payloads::WitnessCondition;
use atipicial_payloads::{Signer, TransactionAttribute};
use atipicial_primitives::{ADDRESS_SIZE, UInt256, WitnessScope};
use atipicial_serialization::json::JArray;
...
fn production_client_native_hashes_use_static_helpers() {
    ...
    for (name, source) in sources {
        assert!(
            !source.contains("AtipicialCoin::new().hash()"),
            "{name} should use the RPC client native-hash helper for ATC"
        );
        assert!(
            !source.contains("AtipicialDollar::new().hash()"),
    ...
    assert!(helper.contains("AtipicialCoin::script_hash()"));
    assert!(helper.contains("AtipicialDollar::script_hash()"));
...
fn as_script_hash_maps_native_contract_name_and_id() {
    let contract = atipicial_native_contracts::standard_native_contracts()
        .into_iter()
        .find(|contract| contract.name() == "AtipicialCoin")
        .expect("AtipicialCoin contract");
...
#[test]
fn rpc_stack_item_conversion_matches_atipicialvm_rules() {
    assert_eq!(
...
fn transaction_roundtrip_json() {
    use atipicial_payloads::witness::Witness as PayloadWitness;
    use atipicial_primitives::WitnessScope;
...
fn transaction_roundtrip_with_custom_signer() {
    use atipicial_payloads::WitnessRuleAction;
    ...
    signer.rules.push(atipicial_payloads::WitnessRule::new(
...
fn transaction_roundtrip_preserves_attributes() {
    use atipicial_payloads::OracleResponseCode;
    use atipicial_payloads::conflicts::Conflicts;
    use atipicial_payloads::not_valid_before::NotValidBefore;
    use atipicial_payloads::notary_assisted::NotaryAssisted;
    use atipicial_payloads::oracle_response::OracleResponse;
