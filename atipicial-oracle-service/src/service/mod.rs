//! # atipicial-oracle-service::service
//!
...
use atipicial_mempool::{TransactionAdmissionOutcome, TransactionOrigin};
...
#[cfg(feature = "oracle")]
pub(super) use super::atipicialfs::OracleAtipicialFsProtocol;
...
pub trait OracleRuntimeProvider:
    ChainSpecProvider<ChainSpec = AtipicialChainSpec>
    + StoreProvider
...
impl<T> OracleRuntimeProvider for T where
    T: ChainSpecProvider<ChainSpec = AtipicialChainSpec>
        + StoreProvider
        + TxAdmission<Origin = TransactionOrigin, Outcome = TransactionAdmissionOutcome>
{
}
...
pub struct OracleService<R, P = atipicial_native_contracts::StandardNativeProvider>
where
    ...
    P: NativeContractProvider + OracleContractReadProvider,
{
    ...
    wallet: RwLock<Option<Arc<Aep6Wallet>>>,
    ...
    #[cfg(feature = "oracle")]
    atipicialfs: OracleAtipicialFsProtocol,
}
