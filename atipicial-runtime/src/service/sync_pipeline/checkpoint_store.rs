...
use atipicial_storage::persistence::{StoreMaintenanceBatch, TableProvider, TransactionalStore};
...
/// therefore cannot enter Atipicial contract scans, store dumps, or state roots.
#[derive(Debug)]
...
pub(crate) fn table_read_error(
    context: &'static str,
    error: atipicial_storage::StorageError,
) -> ServiceError {
    match error {
