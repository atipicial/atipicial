...
//! instead of `Arc<atipicial_system::Node>`. It holds the same concrete handles
//! (blockchain, network, mempool, storage, chain specification, …) but is defined in
//! `atipicial-rpc` itself, so the RPC crate no longer needs to depend on the
//! composition root (`atipicial-system`).
//!
//! The composition root (typically `atipicial-node` or `atipicial-system`) constructs
//! a `NodeContext` from its `Node` and passes it to `RpcServer::new()`.
//!
//! ## Boundary
//!
//! This module belongs to `atipicial-rpc`. It must not import `atipicial_system::Node`
//! in production code. The `From<Node>` conversion lives in `atipicial-system`
...
use atipicial_mempool::{MemoryPool, TransactionAdmissionOutcome, TransactionOrigin};
use atipicial_network::NetworkHandle;
use atipicial_payloads::Transaction;
use atipicial_runtime::{StoreProvider, TxAdmission};
...
use atipicial_storage::persistence::store_cache::StoreCache;

use super::rpc_services::RpcServices;

/// Service handle bundle for the RPC server.
///
/// Replaces `Arc<atipicial_system::Node>` in `atipicial-rpc`. Constructed by the
/// composition root and passed to `RpcServer::new()`.
///
/// Cheap to clone — every field is either `Clone` (handles) or `Arc<T>`.
#[derive(Clone)]
pub struct NodeContext<P = atipicial_native_contracts::StandardNativeProvider, S = RuntimeStore>
where
    ...
    S: Store,
{
    ...
    /// Native-contract provider for AtipicialVM host calls.
    ...
    S: Store + 'static,
{
    ...
    /// concrete `atipicial_system::Node` layout.
    pub fn from_parts(
        chain_spec: Arc<AtipicialChainSpec>,
        storage: Arc<S>,
        blockchain: BlockchainHandle,
    ...
    pub fn chain_spec(&self) -> Arc<AtipicialChainSpec> {
    ...
    pub fn state_store(&self) -> Option<Arc<atipicial_state_service::StateStore<S>>> {
        self.services.state_store()
    }

    /// Returns the indexer service, if one was started.
    pub fn indexer_service(&self) -> Option<Arc<atipicial_indexer::IndexerService>> {
    ...
    pub fn ledger_provider<'a, B: atipicial_storage::CacheRead>(
        &'a self,
        snapshot: &'a DataCache<B>,
    ) -> impl atipicial_blockchain::LedgerProvider + atipicial_blockchain::ChainTipProvider + 'a {
    ...
    S: Store,
{
    ...
    fn provider<'a, B: atipicial_storage::CacheRead>(
        &'a self,
        snapshot: &'a DataCache<B>,
    ) -> Self::Provider<'a, B> {
        self.ledger_provider_factory.provider(snapshot)
