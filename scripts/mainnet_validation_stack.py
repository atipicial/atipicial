...
DEFAULT_NODE_CONFIG = "atipicial_mainnet_validate.toml"
DEFAULT_NODE_BIN = "target/release/atipicial-node"
    ...
    return {
        ...
        "steps": [
            ...
            },
            {
                ...
                "stdout_log": f"{log_dir_arg}/atipicial-node.log",
            },
            {
                "name": "state-root-validator",
                "purpose": "Continuously compare local state roots with Atipicial reference RPCs.",
                "command": [
                    "python3",
...
def command_with_writer_pid(command: Iterable[str], node_pid: int) -> list[str]:
    return [str(node_pid) if value == "<atipicial-node-pid>" else value for value in command]
