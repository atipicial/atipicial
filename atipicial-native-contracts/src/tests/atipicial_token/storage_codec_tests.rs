use super::*;
use crate::atipicial_coin::storage::candidate_signature_account;
use crate::test_support::{sample_committee, seed_committee};
use atipicial_storage::{DataCacheReadObserver, SeekDirection, StorageKey};
use atipicial_vm::{Contract, Interoperable};
use std::sync::Arc;
...
#[test]
fn atipicial_account_state_decodes_struct_fields() {
    ...
    let state = AtipicialCoin::decode_atipicial_account_state(&bytes).unwrap();
...
#[test]
fn atipicial_account_state_interoperable_projection_matches_csharp_shape() {
    let points = sample_committee();
    let state = AtipicialAccountStateView {
    ...
    let mut parsed = AtipicialAccountStateView {
...
fn candidate_state_fast_decode_accepts_canonical_shape_and_falls_back() {
    let canonical = AtipicialCoin::encode_candidate_state(true, &BigInt::from(123)).unwrap();
    assert_eq!(
        AtipicialCoin::decode_canonical_candidate_state(&canonical).unwrap(),
        Some((true, BigInt::from(123)))
    );
    assert_eq!(
        AtipicialCoin::decode_candidate_state(&canonical).unwrap(),
    ...
    .unwrap();
    assert_eq!(
        AtipicialCoin::decode_canonical_candidate_state(&noncanonical).unwrap(),
        None
    );
    assert_eq!(
        AtipicialCoin::decode_candidate_state(&noncanonical).unwrap(),
        (true, BigInt::from(123)),
...
#[test]
fn atipicial_storage_codecs_match_csharp_stackitem_bytes() {
    let points = sample_committee();
    let large_votes = BigInt::parse_bytes(b"9223372036854775808", 10).unwrap();
    let account = AtipicialAccountStateView {
    ...
    .unwrap();
    let encoded_account = AtipicialCoin::encode_atipicial_account_state(&account).unwrap();
    assert_eq!(encoded_account, expected_account);
    let decoded_account = AtipicialCoin::decode_atipicial_account_state(&encoded_account).unwrap();
    ...
    .unwrap();
    let encoded_candidate = AtipicialCoin::encode_candidate_state(true, &large_votes).unwrap();
    assert_eq!(encoded_candidate, expected_candidate);
    assert_eq!(
        AtipicialCoin::decode_candidate_state(&encoded_candidate).unwrap(),
    ...
    .unwrap();
    let encoded_committee = AtipicialCoin::encode_committee(&members).unwrap();
    assert_eq!(encoded_committee, expected_committee);

    let cache = DataCache::new(false);
    cache.add(
        AtipicialCoin::committee_key(),
    ...
    assert_eq!(
        AtipicialCoin::new()
            .read_committee_member_at(&cache, 1)
            .unwrap(),
        members[1]
    );

    let mut malformed_committee = AtipicialCoin::encode_committee(&members).unwrap();
    malformed_committee.push(0xff);
    assert_eq!(
        AtipicialCoin::decode_canonical_committee_member_at(&malformed_committee, 1).unwrap(),
        None,
...
#[test]
fn atipicial_storage_codecs_use_stack_item_projection() {
    ...
    let source = include_str!("../../atipicial_token/storage/account.rs");
    let committee_source = include_str!("../../atipicial_token/storage/committee.rs");
    let account_decoder = slice_between(
        source,
        "fn decode_atipicial_account_state",
        "fn encode_atipicial_account_state",
    );
    assert!(account_decoder.contains("decode_stack_item"));
    assert!(account_decoder.contains("AtipicialAccountStateView::from_stack_item"));
    ...
    let candidate_source = include_str!("../../atipicial_token/storage/candidates.rs");
...
#[test]
fn atipicial_storage_codecs_reject_truncated_records_like_csharp() {
    let short_account = BinarySerializer::serialize(
        &StackItem::from_struct(vec![StackItem::from_int(1)]),
        &ExecutionEngineLimits::default(),
    )
    .unwrap();
    let account_err = match AtipicialCoin::decode_atipicial_account_state(&short_account) {
        Ok(_) => panic!("AtipicialAccountState indexes four fields"),
        Err(err) => err,
    };
    assert!(
    ...
    .unwrap();
    let account = AtipicialCoin::decode_atipicial_account_state(&extra_account)
        .expect("C# AtipicialAccountState ignores extra fields");
    ...
    .unwrap();
    let candidate_err = AtipicialCoin::decode_candidate_state(&short_candidate)
    ...
    .unwrap();
    assert_eq!(
        AtipicialCoin::decode_candidate_state(&extra_candidate).unwrap(),
        (false, BigInt::from(9))
    );
    ...
    let committee_err = AtipicialCoin::new()
        .read_committee_with_votes(&cache)
...
fn candidate_vote_is_votes_or_minus_one() {
    ...
    let store = |pk: &ECPoint, registered: bool, votes: i64| {
        ...
        let key = AtipicialCoin::candidate_key(pk);
...
fn committee_address_matches_multisig_script_hash() {
    ...
    assert_eq!(
        AtipicialCoin::new()
            .compute_committee_address(&cache)
            .unwrap(),
...
fn committee_address_cache_tracks_changed_committee_bytes() {
    ...
    let second = AtipicialCoin::new()
        .compute_committee_address(&cache)
        .unwrap();
    let replacement_script =
        atipicial_vm::script_builder::redeem_script::RedeemScript::multi_sig_redeem_script_from_points(
            AtipicialCoin::committee_threshold(replacement.len()),
            replacement,
        )
        .unwrap();
...
fn committee_memoization_never_changes_the_storage_read_sequence() {
    ...
    let atipicial = AtipicialCoin::new();
    ...
    for _ in 0..2 {
        ...
        atipicial
            .next_block_validator_account(&cache, 2, 1)
    ...
    let replacement = &points[..2];
    let replacement_bytes = AtipicialCoin::encode_committee(
        &replacement
            .iter()
            .cloned()
    ...
    atipicial
        .next_block_validator_account(&cache, 2, 0)
...
fn committee_address_uninitialized_errors() {
    ...
    assert!(AtipicialCoin::new().read_committee_points(&cache).is_err());
}
...
fn committee_address_trait_override_feeds_the_engine_seam() {
    ...
    let atipicial = AtipicialCoin::new();
    assert_eq!(
        NativeContract::<atipicial_execution::native_contract_provider::NoNativeContractProvider>::committee_address(&atipicial, &cache).unwrap(),
        Some(AtipicialCoin::new().compute_committee_address(&cache).unwrap())
    );
    assert!(
        NativeContract::<atipicial_execution::native_contract_provider::NoNativeContractProvider>::committee_address(&atipicial, &DataCache::new(false)).is_err()
    );
}
...
fn balance_of_absent_account_is_zero() {
    ...
    assert_eq!(
        AtipicialCoin::new().balance_of(&cache, &account).unwrap(),
        BigInt::from(0)
    );
}
...
fn total_supply_returns_constant_not_storage_slot() {
    ...
    let cache = DataCache::new(false);
    cache.add(
        AtipicialCoin::total_supply_key(),
    ...
    let mut engine = ApplicationEngine::new_with_native_contract_provider(
        atipicial_primitives::TriggerType::Application,
        None,
        Arc::new(cache),
        None,
    ...
    let result = NativeContract::invoke(&AtipicialCoin::new(), &mut engine, "totalSupply", &[])
        .expect("totalSupply succeeds");

    assert_eq!(
        BigInt::from_signed_bytes_le(&result),
        BigInt::from(ATC_TOTAL_AMOUNT)
    );
}
...
fn register_price_requires_initialized_storage() {
    ...
    assert_eq!(
        AtipicialCoin::new().register_price(&cache).unwrap(),
        500 * 100_000_000
    );
}
...
fn gas_per_block_reads_default_and_storage() {
    ...
    assert_eq!(
        AtipicialCoin::new().gas_per_block_at(&cache, 9),
        BigInt::from(DEFAULT_GAS_PER_BLOCK)
    );
    assert_eq!(
        AtipicialCoin::new().gas_per_block_at(&cache, 20),
...
fn set_gas_per_block_write_round_trips() {
    ...
    let cache = DataCache::new(false);
    assert_eq!(
        AtipicialCoin::new().gas_per_block_at(&cache, 50),
        BigInt::from(DEFAULT_GAS_PER_BLOCK)
    );

    AtipicialCoin::new().put_gas_per_block(&cache, 10, &BigInt::from(7 * 100_000_000i64));
    assert_eq!(
        AtipicialCoin::new().gas_per_block_at(&cache, 9),
        BigInt::from(DEFAULT_GAS_PER_BLOCK)
    );
    assert_eq!(
        AtipicialCoin::new().gas_per_block_at(&cache, 10),
    ...
    AtipicialCoin::new().put_gas_per_block(&cache, 10, &BigInt::from(2 * 100_000_000i64));
    assert_eq!(
        AtipicialCoin::new().gas_per_block_at(&cache, 10),
...
fn account_state_returns_stored_struct_or_none() {
    ...
    // Store a AtipicialAccountState struct [balance, height, voteTo(Null),
    ...
    let bytes = BinarySerializer::serialize(&state, &ExecutionEngineLimits::default()).unwrap();
    let key = AtipicialCoin::account_key(&account);
    cache.add(key, StorageItem::from_bytes(bytes.clone()));
    assert_eq!(
        AtipicialCoin::new().read_account_state(&cache, &account),
        Some(bytes.clone())
    );
