    ...
    B: atipicial_storage::CacheRead,
{
    ...
    pub fn load_script_bytes(
        ...
        // Match Atipicial N3/C# semantics: scripts loaded by the host return all
    ...
    pub fn execute_allow_fault(&mut self) -> VMState {
        ...
        // The local atipicial-vm engine is the sole execution authority. It owns the
        // height-selected Atipicial N3 jump table and the invocation, reference,
    ...
    pub fn check_committee_witness(&self) -> CoreResult<bool> {
        ...
        // committee multisig address. That address is `ATC.GetCommitteeAddress`,
        // computed by AtipicialCoin (which owns the committee cache) and reached here
        // through the native-contract seam — the engine cannot depend on
        // `atipicial-native-contracts` directly. C# `GetCommitteeAddress` faults if the
    ...
    pub fn get_storage_context(&self) -> CoreResult<StorageContext> {
        ...
        // repeated GetContext calls do not deserialize the manifest and AEF.
    ...
    pub fn find_storage_entries(
        ...
        if self.execution_observations_enabled() {
            ...
            let range_direction = if options.contains(FindOptions::Backwards) {
                atipicial_vm::RangeDirection::Reverse
            } else {
