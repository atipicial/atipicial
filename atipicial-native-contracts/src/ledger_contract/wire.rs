...
use atipicial_payloads::{Transaction, TrimmedBlock};
use atipicial_primitives::UInt256;
use atipicial_serialization::BinarySerializer;
use atipicial_vm::{StackItem, VmState as VMState};
...
atipicial_vm::impl_interoperable_via_stack_item!(HashIndexState);
...
impl LedgerContract {
    ...
    pub fn serialize_persisted_transaction_state(
        ...
        let item =
            atipicial_payloads::TransactionState::new(block_index, Some(tx.clone()), vm_state)
                .try_to_stack_item()?;
    ...
    pub fn serialize_conflict_stub(&self, block_index: u32) -> CoreResult<Vec<u8>> {
        let item = atipicial_payloads::TransactionState::new(block_index, None, VMState::NONE)
            .try_to_stack_item()?;
        BinarySerializer::serialize_default(&item)
    ...
    pub(crate) fn signers_to_bytes(
        signers: &[atipicial_payloads::Signer],
        method: &str,
    ) -> CoreResult<Vec<u8>> {
        let item = StackItem::from_array(
            signers
                .iter()
                .map(atipicial_payloads::Signer::to_stack_item)
                .collect::<Vec<_>>(),
        );
        BinarySerializer::serialize_default(&item).map_err(|e| {
            CoreError::invalid_operation(format!("LedgerContract::{method}: serialize: {e}"))
    ...
    pub fn decode_transaction_state(
        bytes: &[u8],
    ) -> CoreResult<atipicial_payloads::TransactionState> {
        let item = crate::support::codec::decode_stack_item(bytes, "TransactionState")?;
        let mut state = atipicial_payloads::TransactionState::new(0, None, VMState::NONE);
        state
