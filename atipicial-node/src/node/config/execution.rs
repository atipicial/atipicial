...
/// outcomes; canonical AtipicialVM verification and publication remain synchronous.
...
pub(in crate::node) struct OptimisticSignatureVerificationRuntime {
    pub(in crate::node) pool:
        Arc<atipicial_blockchain::pipeline::signature_verification::SignatureVerificationPool>,
}

impl OptimisticSignatureVerificationSection {
    pub(in crate::node) fn validate(&self, local_execution: bool) -> anyhow::Result<()> {
        let config =
            atipicial_blockchain::pipeline::signature_verification::SignatureVerificationPoolConfig {
    ...
    pub(in crate::node) fn build_runtime(
        ...
        let pool =
            atipicial_blockchain::pipeline::signature_verification::SignatureVerificationPool::new(
                config,
            )
            .map_err(|error| {
