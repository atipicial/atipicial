...
use atipicial_storage::persistence::StoreDataCache;
use atipicial_storage::persistence::providers::{MemoryStore, RuntimeStore};
use atipicial_storage::persistence::{Store, StoreBackendKind};
use atipicial_system::NodeSystemContext;
    ...
    T: Store + 'static,
{
    ...
    finalized_stream: parking_lot::Mutex<
        Option<
            atipicial_system::FinalizedBlockStream<
                atipicial_storage::persistence::StoreCacheBacking<RuntimeStore>,
    ...
    T: Store + 'static,
{
    fn block_committing_with_live_tip(
        &self,
        block: &atipicial_payloads::Block,
        snapshot: &StoreDataCache<RuntimeStore>,
        application_executed: &[atipicial_payloads::ApplicationExecuted],
        live_tip: u64,
    ) -> bool {
        self.hooks
    ...
    fn spawn_finalized_stream(
        &self,
    ) -> tokio::task::JoinHandle<Result<(), atipicial_system::FinalizedBlockStreamError>> {
        let stream = self
            .finalized_stream
            .lock()
            .take()
...
fn chain_spec_for_settings(settings: &ProtocolSettings) -> Arc<atipicial_config::AtipicialChainSpec> {
    atipicial_test_fixtures::test_chain_spec(settings.clone())
}

fn daemon_context<P, L, T>(
    settings: Arc<ProtocolSettings>,
    snapshot: Arc<StoreDataCache<RuntimeStore>>,
    store_cache: atipicial_storage::persistence::StoreCache<RuntimeStore>,
    state_service: Option<
        Arc<atipicial_state_service::commit_handlers::StateServiceCommitHandlers<RuntimeStore>>,
    >,
    state_service_track_during_catchup: bool,
    indexer_service: Option<Arc<atipicial_indexer::IndexerService>>,
    native_contract_provider: Arc<P>,
    application_logs_service: Option<Arc<atipicial_rpc::application_logs::ApplicationLogsService<L>>>,
...
fn runtime_store_cache(
    ...
    read_only: bool,
) -> atipicial_storage::persistence::StoreCache<RuntimeStore> {
    atipicial_storage::persistence::StoreCache::new_from_store(
...
fn state_service_only_composition_skips_replay_artifact_copies() {
    use atipicial_blockchain::SystemContext;
    ...
    let state_store = Arc::new(
        atipicial_state_service::StateStore::with_mpt_store(false, memory_runtime_store())
            .expect("open runtime-backed StateService MPT"),
    );
    let state_service =
        Arc::new(atipicial_state_service::commit_handlers::StateServiceCommitHandlers::new(state_store));
    let context: TestDaemonContext<atipicial_native_contracts::StandardNativeProvider> = daemon_context(
        Arc::clone(&settings),
        snapshot,
        store_cache,
        Some(state_service),
    ...
    let block = atipicial_blockchain::genesis_block(&chain_spec_for_settings(settings.as_ref()))
        .expect("genesis block");

    assert!(
        !context.requires_replay_artifacts(&block, atipicial_blockchain::BlockPersistContext::live())
    );
}
...
fn static_archive_fences_before_canonical_commit_and_recovers_an_ahead_failure() {
    ...
    use atipicial_static_files::{StaticFileArchiveFactory, StaticFileProviderFactory};
    use atipicial_storage::persistence::StoreCache;
    use atipicial_storage::persistence::providers::memory_store::MemoryStore;
    use atipicial_system::BlockCommitHooks;

    type Hooks =
        DaemonCommitHooks<atipicial_native_contracts::StandardNativeProvider, MemoryStore, MemoryStore>;
    ...
    let provider = Arc::new(atipicial_native_contracts::StandardNativeProvider::new());
    ...
    let archive = atipicial_blockchain::StaticLedgerArchive::new(
    ...
    let discarded_archive = atipicial_blockchain::StaticLedgerArchive::new(
        StaticFileArchiveFactory::default()
    ...
    let ahead_archive = atipicial_blockchain::StaticLedgerArchive::new(
        StaticFileArchiveFactory::default()
            .open(&ahead_path)
            .expect("archive"),
    );
    ...
    drop(ahead);
    drop(ahead_archive);
    let recovered_archive = atipicial_blockchain::StaticLedgerArchiveFactory::default()
        .open(&ahead_path)
        .expect("recover staged archive suffix");
    assert_eq!(recovered_archive.tip(), Some(0));
    let empty_hot = atipicial_storage::DataCache::new(false);
...
fn canonical_archive_publication_prunes_hot_ledger_rows_atomically() {
    ...
    use atipicial_storage::persistence::ReadOnlyStoreGeneric;
    use atipicial_storage::persistence::StoreCache;
    ...
    let store = Arc::new(RuntimeStore::Mdbx(
        atipicial_storage::mdbx::MdbxStoreProvider::new(
            atipicial_storage::persistence::storage::StorageConfig {
    ...
    let block_hash_key = StorageKey::new(atipicial_native_contracts::LedgerContract::ID, block_hash_key);
    let current_block_key = StorageKey::new(
        atipicial_native_contracts::LedgerContract::ID,
        vec![atipicial_native_contracts::ledger_contract::storage::PREFIX_CURRENT_BLOCK],
    );
...
fn daemon_commit_hooks_do_not_own_core_system_resources() {
    ...
    let system_context_source =
        include_str!("../../../../atipicial-system/src/composition/system_context.rs");
    ...
    assert!(
        ...
        "atipicial-system must own the generic core context and static hook boundary"
    );
    assert!(
...
fn daemon_context_skips_state_service_mpt_during_default_cold_catchup() {
    use atipicial_blockchain::{BlockPersistContext, SystemContext};
    use atipicial_payloads::{Block, Header};
    use atipicial_state_service::{StateStore, commit_handlers::StateServiceCommitHandlers};
    use atipicial_storage::persistence::providers::memory_store::MemoryStore;
...
fn state_service_store_opens_mdbx_through_factory() {
    ...
    assert_eq!(
        store.backend_kind(),
        atipicial_storage::persistence::StoreBackendKind::Mdbx
    );
}
...
fn db_probe_replay_uses_explicit_native_provider() {
    let source = include_str!("../../bin/atipicial-db-probe.rs");
    let start = source
...
fn coordinated_mdbx_commit_publishes_ledger_and_state_root_atomically() {
    use atipicial_blockchain::{BlockPersistContext, SystemContext};
    use atipicial_storage::persistence::{RawReadOnlyStore, StoreCache};
...
"#,
    ...
    let settings = Arc::new(ProtocolSettings::default());
    let genesis = Arc::new(
        atipicial_blockchain::genesis_block(&chain_spec_for_settings(settings.as_ref()))
            .expect("construct coordinated genesis"),
    );
    let resources = atipicial_blockchain::NativePersistResources::from_provider(native_provider());
    atipicial_blockchain::persist_block_natives_with_resources(
        Arc::clone(&snapshot),
        Arc::clone(&genesis),
        Arc::clone(&settings),
        atipicial_blockchain::NativePersistOptions::default(),
        &resources,
    )
    ...
    let marker = temp.path().join(".atipicial-local-replay-poisoned");
    let shutdown = tokio_util::sync::CancellationToken::new();
    ...
    assert_eq!(
        state_backing.try_get_bytes(atipicial_state_service::Keys::CURRENT_LOCAL_ROOT_INDEX),
        None
    );
...
"#,
    ...
    assert_eq!(
        control.route(
            atipicial_execution::specialization::FLAMINGO_FACTORY_PAIR_KEY_CANDIDATE_ID,
            atipicial_execution::specialization::FLAMINGO_FACTORY_PAIR_KEY_CANDIDATE_VERSION,
        ),
        atipicial_execution::specialization::SpecializationRouteDecision::Shadow
    );
...
"#,
        temp.path().join("StateRoot_{{0}}").display(),
        temp.path().join("AtipicialIndexer_{{0}}").display(),
...
fn genesis_policy_init_visible_through_fresh_store_cache_after_commit() {
    use atipicial_blockchain::SystemContext;
    use atipicial_blockchain::native_persist::{
...
async fn daemon_context_dispatches_application_logs_handlers() {
    use atipicial_blockchain::SystemContext;
    use atipicial_payloads::{ApplicationExecuted, Block, Header, NotifyEventArgs, Signer, Transaction};
    use atipicial_primitives::{TriggerType, UInt160, WitnessScope};
    use atipicial_rpc::application_logs::{ApplicationLogsService, ApplicationLogsSettings};
    use atipicial_storage::persistence::providers::memory_store::MemoryStore;
    ...
    assert!(ctx.requires_replay_artifacts(&block, atipicial_blockchain::BlockPersistContext::live()));
    assert!(
        !ctx.requires_replay_artifacts(&block, atipicial_blockchain::BlockPersistContext::catch_up())
    );
    ...
    ctx.block_finalized(atipicial_blockchain::FinalizedBlock::new(
        Arc::new(block),
        Some(Arc::clone(&snapshot)),
        vec![executed],
        atipicial_blockchain::BlockPersistContext::live(),
    ))
    .await
...
fn post_canonical_application_logs_do_not_arm_replay_marker() {
    use atipicial_blockchain::BlockPersistContext;
    use atipicial_payloads::{Block, Header};
    use atipicial_rpc::application_logs::{ApplicationLogsService, ApplicationLogsSettings};
    use atipicial_storage::persistence::providers::memory_store::MemoryStore;
    use atipicial_system::BlockCommitHooks;
...
fn sync_batch_policy_never_spans_live_post_canonical_plugin_staging() {
    ...
    assert_eq!(
        ...
        atipicial_blockchain::SyncBatchCommitPolicy::PerBlock,
    ...
    assert_eq!(
        ...
        atipicial_blockchain::SyncBatchCommitPolicy::DeferredCatchUp,
    ...
    assert_eq!(
        ...
        atipicial_blockchain::SyncBatchCommitPolicy::DeferredLive,
...
fn static_archive_bounds_deferred_commit_staging() {
    ...
    let hooks = Hooks::new(
        ...
        None,
        None,
        Some(atipicial_blockchain::StaticLedgerArchive::new(files)),
        Arc::new(crate::node::recovery::LocalReplayGuard::new(
            None,
            tokio_util::sync::CancellationToken::new(),
        )),
...
async fn build_node_restarts_from_durable_mdbx_tip_and_resumes_sync_cursor() {
    ...
    let temp = tempfile::tempdir().expect("temp MDBX root");
    let storage_path = temp.path().join("chain");
    let private_key = atipicial_crypto::Secp256r1Crypto::generate_private_key();
    let public_key_bytes = atipicial_crypto::Secp256r1Crypto::derive_public_key(&private_key)
        .expect("derive validator public key");
    let public_key = atipicial_crypto::ECPoint::from_bytes(&public_key_bytes).expect("validator point");
    ...
    genesis.validators = vec![atipicial_config::GenesisValidator {
        public_key: validator_key,
        name: Some("Restart Test Validator".to_string()),
    }];
    let chain_spec = Arc::new(
        atipicial_config::AtipicialChainSpec::private("restart-test-private", protocol, genesis, None)
    ...
    let genesis = atipicial_blockchain::genesis_block(chain_spec.as_ref()).expect("genesis");
    ...
    let headers = atipicial_payloads::HeadersPayload::create(vec![
