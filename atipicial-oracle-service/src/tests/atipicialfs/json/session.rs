...
use super::super::super::json::atipicialfs_json_session_token;
#[cfg(feature = "atipicialfs-grpc")]
...
#[cfg(feature = "atipicialfs-grpc")]
#[test]
fn atipicialfs_json_session_token_matches_csharp_format() {
    ...
    let token = atipicialfs_v2::session::SessionToken {
        body: Some(atipicialfs_v2::session::session_token::Body {
            id: id_bytes,
            owner_id: Some(atipicialfs_v2::refs::OwnerId { value: owner_bytes }),
            lifetime: Some(atipicialfs_v2::session::session_token::body::TokenLifetime {
                exp: 12,
                nbf: 34,
                iat: 56,
            }),
            session_key,
            context: Some(atipicialfs_v2::session::session_token::body::Context::Object(
                atipicialfs_v2::session::ObjectSessionContext {
                    verb: atipicialfs_v2::session::object_session_context::Verb::Get as i32,
                    target: Some(atipicialfs_v2::session::object_session_context::Target {
                        container: Some(atipicialfs_v2::refs::ContainerId {
                            value: container_bytes,
                        }),
                        objects: vec![atipicialfs_v2::refs::ObjectId {
                            value: object_bytes,
                        }],
                    }),
                },
    ...
    let json = atipicialfs_json_session_token(&token).expect("token json");
    // Note: Rust serializes lifetime values as strings to match Atipicial's JSON format
    let expected = format!(
