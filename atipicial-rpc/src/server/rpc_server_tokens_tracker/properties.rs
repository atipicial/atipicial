//! AEP-11 property RPC handler.
//!
//! The handler invokes the target contract's `properties` method through the
//! application engine and translates the returned map into the Atipicial JSON-RPC
//! compatibility shape.
...
use super::request::Aep11PropertiesRequest;

const AEP11_PROPERTIES: [&str; 4] = ["name", "description", "image", "tokenURI"];

impl RpcServerTokensTracker {
    pub(super) fn get_aep11_properties(
        server: &RpcServer,
        params: &[Value],
        ...
        let request = Aep11PropertiesRequest::parse(params, address_version)?;
        ...
        for (key, value) in map.iter() {
            ...
            if AEP11_PROPERTIES.iter().any(|prop| *prop == key_text) {
