...
//!
//! The block-persist hot path (in atipicial-blockchain) calls [`record_block`] to
//! feed these atomics; the telemetry layer (in atipicial-node) reads them for the
//! Prometheus /metrics endpoint. Using a global avoids any dependency cycle
//! (atipicial-blockchain → atipicial-runtime ← atipicial-node).

use std::sync::atomic::{AtomicU64, Ordering};

/// Global best-known live chain tip height reported by connected peers.
/// Set from the network layer (atipicial-network session) when peers report their
...
static ATC_TOKEN_ONPERSIST_STAGES: [NativeHookMetricSlot; 7] = [
    NativeHookMetricSlot::new(),
    NativeHookMetricSlot::new(),
    NativeHookMetricSlot::new(),
    NativeHookMetricSlot::new(),
...
static ATC_TOKEN_COMMITTEE_COMPUTE_STAGES: [NativeHookMetricSlot; 8] = [
    NativeHookMetricSlot::new(),
    NativeHookMetricSlot::new(),
    NativeHookMetricSlot::new(),
    NativeHookMetricSlot::new(),
...
static ATC_TOKEN_COMMITTEE_CANDIDATE_COUNTS: [CountMetricSlot; 7] = [
    CountMetricSlot::new(),
    CountMetricSlot::new(),
    CountMetricSlot::new(),
    CountMetricSlot::new(),
...
const STANDARD_NATIVE_CONTRACTS: [NativeContractMetricLabel; 11] = [
    ...
    NativeContractMetricLabel::new(-5, "AtipicialCoin"),
    NativeContractMetricLabel::new(-6, "AtipicialDollar"),
...
pub struct NativeContractHookStats {
    ...
    /// Stable native contract id from the Atipicial N3 standard native catalog.
    pub contract_id: i32,
    /// Stable native contract name from the Atipicial N3 standard native catalog.
...
const ATC_TOKEN_ONPERSIST_STAGE_ORDER: [AtipicialCoinOnPersistStage; 7] = [
    AtipicialCoinOnPersistStage::Skip,
    AtipicialCoinOnPersistStage::ReadCachedCommittee,
    AtipicialCoinOnPersistStage::ComputeCommittee,
    AtipicialCoinOnPersistStage::WriteCommittee,
    AtipicialCoinOnPersistStage::CompareCommittee,
    AtipicialCoinOnPersistStage::NotifyCommitteeChanged,
    AtipicialCoinOnPersistStage::RefreshTotal,
];
const ATC_TOKEN_COMMITTEE_COMPUTE_STAGE_ORDER: [AtipicialCoinCommitteeComputeStage; 8] = [
    AtipicialCoinCommitteeComputeStage::ReadVotersCount,
    AtipicialCoinCommitteeComputeStage::StandbyLookup,
    AtipicialCoinCommitteeComputeStage::CandidateScanTotal,
    AtipicialCoinCommitteeComputeStage::CandidateBlockedPrefetch,
    AtipicialCoinCommitteeComputeStage::CandidatePubkeyDecode,
    AtipicialCoinCommitteeComputeStage::CandidateStateDecode,
    AtipicialCoinCommitteeComputeStage::CandidateBlockedLookup,
    AtipicialCoinCommitteeComputeStage::TopCandidateMaintenance,
];
const ATC_TOKEN_COMMITTEE_CANDIDATE_COUNT_ORDER: [AtipicialCoinCommitteeCandidateCount; 7] = [
    AtipicialCoinCommitteeCandidateCount::StorageEntries,
    AtipicialCoinCommitteeCandidateCount::MalformedKeys,
    AtipicialCoinCommitteeCandidateCount::DecodedEntries,
    AtipicialCoinCommitteeCandidateCount::RegisteredEntries,
    AtipicialCoinCommitteeCandidateCount::BlockedRegistered,
    AtipicialCoinCommitteeCandidateCount::EligibleCandidates,
    AtipicialCoinCommitteeCandidateCount::TopCandidates,
];
...
/// Fine-grained stages inside `AtipicialCoin.OnPersist`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum AtipicialCoinOnPersistStage {
...
impl AtipicialCoinOnPersistStage {
    fn label(self) -> &'static str {
        match self {
            Self::Skip => "skip",
...
/// Snapshot of one `AtipicialCoin.OnPersist` stage metric series.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct AtipicialCoinOnPersistStageStats {
...
/// Fine-grained stages inside `AtipicialCoin.ComputeCommitteeMembers`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum AtipicialCoinCommitteeComputeStage {
...
impl AtipicialCoinCommitteeComputeStage {
    fn label(self) -> &'static str {
        match self {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum AtipicialCoinCommitteeCandidateCount {
...
impl AtipicialCoinCommitteeCandidateCount {
    fn label(self) -> &'static str {
        match self {
...
/// Snapshot of one `AtipicialCoin.ComputeCommitteeMembers` stage metric series.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct AtipicialCoinCommitteeComputeStageStats {
    /// Stage label used in Prometheus output.
    pub stage: &'static str,
    /// Total stage observations recorded since process start.
    pub calls: u64,
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub struct AtipicialCoinCommitteeCandidateCountStats {
...
/// Record one fine-grained `AtipicialCoin.OnPersist` stage.
pub fn record_atipicial_coin_onpersist_stage(stage: AtipicialCoinOnPersistStage, elapsed_us: u64) {
    let slot = &ATC_TOKEN_ONPERSIST_STAGES[stage.slot_index()];
    slot.calls.fetch_add(1, Ordering::Relaxed);
    ewma(&slot.avg_us, elapsed_us);
}

/// Record one fine-grained `AtipicialCoin.ComputeCommitteeMembers` stage.
pub fn record_atipicial_coin_committee_compute_stage(
    stage: AtipicialCoinCommitteeComputeStage,
    elapsed_us: u64,
) {
    let slot = &ATC_TOKEN_COMMITTEE_COMPUTE_STAGES[stage.slot_index()];
    slot.calls.fetch_add(1, Ordering::Relaxed);
    ewma(&slot.avg_us, elapsed_us);
}

/// Record one candidate-scan item count for `AtipicialCoin.ComputeCommitteeMembers`.
pub fn record_atipicial_coin_committee_candidate_count(
    kind: AtipicialCoinCommitteeCandidateCount,
    count: u64,
) {
    let slot = &ATC_TOKEN_COMMITTEE_CANDIDATE_COUNTS[kind.slot_index()];
...
/// Snapshot fine-grained `AtipicialCoin.OnPersist` stage metrics.
pub fn atipicial_coin_onpersist_stage_stats() -> Vec<AtipicialCoinOnPersistStageStats> {
    ATC_TOKEN_ONPERSIST_STAGE_ORDER
        .iter()
        .map(|stage| {
            let slot = &ATC_TOKEN_ONPERSIST_STAGES[stage.slot_index()];
            AtipicialCoinOnPersistStageStats {
                stage: stage.label(),
                calls: slot.calls.load(Ordering::Relaxed),
                avg_us: slot.avg_us.load(Ordering::Relaxed),
            }
...
/// Snapshot fine-grained `AtipicialCoin.ComputeCommitteeMembers` stage metrics.
pub fn atipicial_coin_committee_compute_stage_stats() -> Vec<AtipicialCoinCommitteeComputeStageStats> {
    ATC_TOKEN_COMMITTEE_COMPUTE_STAGE_ORDER
        .iter()
        .map(|stage| {
            let slot = &ATC_TOKEN_COMMITTEE_COMPUTE_STAGES[stage.slot_index()];
            AtipicialCoinCommitteeComputeStageStats {
                stage: stage.label(),
                calls: slot.calls.load(Ordering::Relaxed),
                avg_us: slot.avg_us.load(Ordering::Relaxed),
            }
...
/// Snapshot candidate-scan item count metrics for `AtipicialCoin.ComputeCommitteeMembers`.
pub fn atipicial_coin_committee_candidate_count_stats() -> Vec<AtipicialCoinCommitteeCandidateCountStats> {
    ATC_TOKEN_COMMITTEE_CANDIDATE_COUNT_ORDER
        .iter()
        .map(|kind| {
            let slot = &ATC_TOKEN_COMMITTEE_CANDIDATE_COUNTS[kind.slot_index()];
            AtipicialCoinCommitteeCandidateCountStats {
                kind: kind.label(),
...
/// Direct snapshot of the hottest `AtipicialCoin.OnPersist` stage metric.
pub fn atipicial_coin_onpersist_hot_stats() -> Option<AtipicialCoinOnPersistStageStats> {
    let index = hot_timing_slot_index(&ATC_TOKEN_ONPERSIST_STAGES)?;
    let stage = ATC_TOKEN_ONPERSIST_STAGE_ORDER[index];
    Some(AtipicialCoinOnPersistStageStats {
        stage: stage.label(),
        calls: ATC_TOKEN_ONPERSIST_STAGES[index]
            .calls
            .load(Ordering::Relaxed),
        avg_us: ATC_TOKEN_ONPERSIST_STAGES[index]
            .avg_us
            .load(Ordering::Relaxed),
    })
}

/// Direct snapshot of the hottest `AtipicialCoin.ComputeCommitteeMembers` stage metric.
pub fn atipicial_coin_committee_compute_hot_stats() -> Option<AtipicialCoinCommitteeComputeStageStats> {
    let index = hot_timing_slot_index(&ATC_TOKEN_COMMITTEE_COMPUTE_STAGES)?;
    let stage = ATC_TOKEN_COMMITTEE_COMPUTE_STAGE_ORDER[index];
    Some(AtipicialCoinCommitteeComputeStageStats {
        stage: stage.label(),
        calls: ATC_TOKEN_COMMITTEE_COMPUTE_STAGES[index]
            .calls
            .load(Ordering::Relaxed),
        avg_us: ATC_TOKEN_COMMITTEE_COMPUTE_STAGES[index]
            .avg_us
            .load(Ordering::Relaxed),
    })
}
...
pub fn atipicial_coin_committee_candidate_hot_stats() -> Option<AtipicialCoinCommitteeCandidateCountStats> {
    let index = hot_count_slot_index(&ATC_TOKEN_COMMITTEE_CANDIDATE_COUNTS)?;
    let kind = ATC_TOKEN_COMMITTEE_CANDIDATE_COUNT_ORDER[index];
    Some(AtipicialCoinCommitteeCandidateCountStats {
        kind: kind.label(),
        samples: ATC_TOKEN_COMMITTEE_CANDIDATE_COUNTS[index]
            .samples
            .load(Ordering::Relaxed),
        total: ATC_TOKEN_COMMITTEE_CANDIDATE_COUNTS[index]
            .total
            .load(Ordering::Relaxed),
        avg: ATC_TOKEN_COMMITTEE_CANDIDATE_COUNTS[index]
            .avg
            .load(Ordering::Relaxed),
    })
}
