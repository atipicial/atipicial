...
The `atipicial-node` daemon is configured by a single TOML file passed with
`--config` (default: `atipicial_testnet_node.toml`). This page documents every
...
`mainnet-service.toml` for AtipicialFura-style RPC/indexer service providers.
...
flowchart LR
    ...
    E --> G
    F --> G
    G --> H[Arc AtipicialChainSpec + runtime configs]
```
...
Selects which Atipicial network the node joins.
...
Phase 1 verification mode for the `atipicial-state-packs` append engine. MDBX
...
record (`atipicial_state_packs_high_water`) into the MDBX maintenance table inside
...
`atipicial-pack-build --network-magic <u32-or-hex> --mdbx <store> --pack <new-dir>`
...
```bash
atipicial-pack-build --network-magic <u32-or-hex> \
...
MDBX/source data with the current `atipicial-pack-build`; the node does not expose an
...
- Static files are a **precommit-durable cold archive**. The node captures the exact
  ...
  Atipicial contract storage; row deletes and watermark
...
- Reads decompress and checksum the complete containing frame before returning
  ...
  full frame/index parity check. `atipicial-db-probe --static-files-dir <DIR>
...
| `max_connections` | i64 | `40` | Maximum simultaatipicialus peers. `-1` means unlimited. Alias: `MaxConnections`. |
...
(`cargo build --release -p atipicial-node --features hsm`). When `[consensus.hsm]` is
...
| `pin_env` | string | `ATC_HSM_CU_PASSWORD` | Environment variable holding the `C_Login` PIN. |
...
[consensus.hsm]
provider = "aws"
key_label = "atipicial-consensus-validator-1"
# pin_env defaults to ATC_HSM_CU_PASSWORD; export it before starting the node:
#   export ATC_HSM_CU_PASSWORD="crypto_user:hunter2"
```
...
transaction follows the ordinary sequential `atipicial-vm` persistence path.
...
This opt-in overlaps the existing block-header consensus-witness AtipicialVM check
...
always runs the complete canonical AtipicialVM witness script, including parent
...
State-root/MPT support used by Atipicial's StateService RPC methods.
...
| `path` | path | none | Optional auxiliary MDBX directory when the canonical node itself is using the in-memory provider. Persistent MDBX nodes use the canonical environment's `atipicial_state_service` named table and ignore this path. `{0}` is replaced with uppercase 8-digit network magic. Alias: `Path`. |
...
the built-in `AtipicialIndexer` RPC method group.
...
This is the built-in service-oriented indexer for AtipicialFura-style RPC workloads:
...
notification lookup rows under stable `atipicial-indexer:v3:*` prefixes and commits
...
Stores containing the removed `atipicial-indexer:snapshot:v2` whole-snapshot format
...
C# `Atipicial.Plugins.ApplicationLogs`-compatible execution log storage.
...
AEP-11/AEP-17 balance and transfer indexing used by the TokensTracker RPC
method group.

| Key | Type | Default | Meaning |
|-----|------|---------|---------|
| `enabled` | bool | `false` | Track AEP-11/AEP-17 balances and transfers. Alias: `Enabled`. |
...
| `enabled_trackers` | array of string | `["AEP-17", "AEP-11"]` | Standards to track. Alias: `EnabledTrackers`. |
| `exception_policy` | enum | plugin default | Handler exception policy. Alias: `UnhandledExceptionPolicy`. |
...
header-cache count, optional service registration gauges, AtipicialIndexer health and
index counts, AtipicialIndexer lag/sync gauges, and any metrics already registered with the process-wide
...
| `level` | string | `info,atipicial=debug` | Tracing filter directive, e.g. `info`, `debug`, or `info,atipicial=debug`. Alias: `Level`. |
...
| `service_name` | string | `atipicial-node` | Service name sent in payloads. Alias: `ServiceName`. |
...
registration, and AtipicialIndexer readiness/lag/sync.
...
available; startup errors without a source location use a `atipicial-node` fallback
...
|----------|-------|---------|
| `RUST_LOG` | daemon | Log filter (e.g. `info`, `debug`, `info,atipicial=debug`). Default when unset: `info,atipicial=debug`. |
| `ATC_NETWORK` | Docker | `testnet` (default) or `mainnet`; selects the bundled config inside the container. |
| `ATC_STORAGE` | Docker | Primary storage path inside the container. The backend still comes from the TOML preset, which uses MDBX by default. |
| `ATC_CONFIG` | Docker | Custom config path for a bind-mounted TOML. |
| `ATC_PLUGINS_DIR` | Docker | Directory for plugin configs (e.g. `RpcServer.json`). |
| `ATC_RPC_PORT` | Docker | Health-check RPC port override only; the node's actual RPC port still comes from the TOML `[rpc]` section. |
...
for arbitrary TOML keys; `ATC_*` variables apply to the Docker entrypoint, not
the native binary.
