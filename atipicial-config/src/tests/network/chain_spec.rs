...
fn mainnet_spec_pins_identity_genesis_and_hardforks() {
    let spec = AtipicialChainSpec::mainnet().expect("valid embedded MainNet spec");

    assert_eq!(spec.identity().name(), "atipicial-n3-mainnet");
...
fn testnet_spec_is_explicit_and_validated() {
    let spec = AtipicialChainSpec::testnet().expect("valid embedded TestNet spec");
...
fn built_in_genesis_pins_reject_a_different_hash() {
    for spec in [
        AtipicialChainSpec::mainnet().expect("valid embedded MainNet spec"),
        AtipicialChainSpec::testnet().expect("valid embedded TestNet spec"),
    ] {
        let error = spec
...
fn private_spec_has_closed_private_identity() {
    let spec = AtipicialChainSpec::private(
        "private-chain",
...
fn committee_mismatch_is_rejected_at_the_chain_boundary() {
    ...
    let error = AtipicialChainSpec::private("invalid-committee", protocol, genesis, None)
