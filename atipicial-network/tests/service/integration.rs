...
use atipicial_network::{LocalNodeService, NetworkCommand, PeerRegistry};
use atipicial_runtime::NetworkService;
...
async fn local_node_service_uses_supplied_peer_registry() {
    let config = atipicial_network::ChannelsConfig::default();
...
async fn next_peer_connected(
    events: &mut tokio::sync::broadcast::Receiver<atipicial_network::NetworkEvent>,
    ...
    loop {
        ...
        if let atipicial_network::NetworkEvent::PeerConnected { peer_id, address } = event {
