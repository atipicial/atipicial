...
use atipicial_native_contracts::ledger_contract::storage::{PREFIX_BLOCK, PREFIX_CURRENT_BLOCK};
use atipicial_network::BlockSource;
...
fn mainnet_chain_spec() -> Arc<atipicial_config::AtipicialChainSpec> {
...
fn empty_child_block(parent: &Block, index: u32) -> Block {
    ...
    header.witness = Witness::new_with_scripts(Vec::new(), vec![atipicial_vm::OpCode::PUSH1.byte()]);
...
fn local_source_with_block(
    ...
    let genesis =
        Arc::new(atipicial_blockchain::genesis_block(chain_spec.as_ref()).expect("genesis block"));
    ...
    let source = LedgerBlockSource::new(
        Arc::clone(&snapshot),
        Arc::new(atipicial_blockchain::LedgerContext::default()),
        Arc::new(atipicial_mempool::MemoryPool::new_with_native_contract_provider(
            chain_spec,
...
fn local_ledger_block_source_requests_restart_on_static_archive_io_error() {
    ...
    let chain_spec = mainnet_chain_spec();
    let settings = chain_spec.protocol_settings_arc();
    let populated = Arc::new(atipicial_storage::DataCache::new(false));
    let resources = atipicial_blockchain::NativePersistResources::from_provider(Arc::new(
        atipicial_native_contracts::StandardNativeProvider::new(),
    ));
    let genesis =
    ...
    let shutdown = CancellationToken::new();
    let source = LedgerBlockSource::new(
        Arc::new(atipicial_storage::DataCache::new(false)),
        Arc::new(atipicial_blockchain::LedgerContext::default()),
        Arc::new(atipicial_mempool::MemoryPool::new_with_native_contract_provider(
            chain_spec,
            atipicial_mempool::TxPoolConfig::default(),
...
fn operational_ledger_tip_reads_stay_behind_local_provider_boundary() {
    ...
    let system_context_path = std::path::Path::new(env!("CARGO_MANIFEST_DIR"))
        ...
        .join("atipicial-system/src/composition/system_context.rs");
