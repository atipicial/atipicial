//! AEP transfer-history RPC handlers.
//!
...
use crate::plugins::tokens_tracker::{Aep11Tracker, Aep17Tracker};
...
impl RpcServerTokensTracker {
    pub(super) fn get_aep11_transfers(
        server: &RpcServer,
        params: &[Value],
    ) -> Result<Value, RpcException> {
        let service = tracker_service(server)?;
        if !service.settings().enabled_aep11() || !service.settings().track_history {
    ...
    pub(super) fn get_aep17_transfers(
        server: &RpcServer,
        params: &[Value],
    ) -> Result<Value, RpcException> {
        let service = tracker_service(server)?;
        if !service.settings().enabled_aep17() || !service.settings().track_history {
            return Err(RpcException::from(RpcError::method_not_found()));
        }
