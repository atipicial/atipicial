use super::*;
use atipicial_config::{Hardfork, HardforkSchedule};
...
fn native_contract_surface() {
    ...
    let settings = atipicial_config::ProtocolSettings {
        ...
        ..atipicial_config::ProtocolSettings::csharp_default()
    };
    let pre_echidna_state =
        atipicial_execution::native_contract::build_native_contract_state(&c, &settings, 0);
    ...
    // Overloads: (aef, manifest) and (aef, manifest, data: Any).
...
fn has_method_resolves_contract_from_snapshot() {
    use atipicial_manifest::{ContractMethodDescriptor, ContractParameterDefinition};
...
fn has_method_rejects_invalid_utf8_method_name_like_csharp() {
    ...
    let mut engine = ApplicationEngine::new_with_native_contract_provider(
        ...
        None,
        atipicial_config::ProtocolSettings::default(),
        0,
...
fn contract_state_marshals_to_five_element_array() {
    ...
    // (id, updateCounter, hash, aef, manifest) as C# ContractState.ToStackItem.
...
fn deployable_manifest(name: &str) -> ContractManifest {
    use atipicial_manifest::ContractMethodDescriptor;
...
fn check_script_against_abi_validates_offsets_and_uniqueness() {
    use atipicial_manifest::{ContractEventDescriptor, ContractMethodDescriptor};
    ...
    let ret_script = vec![atipicial_vm::OpCode::RET.byte()];
...
fn manifest_is_valid_checks_serialization_and_group_signatures() {
    ...
    use atipicial_manifest::ContractGroup;
...
#[test]
fn parse_aef_checked_validates_container_and_checksum() {
    // Empty payload (C# "AEF file length cannot be zero").
    assert!(ContractManagement::parse_aef_checked(&[], "deploy").is_err());
    // A valid AEF3 container round-trips.
    let aef = AefFile::new(
        "unit-test".to_string(),
        vec![atipicial_vm::OpCode::RET.byte()],
    );
    let bytes = aef.to_bytes();
    let parsed = ContractManagement::parse_aef_checked(&bytes, "deploy").unwrap();
    assert_eq!(parsed.checksum, aef.checksum);
    assert_eq!(parsed.script, vec![atipicial_vm::OpCode::RET.byte()]);
    // Corrupting the trailing checksum fails the parse (the C#
    // AsSerializable<AefFile> checksum verifier).
    let mut corrupted = bytes;
    let last = corrupted.len() - 1;
    corrupted[last] ^= 0xFF;
    assert!(ContractManagement::parse_aef_checked(&corrupted, "deploy").is_err());
}
