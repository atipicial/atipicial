...
//! successful ticket carries an exact-input cache for the canonical AtipicialVM
...
//! chain state. Canonical parent checks, AtipicialVM execution, state publication,
...
use atipicial_payloads::{Header, Witness};
...
pub struct SignatureVerificationPoolMetricsSnapshot {
    ...
    /// Header caches installed on a canonical AtipicialVM application engine.
...
/// header: the canonical witness stage must still execute AtipicialVM and apply all
...
impl HeaderSignaturePreverification {
    ...
    /// Network magic included in the exact Atipicial sign data.
    #[must_use]
    ...
    /// Returns the immutable exact-input ECDSA cache for canonical AtipicialVM.
    #[must_use]
...
impl SignatureVerificationPool {
    ...
    pub fn new(
        ...
        for worker_index in 0..config.workers {
            ...
            let name = format!("atipicial-signature-{worker_index}");
    ...
    /// reads no provider or storage snapshot and runs no AtipicialVM code.
    pub(crate) fn try_submit_header_witness_cancellable(
        ...
        let sign_data = atipicial_payloads::get_sign_data(&header, settings.network)
...
mod tests {
    ...
    fn standard_header(settings: &ProtocolSettings, signature: Option<[u8; 64]>) -> Header {
        let private = [7u8; 32];
        let public = atipicial_crypto::Secp256r1Crypto::derive_public_key(&private).expect("public key");
        let verification =
        ...
        let sign_data = atipicial_payloads::get_sign_data(&header, settings.network).expect("sign data");
        let signature = signature.unwrap_or_else(|| {
            atipicial_crypto::Secp256r1Crypto::sign(&sign_data, &private).expect("signature")
        });
        let mut invocation = vec![atipicial_vm::OpCode::PUSHDATA1.byte(), 64];
