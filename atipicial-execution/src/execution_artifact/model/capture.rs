...
use atipicial_primitives::{UInt256, Verifiable};
use atipicial_storage::{CacheRead, DataCache};
use atipicial_vm::ContractResolutionIdentity;
use std::sync::Arc;
...
fn container_hash(
    container: Option<&Arc<atipicial_payloads::VerifiableContainer>>,
    ...
    B: CacheRead,
{
    ...
    for (index, context) in contexts.iter().enumerate() {
        ...
        let static_values =
            context.with_static_fields_mut(|slot| slot.as_ref().map(atipicial_vm::Slot::to_vec));
