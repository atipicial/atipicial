...
class DependencyHygieneTests(unittest.TestCase):
    def test_root_and_fuzz_use_only_the_workspace_vm(self):
        ...
        self.assertIn("atipicial-vm", root["workspace"]["dependencies"])
        self.assertEqual(fuzz["dependencies"]["atipicial-vm"]["path"], "../atipicial-vm")
        self.assertNotIn("atipicial-vm-rs", root["workspace"]["dependencies"])
        self.assertNotIn("atipicial-vm-rs", fuzz["dependencies"])
    ...
    def test_root_and_fuzz_locks_contain_only_the_workspace_vm(self):
        for relative_path in [Path("Cargo.lock"), Path("fuzz/Cargo.lock")]:
            with self.subTest(lock=relative_path):
                ...
                self.assertIn("atipicial-vm", names)
                self.assertNotIn("atipicial-vm-rs", names)
    ...
    def test_bincode_is_scoped_to_consensus_recovery(self):
        ...
        self.assertEqual(sorted(consumers), ["Cargo.toml", "atipicial-consensus/Cargo.toml"])
        self.assertEqual(
    ...
    def test_atipicial_rpc_does_not_reintroduce_warp_transport(self):
        manifest = (REPO_ROOT / "atipicial-rpc" / "Cargo.toml").read_text(encoding="utf-8")

        self.assertNotIn("warp =", manifest)
        self.assertNotIn("dep:warp", manifest)

    def test_active_atipicial_rpc_sources_do_not_reference_legacy_warp_transport(self):
        paths = sorted((REPO_ROOT / "atipicial-rpc" / "src").rglob("*.rs"))
        matches = []
        for path in paths:
        ...
        self.assertEqual(
            matches,
            [],
            "active atipicial-rpc sources should describe only the jsonrpsee transport path",
        )
