...
fn node_to_context(node: &Node) -> atipicial_rpc::server::NodeContext {
    atipicial_rpc::server::NodeContext::from_parts(
        node.chain_spec(),
        ...
        node.mempool(),
        node.header_cache(),
        atipicial_rpc::server::RpcServices::default(),
...
fn build_server_with_handlers() -> Arc<RwLock<RpcServer>> {
    let node = Node::for_test(atipicial_test_fixtures::test_chain_spec(
        ProtocolSettings::default(),
    ));
    let system: Arc<atipicial_rpc::server::NodeContext> = Arc::new(node_to_context(&node));
...
fn assert_atipicial_error(response: &Value, error: ServerRpcError) {
...
fn assert_atipicial_exception(response: &Value, error: &RpcException) {
    assert_eq!(response["error"]["code"], error.code());
...
async fn protected_method_without_transport_auth_returns_access_denied() {
    ...
    assert_atipicial_error(&response, ServerRpcError::access_denied());
}
...
async fn http_transport_enforces_basic_auth_when_configured() {
    ...
    let authorized: Value = client
        .post(&url)
        .basic_auth("atipicial", Some("secret"))
        .json(&request)
        .send()
        .await
...
async fn initial_read_only_methods_match_registered_handlers() {
    ...
    for (id, method) in JSONRPSEE_SMOKE_METHODS.iter().enumerate() {
        ...
        match direct_handler_result(&server, method) {
            ...
            Err(error) => assert_atipicial_exception(&response, &error),
        }
    }
}
...
#[tokio::test(flavor = "multi_thread")]
async fn rejects_non_array_params_with_atipicial_error() {
    let server = build_server_with_handlers();
    let module = build_jsonrpsee_module(Arc::downgrade(&server)).expect("module");
    let response = raw_response(
        &module,
    ...
    assert_atipicial_error(&response, ServerRpcError::invalid_request());
}

#[tokio::test(flavor = "multi_thread")]
async fn disabled_method_uses_atipicial_access_denied() {
    let server = build_server_with_handlers();
...
async fn dispatch_enforces_configured_process_rate_limit() {
    ...
    assert_atipicial_error(&second, ServerRpcError::too_many_requests());
}

#[tokio::test(flavor = "multi_thread")]
async fn unregistered_method_is_rejected_by_jsonrpsee_before_atipicial_dispatch() {
    let server = build_server_with_handlers();
    let module = build_jsonrpsee_module(Arc::downgrade(&server)).expect("module");
    let response = raw_response(
        &module,
...
#[tokio::test(flavor = "multi_thread")]
async fn handler_error_preserves_atipicial_message_and_data() {
    let node = Node::for_test(atipicial_test_fixtures::test_chain_spec(
        ProtocolSettings::default(),
    ));
    let system: Arc<atipicial_rpc::server::NodeContext> = Arc::new(node_to_context(&node));
