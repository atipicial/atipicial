...
use atipicial_state_service::{StateRoot, StateStore};
...
fn seed_state_root(state_store: &StateStore<RuntimeStore>, index: u32, byte: u8) -> StateRoot {
    let root = StateRoot::new_current(index, atipicial_primitives::UInt256::from([byte; 32]));
...
impl MptStoreSnapshot for MemoryMptStore {
    fn try_get(&self, key: &[u8]) -> atipicial_trie::MptResult<Option<Vec<u8>>> {
        Ok(self.data.lock().get(key).cloned())
    }

    fn put(&self, key: Vec<u8>, value: Vec<u8>) -> atipicial_trie::MptResult<()> {
        self.data.lock().insert(key, value);
        Ok(())
    }

    fn delete(&self, key: Vec<u8>) -> atipicial_trie::MptResult<()> {
