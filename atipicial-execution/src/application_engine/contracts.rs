...
fn trace_block_range() -> Option<(u32, u32)> {
    ...
    *RANGE.get_or_init(|| {
        std::env::var("ATC_TRACE_BLOCK").ok().and_then(|raw| {
    ...
    B: atipicial_storage::CacheRead,
{
    ...
    pub(super) fn fetch_contract(&mut self, hash: &UInt160) -> CoreResult<Arc<ContractState>> {
        ...
        match self.lookup_contract_management_state(hash) {
            Ok(Some(contract)) => {
                if diag {
                    tracing::warn!(target: "atipicial", block_index = block_idx, %hash, id = contract.id, "TRACE: fetch_contract found in snapshot");
                }
                return Ok(self.cache_contract(*hash, contract));
            }
            Ok(None) => {
                if diag {
                    tracing::warn!(target: "atipicial", block_index = block_idx, %hash, "TRACE: fetch_contract NOT found in snapshot (None)");
                }
            }
            Err(e) => {
                if diag {
                    tracing::warn!(target: "atipicial", block_index = block_idx, %hash, error = %e, "TRACE: fetch_contract snapshot error");
                }
        ...
        if let Some(native) = self.native_registry.get(hash) {
            ...
            if let Some(contract) = native.contract_state(&self.protocol_settings, block_height) {
                if diag {
                    tracing::warn!(target: "atipicial", block_index = block_idx, %hash, id = contract.id, "TRACE: fetch_contract found as native");
                }
                return Ok(self.cache_contract(*hash, contract));
            }
        }
        ...
        if diag {
            tracing::warn!(target: "atipicial", block_index = block_idx, %hash, "TRACE: fetch_contract FAILED - not found anywhere");
        }
    ...
    /// AEF/manifest, producing different `_initialize` static fields than C#.
    ...
    pub(super) fn cache_contract(
        ...
        let script = Script::new_relaxed(contract.aef.script.clone());
    ...
    fn contract_script(&mut self, contract: &Arc<ContractState>) -> Script {
        self.contract_scripts
            .entry(contract.hash)
            .or_insert_with(|| Script::new_relaxed(contract.aef.script.clone()))
            .clone()
    }
    ...
    pub(super) fn load_contract_context(
        ...
        // whole-script plan and Script identity, matching ordinary AtipicialVM.
        let script =
    ...
    pub fn call_contract_dynamic(
        ...
        if diag {
            ...
            let current = self
                ...
                .map(|h| h.to_string())
                .unwrap_or_else(|| "none".to_string());
            tracing::warn!(target: "atipicial", block_index = block_idx, %contract_hash, method, args_len = args.len(), caller, current, "TRACE: call_contract_dynamic");
        }
        let method_descriptor = contract
            ...
            .ok_or_else(|| {
                let available: Vec<String> = contract.manifest.abi.methods.iter()
                    ...
                    .collect();
                if diag {
                    tracing::warn!(target: "atipicial", block_index = block_idx, %contract_hash, method, args_len = args.len(), ?available, "TRACE: method NOT FOUND in ABI");
                }
        ...
        if diag {
            tracing::warn!(target: "atipicial", block_index = block_idx, %contract_hash, method, offset = method_descriptor.offset, "TRACE: method found, calling internal");
        }
    ...
    ///   how e.g. `PolicyContract.recoverFund` authorizes AEP-17 sweeps via the
    ...
    /// (used by AEP-17 `transfer` to invoke the recipient's `onAEP17Payment`):
