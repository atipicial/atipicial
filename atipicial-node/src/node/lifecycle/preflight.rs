...
pub(in crate::node) fn run_startup_preflight(
    ...
    match storage_preflight {
        ...
        StoragePreflightMode::ValidateLocal => {
            ...
            info!(target: "atipicial", config = %cli.config.display(), "storage preflight passed");
...
fn print_config_preflight_ok(cli: &NodeCli) {
    info!(target: "atipicial", config = %cli.config.display(), "configuration preflight passed");
