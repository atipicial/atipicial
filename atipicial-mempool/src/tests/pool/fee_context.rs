...
fn conflict_rebate_keys_notary_sponsored_conflicts_by_secondary_payer() {
    let notary = atipicial_native_contracts::Notary::script_hash();
...
impl AdmissionLedgerProvider for FailingContainsTransactionProvider {
    fn current_index<B: atipicial_storage::CacheRead>(
        &self,
    ...
    fn contains_transaction<B: atipicial_storage::CacheRead>(
        &self,
        _snapshot: &atipicial_storage::DataCache<B>,
        _hash: &UInt256,
    ) -> atipicial_error::CoreResult<bool> {
        Err(atipicial_error::CoreError::invalid_operation(
            "injected contains-transaction failure",
        ))
    }

    fn contains_conflict_hash<B: atipicial_storage::CacheRead>(
        &self,
        _snapshot: &atipicial_storage::DataCache<B>,
        _hash: &UInt256,
        _signers: &[UInt160],
