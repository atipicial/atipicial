...
use crate::plugins::tokens_tracker::{
    Aep11BalanceKey, Aep11Tracker, Aep11TransferKey, Aep17BalanceKey, Aep17Tracker,
    Aep17TransferKey, TokenBalance, TokenTransfer, TokensTrackerService, TokensTrackerSettings,
...
use atipicial_execution::ContractState;
use atipicial_execution::application_engine::TEST_MODE_GAS;
use atipicial_io::{Serializable, SerializableExtensions};
...
use atipicial_native_contracts::contract_management::ContractManagement;
use atipicial_native_contracts::{AtipicialDollar, StandardNativeProvider};
...
use atipicial_storage::persistence::providers::{MemoryStore, MemoryStoreProvider, RuntimeStore};
use atipicial_storage::persistence::{Store, StoreSnapshot, WriteStore};
...
fn build_nep11_properties_contract() -> ContractState {
    ...
    let aef = AefFile::new("aep11-properties".to_string(), script.to_array());
    let mut manifest = ContractManifest::new("Aep11Properties".to_string());
    manifest.supported_standards.push("AEP-11".to_string());
    ...
    let hash = ContractState::calculate_hash(&UInt160::zero(), aef.checksum, &manifest.name);
    ContractState::new(9, hash, aef, manifest)
}

#[tokio::test(flavor = "multi_thread")]
async fn get_nep17_balances_reports_asset_metadata() {
    let store = create_tracker_store();
    let system = system_with_tokens_tracker(Arc::clone(&store), vec!["AEP-17".to_string()], true);

    let atipicial_dollar = AtipicialDollar::new();
    let asset = atipicial_dollar.hash();
    let contract = atipicial_dollar
    ...
    let key = Aep17BalanceKey::new(user, asset);
    let (balance_prefix, _, _) = Aep17Tracker::<StandardNativeProvider>::rpc_prefixes();
    ...
    let entries = find_prefix::<_, Aep17BalanceKey, TokenBalance>(store.as_ref(), &prefix)
        .expect("find prefix");
    ...
    let address = atipicial_wallets::wallet_helper::WalletAddress::to_address(
        &user,
    ...
    assert_eq!(entry.get("name").and_then(Value::as_str), Some("AtipicialDollar"));
    assert_eq!(entry.get("symbol").and_then(Value::as_str), Some("ATD"));
...
async fn get_nep11_balances_groups_tokens_by_asset() {
    let store = create_tracker_store();
    let system = system_with_tokens_tracker(Arc::clone(&store), vec!["AEP-11".to_string()], true);
    ...
    let key_a = Aep11BalanceKey::new(user, asset, token_a.clone());
    let key_b = Aep11BalanceKey::new(user, asset, token_b.clone());
    let (balance_prefix, _, _) = Aep11Tracker::<StandardNativeProvider>::rpc_prefixes();
...
async fn get_nep11_transfers_orders_by_timestamp_descending() {
    ...
    let (_, sent_prefix, received_prefix) = Aep11Tracker::<StandardNativeProvider>::rpc_prefixes();
    let token_a = vec![0xAA];
    let token_b = vec![0xBB, 0xCC];

    let sent_key_1 = Aep11TransferKey::new(user, t1, asset, token_a.clone(), 0);
    let sent_key_2 = Aep11TransferKey::new(user, t2, asset, token_b.clone(), 1);
    write_tracker_entry(
        &store,
        sent_prefix,
        &sent_key_1,
    ...
    let received_key = Aep11TransferKey::new(user, t1, asset, token_a.clone(), 0);
    write_tracker_entry(
        &store,
        received_prefix,
        &received_key,
...
async fn get_nep11_properties_returns_expected_fields() {
    let store = create_tracker_store();
    let system = system_with_tokens_tracker(store, vec!["AEP-11".to_string()], true);
...
async fn get_nep17_transfers_orders_by_timestamp_descending() {
    ...
    let (_, sent_prefix, received_prefix) = Aep17Tracker::<StandardNativeProvider>::rpc_prefixes();
    let sent_key_1 = Aep17TransferKey::new(user, t1, asset, 0);
    let sent_key_2 = Aep17TransferKey::new(user, t2, asset, 1);
    write_tracker_entry(
        &store,
        sent_prefix,
        &sent_key_1,
    ...
    let received_key = Aep17TransferKey::new(user, t1, asset, 0);
    write_tracker_entry(
        &store,
        received_prefix,
        &received_key,
    ...
    let sent_pairs =
        find_range::<_, Aep17TransferKey, TokenTransfer>(store.as_ref(), &start_key, &end_key)
    ...
    assert_eq!(
        ...
        Some(
            atipicial_wallets::wallet_helper::WalletAddress::to_address(
                &other,
                server.system().settings().address_version
            )
            .as_str()
