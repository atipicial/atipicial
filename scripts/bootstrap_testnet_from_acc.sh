...
if [[ ! -x "$NODE_BIN" ]]; then
  if [[ -x "$ROOT/target/debug/atipicial-node" ]]; then
    NODE_BIN="$ROOT/target/debug/atipicial-node"
  else
    echo "[$(date '+%F %T')] bootstrap-error atipicial-node binary not found (tried release and debug)" >&2
    exit 1
  fi
fi
...
pkill -f 'scripts/atipicial_node_watchdog.sh' || true
pkill -f 'atipicial-node --config atipicial_testnet_node.toml' || true
...
nohup bash ./scripts/atipicial_node_watchdog.sh >/dev/null 2>&1 &
