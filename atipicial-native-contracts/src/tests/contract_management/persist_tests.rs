...
fn settings_with(hardforks: &[(Hardfork, u32)]) -> ProtocolSettings {
    let schedule = hardforks.iter().copied().fold(
        atipicial_config::HardforkSchedule::new(),
...
fn on_persist_writes_genesis_records_and_deploy_notifications() {
    ...
    // genesis-active ATC/ATD initializers emit Transfer before their
...
/// (UpdateCounter++ + the height-composed AEF/manifest) and an `Update`
...
fn echidna_block_refreshes_manifests_and_runs_policy_reinitialization() {
    ...
    // Pre-Echidna ATC manifest: AEP-17 only, no onAEP17Payment.
    let atipicial_hash = crate::AtipicialCoin::script_hash();
    let pre = ContractManagement::get_contract_from_snapshot(&snapshot, &atipicial_hash)
        .unwrap()
        .unwrap();
    assert_eq!(pre.manifest.supported_standards, ["AEP-17"]);
    ...
    // ATC: refreshed in place — UpdateCounter 1, AEP-27 joins, the Echidna
    // ABI method appears, id/hash unchanged.
    let post = ContractManagement::get_contract_from_snapshot(&snapshot, &atipicial_hash)
        .unwrap()
        .unwrap();
    assert_eq!(post.update_counter, 1);
    assert_eq!(post.id, crate::AtipicialCoin::ID);
    assert_eq!(post.manifest.supported_standards, ["AEP-17", "AEP-27"]);
    assert!(ContractManagement::abi_has_method(
        &post.manifest,
        "onAEP17Payment",
        3
    ...
    let gas_hash_bytes = crate::AtipicialDollar::script_hash().to_bytes();
    assert!(
        engine
            .notifications()
            .iter()
    ...
    let kinds: HashMap<Vec<u8>, String> = engine
        ...
        .collect();
    assert_eq!(
        kinds.get(&atipicial_hash.to_bytes().to_vec()),
