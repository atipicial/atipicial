...
# Exit status used when the C#-vs-AtipicialGo baseline cannot be compared because the
...
require_v310_reference_pair() {
  local network="$1"
  local csharp_rpc="$2"
  local atipicial_rpc="$3"

  if [[ -n "$csharp_rpc" && -n "$atipicial_rpc" ]]; then
    return 0
  fi

  local missing=()
  [[ -z "$csharp_rpc" ]] && missing+=("Atipicial C# v3.10.1")
  [[ -z "$atipicial_rpc" ]] && missing+=("AtipicialGo")
  local missing_text
