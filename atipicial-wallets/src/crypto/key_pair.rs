//! Key pair implementation for Atipicial wallets.
//!
//! This module provides cryptographic key pair functionality,
//! converted from the C# Atipicial KeyPair class (@atipicial-sharp/src/Atipicial/Wallets/KeyPair.cs).

use crate::wallet_helper::WalletAddress;
use aes::Aes256;
use aes::cipher::{BlockDecrypt, BlockEncrypt, KeyInit, generic_array::GenericArray};
use atipicial_crypto::{CryptoError, ECC, ECCurve, ECDsa, Secp256r1Crypto, base58};
use atipicial_error::{CoreError, CoreResult};
use atipicial_primitives::HASH_SIZE;
use atipicial_primitives::UInt160;
use atipicial_primitives::hex_util;
use atipicial_vm::script_builder::redeem_script::RedeemScript;
use scrypt::Params;
use std::fmt;
use subtle::ConstantTimeEq;
use zeroize::{Zeroize, Zeroizing};

/// A cryptographic key pair for Atipicial accounts.
...
impl KeyPair {
    ...
    ///
    /// Uses the shared P-256 key generator from `atipicial-crypto`.
    ...
    /// Creates a key pair from a AEP-2 encrypted private key.
    /// The encrypted_key should be the Base58Check-encoded AEP-2 "6P..." string.
    pub fn from_aep2(
        encrypted_key: &[u8],
        password: &str,
        address_version: u8,
    ) -> CoreResult<Self> {
        // AEP-2 strings are Base58Check-encoded (standard "6P..." form), matching
        // C# Wallet.GetPrivateKeyFromNEP2 -> Base58.Base58CheckDecode.
        let encrypted_str = std::str::from_utf8(encrypted_key)
            .map_err(|_| CoreError::invalid_aep2_key("invalid AEP-2 encrypted key"))?;
        let decoded = base58::decode_check(encrypted_str)
            .map_err(|_| CoreError::invalid_aep2_key("invalid AEP-2 encrypted key"))?;

        let private_key = Self::decrypt_aep2(&decoded, password, address_version)?;
        Self::from_private_key(&private_key)
    }

    /// Creates a key pair from a AEP-2 encrypted private key string.
    /// The encrypted_key should be the Base58Check-encoded AEP-2 "6P..." string.
    pub fn from_aep2_string(
        encrypted_key: &str,
        password: &str,
        address_version: u8,
    ) -> CoreResult<Self> {
        Self::from_aep2(encrypted_key.as_bytes(), password, address_version)
    }

    /// Creates a key pair from a AEP-2 string using explicit scrypt parameters,
    /// as a AEP-6 wallet carries its own `ScryptParameters` (C# `AEP6Account`
    ...
    pub fn from_aep2_string_with_params(
        encrypted_key: &str,
        password: &str,
        address_version: u8,
        n: u32,
        ...
        let decoded = base58::decode_check(encrypted_key)
            .map_err(|_| CoreError::invalid_aep2_key("invalid AEP-2 encrypted key"))?;
        let private_key =
            Self::decrypt_aep2_with_params(&decoded, password, address_version, n, r, p)?;
        Self::from_private_key(&private_key)
    }
    ...
    pub fn public_key_point(&self) -> CoreResult<atipicial_crypto::ECPoint> {
        atipicial_crypto::ECPoint::decode_compressed_with_curve(
            atipicial_crypto::ECCurve::secp256r1(),
    ...
    /// Exports the key pair to AEP-2 format.
    pub fn to_aep2(&self, password: &str, address_version: u8) -> CoreResult<String> {
        let encrypted = Self::encrypt_aep2(&self.private_key, password, address_version)?;
        // AEP-2 strings are Base58Check-encoded (C# KeyPair.Encrypt ->
    ...
    /// Exports the key pair to AEP-2 using explicit scrypt parameters (C#
    /// `KeyPair.Export(password, version, N, r, p)`), so a AEP-6 wallet's own
    /// `ScryptParameters` govern the encryption.
    pub fn to_aep2_with_params(
        &self,
        password: &str,
        address_version: u8,
        n: u32,
        ...
        let encrypted =
            Self::encrypt_aep2_with_params(&self.private_key, password, address_version, n, r, p)?;
        Ok(base58::encode_check(&encrypted))
    }
    ...
    /// Encrypts a private key using AEP-2 standard with the AEP-6 default scrypt
    ...
    fn encrypt_aep2(
        ...
        Self::encrypt_aep2_with_params(private_key, password, address_version, 16384, 8, 8)
    }

    /// Encrypts a private key using AEP-2 with explicit scrypt parameters,
    /// mirroring C# `KeyPair.Export(password, version, N, r, p)`.
    fn encrypt_aep2_with_params(
        private_key: &[u8; HASH_SIZE],
        password: &str,
        address_version: u8,
        n: u32,
        ...
        let address_hash_full = atipicial_crypto::Crypto::hash256(address.as_bytes());
        ...
        // AEP-2 uses AES-256 in ECB mode with no padding over the 32-byte
        ...
        result.extend_from_slice(b"\x01\x42"); // AEP-2 prefix
        result.push(0xe0); // AEP-2 flags: isCompressed (0x80) | isEC (0x40) | reserved (0x20)
    ...
    /// Decrypts a AEP-2 encrypted private key with the AEP-6 default scrypt
    ...
    fn decrypt_aep2(
        ...
        Self::decrypt_aep2_with_params(encrypted_key, password, address_version, 16384, 8, 8)
    }

    /// Decrypts a AEP-2 encrypted private key with explicit scrypt parameters,
    /// mirroring C# `Wallet.GetPrivateKeyFromNEP2(aep2, password, version, N, r, p)`.
    fn decrypt_aep2_with_params(
        encrypted_key: &[u8],
        password: &str,
        address_version: u8,
        n: u32,
        ...
        if encrypted_key.len() != 39 {
            return Err(CoreError::invalid_aep2_key("invalid AEP-2 key length"));
        }

        if &encrypted_key[0..2] != b"\x01\x42" {
            return Err(CoreError::invalid_aep2_key("invalid AEP-2 key prefix"));
        }
        ...
        let address = WalletAddress::to_address(&script_hash, address_version);
        let computed_hash_full = atipicial_crypto::Crypto::hash256(address.as_bytes());
        ...
        if !bool::from(computed_hash.ct_eq(address_hash)) {
            ...
            return Err(CoreError::invalid_password(
                "invalid password for AEP-2 key",
            ));
        }
    ...
    fn try_get_verification_script_for_key(private_key: &[u8; HASH_SIZE]) -> CoreResult<Vec<u8>> {
        ...
        // ASCII, so the AEP-2 scrypt salt (address) diverged and '6P…' keys were
...
atipicial_primitives::impl_display_hex!(KeyPair, compressed_public_key);
