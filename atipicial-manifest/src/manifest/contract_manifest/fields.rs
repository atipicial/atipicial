...
//!
//! Atipicial manifests expose `features` and `extra` as JSON fields on the wire. The
//! rest of the code should not need raw `serde_json::Value` plumbing to express
//! their invariants: `features` must stay empty for Atipicial N3, and `extra` is
...
/// Atipicial N3 manifest `features` field.
///
...
impl ManifestFeatures {
    ...
    /// constructed manifests; deployable Atipicial N3 manifests must still validate
    /// with an empty map.
...
/// Atipicial N3 manifest `extra` field when present.
///
