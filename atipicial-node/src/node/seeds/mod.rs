    ...
    let task = async move {
        for seed in seeds {
            ...
            match tokio::net::lookup_host(&seed).await {
                ...
                Err(err) => {
                    warn!(target: "atipicial", seed = %seed, error = %err, "seed DNS resolution failed");
...
async fn dial_seed_addresses(
    ...
    if let Some(addr) = addrs.into_iter().next() {
        match network.connect_peer(addr).await {
            Ok(id) => info!(target: "atipicial", %addr, ?id, "connected to seed"),
            Err(err) => {
                warn!(target: "atipicial", %addr, error = %err, "seed dial failed");
        ...
        warn!(target: "atipicial", seed = %seed, "seed DNS resolved no addresses");
        if let Some(observability) = observability {
            observability.report_runtime_error(
                "seed_no_addresses",
