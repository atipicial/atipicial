use super::json::{JsonParseError, parse_number_or_string_token};
...
use atipicial_serialization::json::JObject;
...
impl RpcMethodToken {
    ...
    pub fn from_json(json: &JObject) -> CoreResult<Self> {
        ...
        let has_return_value = json
            .get("hasreturnvalue")
            .map(atipicial_serialization::json::JToken::as_boolean)
    ...
    pub fn to_json(&self) -> JObject {
        ...
        json.insert(
            "hash".to_string(),
            atipicial_serialization::json::JToken::String(self.method_token.hash.to_string()),
        );
        json.insert(
            "method".to_string(),
            atipicial_serialization::json::JToken::String(self.method_token.method.clone()),
        );
        json.insert(
            "paramcount".to_string(),
            atipicial_serialization::json::JToken::Number(f64::from(self.method_token.parameters_count)),
        );
        json.insert(
            "hasreturnvalue".to_string(),
            atipicial_serialization::json::JToken::Boolean(self.method_token.has_return_value),
        );
        json.insert(
            "callflags".to_string(),
            atipicial_serialization::json::JToken::String(call_flags_to_string(
...
#[cfg(test)]
#[path = "../tests/types/method_token.rs"]
mod tests;
