...
pub fn scripts_to_witness_json(invocation: &[u8], verification: &[u8]) -> JObject {
    ...
    json.insert(
        ...
        atipicial_serialization::json::JToken::String(general_purpose::STANDARD.encode(invocation)),
    );
    json.insert(
        "verification".to_string(),
        atipicial_serialization::json::JToken::String(general_purpose::STANDARD.encode(verification)),
    );
    json
}
