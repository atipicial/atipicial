...
use super::*;
use atipicial_primitives::ADDRESS_SIZE;
use atipicial_primitives::UInt160;
use atipicial_primitives::hex_util::encode_hex;
...
fn group_condition_json_accepts_uncompressed_ecpoint_like_csharp() {
    let compressed =
        parse_group_bytes("03b209fd4f53a7170ea4444e0cb0a6bb6a53c2bd016926989cf85f9b0fba17a70c")
            .unwrap();
    let point =
        atipicial_crypto::ECPoint::from_bytes_with_curve(atipicial_crypto::ECCurve::Secp256r1, &compressed)
            .unwrap();
    ...
    <WitnessCondition as atipicial_io::Serializable>::serialize(&decoded, &mut writer)
...
fn group_condition_accepts_uncompressed_ecpoint_and_normalizes_to_compressed() {
    ...
    let mut reader = atipicial_io::MemoryReader::new(&wire);
    let condition = <WitnessCondition as atipicial_io::Serializable>::deserialize(&mut reader).unwrap();
    match condition {
