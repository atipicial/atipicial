...
use atipicial_execution::specialization::try_flamingo_pair_key;
use atipicial_primitives::constants::MAINNET_MAGIC;
use atipicial_primitives::{CallFlags, Hardfork, TriggerType, UInt160};
use atipicial_vm::evaluation_stack::EvaluationStack;
use atipicial_vm::interop_service::VmInteropDescriptor;
use atipicial_vm::reference_counter::ReferenceCounter;
...
fn build_plan_with_identity(
    ...
    let key = ExecutionPlanKey::new(
        Arc::<[u8]>::from(script),
        entry,
        ProtocolIdentity::new(network_magic, ProtocolVersion::ATC_N3_V3_10_1),
        hardforks,
...
fn mainnet_factory_script() -> Vec<u8> {
    const CONTRACT_MAP: &str = include_str!(
        "../../data/mainnet-vm-script-contract-map.json"
    );
