...
Running `atipicial-rs` in production: deployment, storage, health checks, observability, security hardening, backups, and upgrades.

This guide covers the `atipicial-node` daemon — the full node behind the `wip` feature. It syncs blocks over P2P, validates the chain, and optionally serves a JSON-RPC API. Persistent storage uses MDBX; in-memory storage is available for ephemeral and remote-ledger runs. Configuration is a single TOML file plus a few CLI flags.
...
flowchart LR
  subgraph Host
    direction TB
    N[atipicial-node daemon]
    DB[(persistent data dir)]
    N <--> DB
  end
  P2P[(Atipicial P2P network)] <-->|sync / relay| N
  subgraph Edge
...
Build the daemon and run it against a config file (the full node is `atipicial-node`'s
...
./target/release/atipicial-node --config atipicial_testnet_node.toml --storage-path /var/atipicial/testnet
```
...
./target/release/atipicial-node \
  ...
  --storage-path /var/atipicial/mainnet \
  --fast-sync
```
...
python3 scripts/run-bounded-mainnet-replay.py \
  --config data/mainnet-stateroot-clean/atipicial_mainnet_validate.toml \
  --node-bin target/release/atipicial-node \
...
When the imported package already satisfies `--stop-at-height`, `atipicial-node` can
...
one real `AtipicialDollar.transfer` invocation per block by default. Set
`ATC_BENCH_TRANSACTIONS_PER_BLOCK` from 1 through 512 to evaluate denser blocks.
...
cargo bench -p atipicial-benches --bench block_import -- mdbx_coordinated --quick

# Repeat the same measured window after 2,048 transaction-bearing prefill blocks.
ATC_BENCH_PREFILL_BLOCKS=2048 \
  cargo bench -p atipicial-benches --bench block_import -- mdbx_coordinated --quick

# Compare the legacy asynchronous MPT ceiling on the in-memory fixture.
ATC_BENCH_MPT_APPLY_BATCH_BLOCKS=16 ATC_BENCH_PREFILL_BLOCKS=2048 \
  cargo bench -p atipicial-benches --bench block_import -- memory_blocks --quick

# Exercise denser transaction-bearing blocks through the same import path.
ATC_BENCH_TRANSACTIONS_PER_BLOCK=8 ATC_BENCH_PREFILL_BLOCKS=256 \
  cargo bench -p atipicial-benches --bench block_import -- mdbx_coordinated --quick
```
...
`.atipicial-local-replay-poisoned` or claim that Ledger rolled back. Treat the local
...
`--remote-ledger-rpc <URL>` starts `atipicial-node` without opening or creating the
...
(StateService, AtipicialIndexer, ApplicationLogs, TokensTracker), and delegates
...
./target/release/atipicial-node \
  --config atipicial_mainnet_node.toml \
  --remote-ledger-rpc https://seed1.atipicial.com:10332
```
...
```ini
# /etc/systemd/system/atipicial-node.service
[Unit]
...
ExecStart=/opt/atipicial/atipicial-node --config /opt/atipicial/atipicial_mainnet_node.toml
Restart=always
RestartSec=5
LimitNOFILE=65535
...
ProtectHome=true
ReadWritePaths=/var/atipicial /var/log/atipicial
...
sudo systemctl enable --now atipicial-node
journalctl -u atipicial-node -f
```

### Docker

A multi-stage `Dockerfile` builds the daemon (`cargo build --release -p atipicial-node`) onto a slim Debian runtime as a non-root `atipicial` user, and a `docker-compose.yml` is provided. The Compose service publishes MainNet (`10332`/`10333`) and TestNet (`20332`/`20333`) ports and persists state under the `/data` volume. The daemon currently accepts only those validated built-in chain specifications.

The canonical `atipicial-vm` implementation is a workspace crate, so Docker, CI, and
...
the TOML `[rpc]` section. `ATC_CONFIG` has highest priority; otherwise
`ATC_PROFILE=service` selects the bundled service-provider preset for the chosen
network.

| Variable | Purpose | Example |
|----------|---------|---------|
| `ATC_NETWORK` | Selects the bundled config (`mainnet` / `testnet`). | `mainnet` |
| `ATC_PROFILE` | Empty/`node` for the standard config, or `service` for `config/<network>-service.toml`. | `service` |
| `ATC_CONFIG` | Path to a custom TOML config (overrides `ATC_NETWORK`). | `/config/custom.toml` |
| `ATC_STORAGE` | Persistent storage directory, passed as `--storage-path`. | `/data/mainnet` |
| `ATC_PLUGINS_DIR` | Plugin configuration directory. | `/data/Plugins` |
| `ATC_RPC_BIND_ADDRESS` | Runtime RPC bind address for bundled service profiles. | `0.0.0.0` |
| `ATC_METRICS_BIND_ADDRESS` | Runtime telemetry bind address for bundled service profiles. | `0.0.0.0` |
| `ATC_MAINNET_METRICS_PORT` / `ATC_TESTNET_METRICS_PORT` | Compose host ports for container telemetry ports `9090` / `9091`. | `19090` / `19091` |
| `ATC_LOGS_DIR` | Log directory created by the entrypoint. | `/data/Logs` |
| `ATC_RPC_PORT` | Port used by the container health check only. | `10332` |
...
| `RUST_LOG` | Log directive. | `info,atipicial_p2p=debug` |
...
service data paths from `./data/<network>/...` to `ATC_STORAGE/...` and bundled
log files to `ATC_LOGS_DIR/...`, so the indexer, ApplicationLogs,
TokensTracker, StateService, and file logs stay inside the mounted volumes.
Explicit `ATC_CONFIG` files are not rewritten.
...
`atipicial-node` reads a TOML file. Bundled samples: `atipicial_mainnet_node.toml`, `atipicial_testnet_node.toml`, `atipicial_production_node.toml`. The daemon consumes the sections below; unknown sections and keys are ignored.
...
| `[rpc]` | `enabled`, `port`, `bind_address`, `rpc_user`, `rpc_pass`, `disabled_methods`, limits | The daemon wires these into `atipicial-rpc` at startup. Built-in Basic auth protects HTTP RPC when credentials are configured; use a proxy for TLS and network-level policy. |
...
| `[state_service]` | `enabled`, `full_state`, `path` | Enables the StateService MPT store used by state proofs and state-root RPC. Persistent nodes use the canonical MDBX environment's `atipicial_state_service` table; `path` is relevant only when an ephemeral canonical node explicitly configures a separate auxiliary service store. |
| `[indexer]` | `enabled`, `store_path` | Enables AtipicialIndexer RPC methods and durable read-side indexes that resume their canonical Index stage automatically. |
| `[application_logs]` | `enabled`, `path`, `debug`, `exception_policy` | Enables C# ApplicationLogs-compatible plugin storage. |
| `[tokens_tracker]` | `enabled`, `db_path`, `enabled_trackers` | Enables AEP-11/AEP-17 token tracker services. |
...
durable AtipicialIndexer/ApplicationLogs/TokensTracker/StateService stack, expose
...
the daemon keeps the expensive AtipicialIndexer worker deferred for sync throughput
...
environment's `atipicial_state_service` named table. Size the volume with comfortable headroom and
...
parity pass. Run it through `atipicial-db-probe` without opening a hot database:

```bash
cargo run -p atipicial-node --bin atipicial-db-probe -- \
...
[[observability.error_endpoints]]
kind = "custom_json"
url = "https://errors.example.com/atipicial-node"
```
...
fall back to a `atipicial-node` report location so the event is accepted and grouped.
...
Sentry events include `release = "atipicial-node@<version>"`, `level = "fatal"` for
...
service state, and AtipicialIndexer readiness/lag/sync for custom webhook monitors.
...
`enabled` / `ready` state, and AtipicialIndexer status counters.
The metrics endpoint itself includes `atipicial_node_up`, `atipicial_node_info`,
`atipicial_node_uptime_seconds`, `atipicial_node_ledger_height`,
`atipicial_node_connected_peers`, mempool counts, header-cache count,
`atipicial_node_service_enabled{service=...}`, AtipicialIndexer health and indexed-record
gauges, `atipicial_node_indexer_blocks_behind`, `atipicial_node_indexer_synced`, and any
...
metrics: `atipicial_sync_headers_downloaded_total`,
`atipicial_sync_headers_verified_total`, `atipicial_sync_headers_checkpoint_height`,
`atipicial_sync_header_fetch_failures_total`,
`atipicial_sync_bodies_checkpoint_height`,
`atipicial_sync_body_header_mismatches_total`, `atipicial_sync_avg_verify_us`,
`atipicial_sync_avg_persist_us`,
`atipicial_sync_avg_commit_us`, the native-persistence substage gauges
...
`atipicial_sync_native_persist_avg_cache_commit_us`, the per-contract native hook
series `atipicial_sync_native_contract_hook_avg_us{trigger=...,contract=...,id=...}`,
the AtipicialCoin refresh breakdown
`atipicial_sync_atipicialtoken_onpersist_stage_avg_us{stage=...}`,
the committee recompute internals
`atipicial_sync_atipicialtoken_committee_compute_stage_avg_us{stage=...}` and
`atipicial_sync_atipicialtoken_committee_candidate_scan_avg_items{kind=...}`,
and the StateService MPT apply gauges `atipicial_state_service_mpt_apply_avg_total_us`,
`atipicial_state_service_mpt_apply_avg_project_us`,
`atipicial_state_service_mpt_apply_avg_trie_us`, and
`atipicial_state_service_mpt_apply_avg_changes`. When StateService MPT is material,
split the apply bucket with
`atipicial_state_service_mpt_apply_stage_avg_us{stage=...}` and
`atipicial_state_service_mpt_apply_avg_items{kind=...}`. These are EWMA values updated
...
initialization/manifest refresh, GAS fee accounting, ATC committee/reward
maintenance, or another native. If `AtipicialCoin` dominates `OnPersist`, split again
by the AtipicialCoin stage labels:
...
|--------|--------|---------------|
| Block height | `/metrics` `atipicial_node_ledger_height` and `getblockcount` | Lag vs. a trusted seed RPC/explorer |
| Peer count | `/metrics` `atipicial_node_connected_peers`, `getconnectioncount`, `getpeers` | Below-threshold or churning connections |
| Staged P2P sync | `/metrics` `atipicial_sync_headers_*`, `atipicial_sync_bodies_checkpoint_height`, `atipicial_sync_body_header_mismatches_total` | Header checkpoint not advancing, repeated fetch rejection, body checkpoint lag, or peers serving bodies from a different header chain |
| Mempool | `/metrics` mempool gauges and `getrawmempool` | Size stuck at 0 or growing past a cap |
| Indexer | `/metrics` `atipicial_node_service_enabled{service="indexer"}`, `atipicial_node_indexer_up`, `atipicial_node_indexer_blocks_behind`, `atipicial_node_indexer_synced`, `listservices`, `getindexerstatus` | Disabled unexpectedly, status read failures, height lag |
| Native hooks | `/metrics` `atipicial_sync_native_contract_hook_*{trigger=...,contract=...}` | Single native contract dominating OnPersist/PostPersist latency |
| AtipicialCoin refresh | `/metrics` `atipicial_sync_atipicialtoken_onpersist_stage_*{stage=...}` | Committee recompute, cache read/write, or notification work dominating AtipicialCoin OnPersist |
| AtipicialCoin committee scan | `/metrics` `atipicial_sync_atipicialtoken_committee_compute_stage_*{stage=...}` and `atipicial_sync_atipicialtoken_committee_candidate_scan_*{kind=...}` | Candidate prefix scan, candidate-state decode, Policy blocked-set prefetch, per-candidate blocked membership check, top-M maintenance, and scanned/eligible counts |
| StateService MPT | `/metrics` `atipicial_state_service_mpt_apply_*`, stage/count labels, `getstateroot`, and checkpoint probes | Apply failures, increasing per-block changes/overlay entries, MPT stage hot spots, or MPT height lag vs. chain |
...
`getstateroot` against official Atipicial seed RPCs and emit a JSON status file. That
...
python3 scripts/plan-mainnet-validation-stack.py \
  --node-config atipicial_mainnet_validate.toml \
...
uses `atipicial-node --check-all`, so it catches a chain/StateService height mismatch
...
python3 scripts/plan-stateroot-recovery.py \
  --node-config atipicial_mainnet_validate.toml \
  --probe-bin target/debug/atipicial-db-probe
```
...
`.atipicial-local-replay-poisoned`. The node fsyncs this write-ahead marker before a
...
MDBX StateService uses the canonical environment's `atipicial_state_service` named
...
python3 scripts/prepare-clean-stateroot-validation.py \
  --base-config atipicial_mainnet_validate.toml \
  --work-root data/mainnet-stateroot-clean
```

The command writes `data/mainnet-stateroot-clean/atipicial_mainnet_validate.toml` and
...
workspace. Run the bounded smoke command first: it starts `atipicial-node` with
`--stop-at-height`,
probes both the chain Ledger height and the StateService MPT current root
height with `atipicial-db-probe`, fetches `getstateroot` from the official MainNet
...
python3 scripts/maintain-stateroot-checkpoints.py \
  --status-file /tmp/stateroot-validation.json \
  --data-dir ./data \
  --writer-pid <atipicial-node-pid> \
  --watch-interval 600 \
  --waiting-interval 30 \
  --execute
```

For the isolated validation preset in `atipicial_mainnet_validate.toml`, pass its
...
`AtipicialDollar::burn` insufficient-balance errors can be enriched with the reference
block transaction whose `sysfee + netfee` matches the failed burn:

```bash
python3 scripts/diagnose-persist-failure.py \
  --log logs/atipicial-node-validate.log \
  --status-file /tmp/stateroot-validation.json
```
...
include the decoded AEP-17 `AccountState` balance in the report:
...
python3 scripts/repair-bounded-replay-gas.py \
  ...
  --log data/bounded-replay/v474701-to-595959-cachefix/atipicial-node.log
```
...
python3 scripts/repair-bounded-replay-gas.py \
  --db data/bounded-replay/v474701-to-595959-cachefix/data \
  --log data/bounded-replay/v474701-to-595959-cachefix/atipicial-node.log \
  --probe-bin target/release/atipicial-db-probe \
  --apply
```
...
python3 scripts/run-bounded-replay-with-repairs.py \
  --config data/bounded-replay/v474701-to-595959-cachefix/atipicial-bounded-replay.toml \
  --db data/bounded-replay/v474701-to-595959-cachefix/data \
  --log data/bounded-replay/v474701-to-595959-cachefix/atipicial-node.log \
...
again after an unrelated timeout. If a timeout produced no new `AtipicialDollar::burn`
balance failure, the supervisor records that fact and starts another bounded
attempt without mutating storage. The bounded runner also uses `atipicial-db-probe`
...
```bash
cargo build -p atipicial-node --bin atipicial-db-probe
...
```bash
RUST_LOG=info,atipicial_p2p=debug ./target/release/atipicial-node --config atipicial_mainnet_node.toml
```
...
```bash
journalctl -u atipicial-node -f
journalctl -u atipicial-node -p warning..alert --since "1 hour ago"
```
...
The daemon wires the `[rpc]` TOML section into the embedded `atipicial-rpc` server,
...
`atipicial-rpc`'s `RpcServerConfig`. Defaults reflect C# parity; keys present in the
...
| Concurrent connections | `max_concurrent_connections` | 100 | Caps simultaatipicialus connections. |
...
> `ATC_NATIVE_STRICT_SECURITY=1` enables extra native-contract guard checks for hardening experiments. Do **not** enable it on production consensus nodes without validating full chain parity for your exact network and dataset — it can diverge from reference behavior.
...
sudo tar czf /backups/atipicial-$(date +%F).tgz /var/atipicial/mainnet
...
| Script | Purpose |
|--------|---------|
| `scripts/run-bounded-mainnet-replay.py --config <toml> --target-height <N>` | Runs `atipicial-node` with `--stop-at-height`; add `--fast-sync` to validate the built-in fast-sync package path or `--import-chain <chain.acc>` to validate an extracted package. With `--db`, `--require-stateroot-height-match`, `--reference`, and `--require-reference-stateroot-match`, also verifies the post-run Ledger height matches the coordinated MDBX StateService table and the local root matches reference `getstateroot`. |
...
| `scripts/compare-offline-gas-storage.py --db <store_path> --address <ADDR>` | Uses `atipicial-db-probe` plus official state-root RPCs to compare selected offline GAS AccountState balances before promoting a checkpoint. |
...
in the canonical environment's `atipicial_state_service` table, so a full-state MDBX
...
`atipicial` user, then start the service. Restoring into a separate validation or
bounded-replay directory can run while unrelated `atipicial-node` processes use other
...
sequenceDiagram
  ...
  participant Svc as atipicial-node
  Op->>Svc: systemctl stop atipicial-node
  Op->>Op: Backup data dir + config
  Op->>Op: Build / pull new binary
  Op->>Svc: atipicial-node --check-all --config <path>
  Op->>Svc: systemctl start atipicial-node
...
3. **Preflight**: `atipicial-node --check-all --config <path>` to catch config/storage issues without starting.
...
- `unclaimedGas` returning `0` in live transfer paths (reverse-prefix iteration that missed AtipicialCoin GAS-per-block records).
- Contract-originated native transfers (e.g., GAS `transfer`) returning `false` unexpectedly (caller-hash resolution fix).
- ATC transfers or mints to contracts running `onNEP17Payment` before deferred GAS reward distribution (native-to-contract callback ordering fix).
- Repeated block-persistence failures such as `AtipicialDollar burn failed ... Insufficient balance for burn` on canonical blocks — treat this as divergent local state, not a recoverable network error.
