...
fn capabilities_omit_tcp_server_until_listening() {
    let identity = LocalIdentity::new(860_833_102, 7, "/atipicial-rs:test/".to_string(), true);
...
fn capabilities_advertise_disable_compression_when_disabled() {
    let identity = LocalIdentity::new(1, 2, "/atipicial-rs:test/".to_string(), false);
    assert!(
        identity
            .capabilities()
            .iter()
...
fn version_payload_carries_identity_fields() {
    let identity = LocalIdentity::new(894_710_606, 42, "/atipicial-rs:test/".to_string(), true);
    ...
    assert_eq!(payload.user_agent, "/atipicial-rs:test/");
    assert!(
        payload
            .capabilities
            .iter()
