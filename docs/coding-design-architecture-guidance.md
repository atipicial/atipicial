...
This guide is the project-wide contract for writing atipicial-rs code that reads as
high-level Atipicial domain logic while keeping protocol, storage, RPC, VM, network,
and runtime mechanics in lower layers. It is inspired by compact Atipicial tooling
codebases such as <https://github.com/atipicialburger/TEE>, where entrypoints read as
business workflows rather than implementation detail.

Do not copy that style mechanically. atipicial-rs is a Rust full node with async
services, hot paths, durable storage, and Atipicial C# parity constraints. The goal is
...
This guide is the atipicial-rs project contract. It is supported by the Rust API
...
Use those references as Rust idiom baselines, then apply the stricter atipicial-rs
...
- Prefer maintained Rust crates for commodity algorithms and data structures.
  Keep local wrappers only for Atipicial protocol byte layout, C# compatibility,
...
- Top-level code reads as ordered Atipicial domain intent.
...
atipicial-rs is a blockchain node, not a generic server. The engineering priority
order is:
...
Atipicial N3 v3.10.1 protocol parity. A faster node that can diverge from mainnet
...
- Do not swap consensus-critical codecs, Merkle roots, MPT state roots, Bloom
  filter wire behavior, native-contract serialization, or Atipicial binary IO to a
  generic crate unless byte-for-byte parity is proven against Atipicial C# / mainnet
...
Prefer maintained Rust crates for commodity algorithms. Local atipicial-rs code should
own Atipicial protocol semantics, not reimplement standard algorithms under familiar
names. A wrapper is justified only when it preserves one of these contracts:

- Atipicial/C# byte layout, endian order, wire framing, or error mapping.
...
bytes, the authoritative reference is Atipicial C# / mainnet replay parity. For
commodity math or data structures, prefer maintained crates such as RustCrypto,
`murmur3`, `bitvec`, `hex`, `bs58`, and `base64`; keep atipicial-rs code as a thin
adapter that names the Atipicial contract being protected. Do not replace a local
consensus primitive merely because a generic crate has the same algorithm name.

Use this matrix when reviewing `atipicial-crypto`, `atipicial-trie`, `atipicial-io`, storage,
wallet, and network code:

| Area | Upstream crate should own | Local atipicial-rs code should own |
| --- | --- | --- |
| Hex, Base58, Base64 | Encoding/decoding algorithms (`hex`, `bs58`, `base64`) | Atipicial address payload versioning, checksum error mapping, and .NET-compatible strict/lenient behavior |
| LZ4 and compression | Compression codec implementation (`lz4_flex` or equivalent) | Atipicial package framing, size limits, checksum policy, and DoS guards |
| Binary readers/writers | byte buffers, endian helpers, and `std::io` traits | Atipicial var-int, `ISerializable`-style contracts, max-size checks, and C# parity errors |
...
| Murmur | Murmur3 hash function | Atipicial Bloom-filter seed schedule and error mapping |
| Bloom filters | bit storage primitives and hash functions | Atipicial wire bit layout, Murmur seed schedule, and network payload compatibility |
| Signatures | curve math and verification (`p256`, `secp256k1`/`k256`, `ed25519-dalek`, `blst`) | raw `r||s` framing, low-s/C# parity, AtipicialFs prefixes, hardfork gates, and native-contract error mapping |
| BIP-32/BIP-39 | mnemonic and derivation primitives when the crate supports the required curve and serialization | Atipicial wallet defaults, AEP-6 account model, P-256 behavior, path policy, and zeroization boundaries |
| Merkle tree | reusable proof helpers only if custom hash/duplication/layout is exact | Atipicial block/MerkleBlock root bytes: odd-leaf duplication, `Hash256(left || right)`, little-endian `UInt256`, and trim/proof shape |
| MPT trie | reference ideas and low-level data structures only | Atipicial C# MPT node types, serialization, empty-node behavior, hash domain, proof shape, and state-root parity |
...
name represents a real Atipicial protocol contract. If a thin adapter remains, its
module rustdoc must state which upstream crate does the commodity work and which
Atipicial-specific behavior the adapter protects.
...
## Reth/Polkadot Layering Priorities for Atipicial

Use reth and Polkadot as architectural references, not as templates to copy
wholesale. Atipicial has dBFT immediate finality and native contracts, so the highest
...
1. **One canonical block-import path.** Consensus, sync, fast sync, and RPC
   submission should depend on `atipicial_runtime::BlockImport` and receive
   `BlockImportOutcome` / `ImportedTip` rather than reaching into
   `atipicial_blockchain::BlockchainCommand`. `atipicial-blockchain` owns the concrete
   validation, execution, native-persist, state-root, and durable-store
   implementation behind that trait. Use `atipicial_runtime::BlockImportQueue` for
   bounded concurrent preverification, but keep ordered import in
   `BlockImport::import_many`. Use `atipicial_system::StagedSyncPipeline` as the
   ...
   Unsolicited peer bursts must use the `atipicial_system::LiveBlockImportPipeline`
   ...
   Downloader code should expose `atipicial_network::BlockDownloader` streams and use
   `atipicial_system::SyncDownloadImportDriver` to convert downloaded batches into
   ordered `atipicial_runtime::SyncPipelineDriver` submissions. Stage flushing and
   crash-resume markers belong in
   `atipicial_runtime::sync_pipeline::{CommitPolicy, SyncStageCheckpointStore}`
   ...
   to `Store`. `atipicial-system` owns the concrete cache and exposes
   `CanonicalCommit<S>` to application hooks; do not pass `StoreCache` into
   `atipicial-node` policy code. Never substitute commit-then-flush because a failed
   ...
   `atipicial_network::CrossPeerBlockRangeScheduler` so downloader policy stays
   ...
   scheduler. Use `atipicial_network::BlockDownloadCoordinator` to compose that
   scheduler with `atipicial_network::OrderedBlockBatchBuffer` and a
   transport-specific `atipicial_network::BlockRangeFetcher`; production P2P range
...
2. **Three explicit block-outcome channels.** Do not force durability
   ...
   rollback. Atipicial finalized projection delivery is committed-only; exceptional
...
5. **Typed tables only adapt established bytes.** Define a zero-sized `Table`
   ...
   abstraction must not invent a different Atipicial serialization. Compact derives
   are limited to explicitly versioned node-local records unless byte parity
   with the established Atipicial codec is proven.
6. **Operational metadata must be physically isolated.** Prune watermarks,
   ...
   the normal Atipicial data table: typed scans can interpret them as `StorageKey`
...
meta-runtime into Atipicial just for symmetry. Keep the part that matters here:
...
Top-level workflows should read like a Atipicial operation:
...
source organization part of the API (`atipicial_blockchain::service::*`,
`atipicial_network::wire::*`, or `atipicial_network::proto::*`). Prefer
`atipicial_blockchain::BlockchainHandle`, `atipicial_network::NetworkHandle`, and root-level
...
- Do not let `serde_json::Value`, raw stack items, unlabeled byte vectors, or
  loosely typed maps escape a boundary that can decode them into a Atipicial type.
- Use named structs and enums for repeated protocol shapes, even when they start
  with two fields.
- Public workflow APIs should pass and return values with Atipicial meaning:
...
```text
atipicial-node/src/node/fast_sync/
...
- Raw JSON, stack items, byte vectors, or primitive tuples crossing boundaries
  where named Atipicial types would explain the contract.
