...
use atipicial_execution::{PreverifiedSignatureCache, preverify_standard_witness_signatures};
use atipicial_payloads::{Block, Header, Witness};
use atipicial_primitives::UInt256;
use atipicial_runtime::BlockOrigin;
...
impl MockConsensusWitnessContext {
    fn new(parent: Option<ParentHeaderContext>) -> Self {
        Self {
            chain_spec: atipicial_test_fixtures::test_chain_spec(ProtocolSettings::default()),
...
impl ConsensusWitnessContext for MockConsensusWitnessContext {
    ...
    fn native_contract_provider(&self) -> Arc<atipicial_native_contracts::StandardNativeProvider> {
...
fn stage(
    parent: Option<ParentHeaderContext>,
) -> AtipicialConsensusWitnessStage<MockConsensusWitnessContext> {
    AtipicialConsensusWitnessStage::new(Arc::new(MockConsensusWitnessContext::new(parent)))
}
...
fn true_witness() -> Witness {
    Witness::new_with_scripts(Vec::new(), vec![atipicial_vm::OpCode::PUSH1.byte()])
}
...
fn signed_standard_witness(header: &Header) -> (Witness, Arc<PreverifiedSignatureCache>) {
    ...
    let sign_data = atipicial_payloads::get_sign_data_vec(header, ProtocolSettings::default().network)
...
fn consensus_witness_stage_owns_concrete_context_type() {
    ...
    assert!(
        source.contains("pub struct AtipicialConsensusWitnessStage<C>"),
    ...
    assert!(
        ...
        "owned homogeatipicialus stage context should not be erased to dyn"
    );
}
...
#[test]
fn consensus_witness_stage_accepts_exact_cache_without_skipping_atipicialvm() {
...
fn consensus_witness_cache_hit_still_executes_the_remaining_verification_script() {
    ...
    aborting_script.push(atipicial_vm::OpCode::ABORT.byte());
...
fn consensus_witness_stage_rejects_wrong_consensus_account() {
    ...
    let parent = ParentHeaderContext {
        ...
        index: 0,
        timestamp: 10,
        next_consensus: Witness::new_with_scripts(Vec::new(), vec![atipicial_vm::OpCode::PUSH2.byte()])
            .script_hash(),
    };
    let block = block(1, parent_hash, 20, witness);
...
fn cached_valid_signature_cannot_bypass_parent_next_consensus() {
    ...
    let parent = ParentHeaderContext {
        ...
        index: 0,
        timestamp: 10,
        next_consensus: Witness::new_with_scripts(Vec::new(), vec![atipicial_vm::OpCode::PUSH1.byte()])
            .script_hash(),
    };
