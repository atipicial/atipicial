...
async fn wallet_api_get_unclaimed_gas_uses_block_count() {
    ...
    let script = build_dynamic_call_script(&atipicial_hash(), "unclaimedGas", &args);
    ...
    let balance = api
        .get_unclaimed_atipicial_dollar(&address)
        .await
...
#[tokio::test]
async fn wallet_api_get_atipicial_and_gas_balances() {
    if !localhost_binding_permitted() {
        return;
    }
    ...
    let args = vec![serde_json::json!(account.to_string())];
    let atipicial_script = build_dynamic_call_script(&atipicial_hash(), "balanceOf", &args);
    let gas_script = build_dynamic_call_script(&gas_hash(), "balanceOf", &args);
    let atipicial_script_b64 = general_purpose::STANDARD.encode(atipicial_script);
    let gas_script_b64 = general_purpose::STANDARD.encode(gas_script);

    let mut server = Server::new_async().await;
    mock_invokescript(
        &mut server,
        &atipicial_script_b64,
    ...
    let atipicial_balance = api.get_atipicial_balance(&address).await.expect("atipicial balance");
    assert_eq!(atipicial_balance, 1_00000000u32);
...
async fn wallet_api_get_account_state_combines_balances() {
    ...
    let unclaimed_script = build_dynamic_call_script(&atipicial_hash(), "unclaimedGas", &unclaimed_args);
    ...
    assert_eq!(state.atipicial_balance, 10_00000000u32);
    assert!((state.gas_balance - 2.1).abs() < f64::EPSILON);
    assert!((state.unclaimed_atipicial_dollar - 0.5).abs() < f64::EPSILON);
}
