//! Murmur3 hash helpers used by Atipicial runtime and native contracts.
//!
...
//! byte-slice calls used by Atipicial's bloom-filter seed schedule.
...
pub fn murmur32(data: &[u8], seed: u32) -> u32 {
    match murmur3_32(&mut Cursor::new(data), seed) {
        Ok(hash) => hash,
        Err(error) => {
            tracing::error!(target: "atipicial_crypto", %error, "murmur32 in-memory hash failed");
            0
        }
    }
}
...
pub fn murmur128(data: &[u8], seed: u32) -> [u8; 16] {
    match murmur3::murmur3_x64_128(&mut Cursor::new(data), seed) {
        ...
        Err(error) => {
            tracing::error!(target: "atipicial_crypto", %error, "murmur128 in-memory hash failed");
            [0; 16]
        }
    }
}
