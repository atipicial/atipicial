...
use super::*;
use atipicial_blockchain::{BlockchainCommand, BlockchainHandle, ImportBlocksReply};
use atipicial_payloads::block::Block;
use atipicial_payloads::{Signer, Transaction, Witness};
...
fn non_empty_block_with_prev_hash(
    index: u32,
    prev_hash: atipicial_primitives::UInt256,
...
fn memory_store_with_ledger_tip(tip: u32, hash: atipicial_primitives::UInt256) -> Arc<MemoryStore> {
...
async fn import_chain_acc_can_stop_count_only_file_before_full_end() {
    ...
    let imported = import_chain_acc_from_reader_until_height(
        ...
        None,
        Some(2),
        None::<Arc<atipicial_storage::persistence::providers::memory_store::MemoryStore>>,
    )
    .await
...
async fn import_chain_acc_rejects_partial_range_prev_hash_mismatch_before_import() {
    let (handle, mut commands, _events) = BlockchainHandle::channel(1, 1);
    let local_tip_hash = atipicial_primitives::UInt256::from([0xAA; 32]);
    let store = memory_store_with_ledger_tip(9, local_tip_hash);
    let wrong_prev_hash = atipicial_primitives::UInt256::from([0xBB; 32]);
...
async fn import_chain_acc_rejects_internal_prev_hash_mismatch_before_import() {
    ...
    let wrong_prev_hash = atipicial_primitives::UInt256::from([0xCC; 32]);
...
async fn import_chain_acc_report_times_only_transaction_bearing_batches_for_transaction_bps() {
    let (handle, mut commands, _events) = BlockchainHandle::channel(1, 1);
    let empty_run = atipicial_blockchain::empty_block_fast_forward::MAX_EMPTY_BLOCK_FAST_FORWARD_BLOCKS;
    ...
    let service = tokio::spawn(async move {
        ...
        reply
            .send(ImportBlocksReply::ok_with_stats(
                import.blocks.len(),
                atipicial_blockchain::ImportBlocksStats {
...
fn chain_acc_batch_keeps_short_mixed_runs_until_normal_boundary() {
    let empty_limit = atipicial_blockchain::empty_block_fast_forward::MAX_EMPTY_BLOCK_FAST_FORWARD_BLOCKS;
...
fn empty_only_chain_acc_batches_flush_at_outer_import_boundary() {
    ...
    let max = atipicial_blockchain::empty_block_fast_forward::MAX_EMPTY_BLOCK_FAST_FORWARD_BLOCKS;
    for _ in 0..max {
        pending.record_pushed(&empty);
    }
