...
The Rust node emits structured trace lines when ATC_TRACE_BLOCK is set:
...
CSHARP_RPC = "http://seed1.atipicial.com:10332"
...
#   2026-04-11T10:00:00.000Z  WARN atipicial: TRACE: tx execution result block_index=125000 tx_hash=0xabc... vm_state=HALT gas_consumed=1234 notif_count=2 exception=None
#   2026-04-11T10:00:00.000Z  WARN atipicial: TRACE: notification block_index=125000 tx_hash=0xabc... notif_idx=0 contract=0xef4073a0f2b305a38ec4050e4d3d28bc40ea63f5 event=Transfer state=["bytes:0x...", "int:100"]
#
...
def parse_rust_log(log_path: str, blocks: set[int] | None = None):
    ...
    with open(log_path, "r", errors="replace") as f:
        for raw_line in f:
            ...
            if "ATC_TX_ARTIFACT" in line:
                payload = line.split("ATC_TX_ARTIFACT", 1)[1].strip()
                try:
                    artifact = json.loads(payload)
                except json.JSONDecodeError as exc:
                    raise ValueError("malformed ATC_TX_ARTIFACT") from exc
                if not isinstance(artifact, dict):
                    raise ValueError("malformed ATC_TX_ARTIFACT")
                try:
...
def rpc_call(url: str, method: str, params: list, timeout: int = 30, retries: int = 5):
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "atipicial-rs-divergence-finder/1.0",
        },
        method="POST",
    )
...
def process_block(
    ...
    for tx_hash in tx_hashes:
        ...
        if require_artifact and "artifact" not in rust_tx:
            lines.append(
                f"  TX {tx_hash}: INCOMPLETE Rust evidence; full ATC_TX_ARTIFACT missing"
            )
            divergent_txs += 1
            continue
