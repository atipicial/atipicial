//! Typed handles for Atipicial's standard native-contract set.
//!
...
use atipicial_payloads::{Block, TransactionAttribute, TransactionState, TrimmedBlock};
...
/// Concrete handle for one of Atipicial N3's canonical native contracts.
...
pub enum StandardNativeContract {
    ...
    /// AtipicialCoin native contract.
    AtipicialCoin(AtipicialCoin),
    /// AtipicialDollar native contract.
    AtipicialDollar(AtipicialDollar),
...
macro_rules! with_standard_contract {
    ($self:expr, $contract:ident => $body:expr) => {
        match $self {
            ...
            StandardNativeContract::AtipicialCoin($contract) => $body,
            StandardNativeContract::AtipicialDollar($contract) => $body,
...
impl StandardNativeContract {
    ...
    pub fn all() -> [Self; crate::STANDARD_NATIVE_CONTRACT_COUNT] {
        [
            ...
            Self::AtipicialCoin(AtipicialCoin::new()),
            Self::AtipicialDollar(AtipicialDollar::new()),
    ...
    pub const fn has_on_persist_hook(&self) -> bool {
        matches!(
            ...
            Self::ContractManagement(_)
                ...
                | Self::AtipicialCoin(_)
                | Self::AtipicialDollar(_)
                | Self::Notary(_)
        )
    }
    ...
    pub const fn has_post_persist_hook(&self) -> bool {
        matches!(
            self,
            Self::LedgerContract(_) | Self::AtipicialCoin(_) | Self::OracleContract(_)
        )
    }
}
