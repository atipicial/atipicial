...
fn token_balance_roundtrip() {
    let entry = RpcAep11TokenBalance {
        token_id: vec![0x01],
    ...
    let parsed = RpcAep11TokenBalance::from_json(&json).unwrap();
...
fn balances_and_tokens_arrays_keep_lossy_parse_behavior() {
    ...
    let valid_token = RpcAep11TokenBalance {
        token_id: vec![0x01],
        amount: BigInt::from(5),
        last_updated_block: 3,
    }
    ...
    let mut valid_balance = RpcAep11Balance {
        asset_hash: UInt160::zero(),
        name: "Test".to_string(),
        symbol: "T".to_string(),
        decimals: 0,
    ...
    let parsed = RpcAep11Balances::from_json(&root, &settings).unwrap();
    assert_eq!(parsed.balances.len(), 1);
