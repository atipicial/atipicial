...
use atipicial_payloads::{Block, Signer, Transaction, Witness, header::Header};
use atipicial_primitives::{CallFlags, UInt160, UInt256, WitnessScope};
use atipicial_state_service::{
    StateRootApplyMetrics, StateStore, commit_handlers::StateServiceCommitHandlers,
};
use atipicial_storage::{
...
use atipicial_system::{BlockCommitHooks, CanonicalCommit, NodeCoreBuilder};
...
impl BenchmarkImportMode {
    fn from_env() -> Self {
        match std::env::var("ATC_BENCH_IMPORT_MODE").as_deref() {
...
fn benchmark_mpt_apply_batch_blocks() -> usize {
    std::env::var("ATC_BENCH_MPT_APPLY_BATCH_BLOCKS")
        .ok()
...
fn benchmark_prefill_blocks() -> usize {
    std::env::var("ATC_BENCH_PREFILL_BLOCKS")
        .ok()
        .and_then(|value| value.parse().ok())
        .unwrap_or(0)
}
...
fn benchmark_transactions_per_block() -> usize {
    std::env::var("ATC_BENCH_TRANSACTIONS_PER_BLOCK")
        .ok()
        .and_then(|value| value.parse().ok())
...
fn benchmark_stage_metrics_enabled() -> bool {
    std::env::var("ATC_BENCH_STAGE_METRICS")
        .is_ok_and(|value| matches!(value.as_str(), "1" | "true" | "TRUE" | "yes" | "YES"))
}

fn benchmark_replay_artifacts_required() -> bool {
    std::env::var("ATC_BENCH_REPLAY_ARTIFACTS")
        .is_ok_and(|value| matches!(value.as_str(), "1" | "true" | "TRUE" | "yes" | "YES"))
}
    ...
    S: Store,
{
    ...
    fn block_committing(
        ...
        _application_executed: &[atipicial_payloads::ApplicationExecuted],
        _live_tip: u64,
...
impl ChainCursor {
    fn after_genesis(chain_spec: &AtipicialChainSpec) -> Self {
        let settings = chain_spec.protocol_settings();
        let genesis = atipicial_blockchain::genesis_block(chain_spec).expect("build benchmark genesis");
        Self {
            next_index: 1,
            next_nonce: 1,
...
fn gas_transfer_script(sender: &UInt160, recipient: &UInt160) -> Vec<u8> {
    ...
    builder.emit_push(&AtipicialDollar::script_hash().to_array());
    builder
        .emit_syscall("System.Contract.Call")
        .expect("emit ATD transfer syscall");
    builder.to_array()
}
...
fn print_native_persist_metrics(backend: &str) {
    use atipicial_runtime::sync_metrics as metrics;
...
{
    let chain_spec = AtipicialChainSpec::mainnet().expect("build benchmark MainNet chain spec");
    ...
    assert_eq!(
        AtipicialDollar::balance_of(snapshot.as_ref(), &recipient).expect("read recipient ATD balance"),
        BigInt::from(TRANSFER_AMOUNT)
            * BigInt::from(
                u64::try_from(transactions_per_block)
                    .expect("benchmark transaction count fits u64"),
            ),
        "benchmark ATD transfer must execute successfully before measurement"
    );
            ...
            benchmark_canonical_import(
                ...
                || {
                    let (canonical_store, canonical_tempdir) =
                        make_mdbx_store("atipicial-canonical-import-mdbx-bench");
                    let state_backing = Arc::new(
                        canonical_store
                            .open_namespace(atipicial_state_service::MDBX_STATE_SERVICE_NAMESPACE)
