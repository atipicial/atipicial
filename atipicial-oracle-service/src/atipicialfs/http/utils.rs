...
pub(crate) fn normalize_atipicialfs_endpoint(endpoint: &str) -> CoreResult<String> {
    let trimmed = endpoint.trim();
    if trimmed.is_empty() {
        return Err(CoreError::other("AtipicialFs endpoint not configured"));
    }
...
pub(crate) fn map_atipicialfs_status(status: StatusCode) -> Option<OracleResponseCode> {
