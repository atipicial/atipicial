...
use atipicial_tests::state::{MemoryWorldState, StateChanges, StorageItem, StorageKey, WorldState};
use atipicial_vm::{ExecutionEngine, OpCode, Script, StackItem, VmError, VmState};
...
fn try_run_script(script: &[u8]) -> Result<ExecutionResult, VmError> {
    ...
    if state == VmState::FAULT {
        let message = engine
            ...
            .unwrap_or_else(|| "local atipicial-vm execution faulted".to_string());
...
fn run_script(script: &[u8]) -> ExecutionResult {
    try_run_script(script).expect("local atipicial-vm should execute script")
}
