...
use atipicial_storage::{StorageKey, StorageResult};

use super::StaticLedgerArchive;

const PRUNED_THROUGH_KEY: &[u8] = b"atipicial.ledger.hot-pruned-through.v1";
...
pub struct HotLedgerPruatipicialOutcome {
...
impl StaticLedgerArchive {
    ...
    pub fn prune_hot_through<S: TransactionalStore>(
        ...
        target: u32,
        max_frames: usize,
    ) -> CoreResult<HotLedgerPruatipicialOutcome> {
        if max_frames == 0 {
        ...
        if previous_watermark.is_some_and(|watermark| watermark >= target) {
            return Ok(HotLedgerPruatipicialOutcome {
                previous_watermark,
                pruned_through: previous_watermark,
                ..HotLedgerPruatipicialOutcome::default()
            });
        }
        ...
        Ok(HotLedgerPruatipicialOutcome {
            previous_watermark,
...
fn storage_error(context: &'static str) -> impl FnOnce(atipicial_storage::StorageError) -> CoreError {
    move |error| CoreError::io(format!("{context}: {error}"))
}

fn table_read_error(context: &'static str, error: atipicial_storage::StorageError) -> CoreError {
    match error {
        atipicial_storage::StorageError::Serialization { .. } => {
...
fn static_file_error(
    context: &'static str,
) -> impl FnOnce(atipicial_static_files::StaticFileError) -> CoreError {
    move |error| CoreError::io(format!("{context}: {error}"))
}
