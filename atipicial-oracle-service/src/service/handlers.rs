...
impl<R, P> atipicial_runtime::CommittingHandler for OracleService<R, P>
where
    ...
    P: NativeContractProvider + OracleContractReadProvider + 'static,
{
    fn blockchain_committing_handler<B: atipicial_storage::CacheRead>(
        &self,
        network: u32,
        _block: &atipicial_payloads::Block,
...
impl<R, P> atipicial_runtime::WalletChangedHandler for OracleService<R, P>
where
    R: OracleRuntimeProvider + 'static,
    P: NativeContractProvider + OracleContractReadProvider + 'static,
{
    type Sender = ();
    type Wallet = Aep6Wallet;
