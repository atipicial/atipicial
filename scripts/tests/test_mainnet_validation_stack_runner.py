...
class MainnetValidationStackRunnerTests(unittest.TestCase):
    def test_start_stack_runs_preflight_and_starts_processes_with_pid_files(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            plan = module.build_plan(
                node_config=Path("atipicial_mainnet_validate.toml"),
                node_bin=Path("target/release/atipicial-node"),
            ...
            self.assertEqual((root / "pids" / "atipicial-node.pid").read_text(encoding="utf-8"), "101\n")
            self.assertEqual(
    ...
    def test_start_stack_returns_preflight_failure_without_spawning_processes(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse((root / "pids" / "atipicial-node.pid").exists())

    def test_stack_status_reports_pid_file_health(self):
        module = load_module()
        with tempfile.TemporaryDirectory() as tmp:
            pid_dir = Path(tmp)
            (pid_dir / "atipicial-node.pid").write_text("101\n", encoding="utf-8")
    ...
    def test_stop_stack_terminates_running_pids_in_reverse_runtime_order(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertFalse((pid_dir / "atipicial-node.pid").exists())
