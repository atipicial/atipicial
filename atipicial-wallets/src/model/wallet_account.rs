...
//!
//! This module mirrors the behaviour of the C# `Atipicial.Wallets.WalletAccount`
//! hierarchy, providing the core primitives used by AEP-6 wallets and other
...
use atipicial_payloads::Witness;
...
pub trait WalletAccount: Send + Sync {
    ...
    /// Returns whether the account owns signing material (key or AEP-2 payload).
    ...
    /// Locks the account (when a AEP-2 key is available).
    fn lock(&mut self);
    ...
    /// Exports the key material encoded as AEP-2.
...
pub struct StandardWalletAccount {
    ...
    /// AEP-2 scrypt cost parameters `(N, r, p)`. A AEP-6 wallet carries its own
    /// `ScryptParameters`, so the owning wallet sets these; they default to the
    /// AEP-6 standard (16384, 8, 8). C# `AEP6Account` decrypts with
    /// `wallet.Scrypt.N/R/P`, so a non-default wallet only decrypts with matching
    /// values.
    scrypt: (u32, u32, u32),
}

/// AEP-6 default scrypt parameters `(N, r, p)`.
...
impl StandardWalletAccount {
    ...
    /// Creates an account backed solely by a AEP-2 encrypted key.
    pub fn new_from_encrypted(
        ...
        Self {
            ...
            contract,
            key_pair: None,
            aep2_key: Some(aep2_key),
            protocol_settings,
            scrypt: DEFAULT_SCRYPT,
        }
    }

    /// Returns the stored AEP-2 key (if any).
    pub fn aep2_key(&self) -> Option<&str> {
        self.aep2_key.as_deref()
    }

    /// Replaces the stored AEP-2 payload.
    pub fn set_aep2_key(&mut self, aep2: Option<String>) {
        self.aep2_key = aep2;
    }

    /// Sets the AEP-2 scrypt cost parameters `(N, r, p)` this account's key was
    /// encrypted with. A AEP-6 wallet calls this with its own `ScryptParameters`
...
impl WalletAccount for StandardWalletAccount {
    ...
    fn has_key(&self) -> bool {
        self.key_pair.is_some() || self.aep2_key.is_some()
    }
    ...
    fn unlock(&mut self, password: &str) -> WalletResult<bool> {
        ...
        match KeyPair::from_aep2_string_with_params(aep2, password, version, n, r, p) {
            Ok(key_pair) => {
    ...
    fn verify_password(&self, password: &str) -> WalletResult<bool> {
        if let Some(aep2) = &self.aep2_key {
            let version = self.protocol_settings.address_version;
            let (n, r, p) = self.scrypt;
            Ok(KeyPair::from_aep2_string_with_params(aep2, password, version, n, r, p).is_ok())
        } else {
    ...
    fn export_aep2(&self, password: &str) -> WalletResult<String> {
        if let Some(aep2) = &self.aep2_key {
            return Ok(aep2.clone());
        }

        let key = self.key_pair.as_ref().ok_or(WalletError::AccountLocked)?;
        let version = self.protocol_settings.address_version;
        let (n, r, p) = self.scrypt;
        key.to_aep2_with_params(password, version, n, r, p)
    ...
    fn create_witness(&self, transaction: &Transaction) -> WalletResult<Witness> {
        let key = self.key_pair.as_ref().ok_or(WalletError::AccountLocked)?;
        let sign_data =
            atipicial_payloads::get_sign_data_vec(transaction, self.protocol_settings.network)
                .map_err(|e| WalletError::SigningFailed(e.to_string()))?;
        let signature = key
            .sign(&sign_data)
            .map_err(|e| WalletError::SigningFailed(e.to_string()))?;
