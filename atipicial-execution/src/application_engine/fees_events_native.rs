...
fn native_on_persist_perf_enabled() -> bool {
    ...
    *ENABLED.get_or_init(|| env_flag_enabled("ATC_PERSIST_PROFILE", false))
}
...
where
    ...
    B: atipicial_storage::CacheRead,
{
    if std::env::var_os("ATC_TRACE_NATIVE_FEES").is_some() {
        return true;
    }
    let Ok(raw) = std::env::var("ATC_TRACE_NATIVE_FEES_TX") else {
        return false;
    };
