use super::*;
use atipicial_crypto::{ECPoint, Secp256k1Crypto, Secp256r1Crypto};
...
#[test]
fn permission_descriptor_projects_to_atipicial_vm_stack_item() {
    let hash = UInt160::from_bytes(&[0x11; 20]).expect("hash");
    let group = group_key();
    let group_bytes = group.encode_point(true).expect("compressed key");

    assert_eq!(
        ContractPermissionDescriptor::create_wildcard().to_stack_item(),
        StackItem::Null
    );
    assert_eq!(
...
#[test]
fn permission_descriptor_reads_from_atipicial_vm_stack_item() {
    let hash = UInt160::from_bytes(&[0x33; 20]).expect("hash");
    let group = group_key();
    let group_bytes = group.encode_point(true).expect("compressed key");

    assert_eq!(
        ContractPermissionDescriptor::from_stack_item(&StackItem::Null).unwrap(),
    ...
    assert_eq!(
        ContractPermissionDescriptor::from_stack_item(&StackItem::from_buffer(group_bytes,))
            .unwrap(),
