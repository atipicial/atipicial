...
**Goal:** Make the daemon read as a short sequence of Atipicial node operations while
each lower layer owns and hides its protocol, execution, networking, and
storage mechanics.

**Protocol constraint:** The refactor must preserve the already completed Atipicial
...
| --- | --- | --- | --- |
| Foundation | `atipicial-primitives` | domain value types | byte codecs and services |
| Infrastructure | `atipicial-io`, `atipicial-error`, `atipicial-crypto`, `atipicial-storage`, `atipicial-config`, `atipicial-vm`, `atipicial-serialization`, `atipicial-manifest` | precise mechanical capabilities | node policy and lifecycle |
| Protocol | `atipicial-payloads`, `atipicial-consensus`, `atipicial-hsm` | blocks, transactions, witnesses, dBFT messages | stores, RPC, process startup |
| Domain services | `atipicial-runtime`, `atipicial-execution`, `atipicial-native-contracts`, `atipicial-state-service`, `atipicial-mempool` | import, execute, persist, state, admission contracts | CLI and transport wiring |
| Node services | `atipicial-blockchain`, `atipicial-network`, `atipicial-wallets`, `atipicial-indexer`, `atipicial-oracle-service` | long-running service handles and provider APIs | channel loops and lower codecs |
| Composition | `atipicial-system` | built node, sync workflow, service lifecycle | concrete worker and channel assembly |
| Plugin boundary | `atipicial-rpc` | operator/client service APIs | core-node construction |
| Application | `atipicial-node`, `atipicial-gui` | start, import, serve, stop use cases | protocol, store, and channel mechanics |
...
tooling path such as `atipicial-db-probe` may use lower mechanical APIs directly, but
...
- Add `atipicial-node/src/node/application/mod.rs`.
- Add `atipicial-node/src/node/application/command.rs`.
- Add `atipicial-node/src/node/application/runtime.rs`.
- Reduce `atipicial-node/src/node/lifecycle/daemon.rs` to the ordered application facade.
- Add `atipicial-node/src/tests/node/application.rs` and register it from the node
  test module.
...
- Existing `atipicial-node` runtime, restart, fast-sync, RPC, and shutdown tests stay
  green.

## Phase 2: Core Composition Ownership (Implemented)

Provider-neutral core assembly moved from
`atipicial-node/src/node/lifecycle/composition.rs` to `atipicial-system`:
...
6. Keep CLI config parsing, HSM credentials, RPC, oracle, telemetry, and
   process observability in `atipicial-node`.

Do not make `atipicial-system` depend on `atipicial-rpc`, `atipicial-oracle-service`, or
`atipicial-node`. Optional application services receive typed handles from the core
composition result.
...
`atipicial-system::Node` does not create a second token or expose a competing
...
- `atipicial-runtime` remains a low-level contract crate;
- `atipicial-system` has no plugin/application dependencies;
- lower service crates never import `atipicial-system` or `atipicial-node`;
...
The enforced service API also keeps `atipicial-blockchain` command-loop internals and
`atipicial-network` wire/protocol module layouts private. Required capabilities and
...
```text
cargo fmt --all -- --check
cargo test -p atipicial-node --bin atipicial-node
cargo test -p atipicial-system --lib
cargo test -p atipicial-runtime --lib
...
- Reusable core composition lives in `atipicial-system`, not in the binary.
