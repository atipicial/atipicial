...
fn expect_uint256_param_with_message_uses_custom_invalid_label() {
    ...
    let err = expect_uint256_param_with_message(&params, 0, "method expects hash", "block hash")
...
fn expect_uint160_param_with_message_parses_valid_hash() {
    ...
    let parsed =
        expect_uint160_param_with_message(&params, 0, "method expects script hash", "script hash")
...
fn parse_script_hash_or_address_accepts_wallet_address() {
    let hash = UInt160::zero();
    let address_version = 0x09;
    let address = atipicial_wallets::wallet_helper::WalletAddress::to_address(&hash, address_version);

    let parsed = parse_script_hash_or_address(&address, address_version).expect("valid address");
...
fn parse_script_hash_or_address_with_error_uses_custom_address_error() {
    let err = parse_script_hash_or_address_with_error("not-an-address", 0x09, |_| {
...
fn expect_base64_param_with_messages_uses_custom_missing_message() {
    let err = expect_base64_param_with_messages(&[], 0, "method requires Base64 payload", |text| {
...
fn expect_base64_param_with_messages_uses_input_aware_decode_message() {
    ...
    let err =
        expect_base64_param_with_messages(&params, 0, "method requires Base64 payload", |text| {
            format!("invalid Base64 payload: {text}")
        })
