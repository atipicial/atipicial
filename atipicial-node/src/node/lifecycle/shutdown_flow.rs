...
use atipicial_storage::persistence::{Store, TransactionalStore};
...
pub(in crate::node) async fn run_daemon_shutdown<S, ServiceS>(
    node: &Arc<atipicial_system::Node<atipicial_native_contracts::StandardNativeProvider, S>>,
    ...
    ServiceS: Store + 'static,
{
    ...
    match shutdown_signalled {
        Ok(signal_name) => {
            info!(target: "atipicial", signal = signal_name, "shutdown signal received; shutting down")
        }
        Err(err) => {
            warn!(target: "atipicial", error = %err, "shutdown-signal handler failed; falling back to pending forever");
