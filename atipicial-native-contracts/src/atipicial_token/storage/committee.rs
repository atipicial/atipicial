//! Committee cache readers and address derivation for ATC.
//!
...
/// committee pubkeys on the hot path. See [`AtipicialCoin::read_committee_with_votes`].
...
///
/// `AtipicialDollar::on_persist` needs only the primary validator's account. This
...
impl AtipicialCoin {
    ...
    /// null deref.
    pub(in crate::atipicial_coin) fn read_committee_with_votes<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
    ) -> CoreResult<Vec<(ECPoint, BigInt)>> {
        let key = Self::committee_key();
        let item = snapshot.get(&key).ok_or_else(|| {
            CoreError::invalid_operation("AtipicialCoin committee cache is not initialized")
        })?;
    ...
    fn decode_committee_with_votes(raw: &[u8]) -> CoreResult<Vec<(ECPoint, BigInt)>> {
        ...
        // `read_committee_with_votes` is on the per-block hot path (AtipicialDollar
    ...
    pub(in crate::atipicial_coin) fn read_committee_points<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
    ...
    pub(crate) fn next_block_validator_account<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
                ...
                if *cached_count == validators_count && cached_bytes.as_slice() == raw.as_ref() {
                    return cached_accounts.get(primary_index).copied().ok_or_else(|| {
                        CoreError::invalid_operation(format!(
                            "AtipicialCoin next-block validator primary index {primary_index} outside the validator set"
                        ))
                    });
                }
            }
    ...
    pub(in crate::atipicial_coin) fn read_committee_member_at<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
        index: usize,
        ...
        Self::decode_committee_with_votes(raw.as_ref())?
            .into_iter()
            .nth(index)
            .ok_or_else(|| CoreError::invalid_operation("AtipicialCoin committee cache too small"))
    }
    ...
    pub(in crate::atipicial_coin) fn encode_committee(
    ...
    pub(in crate::atipicial_coin) fn decode_canonical_committee_member_at(
        value: &[u8],
        index: usize,
    ) -> CoreResult<Option<(ECPoint, BigInt)>> {
        const ARRAY: u8 = atipicial_vm::ATCVM_STACK_ITEM_TYPE_ARRAY;
        const STRUCT: u8 = atipicial_vm::ATCVM_STACK_ITEM_TYPE_STRUCT;
        const BYTE_STRING: u8 = atipicial_vm::ATCVM_STACK_ITEM_TYPE_BYTESTRING;
        const INTEGER: u8 = atipicial_vm::ATCVM_STACK_ITEM_TYPE_INTEGER;

        let Some((&ARRAY, tail)) = value.split_first() else {
            return Ok(None);
        };
        let Some((count, count_width)) = atipicial_io::var_int::VarInt::read_var_int_prefix(tail)
        else {
            return Ok(None);
        };
        if index >= count as usize {
            return Err(CoreError::invalid_operation(
                "AtipicialCoin committee cache too small",
            ));
        }
        ...
        for member_index in 0..count as usize {
            ...
            let Some((key_len, key_len_width)) =
                atipicial_io::var_int::VarInt::read_var_int_prefix(rest)
            else {
                return Ok(None);
            };
            if key_len != 33 {
    ...
    /// C# `AtipicialCoin.ShouldRefreshCommittee(height, committeeMembersCount)`:
    ...
    pub(in crate::atipicial_coin) fn should_refresh_committee(
        height: u32,
    ...
    /// `AtipicialDollar::initialize` can mint the initial ATD distribution to the
    /// standby-validator BFT address (C# AtipicialDollar.cs:33).
    pub(crate) fn bft_address(pubkeys: &[ECPoint]) -> CoreResult<UInt160> {
        atipicial_vm::script_builder::RedeemScript::bft_address(pubkeys)
    ...
    pub(in crate::atipicial_coin) fn committee_sorted<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
    ) -> CoreResult<Vec<ECPoint>> {
    ...
    pub(in crate::atipicial_coin) fn compute_committee_address<B: atipicial_storage::CacheRead>(
        &self,
        snapshot: &DataCache<B>,
    ) -> CoreResult<UInt160> {
        let key = Self::committee_key();
