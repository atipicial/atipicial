//! Stack-item projections for AtipicialCoin storage records.

use super::*;
use atipicial_error::CoreError;

/// Decoded view of a `AtipicialAccountState` (`Struct[Balance, BalanceHeight, VoteTo,
/// LastGasPerVote]`, C# `AtipicialAccountState.FromStackItem`).
#[derive(Debug, Clone, PartialEq, Eq)]
pub(in crate::atipicial_coin) struct AtipicialAccountStateView {
    pub(in crate::atipicial_coin) balance: BigInt,
    pub(in crate::atipicial_coin) balance_height: u32,
    pub(in crate::atipicial_coin) vote_to: Option<ECPoint>,
    pub(in crate::atipicial_coin) last_gas_per_vote: BigInt,
}

impl AtipicialAccountStateView {
    pub(in crate::atipicial_coin) fn to_stack_item(&self) -> StackItem {
    ...
    pub(in crate::atipicial_coin) fn from_stack_item(stack_item: &StackItem) -> CoreResult<Self> {
        let decoder =
            crate::support::codec::StructDecoder::new(stack_item, "atipicial account state")?;
        if decoder.len() < 4 {
            return Err(CoreError::invalid_data(
                "atipicial account state must have at least 4 fields",
            ));
        }
...
atipicial_vm::impl_interoperable_via_stack_item!(AtipicialAccountStateView);

#[derive(Debug, Clone, PartialEq, Eq)]
pub(in crate::atipicial_coin) struct CandidateState {
    pub(in crate::atipicial_coin) registered: bool,
    pub(in crate::atipicial_coin) votes: BigInt,
}

impl CandidateState {
    pub(in crate::atipicial_coin) fn new(registered: bool, votes: BigInt) -> Self {
...
atipicial_vm::impl_interoperable_via_stack_item!(CandidateState);
...
atipicial_vm::impl_interoperable_via_stack_item!(CachedCommittee);
