...
use atipicial_blockchain::{BlockPersistContext, FinalizedBlock, SyncBatchCommitPolicy};
...
use atipicial_storage::persistence::StoreCacheBacking;
use atipicial_storage::persistence::providers::RuntimeStore;
use atipicial_storage::{CacheRead, DataCache, StorageError};
use atipicial_system::{BlockCommitHooks, CanonicalCommit};
use std::sync::Arc;
    ...
    T: Store + 'static,
{
    ...
    fn deferred_import_work_budget(&self) -> Option<usize> {
        ...
        // Override with ATC_COORDINATED_IMPORT_CHANGE_BUDGET (0 disables).
        ...
        self.state_service.as_ref().and_then(|state_service| {
            ...
            let budget = std::env::var("ATC_COORDINATED_IMPORT_CHANGE_BUDGET")
                .ok()
                    ...
                    canonical_commit
                        .commit_durable_with(|canonical, canonical_overlay| {
                            ...
                            let mut shadow_hook = append_shadow.as_ref().map(|shadow| {
                                ...
                                move |entries: atipicial_storage::persistence::ShadowOverlayEntries| {
