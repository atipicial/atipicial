//! # atipicial-state-packs store filesystem operations
//!
...
///
/// Unix fsyncs a directory via an open descriptor. Windows cannot open a
/// directory with a plain read handle (`CreateFile` on a directory requires
/// `FILE_FLAG_BACKUP_SEMANTICS`), and no user-data flush is needed there:
/// NTFS journals metadata, and every pack file was already `sync_all`ed
/// before the directory sync would run. The call is therefore a no-op on
/// Windows — crash-consistency semantics come from the file-level syncs.
#[cfg(unix)]
