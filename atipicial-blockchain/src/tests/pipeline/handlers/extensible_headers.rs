use super::*;
use atipicial_payloads::extensible_payload::ExtensiblePayload;
...
async fn extensible_inventory_verifies_range_whitelist_and_witness() {
    ...
    let sender = atipicial_primitives::UInt160::from_script(&verification);
    ...
    payload.witness = atipicial_payloads::Witness::new_with_scripts(invocation, verification.clone());
    ...
    let foreign_private_key = atipicial_crypto::Secp256r1Crypto::generate_private_key();
    let foreign_public_key = atipicial_crypto::Secp256r1Crypto::derive_public_key(&foreign_private_key)
        .expect("foreign public key");
    let foreign_verification =
        atipicial_vm::script_builder::redeem_script::RedeemScript::signature_redeem_script(
            &foreign_public_key,
        );
    ...
    foreign.sender = atipicial_primitives::UInt160::from_script(&foreign_verification);
    ...
    let foreign_signature =
        atipicial_crypto::Secp256r1Crypto::sign(&foreign_sign_data, &foreign_private_key).expect("sign");
    let mut foreign_invocation = vec![0x0C, 64];
    foreign_invocation.extend_from_slice(&foreign_signature);
    foreign.witness =
        atipicial_payloads::Witness::new_with_scripts(foreign_invocation, foreign_verification);
    let err = service
...
async fn extensible_inventory_rejects_witness_script_hash_mismatch() {
    ...
    let whitelisted_verification =
        atipicial_vm::script_builder::redeem_script::RedeemScript::signature_redeem_script(&public_key);
    let sender = atipicial_primitives::UInt160::from_script(&whitelisted_verification);
    ...
    sign_data.extend_from_slice(&payload.hash().to_bytes());
    let signature =
        atipicial_crypto::Secp256r1Crypto::sign(&sign_data, &foreign_private_key).expect("sign");
    let mut invocation = vec![0x0C, 64];
    invocation.extend_from_slice(&signature);
    payload.witness = atipicial_payloads::Witness::new_with_scripts(invocation, foreign_verification);
...
async fn canonical_duplicate_keeps_header_validation_prefix_exact() {
    ...
    let mut invocation = vec![0x0C, 64];
    invocation.extend_from_slice(&signature);
    child.witness = atipicial_payloads::Witness::new_with_scripts(invocation, verification);
