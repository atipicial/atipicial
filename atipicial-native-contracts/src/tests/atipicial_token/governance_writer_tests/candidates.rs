...
/// [`candidate_pubkey`] (a Atipicial N3 standby validator).
...
fn get_candidates_filters_blocked_and_projects_votes() {
    ...
    cache.add(
        AtipicialCoin::candidate_key(&kept),
        StorageItem::from_bytes(
            AtipicialCoin::encode_candidate_state(true, &BigInt::from(7)).unwrap(),
        ),
    );
    cache.add(
        AtipicialCoin::candidate_key(&candidate_pubkey()),
        StorageItem::from_bytes(
            AtipicialCoin::encode_candidate_state(false, &BigInt::from(3)).unwrap(),
        ),
    );
    let blocked = other_pubkey(1);
    cache.add(
        AtipicialCoin::candidate_key(&blocked),
        StorageItem::from_bytes(
            AtipicialCoin::encode_candidate_state(true, &BigInt::from(9)).unwrap(),
        ),
    );
    ...
    let result = NativeContract::invoke(&AtipicialCoin::new(), &mut engine, "getCandidates", &[])
...
fn get_all_candidates_iterator_filters_and_projects() {
    ...
    let result =
        NativeContract::invoke(&AtipicialCoin::new(), &mut engine, "getAllCandidates", &[])
