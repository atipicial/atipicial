...
use atipicial_vm::{Interoperable, InteroperableError, StackItem};

/// Projects witness-rule types to the local atipicial-vm [`StackItem`] form.
///
/// This lives in atipicial-core (which depends on the VM crate) rather than in atipicial-io
/// so that atipicial-io — a Layer-1 serialization crate — stays free of any VM
...
impl WitnessCondition {
    /// Converts to a atipicial-vm stack item (matches C# `WitnessCondition.ToStackItem` layout).
...
impl WitnessRule {
    /// Converts to a atipicial-vm stack item (matches C# `WitnessRule.ToStackItem` layout).
    pub fn to_stack_item(&self) -> StackItem {
