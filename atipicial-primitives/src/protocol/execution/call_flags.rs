...
//!
//! `CallFlags` defines the permissions granted when one contract calls another,
//! matching the C# Atipicial `CallFlags` enum exactly. It lives in `atipicial-primitives`
//! (a Layer-0 crate) so both the VM host and the smart-contract layer can depend
//! on it without creating a dependency cycle between them.
//!
//! ## Flags
//!
...
#[cfg(test)]
#[path = "../../tests/protocol/execution/call_flags.rs"]
mod tests;
