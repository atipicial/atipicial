...
use atipicial_storage::persistence::read_only_store::RawReadOnlyStore;
...
fn store_checkpoint_store_round_trips_and_overwrites_memory_backend() {
    ...
    assert_eq!(
        ...
        None,
        "sync metadata must not enter the normal Atipicial data table"
    );
    assert!(
        backing
...
fn store_checkpoint_reads_do_not_touch_normal_data_rows() {
    ...
    assert_eq!(
        ...
        "typed maintenance reads must not inspect or mutate normal Atipicial data"
    );
}
...
fn store_verified_header_store_persists_across_memory_reopen_and_finishes_pruned_window() {
    ...
    assert_eq!(
        ...
        None,
        "verified headers must not enter the normal Atipicial data table"
    );
    assert!(
        backing
...
fn sync_metadata_tables_preserve_v1_key_and_value_bytes() {
    ...
    let mut expected_checkpoint_key = b"atipicial.sync.stage-checkpoint.v1.".to_vec();
    ...
    let mut expected_header_key = b"atipicial.sync.verified-header.v1.header.".to_vec();
    ...
    let window_key = VerifiedWindowKeyCodec::encode(&())
        ...
        .into_table_bytes();
    assert_eq!(window_key, b"atipicial.sync.verified-header.v1.window");
    let target_key = TargetHashKeyCodec::encode(&())
        .expect("encode target key")
        .into_table_bytes();
    assert_eq!(target_key, b"atipicial.sync.verified-header.v1.target-hash");
