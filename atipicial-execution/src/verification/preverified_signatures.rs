//! Advisory preverification for canonical Atipicial signature witnesses.
//!
//! ## Boundary
//!
...
//! authorize a witness, skip AtipicialVM execution, or accept externally supplied
//! cache entries.
//!
//! ## Contents
//!
//! - [`PreverifiedSignatureCache`]: opaque exact-key outcomes for one Atipicial sign
//!   data value.
...
/// Each instance is bound to one exact Atipicial `network || hash` sign-data value
...
pub struct PreverifiedSignatureCache {
    sign_data: [u8; ATC_SIGN_DATA_LEN],
...
impl PreverifiedSignatureCache {
    ...
    /// keys, outcomes, or mutation. A canonical AtipicialVM cache miss still falls
...
///
/// The function accepts only Atipicial's 36-byte sign data and canonical single- or
...
fn preverify_multi_signature(
    ...
    // Standard scripts push both collections in forward bytecode order. AtipicialVM
