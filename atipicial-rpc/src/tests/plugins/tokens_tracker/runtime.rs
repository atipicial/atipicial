...
fn token_tracker_engine_paths_use_explicit_native_provider() {
    let engine_sources = [
        (
            "AEP-17 tracker",
            include_str!("../../../plugins/tokens_tracker/trackers/aep_17/aep17_tracker.rs"),
        ),
        (
            "AEP-11 tracker",
            include_str!("../../../plugins/tokens_tracker/trackers/aep_11/aep11_tracker.rs"),
        ),
    ];
...
fn result_action_disables_tracker_when_commit_error_stops_plugin() {
    ...
    let should_continue = tracker.run_tracker_result_action("test", "commit", || {
        Err(atipicial_error::CoreError::other("injected commit failure"))
    });
