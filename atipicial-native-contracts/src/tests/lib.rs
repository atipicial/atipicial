use super::{
    AEP11_PAYMENT_METHOD, AEP17_PAYMENT_METHOD, AEP17_STANDARD, AEP26_STANDARD, AEP27_STANDARD,
    AEP30_STANDARD, AccountState, aep17_account_key, aep17_payment_callback_args,
    aep17_payment_data_item, aep17_total_supply_key, aep17_transfer_notification_state,
    native_supported_standards, read_aep17_total_supply,
};
use atipicial_primitives::{ContractParameterType, UInt160};
use atipicial_storage::StorageItem;
use atipicial_storage::persistence::DataCache;
use atipicial_vm::Interoperable;
...
fn stack_item_struct_eq(a: &atipicial_vm::StackItem, b: &atipicial_vm::StackItem) -> bool {
...
#[test]
fn aep17_key_helpers_match_csharp_prefix_layout() {
    let account = UInt160::from_bytes(&[0xAB; 20]).unwrap();

    let account_key = aep17_account_key(-6, &account);
    assert_eq!(account_key.id(), -6);
    assert_eq!(account_key.key()[0], super::AEP17_PREFIX_ACCOUNT);
    assert_eq!(&account_key.key()[1..], account.as_bytes());
    assert_eq!(
        account_key,
        crate::keys::prefixed_hash160_key(-6, super::AEP17_PREFIX_ACCOUNT, &account)
    );

    let supply_key = aep17_total_supply_key(-6);
    assert_eq!(supply_key.id(), -6);
    assert_eq!(supply_key.key(), &[super::AEP17_PREFIX_TOTAL_SUPPLY]);
    assert_eq!(
        supply_key,
        crate::keys::prefixed_key(-6, super::AEP17_PREFIX_TOTAL_SUPPLY, &[])
    );
}

#[test]
fn aep17_key_helpers_use_shared_storage_key_builders() {
    let source = include_str!("../support/token/aep.rs");
    let start = source
        .find("pub(crate) fn aep17_total_supply_key(")
        .expect("AEP-17 key helpers exist");
    let end = source[start..]
        .find("/// C# `AccountState`")
        .map(|offset| start + offset)
        .expect("account state follows AEP-17 key helpers");
...
#[test]
fn aep17_balance_reader_uses_stack_item_projection() {
    let source = include_str!("../support/token/aep.rs");
    let start = source
        .find("pub(crate) fn read_aep17_balance(")
        .expect("read_aep17_balance helper exists");
    let end = source[start..]
        .find("/// Reads the AEP-17 total supply")
        .map(|offset| start + offset)
        .expect("total supply reader follows read_aep17_balance");
    let helper = &source[start..end];

    // After the FungibleToken-helper extraction, read_aep17_balance delegates
    ...
    assert!(helper.contains("aep17_account_key"));
    assert!(!helper.contains("StackItem::Struct"));
    assert!(!helper.contains("BinarySerializer::deserialize("));
    assert!(!helper.contains("atipicial_vm::StackItem::Struct"));
}

#[test]
fn aep17_total_supply_reader_matches_fungible_token_default() {
    let cache = DataCache::new(false);
    assert_eq!(read_aep17_total_supply(&cache, -6), BigInt::from(0));

    cache.add(
        aep17_total_supply_key(-6),
        StorageItem::from_bytes(crate::bigint_to_storage_bytes(&BigInt::from(123456789i64))),
    );
    assert_eq!(
        read_aep17_total_supply(&cache, -6),
...
fn native_supported_standards_helper_preserves_manifest_order() {
    assert_eq!(
        native_supported_standards(&[AEP17_STANDARD, AEP27_STANDARD]),
        ["AEP-17", "AEP-27"]
    );
    assert_eq!(
        native_supported_standards(&[AEP26_STANDARD, AEP27_STANDARD, AEP30_STANDARD]),
        ["AEP-26", "AEP-27", "AEP-30"]
    );
}

#[test]
fn aep17_transfer_notification_state_matches_fungible_token_shape() {
    let from = UInt160::from([0x11; 20]);
    let to = UInt160::from([0x22; 20]);
    let amount = BigInt::from(42);

    let transfer = aep17_transfer_notification_state(Some(&from), Some(&to), &amount);
    ...
    let mint = aep17_transfer_notification_state(None, Some(&to), &amount);
    assert!(matches!(mint[0], atipicial_vm::StackItem::Null));
    assert_eq!(mint[1].as_bytes().unwrap(), to.to_bytes());
    assert_eq!(mint[2].as_int().unwrap(), amount);

    let burn = aep17_transfer_notification_state(Some(&from), None, &amount);
    assert_eq!(burn[0].as_bytes().unwrap(), from.to_bytes());
    assert!(matches!(burn[1], atipicial_vm::StackItem::Null));
    assert_eq!(burn[2].as_int().unwrap(), amount);
}

#[test]
fn aep17_payment_callback_args_match_on_payment_shape() {
    assert_eq!(AEP17_PAYMENT_METHOD, "onAEP17Payment");

    let from = UInt160::from([0x33; 20]);
    let amount = BigInt::from(99);
    let data = atipicial_vm::StackItem::from_byte_string(vec![0xaa, 0xbb]);

    let transfer = aep17_payment_callback_args(Some(&from), &amount, data.clone());
    assert_eq!(transfer.len(), 3);
    assert_eq!(transfer[0].as_bytes().unwrap(), from.to_bytes());
    assert_eq!(transfer[1].as_int().unwrap(), amount);
    assert_eq!(transfer[2].as_bytes().unwrap(), vec![0xaa, 0xbb]);

    let mint = aep17_payment_callback_args(None, &amount, atipicial_vm::StackItem::null());
    assert!(matches!(mint[0], atipicial_vm::StackItem::Null));
    assert_eq!(mint[1].as_int().unwrap(), amount);
    assert!(matches!(mint[2], atipicial_vm::StackItem::Null));
}

#[test]
fn aep17_payment_data_item_round_trips_any_payload() {
    let empty = aep17_payment_data_item(&[], "test data").expect("empty data");
    assert!(matches!(empty, atipicial_vm::StackItem::Null));

    let original = atipicial_vm::StackItem::from_byte_string(vec![0xaa, 0xbb]);
    ...
    let restored = aep17_payment_data_item(&bytes, "test data").expect("restore data");
    assert_eq!(restored.as_bytes().unwrap(), vec![0xaa, 0xbb]);

    let err = aep17_payment_data_item(&[0xff], "bad callback data")
...
#[test]
fn aep17_method_helpers_match_fungible_token_abi_shape() {
    let symbol = super::aep17_symbol_method();
    ...
    let decimals = super::aep17_decimals_method();
    ...
    let total_supply = super::aep17_total_supply_method(0x04);
    ...
    let balance_of = super::aep17_balance_of_method(0x04);
    ...
    let transfer = super::aep17_transfer_method();
...
#[test]
fn aep_payment_method_helpers_match_callback_abi_shape() {
    let aep17 = super::aep17_payment_method(32, true, 0);
    assert_eq!(aep17.name, AEP17_PAYMENT_METHOD);
    assert_eq!(aep17.cpu_fee, 32);
    assert!(aep17.safe);
    assert_eq!(aep17.required_call_flags, 0);
    assert_eq!(
        aep17.parameters,
        [
            ContractParameterType::Hash160,
    ...
    assert_eq!(aep17.parameter_names, ["from", "amount", "data"]);
    assert_eq!(aep17.return_type, ContractParameterType::Void);

    let aep11 = super::aep11_payment_method(64, false, 0x0f);
    assert_eq!(aep11.name, AEP11_PAYMENT_METHOD);
    assert_eq!(aep11.cpu_fee, 64);
    assert!(!aep11.safe);
    assert_eq!(aep11.required_call_flags, 0x0f);
    assert_eq!(
        aep11.parameters,
        [
            ContractParameterType::Hash160,
    ...
    assert_eq!(aep11.parameter_names, ["from", "amount", "tokenId", "data"]);
    assert_eq!(aep11.return_type, ContractParameterType::Void);
}
