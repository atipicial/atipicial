...
/// `[network]`: which Atipicial network the node joins.
#[derive(Debug, Default, Deserialize)]
...
/// into a `atipicial-state-packs` store at `path` and persists a pack high-water
...
impl StorageSection {
    ...
    pub(super) fn static_file_config(&self) -> atipicial_static_files::StaticFileConfig {
        let mut config = atipicial_static_files::StaticFileConfig::default();
...
pub(super) struct P2pSection {
    ...
    /// Maximum simultaatipicialus peer count. `-1` matches C# "unlimited".
    #[serde(default, alias = "MaxConnections")]
    max_connections: Option<i64>,
    /// Maximum simultaatipicialus peers accepted from one remote IP.
...
impl P2pSection {
    ...
    pub(super) fn channels_config(&self) -> anyhow::Result<atipicial_network::ChannelsConfig> {
        let mut config = atipicial_network::ChannelsConfig::default();
...
impl RpcSection {
    pub(super) fn server_config(
        &self,
        network: u32,
    ) -> anyhow::Result<atipicial_rpc::server::RpcServerConfig> {
        ...
        let mut config = atipicial_rpc::server::RpcServerConfig {
            network,
            bind_address,
...
impl MempoolSection {
    /// Builds the bounded runtime policy owned by `atipicial-mempool`.
    pub(super) fn tx_pool_config(&self) -> anyhow::Result<atipicial_mempool::TxPoolConfig> {
        let Some(max_transactions) = self.max_transactions else {
            return Ok(atipicial_mempool::TxPoolConfig::default());
        };
        let max_transactions = usize::try_from(max_transactions)
            .context("invalid [mempool].max_transactions: expected a positive integer")?;
        atipicial_mempool::TxPoolConfig::new(max_transactions)
...
pub(super) fn default_p2p_port(network: u32) -> u16 {
    match network {
        atipicial_primitives::constants::TESTNET_MAGIC => 20333,
        atipicial_primitives::constants::MAINNET_MAGIC => 10333,
        _ => 0,
    }
}

/// Loads the TOML node configuration and selects an immutable [`AtipicialChainSpec`].
...
pub(super) fn load_config(
...
) -> anyhow::Result<(Arc<AtipicialChainSpec>, NodeConfig)> {
    let config: NodeConfig = if path.exists() {
        ...
        info!(target: "atipicial", path = %path.display(), "config not found; using built-in defaults");
        NodeConfig::default()
    };
    ...
    let chain_spec = match network_type {
        ...
        NetworkType::MainNet | NetworkType::TestNet => {
            ...
            AtipicialChainSpec::from_network_type(network_type)
