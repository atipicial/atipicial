...
async fn state_queries_reject_unknown_contract() {
    let fixture = make_server_with_mpt(true);
    let foreign = atipicial_primitives::UInt160::from_bytes(&[0x99u8; 20]).expect("hash");
...
async fn state_queries_report_resolution_failure_for_unknown_root() {
    let fixture = make_server_with_mpt(true);
    let unknown_root = atipicial_primitives::UInt256::from([0x77u8; 32]);
    let err = call(
        &fixture.server,
        "getstate",
        &[
...
fn state_handlers_depend_on_provider_capabilities_not_mpt_mechanics() {
    ...
    for forbidden in [
        "use atipicial_state_service::mpt_store",
        "MptReadSnapshot",
        ".open_trie(",
        "Self::mpt_store(",
        "Trie::<",
