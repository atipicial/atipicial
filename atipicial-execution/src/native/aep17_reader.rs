//! Concrete [`atipicial_runtime::Aep17MetadataReader`] implementation backed by
//! [`ApplicationEngine`].
//!
//! This is the execution-layer impl of the trait defined in `atipicial-runtime`.
//! It builds a script that calls `decimals` then `symbol` on the target
//! AEP-17 contract (both with `CallFlags::READ_ONLY`) and runs it through
//! a read-only `ApplicationEngine` with a 0.3 GAS budget — matching C#
//! `Atipicial.Wallets.AssetDescriptor` semantics.
//!
//! The wallet layer (`atipicial-wallets`) depends on the trait, not on this
//! concrete impl, breaking the direct L4 → L3 execution dependency.

use std::sync::Arc;

use atipicial_config::ProtocolSettings;
use atipicial_primitives::{CallFlags, TriggerType, UInt160};
use atipicial_runtime::{Aep17Metadata, Aep17MetadataReader, ServiceError};
use atipicial_storage::{CacheRead, DataCache, EmptyCacheBacking};
use atipicial_vm::VmState;
...
/// [`atipicial_runtime::Aep17MetadataReader`] backed by [`ApplicationEngine`].
///
...
#[derive(Clone)]
pub struct Aep17MetadataReaderImpl<P, B = EmptyCacheBacking> {
    snapshot: Arc<DataCache<B>>,
    settings: ProtocolSettings,
    native_contract_provider: Arc<P>,
}

impl<P, B> std::fmt::Debug for Aep17MetadataReaderImpl<P, B> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("Aep17MetadataReaderImpl")
...
impl<P, B> Aep17MetadataReaderImpl<P, B>
where
...
impl<P, B> Aep17MetadataReader for Aep17MetadataReaderImpl<P, B>
where
    P: NativeContractProvider + 'static,
    B: CacheRead,
{
    fn read_metadata(&self, contract_hash: UInt160) -> Result<Aep17Metadata, ServiceError> {
        ...
        Ok(Aep17Metadata { symbol, decimals })
    }
}
...
fn emit_descriptor_call(
    ...
    asset_id: &UInt160,
    method: &str,
) -> Result<(), atipicial_error::CoreError> {
    ...
    builder
        ...
        .map_err(|e| atipicial_error::CoreError::invalid_operation(e.to_string()))?;
    Ok(())
}
