...
use atipicial_payloads::{Block, Transaction};
use atipicial_runtime::{NetworkEvent as RuntimeNetworkEvent, NetworkService, Service, ServiceError};
...
impl LocalNodeService<NoBlockSource> {
    ...
    pub fn new(chain_spec: Arc<AtipicialChainSpec>) -> (Self, NetworkHandle) {
    ...
    B: BlockSource + 'static,
{
    ...
    pub async fn run(mut self) {
        info!(target: "atipicial_network", "local node service run loop started");
        ...
        info!(target: "atipicial_network", "local node service run loop exited");
    }
    ...
    async fn handle_start(&mut self, bind_addr: SocketAddr) -> NetworkResult<SocketAddr> {
        ...
        info!(target: "atipicial_network", %local_addr, "local node tcp listener bound");
    ...
    async fn handle_broadcast_extensible(&self, payload: &atipicial_payloads::ExtensiblePayload) {
        ...
        .and_then(|message| message.to_bytes())
        {
            Ok(bytes) => bytes,
            Err(err) => {
                warn!(target: "atipicial_network", %err, "failed to encode extensible payload");
                return;
            }
        };
        for (peer_id, handle) in self.registry.handles() {
    ...
    async fn handle_broadcast_inv(
        &self,
        inventory_type: atipicial_primitives::InventoryType,
        hashes: Vec<atipicial_primitives::UInt256>,
    ) {
        if hashes.is_empty() {
            return;
        }
        for group in atipicial_payloads::inv_payload::InvPayload::create_group(inventory_type, hashes) {
            let frame =
                    ...
                    .and_then(|message| message.to_bytes())
                {
                    Ok(bytes) => bytes,
                    Err(err) => {
                        warn!(target: "atipicial_network", %err, "failed to encode inv announcement");
                        return;
                    }
                };
            for (peer_id, handle) in self.registry.handles() {
    ...
    async fn handle_relay_inventory(&self, hash: atipicial_primitives::UInt256) {
        trace!(target: "atipicial_network", ?hash, "relay inventory");
    }
...
async fn accept_loop<B>(
    ...
    B: BlockSource + 'static,
{
    info!(target: "atipicial_network", "accept loop started");
    loop {
        tokio::select! {
            _ = shutdown.cancelled() => {
                debug!(target: "atipicial_network", "accept loop cancelled");
                break;
            }
    ...
    info!(target: "atipicial_network", "accept loop exited");
}
