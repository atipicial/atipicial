...
CONFIG_PATH="atipicial_testnet_node.toml"
STORAGE_PATH="$ROOT/data/testnet-full"
BOOTSTRAP_DIR="$ROOT/data/bootstrap-testnet"
NODE_BIN="$ROOT/target/release/atipicial-node"
RPC_URL="http://127.0.0.1:20332"
REF_RPC_CSHARP="http://seed1t5.atipicial.com:20332"
REF_RPC_REFERENCE="http://seed1.atipicial.com:20332"
POLL_SECONDS=15
TARGET_LAG=0
MAX_WAIT_SECONDS=0
SKIP_BOOTSTRAP=0
...
Options:
  --config <path>           atipicial-node config path (default: atipicial_testnet_node.toml)
  ...
  --node-bin <path>         atipicial-node binary path (default: target/release/atipicial-node; falls back to debug)
  --rpc-url <url>           local RPC URL to validate (default: http://127.0.0.1:20332)
  --ref-csharp <url>        C# reference RPC (default: http://seed1t5.atipicial.com:20332)
  --ref-atipicial <url>         AtipicialGo reference RPC (default: http://seed1.atipicial.com:20332)
...
if [[ ! -x "$NODE_BIN" ]]; then
  if [[ "$NODE_BIN" == "$ROOT/target/release/atipicial-node" && -x "$ROOT/target/debug/atipicial-node" ]]; then
    NODE_BIN="$ROOT/target/debug/atipicial-node"
  else
    echo "error: atipicial-node binary not executable: $NODE_BIN" >&2
    exit 1
  fi
fi
...
pick_max_remote_height() {
  ...
  h2="$(rpc_ref_call "$REF_RPC_REFERENCE" getblockcount 2>/dev/null | jq -r '.result // -1')"
  if [[ ! "$h1" =~ ^[0-9]+$ ]]; then
    h1=-1
  fi
  if [[ ! "$h2" =~ ^[0-9]+$ ]]; then
...
LOG_FILE="$(mktemp /tmp/atipicial-testnet-full-sync-XXXX.log)"
IMPORT_LOG="$(mktemp /tmp/atipicial-testnet-import-XXXX.log)"
...
log "starting atipicial-node for tail sync"
"$NODE_BIN" \
  --config "$CONFIG_PATH" \
  --storage "$STORAGE_PATH" \
  --logging-format json \
...
for _ in $(seq 1 180); do
  if ! kill -0 "$NODE_PID" >/dev/null 2>&1; then
    echo "error: atipicial-node exited before RPC became ready" >&2
...
start_ts="$(date +%s)"
ATC_HASH="0xef4073a0f2b305a38ec4050e4d3d28bc40ea63f5"
last_local=-1
...
while true; do
  ...
  if ! kill -0 "$NODE_PID" >/dev/null 2>&1; then
    echo "error: atipicial-node exited during sync loop" >&2
    tail -n 120 "$LOG_FILE" >&2 || true
    exit 1
  fi
  ...
  invoke_json="$(rpc_call invokefunction "[\"${ATC_HASH}\",\"totalSupply\",[]]")"
  ...
  csharp_supply="$(rpc_ref_call "$REF_RPC_CSHARP" invokefunction "[\"${ATC_HASH}\",\"totalSupply\",[]]" | jq -r '.result.stack[0].value // ""')"
  atipicial_supply="$(rpc_ref_call "$REF_RPC_REFERENCE" invokefunction "[\"${ATC_HASH}\",\"totalSupply\",[]]" | jq -r '.result.stack[0].value // ""')"
  if [[ -z "$csharp_supply" || -z "$atipicial_supply" ]]; then
    echo "error: failed to query reference totalSupply values" >&2
    exit 1
  fi
  if [[ "$local_supply" != "$csharp_supply" || "$local_supply" != "$atipicial_supply" ]]; then
    echo "error: totalSupply mismatch local=$local_supply csharp=$csharp_supply atipicial=$atipicial_supply" >&2
    exit 1
  fi
...
echo "  total_supply(local): $(rpc_call invokefunction \"[\\\"${ATC_HASH}\\\",\\\"totalSupply\\\",[]]\" | jq -r '.result.stack[0].value // \"\"')"
