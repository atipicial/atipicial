//! TransactionState - matches C# Atipicial.SmartContract.Native.TransactionState exactly.

use crate::Transaction;
use atipicial_error::CoreError;
use atipicial_io::{BinaryWriter, MemoryReader, Serializable};
use atipicial_vm::{Interoperable, InteroperableError};
...
pub struct TransactionState {
    ...
    /// The execution state (mirrors the atipicial-vm [`VMState`] enum).
    pub state: VMState,
}
...
impl TransactionState {
    ...
    /// Converts to a atipicial-vm stack item, preserving C# failure semantics:
    ...
    /// Updates this transaction state from a atipicial-vm stack item.
