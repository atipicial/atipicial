"""Small Atipicial storage/RPC helpers used by validation scripts."""
...
DEFAULT_REFERENCE_RPC = "http://seed1.atipicial.com:10332"
DEFAULT_ADDRESS_VERSION = 0x35
BASE58_ALPHABET = "123456789ABCDEFGHJKLMNPQRSTUVWXYZabcdefghijkmnopqrstuvwxyz"
AEP17_ACCOUNT_PREFIX = 20
...
def address_to_script_hash(address: str, *, address_version: int = DEFAULT_ADDRESS_VERSION) -> bytes:
    ...
    if len(payload) != 21:
        raise ValueError(
            f"invalid Atipicial address payload length: expected 21 bytes, got {len(payload)}"
        )
    version = payload[0]
    if version != address_version:
        raise ValueError(
            f"invalid Atipicial address version: expected 0x{address_version:02x}, got 0x{version:02x}"
        )
    return payload[1:]
...
def gas_account_storage_key(address: str, *, address_version: int = DEFAULT_ADDRESS_VERSION) -> str:
    ...
    key = bytes([AEP17_ACCOUNT_PREFIX]) + script_hash
...
def decode_aep17_account_balance(value_base64: str | None) -> int:
    if not value_base64:
        return 0
    ...
    if data[0] != STACK_ITEM_STRUCT:
        raise ValueError(f"expected AEP-17 account Struct 0x41, got 0x{data[0]:02x}")
    count, offset = read_var_int(data, 1)
    if count < 1:
        raise ValueError("AEP-17 account Struct has no balance field")
...
def rpc_call(url: str, method: str, params: list, timeout: float = 20.0) -> Any:
    ...
    request = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "atipicial-rs-storage-validator/1.0",
        },
        method="POST",
    )
