...
fn callback_contract(name: &str, marker: u8) -> (AefFile, atipicial_manifest::ContractManifest) {
    ...
    (
        AefFile::new("cache-test".to_string(), script.to_array()),
        manifest,
    )
}
...
fn update_bumps_counter_swaps_payloads_and_notifies() {
    ...
    let new_nef = AefFile::new("updated-compiler".to_string(), vec![OpCode::RET.byte()]);
    ...
    // Same id + hash, UpdateCounter bumped, AEF and manifest swapped.
    ...
    assert_eq!(updated.aef.compiler, "updated-compiler");
    assert_eq!(updated.aef.checksum, new_nef.checksum);
...
fn update_with_null_nef_keeps_the_old_nef() {
    ...
    // supported standard); the AEF stays byte-identical.
    let mut new_manifest = deployable_manifest("NullNefFixture");
    new_manifest.supported_standards = vec!["AEP-17".to_string()];
    ...
    assert_eq!(updated.aef.checksum, original_nef.checksum, "AEF unchanged");
    assert_eq!(updated.aef.compiler, original_nef.compiler);
    assert_eq!(
...
fn update_refreshes_contract_cache_before_deploy_callback() {
    ...
    assert_eq!(
        ...
        vec![0xBB],
        "queued _deploy must execute the updated contract state, not the cached old AEF"
    );
}
...
fn update_validation_failures_fault() {
    // Both args null.
    {
        ...
        assert_eq!(state, VmState::FAULT, "null aef + null manifest must fault");
    }
