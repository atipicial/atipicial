...
mod execution_engine_tests {
    ...
    #[cfg_attr(
        debug_assertions,
        ignore = "run with cargo test --release -p atipicial-vm execution_profiler_overhead_stays_within_budget -- --nocapture"
    )]
    ...
    /// the script length as a normal end of execution, while AtipicialGo rejects it
    ...
    /// (`invokescript` -> HALT) and live AtipicialGo (-> FAULT) on 2026-07-26.
    ///
