...
/// from the environment (`ATC_HSM_CU_PASSWORD`) — never from a TOML file.
...
impl HsmConfig {
    ...
    ///
    /// Reads `ATC_HSM_CU_PASSWORD` from the environment and returns an
    ...
    fn from_env_aws_with(
        ...
        let cu_password = read_env("ATC_HSM_CU_PASSWORD").map_err(|_| {
            HsmError::Init("ATC_HSM_CU_PASSWORD must be set for AWS CloudHSM".to_string())
        })?;
...
mod tests {
    ...
    fn aws_env_config_returns_init_error_when_pin_env_is_missing() {
        ...
        assert!(matches!(err, HsmError::Init(message) if message.contains("ATC_HSM_CU_PASSWORD")));
    }
}
