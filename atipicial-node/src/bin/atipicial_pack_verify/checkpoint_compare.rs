...
use atipicial_state_packs::{CHECKPOINT_NAMESPACE_DIGEST_DOMAIN, PACK_KEY_BYTES, PackStore};
use atipicial_storage::persistence::providers::RuntimeStore;
use atipicial_storage::persistence::{RawReadOnlyStore, Store};
...
pub(super) fn compare_checkpoint_nodes(
    ...
    state_store.visit_raw_entries_with_prefix(&[MPT_NODE_PREFIX], maximum, |key, value| {
        if key.len() != PACK_KEY_BYTES || key.first() != Some(&MPT_NODE_PREFIX) {
            return Err(atipicial_storage::StorageError::invalid_operation(
                "StateService node scan returned a malformed key",
            ));
        }
        if value.len() > AUTHORITY_LOOKUP_MAX_VALUE_BYTES {
            return Err(atipicial_storage::StorageError::invalid_operation(format!(
        ...
        total_value_bytes = total_value_bytes
            ...
            .ok_or_else(|| {
                atipicial_storage::StorageError::invalid_operation(
                    "StateService node value byte count overflows",
                )
            })?;
        let next_total_keys = total_keys.checked_add(1).ok_or_else(|| {
            atipicial_storage::StorageError::invalid_operation("StateService node row count overflows")
        })?;
