...
use atipicial_native_contracts::AtipicialDollar;
use atipicial_serialization::json::{JArray, JObject, JToken};
...
fn load_contract_state_case(manifest_name: &str) -> Option<(String, String)> {
    ...
    path.push("..");
    path.push("atipicial_csharp");
    path.push("node");
    path.push("tests");
    path.push("Atipicial.Network.RPC.Tests");
    path.push("RpcTestCases.json");
    if !path.exists() {
        eprintln!(
            "SKIP: atipicial_csharp submodule not initialized ({})",
            path.display()
        );
        return None;
    }
    ...
    for entry in cases.children() {
        ...
        let contract = if UInt160::parse(&contract).is_ok() {
            ...
            atipicial_native_contracts::standard_native_contracts()
                .into_iter()
...
async fn token_info_uses_contract_state_name() {
    ...
    let Some((contract_hash, contract_state_response)) = load_contract_state_case("AtipicialDollar")
    else {
        return;
    };
    let symbol = "ATD";
    ...
    let api = Aep17Api::new(Arc::new(client));
    ...
    assert_eq!(info.name, "AtipicialDollar");
    assert_eq!(info.symbol, "ATD");
...
async fn token_info_by_contract_accepts_name() {
    ...
    let Some((_contract_hash, contract_state_response)) = load_contract_state_case("AtipicialDollar")
    else {
        return;
    };
    let symbol_b64 = general_purpose::STANDARD.encode("ATD".as_bytes());
    ...
    let _m_contract = server
        .mock("POST", "/")
        .match_body(Matcher::Regex(
            r#""method"\s*:\s*"getcontractstate".*"AtipicialDollar""#.into(),
        ))
        .with_status(200)
        .with_header("content-type", "application/json")
        .with_body(contract_state_response)
    ...
    let info = api
        .get_token_info_by_contract("AtipicialDollar")
        .await
        .expect("token info");
...
async fn token_info_with_balance_fetches_balance() {
    ...
    let settings = atipicial_config::ProtocolSettings::default_settings();
...
async fn create_transfer_tx_with_from_builds_transaction() {
    ...
    let balance_script =
        build_dynamic_call_script(&AtipicialDollar::new().hash(), "balanceOf", &balance_args);
    let balance_script_b64 = general_purpose::STANDARD.encode(balance_script);
    mock_invokescript(
        &mut server,
        &balance_script_b64,
