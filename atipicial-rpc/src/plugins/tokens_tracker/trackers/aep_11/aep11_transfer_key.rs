//! AEP-11 transfer history key.
//!
//! Storage key for AEP-11 (NFT) transfer records.
...
/// Storage key for AEP-11 transfers.
#[derive(Debug, Clone, PartialEq, Eq, Hash, Serialize, Deserialize, Default)]
pub struct Aep11TransferKey {
    /// Base transfer key.
...
impl PartialOrd for Aep11TransferKey {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        Some(self.cmp(other))
    }
}

impl Ord for Aep11TransferKey {
...
impl_serializable! {
    struct Aep11TransferKey {
        base: TokenTransferKey,
        // AEP-11 token IDs are bounded to 64 bytes by the standard.
        token: var_bytes { max: 64 },
    }
}

super::super::impl_token_transfer_key_as_ref!(Aep11TransferKey, base);
