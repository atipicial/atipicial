...
use atipicial_network::BlockDownloader;
    ...
    H: VerifiedHeaderStore,
{
    ...
    #[must_use]
    pub fn downloader_config(&self) -> &atipicial_network::BlockDownloadConfig {
        self.downloader.config()
    }

    /// Drain every available download batch into the canonical import pipeline.
    ///
    /// Import remains ordered by [`atipicial_runtime::SyncPipelineDriver`]; this
    ...
    pub async fn import_all(&mut self) -> ServiceResult<SyncDownloadImportSummary> {
        ...
        while let Some(downloaded) = next_download_batch(&mut self.downloader).await {
            let batch = downloaded.map_err(atipicial_runtime::ServiceError::from)?;
        ...
        if let Some(height) = summary.last_imported_height {
            ...
            if summary.body_checkpoint.is_none() {
                return Err(atipicial_runtime::ServiceError::invalid_state(
...
async fn next_download_batch<D: BlockDownloader>(
    downloader: &mut D,
) -> Option<atipicial_network::NetworkResult<atipicial_network::BlockDownloadBatch>> {
    std::future::poll_fn(|cx| {
        atipicial_network::BlockDownloader::poll_next_batch(Pin::new(&mut *downloader), cx)
    })
    .await
}
