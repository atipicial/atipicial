...
impl MockValidateContext {
    fn new() -> Self {
        ...
        Self {
            chain_spec: atipicial_test_fixtures::test_chain_spec(settings),
            prev_hash: None,
            prev_timestamp: None,
        }
    }
    ...
    fn with_settings(mut self, settings: ProtocolSettings) -> Self {
        self.chain_spec = atipicial_test_fixtures::test_chain_spec(settings);
        self
    }
}
...
fn stage(ctx: MockValidateContext) -> AtipicialValidateStage<MockValidateContext> {
    AtipicialValidateStage::new(Arc::new(ctx))
}
...
fn validate_stage_owns_concrete_context_type() {
    ...
    assert!(
        source.contains(
            "pub struct AtipicialValidateStage<C = SnapshotValidateContext<atipicial_storage::EmptyCacheBacking>>"
        ),
    ...
    assert!(
        ...
        "owned homogeatipicialus validate context should not be erased to dyn"
    );
