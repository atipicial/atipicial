//! # atipicial-node::node::rpc_runtime
//!
...
pub(super) fn start_rpc_server(
...
) -> anyhow::Result<Arc<parking_lot::RwLock<atipicial_rpc::server::RpcServer>>> {
    ...
    // C#-optional plugin method groups plus the built-in AtipicialIndexer: register
    ...
    info!(target: "atipicial", %bind_address, port, "RPC server started");
    Ok(server)
}
