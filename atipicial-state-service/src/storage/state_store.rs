...
use atipicial_storage::{DataCache, StorageResult, TrackState};
use atipicial_trie::{MptError, MptResult};
    ...
    S: Store,
{
    ...
    /// [`atipicial_storage::persistence::providers::RuntimeStore`]
    ...
    /// [`MptReadSnapshot`](crate::MptReadSnapshot) and [`Trie`](atipicial_trie::Trie)
    ...
    pub fn apply_snapshot_changes<B: atipicial_storage::CacheRead>(
        &self,
        block_index: u32,
    ...
    pub(crate) fn project_mpt_changes_into<B: atipicial_storage::CacheRead>(
        snapshot: &DataCache<B>,
    ...
    fn apply_snapshot_mpt_changes_with_root<B: atipicial_storage::CacheRead>(
        &self,
        block_index: u32,
        root_before: Option<UInt256>,
        snapshot: &DataCache<B>,
    ...
    #[cfg(test)]
    fn mpt_changes_from_snapshot<B: atipicial_storage::CacheRead>(
        snapshot: &DataCache<B>,
    ) -> Vec<MptChange> {
    ...
    fn push_mpt_changes_from_snapshot<B: atipicial_storage::CacheRead>(
        snapshot: &DataCache<B>,
        changes: &mut Vec<MptChange>,
    ) {
        snapshot.visit_tracked_items(|key, trackable| {
