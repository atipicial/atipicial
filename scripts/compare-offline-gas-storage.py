#!/usr/bin/env python3
"""Compare offline atipicial-rs GAS account storage against a reference state root."""
...
def local_gas_balance(
    *,
    ...
    payload = probe_runner(
        db_path,
        ["--gas-address", address, "--decode", "aep17-account"],
        probe_bin=probe_bin,
    )
    decoded = payload.get("decoded") or {}
...
def compare_gas_accounts(
    *,
    ...
    for address in addresses:
        ...
        reference_balance = decode_aep17_account_balance(reference_value)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare offline GAS account storage from a atipicial-rs MDBX directory "
            "against Atipicial reference state at the DB's current Ledger height."
        )
    )
    parser.add_argument("--db", required=True, type=Path, help="atipicial-rs chain MDBX path")
    parser.add_argument(
        "--address",
        action="append",
        required=True,
        help="Atipicial address whose GAS AccountState should match the reference root; repeatable",
    )
    parser.add_argument(
        "--probe-bin",
        default=Path("target/debug/atipicial-db-probe"),
        type=Path,
        help="Path to the built atipicial-db-probe binary",
    )
    parser.add_argument(
        "--reference-rpc",
