...
use atipicial_vm::{ExecutionEngineLimits, OpCode, VmState};
...
/// C# `Notary.OnManifestCompose` (Notary.cs:92-102): AEP-27 alone until
/// HF_Faun is enabled at the height, then AEP-27 + AEP-30.
#[test]
fn manifest_standards_gain_nep30_at_faun() {
    let echidna_only = build_native_contract_state(&Notary, &echidna_settings(), 0);
    assert_eq!(echidna_only.manifest.supported_standards, ["AEP-27"]);
    ...
    assert_eq!(before.manifest.supported_standards, ["AEP-27"]);
    let after = build_native_contract_state(&Notary, &settings, 10);
    assert_eq!(after.manifest.supported_standards, ["AEP-27", "AEP-30"]);
}

/// Reads the ATD balance of `account` out of the AEP-17 account record
/// (`Struct[Integer(balance), ...]`), returning 0 when absent.
fn gas_balance(snapshot: &DataCache, account: &UInt160) -> BigInt {
    crate::AtipicialDollar::balance_of(snapshot, account).expect("decode ATD account balance")
}
...
/// NotaryAssisted share `AtipicialDollar::on_persist` withholds from the primary
/// network-fee mint.
#[test]
