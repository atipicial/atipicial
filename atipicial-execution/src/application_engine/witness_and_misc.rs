...
{
    let Ok(raw) = std::env::var("ATC_TRACE_WITNESS_TX") else {
        return false;
    };
    ...
    B: atipicial_storage::CacheRead,
{
    ...
    /// Matches all Atipicial N3 witness condition types.
    ...
    fn contract_matches_group(&self, contract_hash: &UInt160, group: &[u8]) -> CoreResult<bool> {
        ...
        let group_point = atipicial_crypto::ECPoint::from_bytes(group)
    ...
    /// This matches C# Atipicial's ApplicationEngine.Find method exactly.
    ...
    /// This matches C# Atipicial's ApplicationEngine.Find method with FindOptions exactly.
    ...
    pub fn stack_item_to_bytes(item: StackItem) -> CoreResult<Vec<u8>> {
        if matches!(item, StackItem::Null) {
            return atipicial_serialization::binary_serializer::BinarySerializer::serialize(
                &StackItem::null(),
        ...
        atipicial_serialization::binary_serializer::BinarySerializer::serialize(
            &item,
            &ExecutionEngineLimits::default(),
        )
    }
