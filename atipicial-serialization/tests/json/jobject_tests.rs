...
use atipicial_serialization::json::{JObject, JToken, OrderedDictionary};

#[test]
fn test_object_creation_and_access() {
    let mut dict = OrderedDictionary::new();
    dict.insert("name".to_string(), Some(JToken::String("atipicial".to_string())));
    ...
    assert_eq!(object.get("name"), Some(&JToken::String("atipicial".to_string())));
