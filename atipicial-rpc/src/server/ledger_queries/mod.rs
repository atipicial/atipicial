//! # atipicial-rpc::server::ledger_queries
//!
...
use atipicial_blockchain::ledger_provider::{
...
///
/// Routes the index case through the factory supplied by node composition, so
/// historical hashes remain available after their hot rows are pruned.
pub(crate) fn resolve_block_hash<B: CacheRead>(
    factory: &impl LedgerProviderFactory,
    snapshot: &DataCache<B>,
    identifier: &BlockHashOrIndex,
) -> CoreResult<Option<UInt256>> {
    match identifier {
        BlockHashOrIndex::Index(index) => factory.provider(snapshot).block_hash_by_index(*index),
        BlockHashOrIndex::Hash(hash) => Ok(Some(*hash)),
    }
}

/// Returns the current persisted ledger height from the hot current-block row.
///
...
/// narrow capability trait. The current pointer is never cold-tiered, unlike
/// historical block and transaction rows.
...
/// Returns the current persisted ledger hash from the hot current-block row.
...
/// Returns the current persisted block count from the hot current-block row.
...
pub(crate) fn block_hash_by_index<B: CacheRead>(
    ...
    factory.provider(snapshot).block_hash_by_index(index)
}
...
pub(crate) fn current_index_and_next_hash<B: CacheRead>(
    ...
    let provider = factory.provider(snapshot);
    let current_index = current_index(snapshot)?;
...
/// are both delegated through the node-composed hot/cold factory. Provider
/// failures retain the same typed `CoreError` semantics whether the winning row
/// came from mutable storage or the finalized archive.
pub(crate) fn get_full_block<B: CacheRead>(
    factory: &impl LedgerProviderFactory,
    snapshot: &DataCache<B>,
    identifier: &BlockHashOrIndex,
) -> CoreResult<Option<Block>> {
    let Some(hash) = resolve_block_hash(factory, snapshot, identifier)? else {
        return Ok(None);
    };
    factory.provider(snapshot).block_by_hash(&hash)
}
