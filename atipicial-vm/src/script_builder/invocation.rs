...
//! shape used by Atipicial witnesses: `PUSHDATA1 0x40 <64-byte signature>`.
...
impl ScriptBuilder {
    ...
    /// Atipicial witness invocation scripts use. The output is byte-identical
    /// to the hand-rolled `PUSHDATA1 + len + sig` construction that was
    /// previously duplicated across atipicial-consensus and atipicial-node.
    ///
