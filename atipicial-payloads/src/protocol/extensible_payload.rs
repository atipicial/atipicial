use crate::{InventoryType, inventory::Inventory, witness::Witness};
use atipicial_error::CoreResult;
use atipicial_io::macros::ValidateLength;
...
impl atipicial_primitives::SerializablePayload for ExtensiblePayload {
    ...
    fn hash(&self) -> UInt256 {
        self.try_get_hash_data()
            .map(|data| UInt256::from(atipicial_crypto::Crypto::sha256(&data)))
            .unwrap_or_default()
    }
...
atipicial_io::impl_default_via_new!(ExtensiblePayload);

impl atipicial_primitives::Verifiable for ExtensiblePayload {
...
#[cfg(test)]
#[path = "../tests/protocol/extensible_payload.rs"]
mod tests;
