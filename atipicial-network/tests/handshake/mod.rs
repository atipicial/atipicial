//! # atipicial-network::tests::handshake
//!
//! Test module grouping handshake behavior coverage for atipicial-network.
//!
//! ## Boundary
//!
//! This is test/benchmark-only code for atipicial-network; it may assemble fixtures
...
use atipicial_network::MessageCommand;
...
use atipicial_network::{Message, MessageCodec};
use atipicial_payloads::p2p_payloads::{
    GetBlockByIndexPayload, InvPayload, NodeCapability, PingPayload, VersionPayload,
};
use atipicial_primitives::{InventoryType, UInt256};
...
fn test_chain_spec() -> Arc<AtipicialChainSpec> {
    AtipicialChainSpec::mainnet().expect("embedded MainNet chain specification")
}
...
fn decode_version(message: &Message) -> VersionPayload {
    ...
    <VersionPayload as atipicial_io::Serializable>::deserialize(&mut reader).expect("decode version")
}
...
where
    F: FnMut(&atipicial_network::LocalNodeInfo) -> bool,
{
