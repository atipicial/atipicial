...
pub struct ConnectedPeer {
    ...
    /// [`atipicial_runtime::NetworkEvent::PeerConnected`]. Folds with `None`
...
impl LocalNodeState {
    fn new(events: broadcast::Receiver<NetworkEvent>, registry: Option<Arc<PeerRegistry>>) -> Self {
        ...
        Self {
            nonce,
            user_agent: format!("/atipicial-rs:{}/", env!("CARGO_PKG_VERSION")),
...
impl NetworkHandle {
    ...
    pub async fn relay_inventory(&self, hash: atipicial_primitives::UInt256) -> NetworkResult<()> {
        self.cmd_tx
...
/// in `atipicial_blockchain` and `atipicial_runtime`.
