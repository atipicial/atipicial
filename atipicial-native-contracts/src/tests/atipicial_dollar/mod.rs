//! # atipicial-native-contracts::tests::atipicial_dollar
//!
//! Test module grouping Native ATD token state, accounting, and transfer
...
use super::*;
use atipicial_payloads::{TransactionAttribute, VerifiableContainer};
use atipicial_primitives::{CallFlags, ContractParameterType, UInt160};
...
fn gas_transfer_from_calling_contract_uses_contract_as_witness() {
    ...
    use atipicial_payloads::{Signer, Transaction};
    use atipicial_primitives::{TriggerType, WitnessScope};
    use std::sync::Arc;
    ...
    crate::test_support::deploy_native(
        &cache,
        &build_native_contract_state(&AtipicialDollar, &ProtocolSettings::default(), 0),
    );
    ...
    engine
        .load_script(vec![atipicial_vm::OpCode::RET.byte()], CallFlags::ALL, None)
    ...
    engine
        ...
        .expect("load ATD transfer");

    assert_eq!(engine.execute_allow_fault(), atipicial_vm::VmState::HALT);
    assert_eq!(
        AtipicialDollar::new()
...
fn compute_gas_transfer_matches_csharp_balance_arithmetic() {
    ...
    assert_eq!(
        AtipicialDollar::compute_gas_transfer(None, None, false, &BigInt::zero()),
        GasTransferOutcome::NoMovement
    );

    // amount > 0, from has no account -> insufficient.
    assert_eq!(
        AtipicialDollar::compute_gas_transfer(None, None, false, &amt),
        GasTransferOutcome::InsufficientBalance
    );
    // amount > 0, from underfunded -> insufficient.
    assert_eq!(
        AtipicialDollar::compute_gas_transfer(Some(BigInt::from(99)), None, false, &amt),
        GasTransferOutcome::InsufficientBalance
    );
    // from == to with sufficient balance -> no movement.
    assert_eq!(
        AtipicialDollar::compute_gas_transfer(
    ...
    assert_eq!(
        AtipicialDollar::compute_gas_transfer(Some(BigInt::from(100)), None, false, &amt),
...
fn gas_account_storage_round_trips() {
    ...
    assert_eq!(
        cache
            .get(&AtipicialDollar::account_key(&account))
            .unwrap()
            .value_bytes()
            .as_ref(),
        expected.as_slice()
    ...
    // The single-field Struct[Balance] layout is what read_aep17_balance reads.
    assert_eq!(
        crate::read_aep17_balance(&cache, AtipicialDollar::ID, &account).unwrap(),
        BigInt::from(12345)
    );
    AtipicialDollar::new().delete_gas_account(&cache, &account);
    assert!(
        AtipicialDollar::new()
            .read_gas_account(&cache, &account)
            .unwrap()
...
fn gas_account_storage_uses_stack_item_projection() {
    let source = include_str!("../../atipicial_dollar/storage.rs");
...
fn balance_of_absent_account_is_zero() {
    ...
    assert_eq!(
        AtipicialDollar::balance_of(&cache, &account).unwrap(),
        BigInt::from(0)
    );
}
...
fn total_supply_invoke_reads_full_bigint() {
    ...
    cache.add(
        AtipicialDollar::total_supply_key(),
    ...
    let encoded = AtipicialDollar::new()
...
/// generated GAS manifest declares AEP-17 regardless of the hardfork
/// configuration or height.
#[test]
fn manifest_declares_aep17() {
    use atipicial_execution::native_contract::build_native_contract_state;

    let state = build_native_contract_state(&AtipicialDollar, &ProtocolSettings::default(), 0);
    assert_eq!(state.manifest.supported_standards, ["AEP-17"]);
    let later =
        build_native_contract_state(&AtipicialDollar, &ProtocolSettings::default(), u32::MAX);
    assert_eq!(later.manifest.supported_standards, ["AEP-17"]);
}
...
fn gas_burn_debits_balance_and_supply() {
    ...
    AtipicialDollar::new()
        ...
        .unwrap();
    let supply_key = AtipicialDollar::total_supply_key();
    cache.add(
        supply_key.clone(),
...
/// `AtipicialDollar::initialize` and `AtipicialDollar::on_persist` against the C# oracle
/// (AtipicialDollar.cs:29-58): the genesis InitialGasDistribution mint, the
...
mod persist_tests {
    ...
    use atipicial_payloads::{Block, Header, NotaryAssisted, Signer, Transaction};
    use atipicial_primitives::{TriggerType, WitnessScope};
    ...
    /// C# `AtipicialDollar.InitializeAsync` (AtipicialDollar.cs:29-37): the genesis pass
    ...
    fn initialize_mints_initial_gas_distribution_to_bft_address() {
        ...
        NativeContract::initialize(&AtipicialDollar::new(), &mut engine).expect("initialize");

        let bft = crate::AtipicialCoin::bft_address(&settings.standby_validators()).unwrap();
        ...
        assert_eq!(transfer.script_hash, AtipicialDollar::script_hash());
        assert!(
    ...
    /// C# `AtipicialDollar.OnPersistAsync` (AtipicialDollar.cs:39-58): each sender is burned
    ...
    fn on_persist_burns_fees_and_mints_network_fees_to_primary() {
        ...
        NativeContract::on_persist(&AtipicialDollar::new(), &mut engine).expect("on_persist");
        ...
        // Mint: 1.5 ATD total network fees to the primary validator
    ...
    fn on_persist_uses_cached_signature_account_for_primary_reward() {
        let source = include_str!("../../atipicial_dollar/persist.rs");
        ...
        assert!(
            ...
            "AtipicialDollar::on_persist should use the cached primary-validator account path"
        );
        assert!(
            !on_persist.contains("create_signature_redeem_script(primary_key.clone())"),
            "AtipicialDollar::on_persist should not rebuild the primary validator redeem script"
        );
    }
    ...
    fn on_persist_faults_when_a_sender_cannot_pay_its_fees() {
        ...
        assert!(NativeContract::on_persist(&AtipicialDollar::new(), &mut engine).is_err());
    }

    /// C# AtipicialDollar.cs:47-53: a NotaryAssisted transaction deducts
    ...
    fn on_persist_deducts_notary_assisted_share_from_the_primary_mint() {
        ...
        cache.add(
            crate::PolicyContract::attribute_fee_key(
                atipicial_primitives::TransactionAttributeType::NotaryAssisted.to_byte(),
            ),
    ...
    fn fast_forward_mint_state_preserves_zero_mint_noop_rule() {
        ...
        assert!(
            cache.get(&AtipicialDollar::account_key(&account)).is_none(),
            "zero mint must not create a ATD account"
        );
        assert!(
            cache.get(&AtipicialDollar::total_supply_key()).is_none(),
