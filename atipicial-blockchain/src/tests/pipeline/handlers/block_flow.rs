use super::*;
use atipicial_payloads::header::Header;
...
struct FailingSecondCommitContext {
    snapshot: Arc<atipicial_storage::DataCache>,
...
impl_chain_spec_provider_for_field!(FailingSecondCommitContext);
...
impl SystemContext for FailingSecondCommitContext {
    ...
    type CacheBacking = atipicial_storage::EmptyCacheBacking;

    fn current_height(&self) -> u32 {
        0
    }

    fn store_snapshot(&self) -> Option<Arc<atipicial_storage::DataCache>> {
    ...
    fn block_committing(
        &self,
        _block: &Block,
        _snapshot: &atipicial_storage::DataCache,
        _application_executed_list: &[atipicial_payloads::ApplicationExecuted],
    ) -> bool {
...
impl_chain_spec_provider_for_field!(FailingBulkFlushContext);
...
impl_chain_spec_provider_for_field!(FailingDurableCommitContext);
...
impl_chain_spec_provider_for_field!(FailingFinalizedDeliveryContext);
...
impl_chain_spec_provider_for_field!(StopAfterDurableCommitContext);
...
fn failing_durable_commit_fixture() -> (
    ...
    let system = Arc::new(FailingDurableCommitContext {
        snapshot: Arc::new(atipicial_storage::DataCache::new(false)),
...
struct StateServiceEmptyFastPathContext {
    snapshot: Arc<atipicial_storage::DataCache>,
    chain_spec: Arc<AtipicialChainSpec>,
    state_service: Arc<atipicial_state_service::commit_handlers::StateServiceCommitHandlers>,
    fast_path_checks: Arc<AtomicUsize>,
    committing_heights: Arc<parking_lot::Mutex<Vec<u32>>>,
}
impl_chain_spec_provider_for_field!(StateServiceEmptyFastPathContext);
...
impl SystemContext for StateServiceEmptyFastPathContext {
    ...
    fn block_committing(
        &self,
        block: &Block,
        snapshot: &atipicial_storage::DataCache,
        _application_executed_list: &[atipicial_payloads::ApplicationExecuted],
    ) -> bool {
...
async fn initialize_bootstraps_genesis_once_and_inventory_runs_native_hooks() {
    ...
    assert_eq!(
        atipicial_total_supply(&snapshot),
        Some(num_bigint::BigInt::from(100_000_000)),
        "genesis minted the ATC total supply"
    );
    assert!(
    ...
    // the 0.5-ATD committee reward to standby_committee[1 % 21].
    ...
    // It must still descend from genesis: the pre-verified path skips witness
    // verification, not the parent link.
    let genesis_hash = service
        .ledger
        .block_hash_at(0)
        .expect("genesis cached in the ledger");
    let mut header = Header::new();
    header.set_index(1);
    header.set_prev_hash(genesis_hash);
    ...
    let account = atipicial_primitives::UInt160::from_script(&script);
    let mut key = vec![20u8]; // shared AEP-17 Prefix_Account
    key.extend_from_slice(&account.to_bytes());
    assert!(
        snapshot
            .get(&atipicial_storage::StorageKey::new(
...
async fn bulk_import_flush_failure_aborts_batch_before_durable_store_commit() {
    let snapshot = Arc::new(atipicial_storage::DataCache::new(false));
...
async fn inventory_block_batch_continues_after_rejected_block_like_individual_commands() {
    ...
    bad_header.set_prev_hash(atipicial_primitives::UInt256::from([0xAB; 32]));
...
async fn checked_peer_batch_still_requires_consensus_witness_verification() {
    ...
    let queue = atipicial_runtime::BlockImportQueue::new(Arc::new(handle), 1);
    let checked = queue
...
async fn future_block_with_cached_header_hash_mismatch_is_rejected_not_parked() {
    ...
    assert_eq!(
        outcome.frontier.as_ref().map(atipicial_payloads::Header::index),
        Some(2)
    );
...
async fn bulk_import_uses_empty_fast_path_when_only_state_service_is_loaded() {
    let snapshot = Arc::new(atipicial_storage::DataCache::new(false));
    let state_store = Arc::new(atipicial_state_service::StateStore::with_mpt(true));
    let state_service = Arc::new(
        atipicial_state_service::commit_handlers::StateServiceCommitHandlers::new(Arc::clone(
            &state_store,
        )),
    );
...
async fn bulk_import_verify_true_validates_against_prior_batch_block() {
    ...
    let public_key =
        atipicial_crypto::Secp256r1Crypto::derive_public_key(&private_key).expect("public key");
    let point = atipicial_crypto::ECPoint::from_bytes(&public_key).expect("point");
    let mut settings = atipicial_config::ProtocolSettings::default();
...
async fn bulk_import_skips_per_block_mempool_maintenance() {
    ...
    let system = Arc::new(StoreContext {
        snapshot,
        chain_spec: test_chain_spec(settings.clone()),
        requires_replay_artifacts: true,
        state_service: None,
...
async fn peer_block_witness_verification_accepts_valid_and_rejects_tampered() {
    ...
    let signature = atipicial_crypto::Secp256r1Crypto::sign(&sign_data, &private_key).expect("sign");
    let verification =
        atipicial_vm::script_builder::redeem_script::RedeemScript::multi_sig_redeem_script_from_points(
            1,
            &[point],
    ...
    tampered.witness =
        atipicial_payloads::Witness::new_with_scripts(invocation(&tampered_sig), verification.clone());
    let err = service
    ...
    header.witness = atipicial_payloads::Witness::new_with_scripts(invocation(&signature), verification);
    service
...
async fn peer_block_rejects_out_of_range_primary_with_pool_off_and_on() {
    for pool_enabled in [false, true] {
        ...
        let genesis = crate::native_persist::genesis_block(&chain_spec_for_settings(&settings))
            .expect("genesis");
        let verification = atipicial_vm::script_builder::redeem_script::RedeemScript::
...
async fn optimistic_inventory_batch_verifies_next_header_while_persisting_current_block() {
    ...
    let sign = |header: &Header| {
        ...
        atipicial_payloads::Witness::new_with_scripts(invocation, verification.clone())
    };
...
async fn optimistic_inventory_window_stops_at_an_invalid_deep_header() {
    ...
    for index in 1..=4 {
        ...
        header.witness = if index == 3 {
            atipicial_payloads::Witness::new_with_scripts(Vec::new(), vec![atipicial_vm::OpCode::PUSH0.byte()])
        } else {
            sign(&header)
        };
    ...
    assert!(
        metrics.completed >= 2,
        "the deep invalid witness must complete advisory worker preparation"
    );
}
...
async fn canonical_import_does_not_add_transaction_signature_gate() {
    ...
    let transaction_verification =
        atipicial_vm::script_builder::redeem_script::RedeemScript::signature_redeem_script(&[2u8; 33]);
    let transaction_account = atipicial_primitives::UInt160::from_script(&transaction_verification);
    let mut transaction_invocation = vec![atipicial_vm::OpCode::PUSHDATA1.byte(), 64];
    transaction_invocation.extend_from_slice(&[0u8; 64]);
    let mut transaction = Transaction::new();
    transaction.set_script(vec![atipicial_vm::OpCode::PUSH1.byte()]);
    transaction.set_signers(vec![atipicial_payloads::Signer::new(
        transaction_account,
        atipicial_primitives::WitnessScope::NONE,
    )]);
    transaction.set_witnesses(vec![atipicial_payloads::Witness::new_with_scripts(
...
async fn peer_block_witness_verification_does_not_trust_catchup_peer_tip() {
    ...
    atipicial_runtime::sync_metrics::set_peer_live_tip(1_000_000);
    ...
    sign_data.extend_from_slice(&network.to_le_bytes());
    sign_data.extend_from_slice(&header.hash().to_bytes());
    let mut signature = atipicial_crypto::Secp256r1Crypto::sign(&sign_data, &private_key).expect("sign");
    signature[10] ^= 0xFF;
    let verification =
        atipicial_vm::script_builder::redeem_script::RedeemScript::multi_sig_redeem_script_from_points(
            1,
    ...
    let mut invocation = vec![0x0C, 64];
    invocation.extend_from_slice(&signature);
    header.witness = atipicial_payloads::Witness::new_with_scripts(invocation, verification);
...
async fn peer_block_must_match_cached_header_hash() {
    ...
    assert_eq!(
        outcome.frontier.as_ref().map(atipicial_payloads::Header::hash),
        Some(cached.hash())
    );
...
async fn handle_import_block_reports_rejection_and_verifies_witness() {
    ...
    tampered_header.witness = atipicial_payloads::Witness::new_with_scripts(
    ...
    let tampered_tip = atipicial_runtime::ImportedTip::from_block(&tampered_block).expect("tampered tip");
    let rejected = handle
    ...
    assert_eq!(
        rejected,
        atipicial_runtime::BlockImportOutcome::NotImported {
    ...
    let valid_tip = atipicial_runtime::ImportedTip::from_block(&valid_block).expect("valid tip");
    let imported = handle
    ...
    assert_eq!(
        imported,
        atipicial_runtime::BlockImportOutcome::Imported(valid_tip),
...
async fn handle_import_block_reports_durable_tip_after_finalized_delivery_failure() {
    ...
    let expected_tip = atipicial_runtime::ImportedTip::from_block(&block).expect("tip");
    ...
    assert_eq!(
        outcome,
        atipicial_runtime::BlockImportOutcome::Imported(expected_tip)
    );
...
async fn handle_implements_runtime_block_import_contract() {
    ...
    header.witness = atipicial_payloads::Witness::new_with_scripts(invocation, verification);
    let block = Block::from_parts(header, vec![]);
    let expected_tip = atipicial_runtime::ImportedTip::from_block(&block).expect("block hash");

    atipicial_runtime::BlockImport::check(&handle, &block)
        .await
        .expect("check");
    let outcome = atipicial_runtime::BlockImport::import(&handle, block, atipicial_runtime::BlockOrigin::Sync)
        .await
