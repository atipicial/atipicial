...
// Atipicial N3 v3.10 registers 37 base host syscalls plus 4 Faun storage-local
    ...
    B: atipicial_storage::CacheRead,
{
    ...
    /// boundary conditions (the pre-/post-atipicial-vm#543 handlers) and whether a
    /// zero shift consumes and coerces its value operand (atipicial-vm#567).
    ...
    ///
    /// Homogeatipicialus node pipelines should use this direct generic constructor so
    ...
    pub fn new_with_shared_block_and_native_contract_provider(
        ...
        engine.set_interop_service(atipicial_vm::InteropService::with_capacity(
            HOST_SYSCALL_REGISTRATION_CAPACITY,
        ));
        // Match C# Atipicial: no instruction-count cap on the execution path. Bounding
    ...
    pub fn new_with_preloaded_native_and_native_contract_provider(
        ...
        let contract_scripts = contracts
            .iter()
            .map(|(hash, contract)| (*hash, Script::new_relaxed(contract.aef.script.clone())))
            .collect();
    ...
    ///
    /// AtipicialVM still executes the complete verification script. Only an exact
    ...
    #[must_use]
    pub fn vm_execution_profile(&self) -> Option<atipicial_vm::VmExecutionProfile> {
    ...
    pub fn record_transaction_vm_state(&mut self, hash: &UInt256, vm_state: VMState) -> bool {
        // LedgerTransactionStates is owned by atipicial-native-contracts; we don't
        // have a direct reference to it from atipicial-execution. Return false for
    ...
    /// Returns the raw picoATD fee consumed during execution.
