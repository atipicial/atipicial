//! Error types for the Atipicial VM crate.
//!
...
//!
//! ```rust,ignore
//! use atipicial_core::atipicial_vm::error::VmError;
//!
...
// Atipicial-specific error conversions
impl From<atipicial_io::IoError> for VmError {
    fn from(error: atipicial_io::IoError) -> Self {
        match error {
            atipicial_io::IoError::Format => Self::parse("format error"),
            atipicial_io::IoError::InvalidUtf8 => Self::parse("invalid utf-8 data"),
            atipicial_io::IoError::InvalidData { context, value } => {
                Self::parse(format!("{context}: {value}"))
            }
            atipicial_io::IoError::Io(inner) => Self::io(inner.to_string()),
        }
    }
}
...
atipicial_error::impl_error_from_struct!(atipicial_error::CoreError, VmError => InvalidOperation);
