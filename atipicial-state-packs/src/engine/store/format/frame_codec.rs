...
use crate::engine::platform_io::{read_at, read_exact_at};
use super::segment::SEGMENT_HEADER_LEN;
use super::*;

pub(super) const FRAME_METADATA_DIGEST_DOMAIN: &[u8] = b"atipicial-state-packs/frame-v2/metadata\0";
pub(super) const FRAME_VALUE_DIGEST_DOMAIN: &[u8] = b"atipicial-state-packs/frame-v2/values\0";
const FRAME_CONTENT_DIGEST_DOMAIN: &[u8] = b"atipicial-state-packs/frame-v2/content\0";
const FRAME_FOOTER_DIGEST_DOMAIN: &[u8] = b"atipicial-state-packs/frame-v2/footer\0";
...
fn read_available(file: &File, offset: u64, bytes: &mut [u8]) -> Result<usize> {
    ...
    while filled < bytes.len() {
        ...
        let read = read_at(file, &mut bytes[filled..], read_offset)
...
pub(super) fn read_frame_receipt_at(
    ...
    let mut header_bytes = [0u8; FRAME_HEADER_LEN];
    read_exact_at(pack, &mut header_bytes, frame_start)
    ...
    let mut footer = [0u8; FRAME_FOOTER_LEN];
    read_exact_at(pack, &mut footer, footer_start)
...
fn hash_file_section(file: &File, offset: u64, len: u64, domain: &[u8]) -> Result<[u8; 32]> {
    ...
    while consumed < len {
        ...
        read_exact_at(file, &mut buffer[..chunk_len], read_offset)
...
#[cfg(test)]
mod tests {
    use crate::engine::platform_io::write_all_at;
    use super::*;
    ...
    fn later_segment_scan_accepts_nonzero_first_epoch() -> Result<()> {
        ...
        write_all_at(&pack, &[0; SEGMENT_HEADER_LEN], 0)
            .context("write temporary segment header")?;
        write_all_at(&pack, &frame, SEGMENT_HEADER_LEN as u64)
    ...
    fn streaming_walker_limits_callbacks_to_the_selected_prefix() -> Result<()> {
        ...
        write_all_at(&pack, &first, SEGMENT_HEADER_LEN as u64)
            .context("write first streaming-walk frame")?;
        write_all_at(&pack, &second, first_end)
            .context("write second streaming-walk frame")?;
        write_all_at(&pack, b"ordinary torn suffix", last_end)
    ...
    fn streaming_walker_rejects_unknown_version_after_the_selected_prefix() -> Result<()> {
        ...
        write_all_at(&pack, b"N3PACK03", frame_end)
