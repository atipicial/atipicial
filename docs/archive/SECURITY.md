...
# Atipicial-rs Security Documentation

> **Version**: 0.7.0  
> **Last Updated**: 2026-01-28  
> **Target Compatibility**: Atipicial N3 v3.10.1

This document provides comprehensive security information for the atipicial-rs project, a professional Rust implementation of the Atipicial N3 blockchain node.

## Runtime Security Mode

- By default, native contract execution runs in a compatibility-first mode to preserve consensus parity with Atipicial N3 reference nodes.
- Optional strict native checks can be enabled with `ATC_NATIVE_STRICT_SECURITY=1`.
...
Atipicial-rs is designed to operate in adversarial environments where threats can originate from various actors:
...
Atipicial-rs supports multiple elliptic curve signature schemes for different use cases:

#### Primary: ECDSA with secp256r1 (P-256)

```rust
// Primary curve for Atipicial N3
...
use atipicial_crypto::Secp256k1Crypto;
...
```rust
// EdDSA signatures
use atipicial_crypto::Ed25519Crypto;
...
Atipicial-rs implements multiple hash functions with constant-time comparison support:
...
| Hash256 | 32 bytes | Atipicial transaction hashes | Double SHA-256 |
...
```rust
use atipicial_crypto::hash::ct_hash_eq;
...
Script Hash (20 bytes) = Hash160(public_key)
       │
       ▼
Atipicial Address (34 chars) = Base58Check(script_hash)
```
...
Atipicial-rs implements dBFT 2.0 (Delegated Byzantine Fault Tolerance), providing:
...
The Atipicial VM provides multiple layers of sandboxing:
...
- [Atipicial N3 Documentation](https://developers.atipicial.com/) - Official Atipicial documentation
